...
# Adapted from the Realm Core build scripts (Apache-2.0).
# Builds the barq-monorepo xcframework consumed by barq-swift's download-core.sh.
...
if [[ -n $BUILD ]]; then
    ...
    (
        ...
        -D BARQ_VERSION="${VERSION}" \
        -D BARQ_BUILD_LIB_ONLY=ON \
        -D BARQ_ENABLE_GEOSPATIAL=ON \
...
filename="barq-Release-${VERSION}-macosx-devel.tar.gz"
...
for bt in "${BUILD_TYPES[@]}"; do
    ...
    find . -name "barq-${bt}-${VERSION}-*-devel.tar.gz" -maxdepth 1 | while read -r filename; do
        platform="${filename#"./barq-${bt}-${VERSION}-"}"
...
create_xcframework barq-monorepo libbarq-monorepo
...
if [[ -n $COPY ]]; then
    ...
    rm -f "barq-monorepo-xcframework-${VERSION}.tar.xz"
    tar -cJvf "barq-monorepo-xcframework-${VERSION}.tar.xz" core/barq-monorepo*.xcframework
fi
