...
void test_lists_numeric_agg(TestContext& test_context, DBRef sg, const barq::DataType type_id, U null_value = U{},
                            bool optional = false)
{
    auto t = sg->start_write();
    auto table = t->add_table("the_table");
    ...
    CHECK_EQUAL(lst.size(), 0);
    {
        ...
        auto min = lst.min(&ret_ndx);
        CHECK(min);
        CHECK_EQUAL(ret_ndx, barq::npos);
        ret_ndx = barq::npos;
        auto max = lst.max(&ret_ndx);
        CHECK(max);
        CHECK_EQUAL(ret_ndx, barq::npos);
        size_t ret_count = barq::npos;
        auto sum = lst.sum(&ret_count);
        CHECK(sum);
...
TEST(List_AggOps)
{
    ...
    test_lists_numeric_agg<float>(test_context, sg, type_Float, barq::null::get_null_float<float>(), true);
    test_lists_numeric_agg<double>(test_context, sg, type_Double, barq::null::get_null_float<double>(), true);
    test_lists_numeric_agg<Decimal128>(test_context, sg, type_Decimal, Decimal128(barq::null()), true);
}
...
TEST(List_Nested_InMixed)
{
    ...
    DBRef db = DB::create(make_in_barq_history(), path, options);
...
{
    SHARED_GROUP_TEST_PATH(path);
    DBRef db = DB::create(make_in_barq_history(), path);
    auto tr = db->start_write();
