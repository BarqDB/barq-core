...
TEST(TableView_DoubleSortPrecision)
{
    ...
    // If this check fails, it's a bug in this unit test, not in Barq
    CHECK_EQUAL(f1, f2);
    ...
    // If sort is stable, and compare makes a draw because the doubles are accidentally casted to float in Barq,
    // then
...
TEST(TableView_SortNullString)
{
    ...
    t.create_object().set(col, barq::null());            // barq::null()
...
TEST(TableView_IsInSync)
{
    ...
    auto repl = make_in_barq_history();
...
TEST_TYPES(TableView_Distinct, DistinctDirect, DistinctOverLink)
{
    ...
    // to test distinct() with all possible Barq data types.
