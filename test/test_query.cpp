...
#include <barq.hpp>
#include <barq/column_integer.hpp>
...
// valgrind --tool=callgrind --instr-atstart=no barq-tests
...
TEST(Query_Count)
{
    ...
    //   BARQ_MAX_BPNODE_SIZE > [some large number]!)
    ...
    for (int j = 0; j < 100; j++) {
        ...
        size_t rows = random.draw_int_mod(5 * BARQ_MAX_BPNODE_SIZE); // to cross some leaf boundaries
...
TEST(Query_NextGenSyntax)
{
    ...
    match = (untyped.column<int64_t>(c0) > untyped.column<int64_t>(c0)).find();
    CHECK_EQUAL(match, barq::null_key);
...
TEST(Query_NextGen_StringConditions)
{
    ...
    cnt = table2->column<String>(col_str3).equal(barq::null()).count();
    CHECK_EQUAL(cnt, 1);

    cnt = table2->column<String>(col_str3).not_equal(barq::null()).count();
    CHECK_EQUAL(cnt, 4);

    cnt = table2->column<String>(col_str3).contains(barq::null()).count();
    CHECK_EQUAL(cnt, 5);

    cnt = table2->column<String>(col_str3).like(barq::null()).count();
    CHECK_EQUAL(cnt, 1);
    ...
    cnt = table2->column<String>(col_str3).equal(barq::null(), false).count();
    CHECK_EQUAL(cnt, 1);

    cnt = table2->column<String>(col_str3).not_equal(barq::null(), false).count();
    CHECK_EQUAL(cnt, 4);

    cnt = table2->column<String>(col_str3).contains(barq::null(), false).count();
    CHECK_EQUAL(cnt, 5);

    cnt = table2->column<String>(col_str3).like(barq::null(), false).count();
    CHECK_EQUAL(cnt, 1);
    ...
    cnt = table3->link(col_link1).column<String>(col_str3).equal(barq::null()).count();
    CHECK_EQUAL(cnt, 1);

    cnt = table3->link(col_link1).column<String>(col_str3).not_equal(barq::null()).count();
    CHECK_EQUAL(cnt, 4);
    ...
    cnt = table3->link(col_link1).column<String>(col_str3).equal(barq::null(), false).count();
    CHECK_EQUAL(cnt, 1);

    cnt = table3->link(col_link1).column<String>(col_str3).not_equal(barq::null(), false).count();
    CHECK_EQUAL(cnt, 4);

    cnt = table3->link(col_link1).column<String>(col_str3).contains(barq::null(), false).count();
    CHECK_EQUAL(cnt, 4);
    ...
    cnt = table3->link(col_link1).column<String>(col_str3).like(barq::null(), false).count();
    CHECK_EQUAL(cnt, 1);
}
...
TEST(Query_NextGenSyntaxMonkey0)
{
    ...
    for (int iter = 1; iter < 10 + TEST_DURATION * 1000; iter++) {
        const size_t rows = 1 + random.draw_int_mod(2 * BARQ_MAX_BPNODE_SIZE);
        Table table;
...
TEST_TYPES(Query_NextGenSyntaxMonkey, std::true_type, std::false_type)
{
    ...
    for (int iter = 1; iter < 5 * (TEST_DURATION * TEST_DURATION * TEST_DURATION + 1); iter++) {
        ...
        const size_t rows = 1 + random.draw_int_mod<size_t>(BARQ_MAX_BPNODE_SIZE * 20 *
        ...
        // second == 1
        barq::Query q1_0 = table.where().equal(col_int1, 1);
        barq::Query q2_0 = table.column<int64_t>(col_int1) == 1;
        barq::TableView tv_0 = q2_0.find_all();
        tvpos = 0;
        for (Obj o : table) {
        ...
        barq::Query q2_1 = (table.column<int64_t>(col_int0) == 0 || table.column<int64_t>(col_int0) == 1) &&
                            table.column<int64_t>(col_int1) == 1;
        barq::TableView tv_1 = q2_1.find_all();
        tvpos = 0;
        for (Obj o : table) {
        ...
        // first == 0 || (first == 1 && second == 1)
        barq::Query q2_2 = table.column<int64_t>(col_int0) == 0 ||
                            (table.column<int64_t>(col_int0) == 1 && table.column<int64_t>(col_int1) == 1);
        barq::TableView tv_2 = q2_2.find_all();
        tvpos = 0;
        for (Obj o : table) {
            if (o.get<Int>(col_int0) == 0 || (o.get<Int>(col_int0) == 1 && o.get<Int>(col_int1) == 1)) {
        ...
        barq::Query q4_8 = table.column<int64_t>(col_int1) == 0 &&
                            (table.column<int64_t>(col_int0) == 0 || table.column<int64_t>(col_int0) == 2);
        barq::TableView tv_8 = q4_8.find_all();
        tvpos = 0;
        for (Obj o : table) {
        ...
        barq::Query q3_7 = (table.column<int64_t>(col_int0) == 0 || table.column<int64_t>(col_int0) == 2) &&
                            (table.column<int64_t>(col_int0) == 1 || table.column<int64_t>(col_int1) == 1);
        barq::TableView tv_7 = q3_7.find_all();
        tvpos = 0;
        for (Obj o : table) {
        ...
        // (first == 0 || first == 2) || (first == 1 || second == 1)
        barq::Query q4_7 = (table.column<int64_t>(col_int0) == 0 || table.column<int64_t>(col_int0) == 2) ||
                            (table.column<int64_t>(col_int0) == 1 || table.column<int64_t>(col_int1) == 1);
        barq::TableView tv_10 = q4_7.find_all();
        tvpos = 0;
        for (Obj o : table) {
            if ((o.get<Int>(col_int0) == 0 || o.get<Int>(col_int0) == 2) ||
                (o.get<Int>(col_int0) == 1 || o.get<Int>(col_int1) == 1)) {
        ...
        // first == 0 || first == 2 || first == 1 || second == 1
        barq::Query q20 = table.column<int64_t>(col_int0) == 0 || table.column<int64_t>(col_int0) == 2 ||
                           table.column<int64_t>(col_int0) == 1 || table.column<int64_t>(col_int1) == 1;
        tv = q20.find_all();
        tvpos = 0;
        for (Obj o : table) {
        ...
        barq::Query q21 = table.query("first * 2 > second / 2 + third + 1");
        tv = q21.find_all();
        tvpos = 0;
        for (Obj o : table) {
        ...
        barq::Query q22 = table.query("first * 2 > second / 2 + third + 1 + third - third + third - third + third - "
...
TEST(Query_MergeQueriesOverloads)
{
    ...
    // q1_0 && q2_0
    barq::Query q1_110 = table.where().equal(col_int0, 20);
    barq::Query q2_110 = table.where().equal(col_int1, 30);
    barq::Query q3_110 = q1_110.and_query(q2_110);
    c = q1_110.count();
    c = q2_110.count();
    c = q3_110.count();
    ...
    // q1_0 && q2_0
    barq::Query q1_0 = table.where().equal(col_int0, 10).Or().equal(col_int0, 20);
    barq::Query q2_0 = table.where().equal(col_int1, 30);
    barq::Query q3_0 = q1_0 && q2_0;
    c = q3_0.count();
    CHECK_EQUAL(1, c);

    // q2_0 && q1_0 (reversed operand order)
    barq::Query q1_1 = table.where().equal(col_int0, 10).Or().equal(col_int0, 20);
    barq::Query q2_1 = table.where().equal(col_int1, 30);
    c = q1_1.count();

    barq::Query q3_1 = q2_1 && q1_1;
    c = q3_1.count();
    CHECK_EQUAL(1, c);

    // Short test for ||
    barq::Query q1_2 = table.where().equal(col_int0, 10);
    barq::Query q2_2 = table.where().equal(col_int1, 30);
    barq::Query q3_2 = q2_2 || q1_2;
    c = q3_2.count();
    CHECK_EQUAL(2, c);
}
...
TEST(Query_MergeQueries)
{
    ...
    barq::Query q1_0 = table.where().equal(col_int0, 10).Or().equal(col_int0, 20);
    barq::Query q2_0 = table.where().and_query(q1_0).equal(col_int1, 30);
...
TEST(Query_Not)
{
    ...
    barq::Query q0 = table.where().Not().equal(col_int0, 10);
    CHECK_EQUAL(2, q0.count());

    // grouping, after not
    barq::Query q0b = table.where().Not().group().equal(col_int0, 10).end_group();
    CHECK_EQUAL(q0b.validate(), "");
    CHECK_EQUAL(2, q0b.count());

    // grouping, surrounding not
    barq::Query q0c = table.where().group().Not().equal(col_int0, 10).end_group();
    CHECK_EQUAL(2, q0c.count());

    // nested nots (implicit grouping)
    barq::Query q0d = table.where().Not().Not().equal(col_int0, 10);
    CHECK_EQUAL(1, q0d.count()); // FAILS

    barq::Query q0e = table.where().Not().Not().Not().equal(col_int0, 10);
    CHECK_EQUAL(2, q0e.count()); // FAILS

    // just checking the above
    barq::Query q0f = table.where().Not().not_equal(col_int0, 10);
    CHECK_EQUAL(1, q0f.count());

    barq::Query q0g = table.where().Not().Not().not_equal(col_int0, 10);
    CHECK_EQUAL(2, q0g.count()); // FAILS

    barq::Query q0h = table.where().not_equal(col_int0, 10);
    CHECK_EQUAL(2, q0h.count());

    // should apply not to first term, leading to query "not A and A", which is obviously empty:
    barq::Query q1 = table.where().Not().equal(col_int0, 10).equal(col_int0, 10);
    CHECK_EQUAL(0, q1.count());

    // should apply not to first term, leading to query "not A and A", which is obviously empty:
    barq::Query q1b = table.where().group().Not().equal(col_int0, 10).end_group().equal(col_int0, 10);
    CHECK_EQUAL(0, q1b.count());

    // should apply not to first term, leading to query "not A and A", which is obviously empty:
    barq::Query q1c = table.where().Not().group().equal(col_int0, 10).end_group().equal(col_int0, 10);
    CHECK_EQUAL(0, q1c.count());


    // should apply not to second term, leading to query "A and not A", which is obviously empty:
    barq::Query q2 = table.where().equal(col_int0, 10).Not().equal(col_int0, 10);
    CHECK_EQUAL(0, q2.count()); // FAILS

    // should apply not to second term, leading to query "A and not A", which is obviously empty:
    barq::Query q2b = table.where().equal(col_int0, 10).group().Not().equal(col_int0, 10).end_group();
    CHECK_EQUAL(0, q2b.count());

    // should apply not to second term, leading to query "A and not A", which is obviously empty:
    barq::Query q2c = table.where().equal(col_int0, 10).Not().group().equal(col_int0, 10).end_group();
    CHECK_EQUAL(0, q2c.count()); // FAILS


    // should apply not to both terms, leading to query "not A and not A", which has 2 members
    barq::Query q3 = table.where().Not().equal(col_int0, 10).Not().equal(col_int0, 10);
    CHECK_EQUAL(2, q3.count()); // FAILS

    // applying not to an empty query is forbidden
    barq::Query q4 = table.where();
    CHECK_THROW(!q4, barq::Exception);
}
...
TEST(Query_MergeQueriesMonkey)
{
    ...
    for (int iter = 0; iter < 5; iter++) {
        const size_t rows = BARQ_MAX_BPNODE_SIZE * 4;
        Table table;
        auto col_int0 = table.add_column(type_Int, "first");
        auto col_int1 = table.add_column(type_Int, "second");
        ...
        // and_query(second == 1)
        barq::Query q1_0 = table.where().equal(col_int1, 1);
        barq::Query q2_0 = table.where().and_query(q1_0);
        barq::TableView tv_0 = q2_0.find_all();
        tvpos = 0;
        for (Obj o : table) {
            if (o.get<Int>(col_int1) == 1) {
        ...
        barq::Query q1_1 = table.where().equal(col_int1, 1);
        barq::Query q2_1 =
        ...
        // first == 0 || (first == 1 && and_query(second == 1))
        barq::Query q1_2 = table.where().equal(col_int1, 1);
        barq::Query q2_2 = table.where().equal(col_int0, 0).Or().equal(col_int0, 1).and_query(q1_2);
        barq::TableView tv_2 = q2_2.find_all();
        tvpos = 0;
        for (Obj o : table) {
            if (o.get<Int>(col_int0) == 0 || (o.get<Int>(col_int0) == 1 && o.get<Int>(col_int1) == 1)) {
        ...
        barq::Query q1_3 = table.where().equal(col_int0, 0);
        barq::Query q2_3 = table.where().and_query(q1_3).Or().equal(col_int0, 1).equal(col_int1, 1);
        barq::TableView tv_3 = q2_3.find_all();
        tvpos = 0;
        for (Obj o : table) {
            if (o.get<Int>(col_int0) == 0 || (o.get<Int>(col_int0) == 1 && o.get<Int>(col_int1) == 1)) {
        ...
        barq::Query q2_4 = table.where().equal(col_int0, 1).equal(col_int1, 1);
        barq::Query q1_4 = table.where().equal(col_int0, 0).Or().and_query(q2_4);
        barq::TableView tv_4 = q1_4.find_all();
        tvpos = 0;
        for (Obj o : table) {
            if (o.get<Int>(col_int0) == 0 || (o.get<Int>(col_int0) == 1 && o.get<Int>(col_int1) == 1)) {
        ...
        barq::Query q2_5 = table.where().equal(col_int0, 0).Or().equal(col_int0, 2);
        barq::Query q1_5 = table.where().equal(col_int0, 1).equal(col_int1, 1);
        barq::Query q3_5 = table.where().and_query(q2_5).Or().and_query(q1_5);
        barq::TableView tv_5 = q3_5.find_all();
        tvpos = 0;
        for (Obj o : table) {
            if ((o.get<Int>(col_int0) == 0 || o.get<Int>(col_int0) == 2) ||
                (o.get<Int>(col_int0) == 1 && o.get<Int>(col_int1) == 1)) {
        ...
        barq::Query q1_6 = table.where().equal(col_int0, 0);
        barq::Query q2_6 = table.where().equal(col_int1, 1);
        barq::Query q3_6 = table.where().and_query(q1_6).and_query(q2_6);
        barq::TableView tv_6 = q3_6.find_all();
        tvpos = 0;
        for (Obj o : table) {
        ...
        // and_query(first == 0 || first == 2) && and_query(first == 1 || second == 1)
        barq::Query q2_7 = table.where().equal(col_int0, 0).Or().equal(col_int0, 2);
        barq::Query q1_7 = table.where().equal(col_int0, 1).equal(col_int0, 1).Or().equal(col_int1, 1);
        barq::Query q3_7 = table.where().and_query(q2_7).and_query(q1_7);
        barq::TableView tv_7 = q3_7.find_all();
        tvpos = 0;
        for (Obj o : table) {
            if ((o.get<Int>(col_int0) == 0 || o.get<Int>(col_int0) == 2) &&
        ...
        barq::Query q2_8 = table.where().equal(col_int0, 2);
        barq::Query q3_8 = table.where().equal(col_int0, 0).Or().and_query(q2_8);
        barq::Query q4_8 = table.where().equal(col_int1, 0).and_query(q3_8);
        barq::TableView tv_8 = q4_8.find_all();
        tvpos = 0;
        for (Obj o : table) {
            if (o.get<Int>(col_int1) == 0 && ((o.get<Int>(col_int0) == 0) || o.get<Int>(col_int0) == 2)) {
        ...
        // second == 0 && and_query(first == 0 || and_query(first == 2))
        barq::Query q2_9 = table.where().equal(col_int0, 2);
        barq::Query q5_9 = table.where().equal(col_int0, 0);
        barq::Query q3_9 = table.where().and_query(q5_9).Or().and_query(q2_9);
        barq::Query q4_9 = table.where().equal(col_int1, 0).and_query(q3_9);
        barq::TableView tv_9 = q4_9.find_all();
        tvpos = 0;
        for (Obj o : table) {
            if (o.get<Int>(col_int1) == 0 && ((o.get<Int>(col_int0) == 0) || o.get<Int>(col_int0) == 2)) {
        ...
        barq::Query q2_10 = table.where().equal(col_int0, 0);
        barq::Query q5_10 = table.where().and_query(q2_10);
        barq::Query q3_10 = table.where().and_query(q5_10);
        barq::Query q4_10 = table.where().and_query(q3_10);
        barq::TableView tv_10 = q4_10.find_all();
        tvpos = 0;
        for (Obj o : table) {
...
TEST(Query_MergeQueriesMonkeyOverloads)
{
    ...
    for (int iter = 0; iter < 5; iter++) {
        ...
        // and_query(second == 1)
        barq::Query q1_0 = table.where().equal(col_int1, 1);
        barq::Query q2_0 = table.where() && q1_0;
        barq::TableView tv_0 = q2_0.find_all();
        tvpos = 0;
        for (Obj o : table) {
            if (o.get<Int>(col_int1) == 1) {
        ...
        // and_query(second == 1)
        barq::Query q1_10 = table.where().equal(col_int1, 1);
        barq::Query q2_10 = q1_10 && table.where();
        barq::TableView tv_10 = q2_10.find_all();
        tvpos = 0;
        for (Obj o : table) {
            if (o.get<Int>(col_int1) == 1) {
                CHECK_EQUAL(o.get_key(), tv_10.get_key(tvpos));
        ...
        // (first == 0 || first == 1) && and_query(second == 1)
        barq::Query q1_1 = table.where().equal(col_int0, 0);
        barq::Query q2_1 = table.where().equal(col_int0, 1);
        barq::Query q3_1 = q1_1 || q2_1;
        barq::Query q4_1 = table.where().equal(col_int1, 1);
        barq::Query q5_1 = q3_1 && q4_1;

        barq::TableView tv_1 = q5_1.find_all();
        tvpos = 0;
        for (Obj o : table) {
            if ((o.get<Int>(col_int0) == 0 || o.get<Int>(col_int0) == 1) && o.get<Int>(col_int1) == 1) {
                CHECK_EQUAL(o.get_key(), tv_1.get_key(tvpos));
        ...
        barq::TableView tv_20 = q1_20.find_all();
        tvpos = 0;
        for (Obj o : table) {
            if ((o.get<Int>(col_int0) == 0 || o.get<Int>(col_int0) == 1) && o.get<Int>(col_int1) == 1) {
        ...
        // and_query(first == 0) || (first == 1 && second == 1)
        barq::Query q1_3 = table.where().equal(col_int0, 0);
        barq::Query q2_3 = table.where().equal(col_int0, 1);
        barq::Query q3_3 = table.where().equal(col_int1, 1);
        barq::Query q4_3 = q1_3 || (q2_3 && q3_3);
        barq::TableView tv_3 = q4_3.find_all();
        tvpos = 0;
        for (Obj o : table) {
            if (o.get<Int>(col_int0) == 0 || (o.get<Int>(col_int0) == 1 && o.get<Int>(col_int1) == 1)) {
                CHECK_EQUAL(o.get_key(), tv_3.get_key(tvpos));
        ...
        barq::Query q1_30 = table.where().equal(col_int0, 0);
        barq::Query q3_30 = table.where().equal(col_int1, 1);
        barq::Query q4_30 = table.where().equal(col_int0, 0) || (table.where().equal(col_int0, 1) && q3_30);
        barq::TableView tv_30 = q4_30.find_all();
        tvpos = 0;
        for (Obj o : table) {
            if (o.get<Int>(col_int0) == 0 || (o.get<Int>(col_int0) == 1 && o.get<Int>(col_int1) == 1)) {
...
TEST(Query_StrIndexCrash)
{
    ...
    for (int iter = 0; iter < 5; ++iter) {
        ...
        for (int i = 0; i < BARQ_MAX_BPNODE_SIZE * 2; ++i) {
...
TEST(Query_IntIndex)
{
    ...
    q = origin->link(col_link).column<Int>(col) == barq::null();
    cnt = q.count();
...
TEST(Query_StringIndexNull)
{
    ...
    auto q = origin->link(col_link).column<String>(col) == barq::null();
    auto cnt = q.count();
    CHECK_EQUAL(cnt, nulls);
}
...
struct AggregateValues<Decimal128> {
    ...
    static const constexpr barq::null null = barq::null();
};
...
ColKey generate_all_combinations(Table& table)
{
    ...
    for (size_t k = 1; k <= n; ++k) {
        ...
        do {
            ...
            BARQ_ASSERT(std::is_sorted(selected_values.begin(), selected_values.end()));
            do {
...
TEST(Query_TwoColsEqualVaryWidthAndValues)
{
...
#ifdef BARQ_DEBUG
    for (int i = 0; i < BARQ_MAX_BPNODE_SIZE * 5; i++) {
#else
    ...
    barq::TableView t1 = table.where().equal(col_int0, col_int1).find_all();
    barq::TableView t2 = table.where().equal(col_int2, col_int3).find_all();
    barq::TableView t3 = table.where().equal(col_int4, col_int5).find_all();

    barq::TableView t4 = table.where().equal(col_float6, col_float7).find_all();
    barq::TableView t5 = table.where().equal(col_double8, col_double9).find_all();
...
TEST(Query_TwoCols0)
{
    ...
    barq::TableView t1 = table.where().equal(col0, col1).find_all();
    CHECK_EQUAL(50, t1.size());

    barq::TableView t2 = table.where().less(col0, col1).find_all();
...
TEST(Query_OnTableView_where)
{
    ...
    for (int iter = 0; iter < 50 * (1 + TEST_DURATION * TEST_DURATION); iter++) {
        ...
        size_t cnt1 = 0;
        size_t cnt0 = 0;
        size_t limit = random.draw_int_max(BARQ_MAX_BPNODE_SIZE * 10);

        for (size_t i = 0; i < BARQ_MAX_BPNODE_SIZE * 10; i++) {
...
#if BARQ_MAX_BPNODE_SIZE > 256
    constexpr int node_size = 256;
#else
    constexpr int node_size = 4;
#endif

#if defined BARQ_DEBUG || BARQ_ANDROID
    for (int N = 0; N < 4; N++) {
#else
    for (int N = 0; N < 20; N++) {
#endif
...
TEST(Query_StrEnum)
{
    ...
    for (int i = 0; i < 100; ++i) {
        ttt.clear();
        aa = 0;
        for (size_t t = 0; t < BARQ_MAX_BPNODE_SIZE * 2; ++t) {
...
{
    SHARED_GROUP_TEST_PATH(path);
    std::unique_ptr<Replication> hist_w(make_in_barq_history());
...
TEST(Query_Sort_And_Requery_Untyped_Monkey2)
{
    ...
    for (int iter = 0; iter < 1; iter++) {
        ...
        for (size_t t = 0; t < 2 * BARQ_MAX_BPNODE_SIZE; t++) {
            int64_t val1 = rand() % 5;
            int64_t val2 = rand() % 5;
        ...
        for (size_t t = 0; t < tv2.size(); t++) {
            ...
            BARQ_ASSERT_EX(b < tv.size(), b, tv.size());
        ...
        for (size_t t = 0; t < tv3.size(); t++) {
            ...
            BARQ_ASSERT_EX(b < tv2.size(), b, tv2.size());
