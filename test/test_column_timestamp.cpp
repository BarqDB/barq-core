...
TEST(TimestampColumn_Operators)
{
    ...
    // if one of the Timestamps are null! Please use barq::Greater, barq::Equal, etc instead.

    // Test A. Note that Timestamp{} is null and Timestamp(0, 0) is non-null
    // -----------------------------------------------------------------------------------------
    CHECK(compare(Timestamp{}, Timestamp{}, barq::Equal()));
    CHECK(compare(Timestamp(0, 0), Timestamp(0, 0), barq::Equal()));
    CHECK(compare(Timestamp(1, 2), Timestamp(1, 2), barq::Equal()));
    CHECK(compare(Timestamp(-1, -2), Timestamp(-1, -2), barq::Equal()));

    // Test B
    // -----------------------------------------------------------------------------------------
    CHECK(!compare(Timestamp{}, Timestamp(0, 0), barq::Equal()));
    CHECK(!compare(Timestamp(0, 0), Timestamp{}, barq::Equal()));
    CHECK(!compare(Timestamp(0, 0), Timestamp(0, 1), barq::Equal()));
    CHECK(!compare(Timestamp(0, 1), Timestamp(0, 0), barq::Equal()));
    CHECK(!compare(Timestamp(1, 0), Timestamp(0, 0), barq::Equal()));
    CHECK(!compare(Timestamp(0, 0), Timestamp(1, 0), barq::Equal()));

    // Test C: !compare(..., Equal) == compare(..., NotEqual)
    // -----------------------------------------------------------------------------------------
    CHECK(compare(Timestamp{}, Timestamp(0, 0), barq::NotEqual()));
    CHECK(compare(Timestamp(0, 0), Timestamp{}, barq::NotEqual()));
    CHECK(compare(Timestamp(0, 0), Timestamp(0, 1), barq::NotEqual()));
    CHECK(compare(Timestamp(0, 1), Timestamp(0, 0), barq::NotEqual()));
    CHECK(compare(Timestamp(1, 0), Timestamp(0, 0), barq::NotEqual()));
    CHECK(compare(Timestamp(0, 0), Timestamp(1, 0), barq::NotEqual()));
    ...
    // -----------------------------------------------------------------------------------------
    CHECK(compare(Timestamp{}, Timestamp{}, barq::GreaterEqual()));
    CHECK(compare(Timestamp(0, 0), Timestamp(0, 0), barq::GreaterEqual()));
    CHECK(compare(Timestamp(1, 2), Timestamp(1, 2), barq::GreaterEqual()));
    CHECK(compare(Timestamp(-1, -2), Timestamp(-1, -2), barq::GreaterEqual()));

    CHECK(compare(Timestamp{}, Timestamp{}, barq::LessEqual()));
    CHECK(compare(Timestamp(0, 0), Timestamp(0, 0), barq::LessEqual()));
    CHECK(compare(Timestamp(1, 2), Timestamp(1, 2), barq::LessEqual()));
    CHECK(compare(Timestamp(-1, -2), Timestamp(-1, -2), barq::LessEqual()));
    ...
    CHECK(compare(Timestamp{}, Timestamp{}, barq::Greater()) ==
          compare(StringData{}, StringData{}, barq::Greater()));

    // Compare null with non-nulls (Timestamp(0, 0) is non-null and StringData("") is non-null
    CHECK(compare(Timestamp(0, 0), Timestamp{}, barq::Greater()) ==
          compare(StringData(""), StringData{}, barq::Greater()));

    // All four elements are non-nulls
    CHECK(compare(Timestamp(0, 0), Timestamp(0, 0), barq::Greater()) ==
          compare(StringData(""), StringData(""), barq::Greater()));

    // Repeat with other operators than Greater
    CHECK(compare(Timestamp{}, Timestamp{}, barq::Less()) == compare(StringData{}, StringData{}, barq::Less()));
    CHECK(compare(Timestamp(0, 0), Timestamp{}, barq::Less()) ==
          compare(StringData(""), StringData{}, barq::Less()));
    CHECK(compare(Timestamp(0, 0), Timestamp(0, 0), barq::Less()) ==
          compare(StringData(""), StringData(""), barq::Less()));

    CHECK(compare(Timestamp{}, Timestamp{}, barq::Equal()) == compare(StringData{}, StringData{}, barq::Equal()));
    CHECK(compare(Timestamp(0, 0), Timestamp{}, barq::Equal()) ==
          compare(StringData(""), StringData{}, barq::Equal()));
    CHECK(compare(Timestamp(0, 0), Timestamp(0, 0), barq::Equal()) ==
          compare(StringData(""), StringData(""), barq::Equal()));

    CHECK(compare(Timestamp{}, Timestamp{}, barq::NotEqual()) ==
          compare(StringData{}, StringData{}, barq::NotEqual()));
    CHECK(compare(Timestamp(0, 0), Timestamp{}, barq::NotEqual()) ==
          compare(StringData(""), StringData{}, barq::NotEqual()));
    CHECK(compare(Timestamp(0, 0), Timestamp(0, 0), barq::NotEqual()) ==
          compare(StringData(""), StringData(""), barq::NotEqual()));

    CHECK(compare(Timestamp{}, Timestamp{}, barq::GreaterEqual()) ==
          compare(StringData{}, StringData{}, barq::GreaterEqual()));
    CHECK(compare(Timestamp(0, 0), Timestamp{}, barq::GreaterEqual()) ==
          compare(StringData(""), StringData{}, barq::GreaterEqual()));
    CHECK(compare(Timestamp(0, 0), Timestamp(0, 0), barq::GreaterEqual()) ==
          compare(StringData(""), StringData(""), barq::GreaterEqual()));

    CHECK(compare(Timestamp{}, Timestamp{}, barq::LessEqual()) ==
          compare(StringData{}, StringData{}, barq::LessEqual()));
    CHECK(compare(Timestamp(0, 0), Timestamp{}, barq::LessEqual()) ==
          compare(StringData(""), StringData{}, barq::LessEqual()));
    CHECK(compare(Timestamp(0, 0), Timestamp(0, 0), barq::LessEqual()) ==
          compare(StringData(""), StringData(""), barq::LessEqual()));
}
...
TEST(TimestampColumn_ForceReallocate)
{
    ...
    int32_t items_count = BARQ_MAX_BPNODE_SIZE * 5;
...
TEST(TimestampColumn_AddColumnAfterRows)
{
    ...
    std::vector<ObjKey> keys;
    t.create_objects(BARQ_MAX_BPNODE_SIZE * 2 + 1, keys);
