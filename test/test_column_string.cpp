...
TEST(ColumnString_AdaptiveStringLeak)
{
    ...
    for (size_t i = 0; i != 2 * BARQ_MAX_BPNODE_SIZE; ++i) {
...
TEST(ColumnString_Null)
{
...

    {
        ...
        // Test insert
        a.insert(0, barq::null());
        a.insert(2, barq::null());
        a.insert(4, barq::null());
    ...
    for (size_t t = 0; t < 50; t++) {
        ...
        for (size_t i = 0; i < 100; i++) {
            ...
            if (rnd < 80 && a.size() > 0) {
                ...
                // Generate string with good probability of being empty or barq::null()
...
TEST(ColumnString_Index)
{
    ...
    // 17 elements, to test node splits with BARQ_MAX_BPNODE_SIZE = 3 or other small number
    asc.add("HEJSA"); // 0
    asc.add("1");
    asc.add("HEJSA");
    asc.add("3");
...
TEST(ColumnString_NonLeafRoot)
{
    // Small strings
    {
        ...
        CHECK_EQUAL(c.get(BARQ_MAX_BPNODE_SIZE), util::to_string(BARQ_MAX_BPNODE_SIZE));
        CHECK_EQUAL(c.get(BARQ_MAX_BPNODE_SIZE + 1), util::to_string(BARQ_MAX_BPNODE_SIZE + 1));
        c.erase(BARQ_MAX_BPNODE_SIZE);
        CHECK_EQUAL(c.get(BARQ_MAX_BPNODE_SIZE), util::to_string(BARQ_MAX_BPNODE_SIZE + 1));
        c.erase(BARQ_MAX_BPNODE_SIZE);
        CHECK_EQUAL(c.size(), BARQ_MAX_BPNODE_SIZE);
    ...
    // Medium strings
    {
        ...
        for (int i = 1; i < (BARQ_MAX_BPNODE_SIZE + 2); i++) {
            std::string s = util::to_string(i);
            c.add(s);
        }
    ...
    // Upgrade leaf from medium to big
    {
        ...
        c.set(BARQ_MAX_BPNODE_SIZE, "This is a medium long string");
        c.set(BARQ_MAX_BPNODE_SIZE + 1, "This is a rather long string, that should not be very much shorter");
        CHECK_EQUAL(c.get(0), "0");
        CHECK_EQUAL(c.get(BARQ_MAX_BPNODE_SIZE), "This is a medium long string");
        CHECK_EQUAL(c.get(BARQ_MAX_BPNODE_SIZE + 1),
    ...
    // Upgrade leaf from medium to big while inserting
    {
        ...
        c.add("This is a medium long string");
        for (int i = 1; i < BARQ_MAX_BPNODE_SIZE + 1; i++) {
            std::string s = util::to_string(i);
            c.add(s);
        }
        c.add("This is a rather long string, that should not be very much shorter");
