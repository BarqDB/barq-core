...
TEST(Dictionary_Basics)
{
...

    {
        ...
        CHECK_EQUAL(dict.find_any(9), barq::npos);
...
static void do_Dictionary_HashCollisionTransaction(barq::test_util::unit_test::TestContext& test_context,
                                                   int64_t nb_entries)
{
    SHARED_GROUP_TEST_PATH(path);
    auto hist = make_in_barq_history();
