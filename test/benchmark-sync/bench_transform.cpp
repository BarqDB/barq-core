...
void transform_transactions(TestContext& test_context)
{
    ...
    for (size_t i = 0; i < num_iterations; ++i) {
        ...
        session_config.on_sync_client_event_hook = [&](const SyncClientHookData& data) {
            ...
            switch (data.event) {
                case barq::SyncClientHookEvent::DownloadMessageReceived:
                    t.reset();
                    break;
                case barq::SyncClientHookEvent::DownloadMessageIntegrated:
...
int main()
{
    std::string results_file_stem = barq::test_util::get_test_path_prefix() + "results";
    bench::results =
