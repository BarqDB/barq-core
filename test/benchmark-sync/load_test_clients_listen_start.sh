...
if [ "$#" -ne 6 ]; then
    echo "Usage sh $0 path/to/client_binary barq_url path/to/token machine_id number_of_clients starting_client_id"
    exit 1
fi
...
statsd_host=beryllium.nl.sync.barq.io
...
if [ $number_of_clients -eq 1 ]; then
    ...
    $client_path $barq_url $client_folder --token=$token_path \
...
do
    ...
    mkdir $client_folder
    $client_path $barq_url $client_folder --token=$token_path --machine-id=$machine_id --listen --client-id=$id --statsd-host=$statsd_host >/dev/null 2>&1 &
done
