...
#if !defined _WIN32 && !BARQ_PLATFORM_APPLE
...
using namespace barq::test_util::unit_test;
...
void display_build_config()
{
...
#ifdef BARQ_COMPILER_AVX
    const char* compiler_avx = "Yes";
#else
    const char* compiler_avx = "No";
#endif

    const char* cpu_sse = barq::sseavx<42>() ? "4.2" : (barq::sseavx<30>() ? "3.0" : "None");

    const char* cpu_avx = barq::sseavx<1>() ? "Yes" : "No";
    ...
    std::cout << std::endl
              ...
              << "\n"
              << "BARQ_MAX_BPNODE_SIZE = " << BARQ_MAX_BPNODE_SIZE << "\n"
              << "BARQ_MEMDEBUG = " << memdebug << "\n"
              << "\n"
...
bool run_tests(const std::shared_ptr<barq::util::Logger>& logger = nullptr)
{
    {
    ...
    const bool running_spawned_process = getenv("BARQ_SPAWNED");
...
int test_all(const std::shared_ptr<barq::util::Logger>& logger)
{
    ...
    if (num_open_files >= 0) {
        ...
        BARQ_ASSERT(num_open_files_2 >= 0);
