...
TEST(Unresolved_Basic)
{
...

    {
        ...
        auto finn = persons->create_object_with_primary_key("finn.schiermer-andersen@barq.test");
        auto mathias = persons->create_object_with_primary_key("mathias@barq.test");
    ...
    auto finn = persons->get_object_with_primary_key("finn.schiermer-andersen@barq.test");
...
TEST(Unresolved_LinkList)
{
    ...
    // If BARQ_MAX_BPNODE_SIZE is 4, we test that context flag is copied over when replacing root
    CHECK_EQUAL(stock1.size(), 4);
    CHECK_EQUAL(stock2.size(), 4);
