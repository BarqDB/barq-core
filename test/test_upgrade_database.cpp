...
TEST_IF(Upgrade_DatabaseWithCallback, BARQ_MAX_BPNODE_SIZE == 1000)
{
    std::string path = test_util::get_test_resource_path() + "test_upgrade_database_20.barq";
...
TEST_IF(Upgrade_DatabaseWithCallbackWithException, BARQ_MAX_BPNODE_SIZE == 1000)
{
    std::string path = test_util::get_test_resource_path() + "test_upgrade_database_20.barq";
    CHECK_OR_RETURN(File::exists(path));
...
TEST(Upgrade_Disabled)
{
    ...
    CHECK_THROW_ANY(DB::create(make_in_barq_history(), temp_copy, options));
...
{
    std::string path = test_util::get_test_resource_path() + "test_upgrade_database_1000_1.barq";
    CHECK_OR_RETURN(File::exists(path));
...
{
    // Build a barq file with format 200
...
TEST_IF(Upgrade_Database_10_11, BARQ_MAX_BPNODE_SIZE == 4 || BARQ_MAX_BPNODE_SIZE == 1000)
{
    std::string path = test_util::get_test_resource_path() + "test_upgrade_database_" +
                       util::to_string(BARQ_MAX_BPNODE_SIZE) + "_10_to_11.barq";
...
{
    std::string path = test_util::get_test_resource_path() + "test_upgrade_database_11.barq";
    std::vector<int64_t> ids = {0, 2, 3, 15, 42, 100, 7000};
...
TEST_IF(Upgrade_Database_20, BARQ_MAX_BPNODE_SIZE == 1000)
{
    std::string path = test_util::get_test_resource_path() + "test_upgrade_database_20.barq";
    std::vector<int64_t> ids = {0, 2, 3, 15, 42, 100, 7000};
    std::vector<std::string> names = {"adam", "bart", "carlo", "david", "eric", "frank", "gary"};
...
{
    SHARED_GROUP_TEST_PATH(path);
    std::string prefix = barq::BackupHandler::get_prefix_from_path(path);
    // clear out any leftovers from potential earlier crash of unittest
    File::try_remove(prefix + "v200.backup.barq");
    ...
    CHECK(File::exists(prefix + "v200.backup.barq"));
...
NONCONCURRENT_TEST(Upgrade_BackupAtoBbypassAtoC)
{
    ...
    // clear out any leftovers from potential earlier crash of unittest
    File::try_remove(prefix + "v200.backup.barq");
    File::try_remove(prefix + "v201.backup.barq");
    ...
    CHECK(File::exists(prefix + "v200.backup.barq"));
    CHECK(File::exists(prefix + "v201.backup.barq"));
...
TEST(Upgrade_RecoverAsymmetricTables)
{
    ...
    std::string path = test_util::get_test_resource_path() + "downgrade_asymmetric.barq";
    SHARED_GROUP_TEST_PATH(temp_copy);
    File::copy(path, temp_copy);
    auto hist = make_in_barq_history();
...
TEST_IF(Upgrade_Database_22, BARQ_MAX_BPNODE_SIZE == 4 || BARQ_MAX_BPNODE_SIZE == 1000)
{
    std::string path = test_util::get_test_resource_path() + "test_upgrade_database_" +
                       util::to_string(BARQ_MAX_BPNODE_SIZE) + "_22.barq";
    ...
    for (auto& val : set_values) {
        CHECK(set.find(val) != barq::npos);
    }
    CHECK(set.find(obj1.get_link()) != barq::npos);
...
TEST_IF(Upgrade_Database_23, BARQ_MAX_BPNODE_SIZE == 4 || BARQ_MAX_BPNODE_SIZE == 1000)
{
    // We should test that we can convert backlink arrays bigger that node size to BPlusTrees
    std::string path = test_util::get_test_resource_path() + "test_upgrade_database_" +
                       util::to_string(BARQ_MAX_BPNODE_SIZE) + "_23.barq";

    const size_t cnt = 10 * BARQ_MAX_BPNODE_SIZE;
...

    {
        ...
        for (size_t i = 0; i < set_values.size(); ++i) {
            ...
            CHECK_NOT_EQUAL(mixed_set1.find(set_values[i]), barq::not_found);
            if (set_values[i].is_type(type_String) || set_values[i].is_null()) {
                CHECK_NOT_EQUAL(string_set1.find(set_values[i].get<StringData>()), barq::not_found);
            }
            if (set_values[i].is_type(type_Binary) || set_values[i].is_null()) {
                CHECK_NOT_EQUAL(binary_set1.find(set_values[i].get<BinaryData>()), barq::not_found);
            }
        }
