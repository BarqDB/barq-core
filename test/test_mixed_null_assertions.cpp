 ...
 *
 * See: https://github.com/barq/barq-core/issues/4304
 *
 **************************************************************************/
...
TEST(Mixed_List_unresolved_as_null)
{
...

    {
        ...
        list.find_all(barq::null(), [&](size_t pos) {
            found.push_back(pos);
        });
...

    {
        ...
        auto index = list.find_any(barq::null());
        CHECK_EQUAL(index, 0);
...

    {
        ...
        CHECK_EQUAL(list.find_any(barq::null()), 0);
    }

    {
        ...
        list.remove(1);
        CHECK_EQUAL(list.find_any(barq::null()), npos);
...

    {
        ...
        auto index_any = list.find_any(barq::null());
        auto index_first = list.find_first(barq::null());
...
TEST(Mixed_Set_unresolved_links)
{
    ...
    {
        int cnt = 0;
        set.find_all(barq::null(), [this, &set, &cnt](size_t pos) {
            CHECK(pos != not_found);
    ...
    {
        auto index = set.find_any(barq::null());
...
struct SingleLink {
    ...
    void set_links(Obj from, std::vector<ObjLink> to)
    {
        ...
        BARQ_ASSERT(to[0].get_table_key() == dest_table_key);
