...
/// Each of the clients insert \a nrows empty rows in their Barq.
...
TEST(Transform_Set)
{
    ...
    assoc.for_each_permutation([&](auto& it) {
        ...
        CHECK_NOT_EQUAL(set.find("Hello"), barq::npos);
        CHECK_NOT_EQUAL(set.find(123.f), barq::npos);
        CHECK_NOT_EQUAL(set.find("World"), barq::npos);
        CHECK_NOT_EQUAL(set.find(456.f), barq::npos);
        CHECK_EQUAL(set.find(999), barq::npos);
    });
}
