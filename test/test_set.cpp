...
TEST(Set_Basics)
{
    ...
    }
    {
        ...
        CHECK_NOT_EQUAL(ndx, barq::npos);
        CHECK_NOT_EQUAL(ndx2, barq::npos);
...
TEST(Set_Mixed)
{
    ...
    std::vector<Mixed> ref_values{{},
                                  ...
                                  bar.get_link()};
    // Sets of Mixed should be ordered by the rules defined for Set<Mixed>. Refer to "barq/set.hpp".
...
TEST(Set_Links)
{
    ...
    CHECK_NOT_EQUAL(set_links.find(bar1.get_key()), barq::npos);
    CHECK_NOT_EQUAL(set_links.find(bar2.get_key()), barq::npos);
    CHECK_NOT_EQUAL(set_links.find(bar3.get_key()), barq::npos);
    CHECK_EQUAL(set_links.find(bar4.get_key()), barq::npos);
    ...
    CHECK_NOT_EQUAL(set_mixeds.find(bar1.get_link()), barq::npos);
    CHECK_NOT_EQUAL(set_mixeds.find(bar2.get_link()), barq::npos);
    CHECK_NOT_EQUAL(set_mixeds.find(cab1.get_link()), barq::npos);
    CHECK_NOT_EQUAL(set_mixeds.find(cab2.get_link()), barq::npos);
    CHECK_EQUAL(set_mixeds.find(bar3.get_link()), barq::npos);
    ...
    CHECK_EQUAL(set_links.find(bar1.get_key()), barq::npos);
    CHECK_EQUAL(set_mixeds.find(bar1.get_link()), barq::npos);
    ...
    CHECK_EQUAL(set_links.find(bar2_key), barq::npos);               // The original bar2 key is no longer in the set
    CHECK_NOT_EQUAL(set_links.find(bar2.get_key()), barq::npos);     // The unresolved bar2 key is in the set
    CHECK_EQUAL(lnkset_links->find_any(bar2.get_key()), barq::npos); // The unresolved bar2 key is hidden by LnkSet
    CHECK_EQUAL(lnkset_links->find_any(bar3.get_key()), 0);
    CHECK_EQUAL(lnkset_links->find_any(bar4.get_key()), 1);
    CHECK_EQUAL(set_mixeds.find(bar2_link), barq::npos);
