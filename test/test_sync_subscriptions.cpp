...
TEST(Sync_SubscriptionStoreStateUpdates)
{
    ...
    query_b.equal(fixture.foo_col, "Barq");
...
// Only runs if BARQ_MAX_BPNODE_SIZE is 1000 since that's what the pre-created
// test barq files were created with
TEST_IF(Sync_SubscriptionStoreInternalSchemaMigration, BARQ_MAX_BPNODE_SIZE == 1000)
{
    SHARED_GROUP_TEST_PATH(sub_store_path)
    ...
    //     harness.load_initial_data([&](SharedBarq barq) {
    //     auto config = barq->config();
    //     config.path = "test_flx_metadata_tables_v1.barq";
    //     config.cache = false;
    //     barq->convert(config, false);
    //   });
    auto path = std::filesystem::path(test_util::get_test_resource_path()) / "test_flx_metadata_tables_v1.barq";
