...
cmake -D CMAKE_BUILD_TYPE=${build_mode} 
      ...
      -D BARQ_ENABLE_ENCRYPTION=OFF 
      -G Ninja 
      ..
cmake --build . --target barq-afl++
afl-fuzz -t "$time_out" 
        -m "$memory" 
...
cmake -D BARQ_LIBFUZZER=ON 
      ...
      -G Ninja 
      ..
cmake --build . --target barq-libfuzz
./barq_libfuzz <corpus>
```
...
Builds `barq-core` and `object-store` in `Debug` mode using the afl++ compiler `afl-cc` and starts 1 instance of `afl-fuzz`.
...
Builds `barq-core` and `object-store` in `Debug` mode using the clang compiler and starts `barq-libfuzz`.
Optionally, the following arguments can be passed to the script:
