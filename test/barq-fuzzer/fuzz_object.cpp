...
const size_t add_empty_row_max = BARQ_MAX_BPNODE_SIZE * BARQ_MAX_BPNODE_SIZE + 1000;
...
void FuzzObject::remove_search_index(Group& group, FuzzLog& log, State& s)
{
    ...
    if (!column_keys.empty()) {
        ...
        // because Barq will just do a no-op at worst (no exception or assert).
...
void FuzzObject::commit(SharedBarq shared_barq, FuzzLog& log)
{
    log << "FuzzObject::commit();\n";
    log << "FuzzObject::commit() - shared_barq->is_in_transaction();\n";
    if (shared_barq->is_in_transaction()) {
        log << "FuzzObject::commit() - shared_barq->commit_transaction();\n";
        shared_barq->commit_transaction();
        auto& group = shared_barq->read_group();
        BARQ_DO_IF_VERIFY(log, group.verify());
    }
}

void FuzzObject::rollback(SharedBarq shared_barq, Group& group, FuzzLog& log)
{
    log << "FuzzObject::rollback()\n";
    if (!shared_barq->is_in_async_transaction() && !shared_barq->is_in_transaction()) {
        shared_barq->begin_transaction();
        BARQ_DO_IF_VERIFY(log, group.verify());
        log << "shared_barq->cancel_transaction();\n";
        shared_barq->cancel_transaction();
        BARQ_DO_IF_VERIFY(log, shared_barq->read_group().verify());
    }
}

void FuzzObject::advance(barq::SharedBarq shared_barq, FuzzLog& log)
{
    log << "FuzzObject::advance();\n";
    shared_barq->notify();
}

void FuzzObject::close_and_reopen(SharedBarq& shared_barq, FuzzLog& log, const Barq::Config& config)
{
    log << "Open/close barq\n";
    shared_barq->close();
    shared_barq.reset();
    shared_barq = Barq::get_shared_barq(config);
    log << "Verify group after barq got reopened\n";
    auto& group = shared_barq->read_group();
    BARQ_DO_IF_VERIFY(log, group.verify());
}
