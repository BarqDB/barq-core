                           ...
                           const std::string& path)
{
    ...
    try {
        ...
        BARQ_ASSERT(&fuzzer == &cnf.get_fuzzer());
        do_fuzz(cnf);
    }
...
{
    const auto path = cnf.get_barq_path();
    auto& log = cnf.get_logger();
    auto& state = cnf.get_state();
    auto& fuzzer = cnf.get_fuzzer();
    auto shared_barq = Barq::get_shared_barq(cnf.get_config());
    ...
    auto begin_write = [&log](SharedBarq shared_barq) -> Group& {
        log << "begin_write() - check : shared_barq->is_in_transaction()\n";
        if (!shared_barq->is_in_transaction() && !shared_barq->is_in_async_transaction()) {
            log << "begin_write() - open transaction : shared_barq->begin_transaction()\n";
            try {
        ...
        log << "begin_write() - return shared_barq->read_group();\n";
        return shared_barq->read_group();
    };
    ...
    for (;;) {
        ...
        try {
            Group& group = begin_write(shared_barq);
            if (instr == Add_Table && group.size() < max_tables) {
                ...
                fuzzer.commit(shared_barq, log);
            }
            else if (instr == Rollback) {
                fuzzer.rollback(shared_barq, group, log);
            }
            else if (instr == Advance) {
                fuzzer.advance(shared_barq, log);
            }
            else if (instr == Close_And_Reopen) {
                fuzzer.close_and_reopen(shared_barq, log, cnf.get_config());
            }
