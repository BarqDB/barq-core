...
FuzzConfigurator::FuzzConfigurator(FuzzObject& fuzzer, const std::string& input, bool use_input_file,
    ...
    , m_fuzz_name(name)
{
    barq::disable_sync_to_disk();
    init(input);
    setup_barq_config();
}

void FuzzConfigurator::setup_barq_config()
{
    m_config.path = m_path;
    m_config.schema_version = 0;
    m_config.scheduler = barq::util::Scheduler::make_dummy();
    if (m_use_encryption) {
...
const barq::Barq::Config& FuzzConfigurator::get_config() const
{
    return m_config;
}
...
const std::string& FuzzConfigurator::get_barq_path() const
{
    return m_path;
}
...
void FuzzConfigurator::init(const std::string& input)
{
    ...
    barq::test_util::BarqPathInfo test_context{db_name};
...
void FuzzConfigurator::print_cnf()
{
    ...
    m_log << "// Test case generated in " BARQ_VER_CHUNK " on " << m_fuzzer.get_current_time_stamp() << ".\n";
    m_log << "// BARQ_MAX_BPNODE_SIZE is " << BARQ_MAX_BPNODE_SIZE << "\n";
    m_log << "// ----------------------------------------------------------------------\n";
