...
TEST(Links_SetLinkLogicErrors)
{
    ...
    CHECK_THROW(obj.set(col0, ObjKey(10)), barq::InvalidTableRef);
}
...
TEST(Links_ClearColumnWithTwoLevelBptree)
{
    ...
    std::vector<ObjKey> keys;
    origin->create_objects(BARQ_MAX_BPNODE_SIZE + 1, keys);
    origin->clear();
...
TEST(Links_ClearLinkListWithTwoLevelBptree)
{
    ...
    for (size_t i = 0; i < BARQ_MAX_BPNODE_SIZE + 1; ++i)
        ll.add(k);
    ll.clear();
    group.verify();
}
...
TEST(Links_CascadeRemove_ColumnLink)
{
    ...
    {
        Fixture f;
        f.get_origin_obj(0).set(f.col_link, null_key); // origin[0].o_1 -> barq::null()
    ...
    {
        Fixture f;
        f.get_origin_obj(1).set(f.col_link, null_key); // origin[1].o_1 -> barq::null()
    ...
    {
        Fixture f;
        f.get_origin_obj(2).set(f.col_link, null_key); // origin[0].o_1 -> barq::null()
