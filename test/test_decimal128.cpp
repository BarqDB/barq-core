...
TEST(Decimal_Array)
{
    ...
    arr.create();
    arr.add(Decimal128(barq::null()));
    CHECK_EQUAL(arr.get_width(), 0);
    CHECK_EQUAL(arr.get(0), Decimal128(barq::null()));
    CHECK(arr.is_null(0));
    arr.add(Decimal128());
    ...
    CHECK_EQUAL(arr.get_width(), 4);
    CHECK_EQUAL(arr.get(0), Decimal128());
    CHECK_EQUAL(arr.get(1), Decimal128(barq::null()));
    CHECK_NOT(arr.is_null(0));
    CHECK(arr.is_null(1));
    ...
    CHECK_EQUAL(arr.find_first(Decimal128(barq::null())), 3);
    ...
    CHECK_EQUAL(arr1.get(1), Decimal128(barq::null()));
    ...
    CHECK_EQUAL(arr.get(2), Decimal128(barq::null()));
    CHECK_EQUAL(arr1.get(0), Decimal128(str1));
...
TEST(Decimal_Query)
{
    ...
    }
    {
        ...
        Query q2 = table->column<Decimal>(col) == barq::null();
        CHECK_EQUAL(q2.count(), 1);
        q2 = table->where().equal(col, barq::null());
        CHECK_EQUAL(q2.count(), 1);
