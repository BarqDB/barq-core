...
TEST(Link_FindNullLink)
{
    ...
    */
    ...
    // We have not yet defined behaviour of finding barq::null()-links in a linked-to table, so we just throw. Todo.
...
TEST(LinkList_QueryLinkNull)
{
    ...
    CHECK_EQUAL(1, data_table->where().and_query(data_table->column<String>(c0) == barq::null()).count());
    CHECK_EQUAL(2, data_table->where().and_query(data_table->column<String>(c0) != barq::null()).count());

    CHECK_EQUAL(1, data_table->where().and_query(data_table->column<Int>(c2) == barq::null()).count());
    CHECK_EQUAL(1, data_table->where().and_query(data_table->column<Double>(c3) == barq::null()).count());
    CHECK_EQUAL(1, data_table->where().and_query(data_table->column<Timestamp>(c4) == barq::null()).count());

    CHECK_EQUAL(2, data_table->where().and_query(data_table->link(c1).column<String>(c0) == barq::null()).count());
    CHECK_EQUAL(1, data_table->where().and_query(data_table->link(c1).column<String>(c0) != barq::null()).count());
    CHECK_EQUAL(o0.get_key(), data_table->where()
                                  .and_query(data_table->link(c1).column<String>(c0) != barq::null())
                                  .find_all()
                                  .get_key(0));

    CHECK_EQUAL(2, data_table->where().and_query(data_table->link(c1).column<Int>(c2) == barq::null()).count());
    CHECK_EQUAL(1, data_table->where().and_query(data_table->link(c1).column<Int>(c2) != barq::null()).count());

    CHECK_EQUAL(2, data_table->where().and_query(data_table->link(c1).column<Double>(c3) == barq::null()).count());
    CHECK_EQUAL(1, data_table->where().and_query(data_table->link(c1).column<Double>(c3) != barq::null()).count());

    CHECK_EQUAL(2,
                data_table->where().and_query(data_table->link(c1).column<Timestamp>(c4) == barq::null()).count());
    CHECK_EQUAL(1,
                data_table->where().and_query(data_table->link(c1).column<Timestamp>(c4) != barq::null()).count());

    CHECK_EQUAL(2,
                data_table->where().and_query(data_table->link(c1).column<String>(c0).equal(barq::null())).count());
    CHECK_EQUAL(
        1, data_table->where().and_query(data_table->link(c1).column<String>(c0).not_equal(barq::null())).count());

    CHECK_EQUAL(2, data_table->where()
                       .Not()
                       .and_query(data_table->link(c1).column<String>(c0).not_equal(barq::null()))
                       .count());
    ...
    CHECK_EQUAL(
        1, data_table->where().Not().and_query(data_table->link(c1).column<String>(c0).equal(barq::null())).count());
    CHECK_EQUAL(o0.get_key(), data_table->where()
                                  .Not()
                                  .and_query(data_table->link(c1).column<String>(c0).equal(barq::null()))
                                  .find_all()
                                  .get_key(0));

    CHECK_EQUAL(1, (data_table->column<Link>(c1) == barq::null()).count());
    CHECK_EQUAL(2, (data_table->column<Link>(c1) != barq::null()).count());
}
...
TEST(LinkList_QueryOnIndexedPropertyOfLinkListMultipleMatches)
{
    ...
    const size_t count = round_up(std::max(BARQ_MAX_BPNODE_SIZE * 8, 100), 4);
