...
TEST_IF(Transactions_ConcurrentFrozenQueryAndObjAndTransactionClose, !BARQ_TSAN)
{
    SHARED_GROUP_TEST_PATH(path);
    std::unique_ptr<Replication> hist_w(make_in_barq_history());
    DBRef db = DB::create(*hist_w, path);
...
public:
                        ...
                        BinaryIterator* buffer) const noexcept override
    {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            BARQ_ASSERT(j != m_changesets.end());
            const ChangeSet& changeset = j->second;
            BARQ_ASSERT(changeset.finalized); // Must have been finalized
...
TEST(LangBindHelper_AdvanceReadTransact_AddTableWithFreshSharedGroup)
{
    ...
    // Add the first table
    {
        std::unique_ptr<Replication> hist_w(barq::make_in_barq_history());
    ...
    std::unique_ptr<Replication> hist(barq::make_in_barq_history());
    DBRef sg = DB::create(*hist, path, DBOptions(crypt_key()));
    TransactionRef rt = sg->start_read();
...
NONCONCURRENT_TEST_IF(LangBindHelper_AdvanceReadTransact_CreateManyTables, testing_supports_spawn_process)
{
    ...
    if (process->is_child()) {
        ...
        {
            std::unique_ptr<Replication> hist_w2(barq::make_in_barq_history());
...
NONCONCURRENT_TEST_IF(LangBindHelper_AdvanceReadTransact_RemoveTableWithColumns, testing_supports_spawn_process)
{
    SHARED_GROUP_TEST_PATH(path);
    if (test_util::SpawnedProcess::is_parent()) {
        std::unique_ptr<Replication> hist_parent(make_in_barq_history());
...
TEST(LangBindHelper_AdvanceReadTransact_IntIndex)
{
    ...
    target->create_objects(BARQ_MAX_BPNODE_SIZE + 1, obj_keys);
...
TEST(LangBindHelper_RollbackAndContinueAsRead_IntIndex)
{
    ...
    std::vector<ObjKey> keys;
    target->create_objects(BARQ_MAX_BPNODE_SIZE + 1, keys);
    g->commit_and_continue_as_read();
    g->promote_to_write();
...
TEST(LangBindHelper_ImplicitTransactions_OverSharedGroupDestruction)
{
    ...
    std::unique_ptr<Replication> hist1(make_in_barq_history());
    {
    ...
    {
        std::unique_ptr<Replication> hist2(make_in_barq_history());
...
void multiple_trackers_reader_thread(TestContext& test_context, DBRef db)
{
    ...
    while (tc->size() == 0) {
        ...
        if (std::chrono::steady_clock::now() - wait_start > max_wait_seconds) {
            ...
            BARQ_ASSERT(reader_process_timed_out);
        }
    }
}
...
TEST(LangBindHelper_SubqueryHandoverDependentViews)
{
    ...
    ColKey col1;
    {
        ...
        {
            barq::TableView tv = qq2->equal(col1, true).find_all();
    ...
    while (true) {
        ...
        for (size_t r = 0; r < num_rows; ++r) {
            ...
            if (c == "stop reading") {
                ...
                BARQ_ASSERT_EX(c.size() == 1, c.size());
            }
            BARQ_ASSERT_EX(t->get_name() == StringData("class_Table_Emulation_Name"), t->get_name().data());
            BARQ_ASSERT_EX(t->get_column_name(0) == StringData("count"), t->get_column_name(0).data());
            BARQ_ASSERT_EX(t->get_column_name(1) == StringData("char"), t->get_column_name(1).data());
            BARQ_ASSERT_EX(t->get_column_name(2) == StringData("payload"), t->get_column_name(2).data());
            ...
            BARQ_ASSERT_EX(s.size() == validator.size(), r, s.size(), validator.size());
            for (size_t i = 0; i < s.size(); ++i) {
                BARQ_ASSERT_EX(s[i] == validator[i], r, i, s[i], validator[i]);
            }
            BARQ_ASSERT_EX(s == validator, r, s.size(), validator.size());
        }
    }
}
...
// in with many reader writer threads on an encrypted barq and it is
...
TEST_IF(Thread_AsynchronousIODataConsistency, false)
{
    ...
    const int num_rows = 200; // 2 + BARQ_MAX_BPNODE_SIZE;
    const char* key = crypt_key(true);
    std::unique_ptr<Replication> hist(make_in_barq_history());
    DBRef sg = DB::create(*hist, path, DBOptions(key));
    {
...
TEST(LangBindHelper_HandoverWithLinkQueries)
{
    ...
    barq::Query query = table1->link(col_link2).column<String>(col_str) == "nabil";
    barq::TableView tv4 = query.find_all();
    ...
    CHECK_EQUAL(1, query.count());
    {
        ...
        barq::TableView tv2 = q2->find_all();
        CHECK_EQUAL(0, tv2.size());
    }
}
...
TEST(LangBindHelper_HandoverQuerySubQuery)
{
    ...
    std::unique_ptr<Query> query;
    {
        ...
        barq::Query query_2 = source->column<Link>(col_link, target->column<String>(col_name) == "C").count() == 1;
        reader = group_w->duplicate();
...
TEST(LangBindHelper_CompactLargeEncryptedFile)
{
    ...
    std::vector<char> data(barq::util::page_size());
    const size_t N = 32;
...
TEST_IF(LangBindHelper_HandoverFuzzyTest, TEST_DURATION > 0)
{
    ...
    ColKey c0, c1, c2, c3;
    {
        ...
        rt->verify();
        {
            barq::Query query = dog->link(c3).column<String>(c0) == "owner" + to_string(rand() % numberOfOwner);
...
TEST(LangBindHelper_SessionHistoryConsistency)
{
    ...
    // When starting with an empty Barq, all history types are allowed, but all
...
TEST(LangBindHelper_InBarqHistory_Upgrade)
{
...
TEST(LangBindHelper_InBarqHistory_Downgrade)
{
    SHARED_GROUP_TEST_PATH(path);
    {
        // In-Barq history
...
TEST(LangBindHelper_Bug2321)
{
    ...
    ColKey col;
    {
        ...
        target->create_objects(BARQ_MAX_BPNODE_SIZE + 2, target_keys);
        TableRef origin = group.add_table("origin");
...

    {
        ...
        for (i = 0; i < (BARQ_MAX_BPNODE_SIZE - 1); i++) {
...
TEST(LangBindHelper_OpenAsEncrypted)
{
    ...
    {
        const char* key = crypt_key(true);
        std::unique_ptr<Replication> hist_encrypt(make_in_barq_history());
...
// Test case generated in [barq-core-4.0.4] on Mon Dec 18 13:33:24 2017.
...
{
    SHARED_GROUP_TEST_PATH(path);
    auto callback = [&](const std::string& barq_path) {
        CHECK(barq_path.compare(path) == 0);
    };
...
{
    DBRef sg = DB::create(make_in_barq_history());
    ColKey col;
    auto rt = sg->start_read();
    {
        auto wt = sg->start_write();
