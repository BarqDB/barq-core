...
struct value_copier<Optional<T1>, T2> {
    ...
    T2 operator()(Optional<T1> from_value, bool)
    {
        if (bool(from_value))
            ...
            if (m_throw_on_null)
                throw barq::LogicError(ErrorCodes::BrokenInvariant, "Null found");
            else
...
{
    // Tests against various aggregate bugs on TableViews: https://github.com/barq/barq-core/pull/2360
    {
        Table table;
...
TEST_TYPES(Table_SortFloat, float, double, Decimal128)
{
    ...
    for (size_t i = 0; i < keys.size(); i += 3) {
        ...
        table.get_object(keys[i + 2]).set(col, barq::nan<TEST_TYPE>(nan_tag));
    }
    ...
    for (size_t i = 300; i < 600; ++i) {
        CHECK(barq::isnan(sorted.get_object(i).get<TEST_TYPE>(col)));
    }
...
TEST(Table_NullInEnum)
{
    ...
    obj50.set<String>(col, barq::null());
    r = table->where().equal(col, "hello").count();
    CHECK_EQUAL(99, r);
    ...
    r = table->where().equal(col, barq::null()).count();
    CHECK_EQUAL(1, r);

    table->get_object(ObjKey(55)).set(col, barq::null());
    r = table->where().equal(col, barq::null()).count();
    CHECK_EQUAL(2, r);
...
#if TEST_DURATION > 0
#define TBL_SIZE BARQ_MAX_BPNODE_SIZE * 10
#else
#define TBL_SIZE 10
#endif // TEST_DURATION
...
TEST(Table_AddColumnWithThreeLevelBptree)
{
    ...
    table.create_objects(BARQ_MAX_BPNODE_SIZE * BARQ_MAX_BPNODE_SIZE + 1, keys);
...
TEST(Table_ClearWithTwoLevelBptree)
{
    ...
    table.create_objects(BARQ_MAX_BPNODE_SIZE + 1, keys);
    table.clear();
    table.verify();
}
...
TEST(Table_NullableChecks)
{
    ...
    BinaryData bd; // null
    obj.set(str_col, sd);
    obj.set(int_col, barq::null());
    obj.set(bool_col, barq::null());
    obj.set(ts_col, ts);
    obj.set(float_col, barq::null());
    obj.set(double_col, barq::null());
...
TEST(Table_Nulls)
{
    ...
    for (size_t round = 0; round < 5; round++) {
        ...
        CHECK_EQUAL(1, t.count_string(col_str, barq::null()));

        CHECK_EQUAL(keys[0], t.find_first_string(col_str, "foo"));
        CHECK_EQUAL(keys[1], t.find_first_string(col_str, ""));
        CHECK_EQUAL(keys[2], t.find_first_string(col_str, barq::null()));
        ...
        tv = t.find_all_string(col_str, barq::null());
        CHECK_EQUAL(1, tv.size());
...
TEST(Table_ObjectsWithNoColumns)
{
    Table table;
    std::vector<ObjKey> keys;
    table.create_objects(BARQ_MAX_BPNODE_SIZE * 2, keys);
    CHECK_NOT(table.is_empty());
    CHECK_EQUAL(table.size(), BARQ_MAX_BPNODE_SIZE * 2);
    for (ObjKey k : keys) {
...
{
    // This test works best for BARQ_MAX_BPNODE_SIZE == 8.
    // To be used mostly as a help when debugging new implementation

    int nb_rows = BARQ_MAX_BPNODE_SIZE * 8;
    Table table;
...
TEST(Table_QueryNullOnNonNullSearchIndex)
{
    ...
    {
        Query q0 = t->column<Int>(col0) == barq::null();
        CHECK_EQUAL(q0.count(), 0);
        Query q1 = t->link(col_link).column<Int>(col0) == barq::null();
        CHECK_EQUAL(q1.count(), 0);
        Query q2 = t->link(col_link).link(col_link).column<Int>(col0) == barq::null();
           ...
           NullableIndexed<Timestamp>, NullableIndexed<StringData>, NullableIndexed<UUID>)
{
    ...
    {
        Query q0 = t->column<underlying_type>(col0) == barq::null();
        CHECK_EQUAL(q0.count(), num_nulls);
        Query q1 = t->link(col_link).column<underlying_type>(col0) == barq::null();
        CHECK_EQUAL(q1.count(), num_nulls);
        Query q2 = t->link(col_link).link(col_link).column<underlying_type>(col0) == barq::null();
...
struct Tester {
    ...
    static void run(DBRef db, barq::DataType type)
    {
    ...
    {
        std::string s = barq::util::to_string(fastrand(5));
        return s;
    }
    template <typename Type = T2>
...
template <typename T>
void test_lists(TestContext& test_context, DBRef sg, const barq::DataType type_id, bool optional = false)
{
...
template <typename T>
void test_tables(TestContext& test_context, DBRef sg, const barq::DataType type_id, bool optional = false)
{
    auto t = sg->start_write();
    auto table = t->add_table("the_table");
...
void test_dynamic_conversion(TestContext& test_context, DBRef sg, barq::DataType type_id, bool from_nullable,
                             bool to_nullable)
{
...
template <typename TFrom, typename TTo>
void test_dynamic_conversion_list(TestContext& test_context, DBRef sg, barq::DataType type_id, bool from_nullable,
                                  bool to_nullable)
{
    // Create values of type TFrom and ask for dynamic conversion to TTo
    auto t = sg->start_write();
...
template <typename T>
void test_dynamic_conversion_combi(TestContext& test_context, DBRef sg, barq::DataType type_id)
{
...
template <typename T>
void test_dynamic_conversion_combi_sametype(TestContext& test_context, DBRef sg, barq::DataType type_id)
{
...
template <typename T>
void test_dynamic_conversion_list_combi(TestContext& test_context, DBRef sg, barq::DataType type_id)
{
...
template <typename T>
void test_dynamic_conversion_list_combi_sametype(TestContext& test_context, DBRef sg, barq::DataType type_id)
{
...
TEST(Table_LoggingMutations)
{
    ...
    options.logger->set_level_threshold("Barq", util::Logger::Level::all);
