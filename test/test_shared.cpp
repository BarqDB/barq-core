...
#include <barq/impl/copy_replication.hpp>
...
DWORD winfork(std::string unit_test_name)
{
    ...
    if (success == 0 || success == MAX_PATH) {
        ...
        BARQ_ASSERT_EX(false, err, MAX_PATH, filename);
    }

    GetModuleFileName(nullptr, filename, MAX_PATH);

    std::string environment;
    environment.append("BARQ_SPAWNED=1");
...
#if !defined(_WIN32) && !BARQ_ENABLE_ENCRYPTION
...
#if !defined(_WIN32) && !BARQ_ENABLE_ENCRYPTION && !BARQ_ANDROID
...
TEST_IF(Shared_PipelinedWritesWithKills, false)
{
    ...
    // grow a Barq file to gigantic sizes over time (100 gigabytes per 20
    // minutes).
    //
...
// This unit test will test the case where the .barq file exceeds the available disk space. To run it, do
// following:
//
// 1: Create a drive that has around 10 MB free disk space *after* the barq-tests binary has been copied to it
// (you can fill up the drive with random data files until you hit 10 MB).
//
// Repeatedly run the barq-tests binary in a loop, like from a bash script. You can even make the bash script
// invoke `pkill barq-tests` with some intervals to test robustness too (if so, start the unit tests with `&`,
// i.e. `barq-tests&` so it runs in the background.

ONLY(Shared_DiskSpace)
{
    for (;;) {
        if (!File::exists("x")) {
            File f("x", barq::util::File::mode_Write);
            f.write(std::string(18 * 1024 * 1024, 'x'));
            f.close();
        }

        std::string path = "test.barq";
...
TEST(Shared_ReadAfterCompact)
{
    SHARED_GROUP_TEST_PATH(path);
    {
        DBRef sg = DB::create(make_in_barq_history(), path);
        WriteTransaction wt(sg);
    ...
    }
    {
        ...
        DBRef sg = DB::create(make_in_barq_history(), path, options);
...
TEST(Shared_InitialMem_StaleFile)
{
    ...
    // Create a MemOnly barq at the path so that a lock file gets initialized
...
#elif BARQ_DEBUG // this test is disproportionately slower in debug
    ...
    auto reader = [path_str]() {
        ...
        try {
            ...
            for (int i = 0; i < 1000; ++i) {
                ...
                BARQ_ASSERT(t->get_object(0).get<StringData>(col_key) == "string");
                rt.get_group().verify();
            }
        }
            ...
            BARQ_ASSERT_EX(unexpected_exception, e.what());
        }
    };
...
TEST(Shared_WritesSpecialOrder)
{
    ...
    const int num_rows =
        5; // FIXME: Should be strictly greater than BARQ_MAX_BPNODE_SIZE, but that takes too long time.
...
#if !BARQ_ENABLE_ENCRYPTION && defined(ENABLE_ROBUST_AGAINST_DEATH_DURING_WRITE)
...
// not ios or android
// #endif // defined TEST_ROBUSTNESS && defined ENABLE_ROBUST_AGAINST_DEATH_DURING_WRITE && !BARQ_ENABLE_ENCRYPTION
...
TEST_IF(Shared_StringIndexBug1, TEST_DURATION >= 1)
{
...

    {
        ...
        for (int i = 0; i < BARQ_MAX_BPNODE_SIZE + 1; ++i)
            table->create_object();
        for (int i = 0; i < BARQ_MAX_BPNODE_SIZE + 1; ++i)
...
{
    // We can't use SHARED_GROUP_TEST_PATH() because it will attempt to clean up the .barq file at the end,
    // and hence throw if the other processstill has the .barq file open
    std::string path = get_test_path("Shared_InterprocessWaitForChange", ".barq");
    ...
    // An old .barq file with random contents can exist (such as a leftover from earlier crash) with random
...
/*
#if !defined(_WIN32)
// Check what happens when Barq cannot allocate more virtual memory
...
NONCONCURRENT_TEST(Shared_OutOfMemory)
{
    ...
    // Attempt to open Barq, should fail because we hold too much already.
    try {
    ...
    // Barq should succeed to open now.
...
NONCONCURRENT_TEST(Shared_StaticFuzzTestRunSanityCheck)
{
    ...
    // a very simple fuzz test that just uses a random generator for generating Barq actions.
    std::string filename = "";
...
TEST_IF(Shared_CompactEncrypt, BARQ_ENABLE_ENCRYPTION)
{
    SHARED_GROUP_TEST_PATH(path);
...
NONCONCURRENT_TEST(Shared_TopSizeNotEqualNine)
{
    ...
    BARQ_ASSERT_RELEASE(sg->compact());
    DBRef sg2 = DB::create(path, DBOptions(crypt_key()));
    {
...
TEST(Shared_Bptree_insert_failure)
{
    ...
    writer->commit();
    BARQ_ASSERT_RELEASE(sg_w->compact());
#if 0
    {
...
TEST(Shared_LockFileOfWrongSizeThrows)
{
    ...
    std::thread t([&]() {
        ...
        File::Map<char> mem(f, barq::util::File::access_ReadWrite, 1);
...
#if !BARQ_ANDROID // FIXME
TEST_IF(Shared_DecryptExisting, BARQ_ENABLE_ENCRYPTION)
{
    // Page size of system that reads the .barq file must be the same as on the system
    ...
    std::string path = test_util::get_test_resource_path() + "test_shared_decrypt_" +
                       barq::util::to_string(page_size() / 1024) + "k_page.barq";

#if 0 // set to 1 to generate the .barq file
    {
...
{
    SHARED_GROUP_TEST_PATH(path);
    std::unique_ptr<Replication> hist_r(make_in_barq_history());
    std::unique_ptr<Replication> hist_w(make_in_barq_history());
...
{
    // Test case generated in [barq-core-6.0.0-rc1] on Wed Apr 11 16:08:05 2018.
    // BARQ_MAX_BPNODE_SIZE is 4
    // ----------------------------------------------------------------------
    SHARED_GROUP_TEST_PATH(path);
...
{
    // Test case generated in [barq-core-6.0.0-alpha.0] on Mon Aug 13 14:43:06 2018.
    // BARQ_MAX_BPNODE_SIZE is 1000
    // ----------------------------------------------------------------------
    SHARED_GROUP_TEST_PATH(path);
    std::unique_ptr<Replication> hist_w(make_in_barq_history());
    DBRef db_w = DB::create(*hist_w, path);
...
#if !BARQ_ANDROID // FIXME
TEST_IF(Shared_MoreVersionsInUse, BARQ_ENABLE_ENCRYPTION)
{
    SHARED_GROUP_TEST_PATH(path);
...
TEST_IF(Shared_LinksToSameCluster, BARQ_ENABLE_ENCRYPTION)
{
...
TEST_IF(Shared_LargeFile, TEST_DURATION > 0 && !BARQ_ANDROID)
{
    SHARED_GROUP_TEST_PATH(path);
    DBOptions options;
...
TEST(Shared_MultipleDBInstances)
{
    ...
    auto hist1 = make_in_barq_history();
    DBRef db1 = DB::create(*hist1, path);
    auto hist2 = make_in_barq_history();
...
TEST(Shared_CompareGroups)
{
    ...
    DBRef db1 = DB::create(make_in_barq_history(), path1);
    DBRef db2 = DB::create(make_in_barq_history(), path2);
...
TEST(Shared_WriteTo)
{
    ...
    // The difference between the two barqs should now be that the remote db has an
    // extra baa object with pk 333.
    CHECK_NOT(*tr == *dest);
    tr->promote_to_write();
    // So if we add this to the local barq, they should match
...
{
    auto path = barq::test_util::get_test_path(test_context.get_test_name(), ".test-dir");
...
TEST(Shared_ClearOnError_CannotClearWhileFileIsOpen)
{
    ...
    DBOptions options;
    options.clear_on_invalid_file = true;
    auto db = DB::create(make_in_barq_history(), path, options);
    options.encryption_key = crypt_key(true);
    CHECK_THROW(DB::create(make_in_barq_history(), path, options), InvalidDatabase);
}
#endif
