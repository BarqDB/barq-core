...
TEST(Util_Uri_Basics)
{
    // normal uri
    {
        const std::string input = "http://www.barq.io/foo?bar#zob";
        auto u = Uri(input);

        CHECK_EQUAL(u.get_scheme(), "http:");
        CHECK_EQUAL(u.get_auth(), "//www.barq.io");
...

        {
            ...
            CHECK(port.empty());
            CHECK_EQUAL(host, "www.barq.io");
        }
    }

    // complex authority
    {
        const std::string input = "http://myuser:mypass@www.barq.io:12345/foo?bar#zob";
        auto u = Uri(input);

        CHECK_EQUAL(u.get_scheme(), "http:");
        CHECK_EQUAL(u.get_auth(), "//myuser:mypass@www.barq.io:12345");
        CHECK_EQUAL(u.get_path(), "/foo");
        CHECK_EQUAL(u.get_query(), "?bar");
        CHECK_EQUAL(u.get_frag(), "#zob");
        CHECK_EQUAL(u.recompose(), input);
...
TEST(Util_UriPercentEncoding_1)
{
    ...
    BARQ_ASSERT(unescaped_strings.size() == escaped_strings.size());
