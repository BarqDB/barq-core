...
class CaptureHelper {
public:
    CaptureHelper(TransactionRef group, SharedBarq const& r, LnkLst& lv, TableKey table_key)
        : m_barq(r)
        , m_group(group)
        , m_list(lv)
    ...
    {
        return m_barq->is_in_transaction();
    }
...
#define VALIDATE_CHANGES(out)                                                                                        \
    ...
    SECTION("object change information") {
        ...
        auto origin = barq->read_group().get_table("class_origin");
        auto target = barq->read_group().get_table("class_target");
        ...
        barq->read_group().get_table("class_origin 2")->create_object_with_primary_key(48);
        ...
        auto observe = [&](std::initializer_list<Obj> rows, auto&& fn) {
            ...
            observer.barq = barq2;
            barq2->m_binding_context.reset(&observer);
            ...
            barq2->refresh();
            barq2->m_binding_context.release();
        ...
        SECTION("array: modifying different table does not produce changes") {
            auto changes = observe({o}, [&] {
                barq->read_group().get_table("class_origin 2")->begin()->get_linklist("array").add(target_keys[0]);
            });
            REQUIRE_FALSE(changes.modified(0, target_cols[2]));
        }
...
namespace cf = barq::collection_fixtures;
...
TEST_CASE("DeepChangeChecker singular links", "[notifications]") {
    ...
    auto get_link = [&](Obj obj, ColKey col) -> ObjLink {
        ...
        BARQ_ASSERT(col.get_type() == col_type_Link);
