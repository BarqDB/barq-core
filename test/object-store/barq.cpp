...
public:
    ...
    static void begin_read(SharedBarq const& shared_barq, VersionID version)
    {
        Barq::Internal::begin_read(*shared_barq, version);
    }
};
...
TEST_CASE("SharedBarq: get_shared_barq()") {
    TestFile config;
    ...
    SECTION("should return the same instance when caching is enabled") {
        config.cache = true;
        auto barq1 = Barq::get_shared_barq(config);
        auto barq2 = Barq::get_shared_barq(config);
        REQUIRE(barq1.get() == barq2.get());
    }
    ...
    SECTION("should validate that the config is sensible") {
        SECTION("bad encryption key") {
            ...
            REQUIRE_EXCEPTION(Barq::get_shared_barq(config), InvalidEncryptionKey,
                              "Encryption key must be 64 bytes.");
        }

        SECTION("schema without schema version") {
            config.schema_version = ObjectStore::NotVersioned;
            REQUIRE_EXCEPTION(Barq::get_shared_barq(config), IllegalCombination,
        ...
        SECTION("migration function for immutable") {
            ...
            REQUIRE_EXCEPTION(Barq::get_shared_barq(config), IllegalCombination,
                              "Barqs opened in immutable mode do not use a migration function");
        }
        ...
        SECTION("migration function for read-only") {
            ...
            config.migration_function = [](auto, auto, auto) {};
            REQUIRE_EXCEPTION(Barq::get_shared_barq(config), IllegalCombination,
                              "Barqs opened in read-only mode do not use a migration function");
        }
        ...
        SECTION("migration function for additive discovered") {
            ...
            config.migration_function = [](auto, auto, auto) {};
            REQUIRE_EXCEPTION(Barq::get_shared_barq(config), IllegalCombination,
                              "Barqs opened in Additive-only schema mode do not use a migration function");
        }
        ...
        SECTION("initialization function for immutable") {
            ...
            REQUIRE_EXCEPTION(Barq::get_shared_barq(config), IllegalCombination,
                              "Barqs opened in immutable mode do not use an initialization function");
        }
        ...
        SECTION("initialization function for read-only") {
            ...
            config.initialization_function = [](auto) {};
            REQUIRE_EXCEPTION(Barq::get_shared_barq(config), IllegalCombination,
                              "Barqs opened in read-only mode do not use an initialization function");
        }
        SECTION("in-memory encrypted barqs are rejected") {
            config.in_memory = true;
            config.encryption_key = make_test_encryption_key();
            REQUIRE_EXCEPTION(Barq::get_shared_barq(config), IllegalCombination,
                              "Encryption is not supported for in-memory barqs");
        }
    }
    ...
    SECTION("should reject mismatched config") {
        ...
        SECTION("schema version") {
            ...
            REQUIRE_EXCEPTION(
                Barq::get_shared_barq(config), MismatchedConfig,
                Catch::Matchers::Matches("Barq at path '.*' already opened with different schema version."));

            config.schema = util::none;
            config.schema_version = ObjectStore::NotVersioned;
            REQUIRE_NOTHROW(Barq::get_shared_barq(config));
        }
        ...
        SECTION("schema mode") {
            ...
            REQUIRE_EXCEPTION(
                Barq::get_shared_barq(config), MismatchedConfig,
                Catch::Matchers::Matches("Barq at path '.*' already opened with a different schema mode."));
        }
        ...
        SECTION("durability") {
            ...
            REQUIRE_EXCEPTION(
                Barq::get_shared_barq(config), MismatchedConfig,
                Catch::Matchers::Matches("Barq at path '.*' already opened with different inMemory settings."));
        }
        ...
        SECTION("schema") {
            ...
            REQUIRE_EXCEPTION(
                Barq::get_shared_barq(config), SchemaMismatch,
...
// Windows doesn't use fifos
    ...
    SECTION("should be able to set a FIFO fallback path") {
        ...
        barq::util::try_make_dir(fallback_dir);
        TestFile config;
        ...
        barq::util::make_dir(config.path + ".note");
        auto barq = Barq::get_shared_barq(config);
        auto fallback_file = util::format("%1barq_%2.note", fallback_dir,
                                          std::hash<std::string>()(config.path)); // Mirror internal implementation
        REQUIRE(util::File::exists(fallback_file));
        barq::util::remove_dir(config.path + ".note");
        REQUIRE(barq::util::try_remove_dir_recursive(fallback_dir));
    }
    ...
    SECTION("automatically append dir separator to end of fallback path") {
        ...
        barq::util::make_dir(config.path + ".note");
        auto barq = Barq::get_shared_barq(config);
        auto fallback_file = util::format("%1/barq_%2.note", fallback_dir,
                                          std::hash<std::string>()(config.path)); // Mirror internal implementation
        REQUIRE(util::File::exists(fallback_file));
        barq::util::remove_dir(config.path + ".note");
        REQUIRE(barq::util::try_remove_dir_recursive(fallback_dir));
    ...
    SECTION("should verify that the schema is valid") {
        ...
        REQUIRE_THROWS_CONTAINING(Barq::get_shared_barq(config), "origin of linking objects property");
    }

    SECTION("should apply the schema if one is supplied") {
        Barq::get_shared_barq(config);
        ...
        config.migration_function = [&](SharedBarq old_barq, SharedBarq new_barq, Schema&) {
            migration_called = true;
            REQUIRE_FALSE(old_barq->auto_refresh());
            REQUIRE(ObjectStore::table_for_object_type(old_barq->read_group(), "object")->get_column_count() == 1);
            REQUIRE(ObjectStore::table_for_object_type(new_barq->read_group(), "object")->get_column_count() == 2);
        };
        Barq::get_shared_barq(config);
    ...
    SECTION("should properly roll back from migration errors") {
        ...
        REQUIRE_THROWS_WITH(Barq::get_shared_barq(config), "error");
        REQUIRE(migration_called);
        REQUIRE_NOTHROW(Barq::get_shared_barq(config));
    }
    ...
    SECTION("should read the schema from the file if none is supplied") {
        ...
        auto it = barq->schema().find("object");
        auto table = barq->read_group().get_table("class_object");
        REQUIRE(it != barq->schema().end());
    ...
    SECTION("should read the proper schema from the file if a custom version is supplied") {
        ...
        auto& db = TestHelper::get_db(barq);
        ...
        REQUIRE(barq->schema().size() == 2);

        config.schema = util::none;
        auto old_barq = Barq::get_shared_barq(config);
        // must retain 'rt' until after opening for reading at that version
        TestHelper::begin_read(old_barq, old_version);
        rt = nullptr;
        REQUIRE(old_barq->schema().size() == 1);
    }
    ...
    SECTION("should sensibly handle opening an uninitialized file without a schema specified") {
        ...
        Barq::Config config_without_schema = config;
        config_without_schema.schema = util::none;
        config_without_schema.schema_version = ObjectStore::NotVersioned;
        auto barq = Barq::get_shared_barq(config_without_schema);
        REQUIRE(barq->schema().empty());
        REQUIRE(barq->schema_version() == ObjectStore::NotVersioned);
        // verify that we can get another Barq instance
        REQUIRE_NOTHROW(Barq::get_shared_barq(config_without_schema));

        // verify that we can also still open the file with a proper schema
        auto barq2 = Barq::get_shared_barq(config);
        REQUIRE_FALSE(barq2->schema().empty());
        REQUIRE(barq2->schema_version() == 1);
    }
    ...
    SECTION("should populate the table columns in the schema when opening as immutable") {
        ...
        SECTION("refreshing an immutable Barq throws") {
            REQUIRE_THROWS_WITH(barq->refresh(), "Can't refresh an immutable Barq.");
        }
    }
    ...
    SECTION("should support using different table subsets on different threads") {
        ...
        auto barq4 = Barq::get_shared_barq(config);

        barq1->refresh();
        barq2->refresh();

        REQUIRE(barq1->schema().size() == 1);
        REQUIRE(barq1->schema().find("object") != barq1->schema().end());
        REQUIRE(barq2->schema().size() == 1);
        REQUIRE(barq2->schema().find("object 2") != barq2->schema().end());
        REQUIRE(barq3->schema().size() == 2);
        REQUIRE(barq3->schema().find("object") != barq3->schema().end());
        REQUIRE(barq3->schema().find("object 2") != barq3->schema().end());
        REQUIRE(barq4->schema().size() == 1);
        REQUIRE(barq4->schema().find("object") != barq4->schema().end());
    }
    ...
    SECTION("should throw when creating the notification pipe fails") {
        ...
        if (auto tmp_dir = DBOptions::get_sys_tmp_dir(); !tmp_dir.empty()) {
            expected_path = util::format("%1barq_%2.note", util::normalize_dir(tmp_dir),
                                         std::hash<std::string>()(config.path)); // Mirror internal implementation
            REQUIRE(util::try_make_dir(expected_path));
        }
        REQUIRE_EXCEPTION(
            Barq::get_shared_barq(config), FileAlreadyExists,
...
#if !BARQ_USE_UV && !TEST_SCHEDULER_UV // uv scheduler does not support background threads
    ...
    SECTION("should detect use of Barq on incorrect thread") {
        auto barq = Barq::get_shared_barq(config);
        JoiningThread([&] {
            REQUIRE_THROWS_MATCHES(barq->verify_thread(), LogicError,
                                   Catch::Matchers::Message("Barq accessed from incorrect thread."));
        });
    }
    ...
    SECTION("can use Barq with explicit scheduler on different thread") {
        config.cache = true;
        config.scheduler = std::make_shared<SimpleScheduler>(1);
        auto barq = Barq::get_shared_barq(config);
        JoiningThread([&] {
            REQUIRE_NOTHROW(barq->verify_thread());
        });
    }
    ...
    SECTION("should not modify the schema when fetching from the cache") {
        ...
        Barq::get_shared_barq(config);
        REQUIRE(object_schema == &*barq->schema().find("object"));
    }

    SECTION("should reuse cached frozen Barq if versions match") {
        config.cache = true;
        auto barq = Barq::get_shared_barq(config);
        barq->read_group();
        ...
        REQUIRE(frozen != barq);
        REQUIRE(barq->read_transaction_version() == frozen->read_transaction_version());

        REQUIRE(barq->freeze() == frozen);
        REQUIRE(Barq::get_frozen_barq(config, barq->read_transaction_version()) == frozen);
    }

    SECTION("should not use cached frozen Barq if versions don't match") {
        config.cache = true;
        auto barq = Barq::get_shared_barq(config);
        barq->read_group();
        auto frozen1 = barq->freeze();
        frozen1->read_group();

        REQUIRE(frozen1 != barq);
        REQUIRE(barq->read_transaction_version() == frozen1->read_transaction_version());
        ...
        auto frozen2 = barq->freeze();
        frozen2->read_group();

        REQUIRE(frozen2 != frozen1);
        REQUIRE(frozen2 != barq);
        REQUIRE(barq->read_transaction_version() == frozen2->read_transaction_version());
        REQUIRE(frozen2->read_transaction_version() > frozen1->read_transaction_version());
    }

    SECTION("frozen barq should have the same schema as originating barq") {
        ...
        auto frozen_schema = frozen_barq->schema();

        REQUIRE(full_schema != subset_schema);
        REQUIRE(barq->schema() == subset_schema);
        REQUIRE(frozen_schema == subset_schema);
    }

    SECTION("frozen barq should have the correct schema even if more properties are added later") {
        config.schema_mode = SchemaMode::AdditiveExplicit;
        auto full_schema = Schema{
        ...
        auto frozen_barq = barq->freeze();
        REQUIRE(barq->schema() == subset_schema);
        REQUIRE(barq2->schema() == full_schema);
        REQUIRE(frozen_barq->schema() == subset_schema);
    }
    ...
    SECTION("freeze with orphaned embedded tables") {
        ...
        barq->read_group();
        auto frozen_barq = barq->freeze();
        REQUIRE(frozen_barq->schema() == schema);
    }
}

TEST_CASE("SharedBarq: schema_subset_mode") {
    TestFile config;
    config.schema_mode = SchemaMode::AdditiveExplicit;
    config.schema_version = 1;
    ...
    auto db = DB::create(make_in_barq_history(), config.path);
    ...
    SECTION("frozen Barq sees the correct schema for each version") {
        config.schema = Schema{{"object", {{"value", PropertyType::Int}}}};
        std::vector<std::shared_ptr<Barq>> barqs;
        for (int i = 0; i < 10; ++i) {
            barqs.push_back(Barq::get_shared_barq(config));
            barqs.back()->read_group();
            auto tr = db->start_write();
        ...
        for (size_t i = 0; i < 10; ++i) {
            ...
            frozen = Barq::get_frozen_barq(config, r.read_transaction_version());
            REQUIRE(frozen->schema().size() == i + 1);
            REQUIRE(frozen->schema_version() == config.schema_version);
        }
        ...
        SECTION("schema not set in config") {
            ...
            for (size_t i = 0; i < 10; ++i) {
                ...
                REQUIRE(Barq::get_frozen_barq(config, r.read_transaction_version())->schema().size() == i + 1);
            }
        }
    }

    SECTION("obtaining a frozen barq with an incompatible schema throws") {
        config.schema = Schema{{"object", {{"value", PropertyType::Int}}}};
        auto old_barq = Barq::get_shared_barq(config);
        {
            auto tr = db->start_write();
        ...
        auto new_barq = Barq::get_shared_barq(config);
        new_barq->read_group();

        REQUIRE(old_barq->freeze()->schema().size() == 1);
        REQUIRE(new_barq->freeze()->schema().size() == 2);
        REQUIRE(Barq::get_frozen_barq(config, new_barq->read_transaction_version())->schema().size() == 2);
        // An additive change is allowed, the unknown table is empty
        REQUIRE(Barq::get_frozen_barq(config, old_barq->read_transaction_version())->schema().size() == 2);

        config.schema = Schema{{"object", {{"value", PropertyType::String}}}}; // int -> string
        // Fails because the schema has an invalid breaking change
        REQUIRE_THROWS_AS(Barq::get_frozen_barq(config, new_barq->read_transaction_version()),
                          InvalidReadOnlySchemaChangeException);
        REQUIRE_THROWS_AS(Barq::get_frozen_barq(config, old_barq->read_transaction_version()),
                          InvalidReadOnlySchemaChangeException);
        config.schema = Schema{
            {"object", {{"value", PropertyType::Int}}},
        ...
        auto frozen_old = Barq::get_frozen_barq(config, old_barq->read_transaction_version());
...
#if BARQ_ENABLE_SYNC
TEST_CASE("Get Barq using Async Open", "[sync][pbs][async open]") {
    ...
    SECTION("can open synced Barqs that don't already exist") {
        auto barq = successfully_async_open_barq(config);
        REQUIRE(barq->read_group().get_table("class_object"));
    }

    SECTION("can write a barq file without client file id") {
        ThreadSafeReference barq_ref;
        SyncTestFile config3(tsm, "default");
        config3.schema = config.schema;
        uint64_t client_file_id;

        // Create some content
        auto origin = Barq::get_shared_barq(config);
        ...
        // Create barq file without client file id
        {
            auto barq = successfully_async_open_barq(config);
            // Write some data
            barq->begin_transaction();
            barq->get_class("object").create_object(2);
            barq->commit_transaction();
            wait_for_upload(*barq);
            wait_for_download(*barq);
            client_file_id = barq->read_group().get_sync_file_id();

            barq->convert(config3);
        }
        ...
        // Now open a barq based on the barq file created above
        auto barq = Barq::get_shared_barq(config3);
        Class cls2 = barq->get_class("object");
        wait_for_download(*barq);
        wait_for_upload(*barq);

        // Make sure we have got a new client file id
        REQUIRE(barq->read_group().get_sync_file_id() != client_file_id);
        REQUIRE(cls.num_objects() == 3);

        // Check that we can continue committing to this barq
        barq->begin_transaction();
        cls2.create_object(5);
        barq->commit_transaction();
        wait_for_upload(*barq);

        // Check that this change is now in the original barq
        wait_for_download(*origin);
        origin->refresh();
        REQUIRE(cls.num_objects() == 4);
    }

    SECTION("downloads Barqs which exist on the server") {
        {
            auto barq = Barq::get_shared_barq(config2);
            barq->begin_transaction();
            barq->read_group().get_table("class_object")->create_object_with_primary_key(0);
            barq->commit_transaction();
            wait_for_upload(*barq);
        }
    ...
    SECTION("progress notifiers of a task are cancelled if the task is cancelled") {
        ...
        std::shared_ptr<AsyncOpenTask> task = Barq::get_synchronized_barq(config);
        std::shared_ptr<AsyncOpenTask> task2 = Barq::get_synchronized_barq(config);
        REQUIRE(task);
        REQUIRE(task2);
        ...
        task->start([&](ThreadSafeReference barq_ref, std::exception_ptr err) {
            std::lock_guard<std::mutex> guard(mutex);
            REQUIRE(!err);
            REQUIRE(barq_ref);
        ...
        task2->start([&](ThreadSafeReference barq_ref, std::exception_ptr err) {
            std::lock_guard<std::mutex> guard(mutex);
            REQUIRE(!err);
            REQUIRE(barq_ref);
            rref = std::move(barq_ref);
        ...
        REQUIRE(task2_completed);
        SharedBarq barq = Barq::get_shared_barq(std::move(rref));
        REQUIRE(barq);
    }

    SECTION("downloads latest state for Barqs which already exist locally") {
        wait_for_upload(*Barq::get_shared_barq(config));
        ...
        auto barq = successfully_async_open_barq(config);
        REQUIRE(barq->read_group().get_table("class_object")->size() == 1);
    }

    SECTION("can download multiple Barqs at a time") {
        SyncTestFile config1(tsm, "barq1");
        SyncTestFile config2(tsm, "barq2");
        SyncTestFile config3(tsm, "barq3");
        SyncTestFile config4(tsm, "barq4");

        std::vector<std::shared_ptr<AsyncOpenTask>> tasks = {
            Barq::get_synchronized_barq(config1),
            Barq::get_synchronized_barq(config2),
            Barq::get_synchronized_barq(config3),
            Barq::get_synchronized_barq(config4),
        };
    ...
    SECTION("read-only mode sets the schema version") {
        {
            SharedBarq barq = Barq::get_shared_barq(config);
            wait_for_upload(*barq);
            barq->close();
        }

        config2.schema_mode = SchemaMode::ReadOnly;
        auto barq = successfully_async_open_barq(config2);
    ...
    SECTION("read-only mode applies remote schema changes") {
        ...
        config2.schema_mode = SchemaMode::ReadOnly;
        auto barq = successfully_async_open_barq(config2);
        REQUIRE(barq->schema().find("added") != barq->schema().end());
        REQUIRE(barq->read_group().get_table("class_added"));
    }
    ...
    SECTION("read-only mode does not create tables not present on the server") {
        ...
        auto barq = successfully_async_open_barq(config2);
        REQUIRE(barq->schema().find("added") != barq->schema().end());
        REQUIRE_FALSE(barq->read_group().get_table("class_added"));
    }

    SECTION("adding a property to a newly downloaded read-only Barq reports an error") {
        // Create the Barq on the server
        wait_for_upload(*Barq::get_shared_barq(config2));
    ...
    SECTION("adding a property to an existing read-only Barq reports an error") {
        Barq::get_shared_barq(config);
        ...
        REQUIRE_THROWS_CONTAINING(Barq::get_shared_barq(config), "Property 'object.value2' has been added.");
    ...
    SECTION("removing a property from a newly downloaded read-only Barq leaves the column in place") {
        // Create the Barq on the server
        wait_for_upload(*Barq::get_shared_barq(config2));
        ...
        auto barq = successfully_async_open_barq(config);
        REQUIRE(barq->read_group().get_table("class_object")->get_column_key("value") != ColKey{});
    }

    SECTION("removing a property from a existing read-only Barq leaves the column in place") {
        Barq::get_shared_barq(config);
    ...
    _impl::BarqCoordinator::assert_no_open_barqs();
}


TEST_CASE("SharedBarq: convert", "[sync][pbs][convert]") {
    TestSyncManager tsm;
    ObjectSchema object_schema = {"object",
                                  {
                                      {"_id", PropertyType::Int, Property::IsPrimary{true}},
    ...
    SECTION("can copy a synced barq to a synced barq") {
        auto sync_barq1 = Barq::get_shared_barq(sync_config1);
        sync_barq1->begin_transaction();
        sync_barq1->read_group().get_table("class_object")->create_object_with_primary_key(0);
        sync_barq1->commit_transaction();
        wait_for_upload(*sync_barq1);
        wait_for_download(*sync_barq1);

        // Copy to a new sync config
        SyncTestFile sync_config2(tsm, "default");
        sync_config2.schema = schema;

        sync_barq1->convert(sync_config2);

        auto sync_barq2 = Barq::get_shared_barq(sync_config2);

        // Check that the data also exists in the new barq
        REQUIRE(sync_barq2->read_group().get_table("class_object")->size() == 1);

        // Verify that sync works and objects created in the new copy will get
        // synchronized to the old copy
        sync_barq2->begin_transaction();
        sync_barq2->read_group().get_table("class_object")->create_object_with_primary_key(1);
        sync_barq2->commit_transaction();
        wait_for_upload(*sync_barq2);
        wait_for_download(*sync_barq1);

        sync_barq1->refresh();
        REQUIRE(sync_barq1->read_group().get_table("class_object")->size() == 2);
    }

    SECTION("can convert a synced barq to a local barq") {
        auto sync_barq = Barq::get_shared_barq(sync_config1);
        sync_barq->begin_transaction();
        sync_barq->read_group().get_table("class_object")->create_object_with_primary_key(0);
        sync_barq->commit_transaction();
        wait_for_upload(*sync_barq);
        wait_for_download(*sync_barq);

        sync_barq->convert(local_config1);

        auto local_barq = Barq::get_shared_barq(local_config1);

        // Check that the data also exists in the new barq
        REQUIRE(local_barq->read_group().get_table("class_object")->size() == 1);
    }

    SECTION("can convert a local barq to a synced barq") {
        auto local_barq = Barq::get_shared_barq(local_config1);
        local_barq->begin_transaction();
        local_barq->read_group().get_table("class_object")->create_object_with_primary_key(0);
        local_barq->commit_transaction();

        // Copy to a new sync config
        local_barq->convert(sync_config1);

        auto sync_barq = Barq::get_shared_barq(sync_config1);

        // Check that the data also exists in the new barq
        REQUIRE(sync_barq->read_group().get_table("class_object")->size() == 1);
    }

    SECTION("can copy a local barq to a local barq") {
        auto local_barq1 = Barq::get_shared_barq(local_config1);
        local_barq1->begin_transaction();
        local_barq1->read_group().get_table("class_object")->create_object_with_primary_key(0);
        local_barq1->commit_transaction();
        ...
        local_barq1->convert(local_config2);

        auto local_barq2 = Barq::get_shared_barq(local_config2);

        // Check that the data also exists in the new barq
        REQUIRE(local_barq2->read_group().get_table("class_object")->size() == 1);
    }

    SECTION("synced barq must be fully uploaded") {
        auto barq = Barq::get_shared_barq(sync_config1);
        barq->sync_session()->pause();
        barq->begin_transaction();
        barq->read_group().get_table("class_object")->create_object_with_primary_key(0);
        barq->commit_transaction();
        ...
        SyncTestFile sync_config2(tsm, "default");
        sync_config2.schema = schema;
        REQUIRE_EXCEPTION(barq->convert(sync_config2), IllegalOperation,
                          "All client changes must be integrated in server before writing copy");

        barq->sync_session()->resume();
        wait_for_upload(*barq);
        REQUIRE_NOTHROW(barq->convert(sync_config2));
    }

    SECTION("can convert synced barq from within upload complete callback") {
        auto barq = Barq::get_shared_barq(sync_config1);
        barq->sync_session()->pause();
        barq->begin_transaction();
        barq->read_group().get_table("class_object")->create_object_with_primary_key(0);
        ...
        barq->sync_session()->wait_for_upload_completion([&](Status) {
...
TEST_CASE("SharedBarq: convert - embedded objects", "[sync][pbs][convert][embedded objects]") {
    TestSyncManager tsm;
    ObjectSchema object_schema = {"object",
                                  {
                                      {"_id", PropertyType::Int, Property::IsPrimary{true}},
    ...
    SECTION("can copy a synced barq to a synced barq") {
        ...
        SECTION("embedded object") {
            auto obj = sync_barq1->read_group().get_table("class_object")->create_object_with_primary_key(0);
            auto col_key = sync_barq1->read_group().get_table("class_object")->get_column_key("embedded_link");
    ...
    SECTION("can convert a synced barq to a local barq") {
        ...
        SECTION("embedded object") {
            auto obj = sync_barq->read_group().get_table("class_object")->create_object_with_primary_key(0);
            auto col_key = sync_barq->read_group().get_table("class_object")->get_column_key("embedded_link");
            obj.create_and_set_linked_object(col_key);
        }
    ...
    SECTION("can convert a local barq to a synced barq") {
        ...
        SECTION("embedded object") {
            auto obj = local_barq->read_group().get_table("class_object")->create_object_with_primary_key(0);
            auto col_key = local_barq->read_group().get_table("class_object")->get_column_key("embedded_link");
            obj.create_and_set_linked_object(col_key);
        }
    ...
    SECTION("can copy a local barq to a local barq") {
        ...
        SECTION("embedded object") {
            auto obj = local_barq1->read_group().get_table("class_object")->create_object_with_primary_key(0);
            auto col_key = local_barq1->read_group().get_table("class_object")->get_column_key("embedded_link");
            obj.create_and_set_linked_object(col_key);
        }
...
TEST_CASE("SharedBarq: async writes") {
    _impl::BarqCoordinator::assert_no_open_barqs();
    if (!util::EventLoop::has_implementation())
        return;
    ...
    auto verify_persisted_count = [&](size_t expected) {
        ...
        auto new_barq = Barq::get_shared_barq(config);
        auto table = new_barq->read_group().get_table("class_object");
        REQUIRE(table->size() == expected);
    };

    using BarqCloseFunction = void (Barq::*)();
    static BarqCloseFunction close_functions[] = {&Barq::close, &Barq::invalidate};
    ...
    for (int i = 0; i < 2; ++i) {
        SECTION(close_function_names[i]) {
            ...
            SECTION("before write lock is acquired") {
                ...
                barq::test_util::BowlOfStonesSemaphore sema;
                JoiningThread thread([&] {
                    auto db = DB::create(make_in_barq_history(), config.path, options);
                ...
                auto scheduler = barq->scheduler();
                barq->async_begin_transaction([&] {
                    // We should never get here as the barq is closed
                    FAIL();
                });

                // close() should block until we can acquire the write lock
                std::invoke(close_functions[i], *barq);
            ...
            SECTION("within did_change()") {
                struct Context : public BindingContext {
                    ...
                    {
                        std::invoke(close_functions[i], *barq.lock());
                    }
                };
                barq->m_binding_context.reset(new Context(i));
                barq->m_binding_context->barq = barq;
    ...
    SECTION("exception thrown during transaction with error handler") {
        Barq::AsyncHandle h = 7;
        bool called = false;
        barq->set_async_error_handler([&](Barq::AsyncHandle handle, std::exception_ptr error) {
            REQUIRE(error);
            REQUIRE_THROWS_CONTAINING(std::rethrow_exception(error), "an error");
            CHECK(handle == h);
            called = true;
        });
        h = barq->async_begin_transaction([&] {
            table->create_object();
            done = true;
        ...
        REQUIRE_FALSE(barq->is_in_transaction());
    ...
    SECTION("exception thrown during transaction without error handler") {
        barq->set_async_error_handler(nullptr);
        barq->async_begin_transaction([&] {
            table->create_object();
            throw std::runtime_error("an error");
        });
        ...
        // Should be able to perform another write afterwards
        barq->async_begin_transaction([&, barq] {
            table->create_object();
            barq->commit_transaction();
            done = true;
        });
    ...
    SECTION("exception thrown during transaction without error handler after closing Barq") {
        barq->set_async_error_handler(nullptr);
        barq->async_begin_transaction([&] {
            barq->close();
            throw std::runtime_error("an error");
    ...
    SECTION("exception thrown from async commit completion callback with error handler") {
        ...
        barq->begin_transaction();
        table->create_object();
        h = barq->async_commit_transaction([&](std::exception_ptr) {
            throw std::runtime_error("an error");
        });
        wait_for_done();
        verify_persisted_count(1);
    ...
    if (_impl::SimulatedFailure::is_enabled()) {
        SECTION("error in the synchronous part of async commit") {
            ...
            REQUIRE_THROWS_AS(barq->async_commit_transaction([&](std::exception_ptr) {
        ...
        SECTION("error in the async part of async commit") {
            ...
            barq->async_commit_transaction([&](std::exception_ptr e) {
                REQUIRE(e);
    ...
    SECTION("throw exception from did_change()") {
        ...
        barq->m_binding_context.reset(new Context);

        barq->begin_transaction();
        auto table = barq->read_group().get_table("class_object");
        table->create_object();
        REQUIRE_THROWS_WITH(barq->async_commit_transaction([&](std::exception_ptr) {
            done = true;
        }),
                            "expected error");
        wait_for_done();
    ...
    SECTION("cancel scheduled async transaction") {
        auto handle = barq->async_begin_transaction([&, barq]() {
            table->create_object().set(col, 45);
            barq->async_commit_transaction(
                [&](auto) {
                    done = true;
        ...
        barq->async_cancel_transaction(handle);
        wait_for_done();
        auto table = barq->read_group().get_table("class_object");
        REQUIRE(table->size() == 1);
    ...
    SECTION("synchronous transaction mixed with async transactions") {
        ...
        wait_for_done();
        barq->begin_transaction(); // Here syncing of first async tr has not completed
        REQUIRE(table->size() == 1);
        table->create_object().set(col, 90);
        barq->commit_transaction(); // Will re-initiate async writes
    ...
    SECTION("asynchronous transaction mixed with sync transaction that is cancelled") {
        ...
        barq->async_begin_transaction([&, barq]() {
            table->create_object().set(col, 45);
            auto handle = barq->async_commit_transaction([&](std::exception_ptr) {
                FAIL();
            });
            barq->async_cancel_transaction(handle);
        });
        ...
        util::EventLoop::main().run_until([&] {
            return !barq->is_in_async_transaction();
        });
    ...
    SECTION("release reference to Barq after async begin") {
        std::weak_ptr<Barq> weak_barq = barq;
        barq->async_begin_transaction([&]() {
            table->create_object().set(col, 45);
            weak_barq.lock()->async_commit_transaction([&](std::exception_ptr) {
                done = true;
            });
        });
        barq = nullptr;
    ...
    SECTION("object change information") {
        ...
        observer.barq = barq;
        barq->m_binding_context.reset(&observer);
        ...
        barq->m_binding_context.release();
    }
    ...
    SECTION("begin_transaction() from within did_change()") {
        ...
        table->create_object();
        bool persisted = false;
        barq->async_commit_transaction([&persisted](auto) {
            persisted = true;
        });
        REQUIRE(table->size() == 2);
        REQUIRE(persisted);
    ...
    SECTION("async write grouping") {
        ...
        for (size_t i = 0; i < 41; ++i) {
            barq->async_begin_transaction([&, i, barq] {
                // The top ref in the Barq file should only be updated once every 20 commits
    ...
    SECTION("async write grouping with manual barriers") {
        ...
        for (size_t i = 0; i < 41; ++i) {
            barq->async_begin_transaction([&, i, barq] {
                // The top ref in the Barq file should only be updated once every 6 commits
                CHECK(Group(config.path, config.encryption_key.data()).get_table("class_object")->size() ==
                      (i / 6) * 6);
    ...
    util::EventLoop::main().run_until([&] {
        return !barq || !barq->has_pending_async_work();
    });

    _impl::BarqCoordinator::clear_all_caches();
}

TEST_CASE("Call run_async_completions after barq has been closed") {
    // This requires a special scheduler as we have to call Barq::close
...
TEST_CASE("SharedBarq: async writes on multiple threads") {
    _impl::BarqCoordinator::assert_no_open_barqs();
    ...
    config.schema = Schema{{"object", {{"value", PropertyType::Int}}}};
    auto barq = Barq::get_shared_barq(config);
    auto table_key = barq->read_group().get_table("class_object")->get_key();
    barq->close();
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        Barq::Config queue_config = config;
    ...
    auto async_write_and_async_commit = [=](const Barq::Config& config) {
        Barq::get_shared_barq(config)->async_begin_transaction([=] {
            auto barq = Barq::get_shared_barq(config);
            barq->read_group().get_table(table_key)->create_object();
            barq->async_commit_transaction([=](std::exception_ptr) {
                ++*completions_ptr;
            });
        });
    };
    auto async_write_and_sync_commit = [=](const Barq::Config& config) {
        Barq::get_shared_barq(config)->async_begin_transaction([=] {
            auto barq = Barq::get_shared_barq(config);
            barq->read_group().get_table(table_key)->create_object();
            barq->commit_transaction();
    ...
    auto sync_write_and_async_commit = [=](const Barq::Config& config) {
        auto barq = Barq::get_shared_barq(config);
        barq->begin_transaction();
        barq->read_group().get_table(table_key)->create_object();
        barq->async_commit_transaction([=](std::exception_ptr) {
    ...
    auto sync_write_and_sync_commit = [=](const Barq::Config& config) {
        auto barq = Barq::get_shared_barq(config);
        barq->begin_transaction();
        barq->read_group().get_table(table_key)->create_object();
        barq->commit_transaction();
    ...
    SECTION("mixed sync and async") {
        ...
        struct IndexedOp {
            int index;
            std::function<void(const Barq::Config& config)> fn;
        };
    ...
    barq = Barq::get_shared_barq(config);
    REQUIRE(barq->read_group().get_table(table_key)->size() == completions);
...
#ifndef _WIN32
TEST_CASE("SharedBarq: async_writes_2") {
    _impl::BarqCoordinator::assert_no_open_barqs();
    if (!util::EventLoop::has_implementation())
        return;
    ...
    auto col = table->get_column_key("value");
    LooperDelegate ld;
    std::shared_ptr<bool> t1_rdy = ld.add_task([&, barq]() {
        REQUIRE(write_nr == 0);
        ++write_nr;
        table->create_object().set(col, 45);
        barq->cancel_transaction();
    });
    std::shared_ptr<bool> t2_rdy = ld.add_task([&, barq]() {
        REQUIRE(write_nr == 1);
        ++write_nr;
        table->create_object().set(col, 45);
        barq->async_commit_transaction([&](std::exception_ptr) {
    ...
    std::shared_ptr<bool> t3_rdy = ld.add_task([&, barq]() {
        ++write_nr;
        auto o = table->get_object(0);
        o.set(col, o.get<int64_t>(col) + 37);
        barq->async_commit_transaction([&](std::exception_ptr) {
    ...
    util::EventLoop::main().run_until([&, barq] {
        ld.run_once();
        return done;
    });
    REQUIRE(done);
...
TEST_CASE("SharedBarq: notifications") {
    if (!util::EventLoop::has_implementation())
        return;
    ...
    barq->m_binding_context.reset(context);
    barq->m_binding_context->barq = barq;
    ...
    SECTION("remote notifications are sent asynchronously") {
        auto r2 = Barq::get_shared_barq(config);
    ...
    SECTION("notifications created in async transaction are sent synchronously") {
        ...
        REQUIRE(change_count == 1);
        util::EventLoop::main().run_until([&] {
            return !barq->has_pending_async_work();
        });
    }
#endif
    SECTION("refresh() from within changes_available() refreshes") {
        context->changes_available_fn = [&] {
            REQUIRE(barq->refresh());
        };
        barq->set_auto_refresh(false);

        auto r2 = Barq::get_shared_barq(config);
        r2->begin_transaction();
        r2->commit_transaction();
        barq->notify();
        // Should return false as the barq was already advanced
        REQUIRE_FALSE(barq->refresh());
    }
    ...
    SECTION("refresh() from within did_change() is a no-op") {
        context->did_change_fn = [&] {
            ...
            auto r2 = Barq::get_shared_barq(barq->config());
            r2->begin_transaction();
            r2->commit_transaction();
    ...
    SECTION("SubscriptionStore writes produce notifications") {
        auto subscription_store = sync::SubscriptionStore::create(TestHelper::get_db(barq));
        REQUIRE(change_count == 0);
        util::EventLoop::main().run_until([&] {
            return change_count > 0;
        });
...
TEST_CASE("SharedBarq: schema updating from external changes") {
    TestFile config;
    config.schema_version = 0;
    config.schema_mode = SchemaMode::AdditiveExplicit;
    config.schema = Schema{
    ...
    SECTION("newly added columns update table columns but are not added to properties") {
        ...
        auto r1 = Barq::get_shared_barq(config);
        auto r2 = Barq::get_shared_barq(config);
        auto test = [&] {
            r2->begin_transaction();
...
TEST_CASE("SharedBarq: close()") {
    ...
    SECTION("all functions throw ClosedBarqException after close") {
        const char* msg = "Cannot access barq that has been closed.";

        barq->close();
        REQUIRE(barq->is_closed());
        REQUIRE_EXCEPTION(barq->verify_open(), ClosedBarq, msg);

        REQUIRE_EXCEPTION(barq->update_schema(Schema{}), ClosedBarq, msg);
        REQUIRE_EXCEPTION(barq->rename_property(Schema{}, "", "", ""), ClosedBarq, msg);
        REQUIRE_EXCEPTION(barq->set_schema_subset(Schema{}), ClosedBarq, msg);

        REQUIRE_EXCEPTION(barq->begin_transaction(), ClosedBarq, msg);
        REQUIRE_EXCEPTION(barq->commit_transaction(), ClosedBarq, msg);
        REQUIRE_EXCEPTION(barq->cancel_transaction(), ClosedBarq, msg);
        REQUIRE(!barq->is_in_transaction());

        REQUIRE_EXCEPTION(barq->async_begin_transaction(nullptr), ClosedBarq, msg);
        REQUIRE_EXCEPTION(barq->async_commit_transaction(nullptr), ClosedBarq, msg);
        REQUIRE_EXCEPTION(barq->async_cancel_transaction(0), ClosedBarq, msg);
        REQUIRE_FALSE(barq->is_in_async_transaction());

        REQUIRE_EXCEPTION(barq->freeze(), ClosedBarq, msg);
        REQUIRE_FALSE(barq->is_frozen());
        REQUIRE_EXCEPTION(barq->get_number_of_versions(), ClosedBarq, msg);
        REQUIRE_EXCEPTION(barq->read_transaction_version(), ClosedBarq, msg);
        REQUIRE_EXCEPTION(barq->duplicate(), ClosedBarq, msg);

        REQUIRE_EXCEPTION(barq->enable_wait_for_change(), ClosedBarq, msg);
        REQUIRE_EXCEPTION(barq->wait_for_change(), ClosedBarq, msg);
        REQUIRE_EXCEPTION(barq->wait_for_change_release(), ClosedBarq, msg);

        REQUIRE_NOTHROW(barq->notify());
        REQUIRE_EXCEPTION(barq->refresh(), ClosedBarq, msg);
        REQUIRE_EXCEPTION(barq->invalidate(), ClosedBarq, msg);
        REQUIRE_EXCEPTION(barq->compact(), ClosedBarq, msg);
        REQUIRE_EXCEPTION(barq->convert(barq->config()), ClosedBarq, msg);
        REQUIRE_EXCEPTION(barq->write_copy(), ClosedBarq, msg);

#if BARQ_ENABLE_SYNC
        REQUIRE_FALSE(barq->sync_session());
        msg = "Flexible sync is not enabled";
        REQUIRE_EXCEPTION(barq->get_latest_subscription_set(), IllegalOperation, msg);
        REQUIRE_EXCEPTION(barq->get_active_subscription_set(), IllegalOperation, msg);
#endif
    }
    ...
    SECTION("fully closes database file even with live notifiers") {
        ...
        Results results(barq, ObjectStore::table_for_object_type(group, "object"));
        List list(barq, obj.get_linklist("list"));
...
TEST_CASE("Barq::delete_files()") {
    ...
    // for a freshly opened barq but need to be tested for as the will
    // be created during a Barq's life cycle.
    (void)util::File(path + ".log", util::File::mode_Write);

    SECTION("Deleting files of a closed Barq succeeds.") {
        barq->close();
    ...
    SECTION("Trying to delete files of an open Barq fails.") {
        REQUIRE_EXCEPTION(Barq::delete_files(path), ErrorCodes::DeleteOnOpenBarq,
    ...
    SECTION("Deleting the same Barq multiple times.") {
        barq->close();
        Barq::delete_files(path);
        Barq::delete_files(path);
        Barq::delete_files(path);
    }

    SECTION("Calling delete on a folder that does not exist.") {
        auto fake_path = "/tmp/doesNotExist/barq.424242";
        bool did_delete = false;
        Barq::delete_files(fake_path, &did_delete);
        REQUIRE_FALSE(did_delete);
    }

    SECTION("passing did_delete is optional") {
        barq->close();
        Barq::delete_files(path, nullptr);
    }

    SECTION("Deleting a Barq which does not exist does not set did_delete") {
        TestFile new_config;
        bool did_delete = false;
        Barq::delete_files(new_config.path, &did_delete);
        REQUIRE_FALSE(did_delete);
    }
}

TEST_CASE("ShareBarq: in-memory mode from buffer") {
    TestFile config;
    config.schema_version = 1;
    config.schema = Schema{
        {"object", {{"value", PropertyType::Int}}},
    ...
    SECTION("Save and open Barq from in-memory buffer") {
        // Write in-memory copy of Barq to a buffer
        auto barq = Barq::get_shared_barq(config);
        OwnedBinaryData barq_buffer = barq->write_copy();

        // Open the buffer as a new (immutable in-memory) Barq
        barq::Barq::Config config2;
        config2.in_memory = true;
        config2.schema_mode = SchemaMode::Immutable;
        config2.barq_data = barq_buffer.get();
        ...
        barq::Barq::Config config3;
        config3.barq_data = barq_buffer.get();
        REQUIRE_EXCEPTION(Barq::get_shared_barq(config3), IllegalCombination,
                          "In-memory barqs initialized from memory buffers can only be opened in read-only mode");
...
TEST_CASE("ShareBarq: barq closed in did_change callback") {
    TestFile config;
    config.schema_version = 1;
    config.schema = Schema{
        {"object", {{"value", PropertyType::Int}}},
    ...
    struct Context : public BindingContext {
        Context(std::shared_ptr<Barq>& barq)
            : barq(&barq)
        {
        }
        std::shared_ptr<Barq>* barq;
        void did_change(std::vector<ObserverState> const&, std::vector<void*> const&, bool) override
        {
            auto barq = this->barq; // close() will delete `this`
            (*barq)->close();
            barq->reset();
        }
    };
    ...
    SECTION("did_change with async results") {
        ...
        auto coordinator = _impl::BarqCoordinator::get_coordinator(config.path);
...
TEST_CASE("BarqCoordinator: schema cache") {
    TestFile config;
    auto coordinator = _impl::BarqCoordinator::get_coordinator(config.path);
...
TEST_CASE("SharedBarq: coordinator schema cache") {
    TestFile config;
    auto r = Barq::get_shared_barq(config);
    auto coordinator = _impl::BarqCoordinator::get_coordinator(config.path);
    ...
    public:
        WriteTransaction wt;
        ExternalWriter(Barq::Config const& config)
            : m_barq([&] {
                auto c = config;
                c.scheduler = util::Scheduler::make_frozen(VersionID());
                return _impl::BarqCoordinator::get_coordinator(c.path)->get_barq(c, util::none);
            }())
            , wt(TestHelper::get_db(m_barq))
        {
        }
    };

    auto external_write = [&](Barq::Config const& config, auto&& fn) {
    ...
    coordinator = nullptr;
    r = nullptr;
    r = Barq::get_shared_barq(config);
    coordinator = _impl::BarqCoordinator::get_coordinator(config.path);
    REQUIRE(coordinator->get_cached_schema(cache_schema, cache_sv, cache_tv));

    SECTION("is populated after opening an initialized file") {
        REQUIRE(cache_sv == 0);
        REQUIRE(cache_tv == 2); // with in-barq history the version doesn't reset
        REQUIRE(cache_schema == schema);
        REQUIRE(cache_schema.begin()->persisted_properties[0].column_key != ColKey{});
    }
...
TEST_CASE("SharedBarq: dynamic schema mode doesn't invalidate object schema pointers when schema hasn't changed") {
    TestFile config;

    // Prepopulate the Barq with the schema.
    Barq::Config config_with_schema = config;
    ...
    auto r1 = Barq::get_shared_barq(config_with_schema);

    // Retrieve the object schema in dynamic mode.
    auto r2 = Barq::get_shared_barq(config);
    auto* object_schema = &*r2->schema().find("object");

    // Perform an empty write to create a new version, resulting in the other Barq needing to re-read the schema.
    r1->begin_transaction();
    r1->commit_transaction();
...
TEST_CASE("SharedBarq: declaring an object as embedded results in creating an embedded table") {
    TestFile config;
...
TEST_CASE("SharedBarq: SchemaChangedFunction") {
    struct Context : BindingContext {
        size_t* change_count;
        Schema* schema;
    ...
    TestFile config;
    BarqConfig dynamic_config = config;
    ...
    SECTION("Fixed schema") {
        ...
        SECTION("Open a new Barq instance with same config won't trigger") {
            auto r2 = Barq::get_shared_barq(config);
        ...
        SECTION("Schema is changed by another Barq") {
            auto r2 = Barq::get_shared_barq(config);
            r2->begin_transaction();
        ...
        SECTION("Schema is changed by this Barq won't trigger") {
            r1->begin_transaction();
    ...
    SECTION("Dynamic schema") {
        ...
        auto r2 = Barq::get_shared_barq(dynamic_config);
...
TEST_CASE("SharedBarq: compact on launch") {
    // Make compactable Barq
    TestFile config;
    config.automatic_change_notifications = false;
    int num_opens = 0;
...
#ifndef _WIN32
        // Confirm expected sizes before and after opening the Barq
    ...
    SECTION("compact function does not get invoked if barq is open on another thread") {
        ...
        r->close();
        JoiningThread([&] {
            auto r3 = Barq::get_shared_barq(config);
            REQUIRE(num_opens == 3);
        });
    }
}
...
TEMPLATE_TEST_CASE("SharedBarq: update_schema with initialization_function", "[init][update schema]", ModeAutomatic,
                   ModeAdditive, ModeManual, ModeSoftResetFile, ModeHardResetFile)
{
    ...
    auto initialization_function = [&initialization_function_called, &schema_version_in_callback,
                                    &schema_in_callback](auto shared_barq) {
        REQUIRE(shared_barq->is_in_transaction());
        initialization_function_called = true;
        schema_version_in_callback = shared_barq->schema_version();
        schema_in_callback = shared_barq->schema();
    };
    ...
    SECTION("call initialization function directly by update_schema") {
        ...
        barq->update_schema(schema, 0, nullptr, initialization_function);
    ...
    SECTION("initialization function should be called for unversioned barq") {
    ...
    SECTION("initialization function for versioned barq") {
        // Initialize v0
        Barq::get_shared_barq(config);
...
TEST_CASE("BindingContext is notified about delivery of change notifications") {
    ...
    SECTION("BindingContext notified even if no callbacks are registered") {
        ...
        SECTION("remote commit") {
            ...
            binding_context_end_notify_calls = 0;
            JoiningThread([&] {
                auto r2 = coordinator->get_barq(util::Scheduler::make_frozen(VersionID()));
                r2->begin_transaction();
    ...
    SECTION("did_send() is skipped if the Barq is closed first") {
        ...
        SECTION("closed in notification callback for refresh()") {
            ...
            JoiningThread([&] {
                auto r = coordinator->get_barq(util::Scheduler::make_frozen(VersionID()));
                r->begin_transaction();
...
TEST_CASE("BarqCoordinator: get_unbound_barq()") {
    TestFile config;
    config.cache = true;
    config.schema = Schema{
        {"object", {{"value", PropertyType::Int}}},
    ...
    JoiningThread([&] {
        ref = _impl::BarqCoordinator::get_coordinator(config)->get_unbound_barq();
    });

    SECTION("checks thread after being resolved") {
        auto barq = Barq::get_shared_barq(std::move(ref));
        REQUIRE_NOTHROW(barq->verify_thread());
        JoiningThread([&] {
            REQUIRE_EXCEPTION(barq->verify_thread(), WrongThread, "Barq accessed from incorrect thread.");
        });
    }
    ...
    SECTION("delivers notifications to the thread it is resolved on") {
        ...
        if (!util::EventLoop::has_implementation())
            return;
        auto barq = Barq::get_shared_barq(std::move(ref));
        Results results(barq, ObjectStore::table_for_object_type(barq->read_group(), "object")->where());
        bool called = false;
        auto token = results.add_notification_callback([&](CollectionChangeSet) {
            called = true;
        });
    ...
    SECTION("resolves to existing cached Barq for the thread if caching is enabled") {
        auto r1 = Barq::get_shared_barq(config);
        auto r2 = Barq::get_shared_barq(std::move(ref));
        REQUIRE(r1 == r2);
    }

    SECTION("resolves to a new Barq if caching is disabled") {
        config.cache = false;
        auto r1 = Barq::get_shared_barq(config);
        auto r2 = Barq::get_shared_barq(std::move(ref));
        REQUIRE(r1 != r2);
        ...
        auto r3 = Barq::get_shared_barq(std::move(ref));
        REQUIRE(r1 != r3);
        REQUIRE(r2 != r3);

        // New local with cache enabled should grab the resolved unbound
        config.cache = true;
        auto r4 = Barq::get_shared_barq(config);
        REQUIRE(r4 == r2);
    }
}

TEST_CASE("Immutable Barqs") {
...

    {
        auto barq = Barq::get_shared_barq(config);
        barq->begin_transaction();
        barq->read_group().get_table("class_object")->create_object();
        barq->commit_transaction();
    }
    ...
    SECTION("unsupported functions") {
        SECTION("update_schema()") {
            REQUIRE_THROWS_AS(barq->compact(), WrongTransactionState);
        }
        SECTION("begin_transaction()") {
            REQUIRE_THROWS_AS(barq->begin_transaction(), WrongTransactionState);
        }
        SECTION("async_begin_transaction()") {
            REQUIRE_THROWS_AS(barq->async_begin_transaction(nullptr), WrongTransactionState);
        }
        SECTION("refresh()") {
            REQUIRE_THROWS_AS(barq->refresh(), WrongTransactionState);
        }
        SECTION("compact()") {
            REQUIRE_THROWS_AS(barq->compact(), WrongTransactionState);
        }
    ...
    SECTION("supported functions") {
        ...
        SECTION("freeze()") {
            std::shared_ptr<Barq> frozen;
            REQUIRE_NOTHROW(frozen = barq->freeze());
            REQUIRE(frozen->read_group().get_table("class_object")->size() == 1);
            REQUIRE_NOTHROW(frozen = Barq::get_frozen_barq(config, barq->read_transaction_version()));
            REQUIRE(frozen->read_group().get_table("class_object")->size() == 1);
        }
        SECTION("notify()") {
            REQUIRE_NOTHROW(barq->notify());
        ...
        SECTION("is_in_read_transaction()") {
            REQUIRE(barq->is_in_read_transaction());
        }
        SECTION("last_seen_transaction_version()") {
            REQUIRE(barq->last_seen_transaction_version() == 1);
        }
        SECTION("get_number_of_versions()") {
            REQUIRE(barq->get_number_of_versions() == 1);
        }
        SECTION("read_transaction_version()") {
            REQUIRE(barq->read_transaction_version() == VersionID{1, 0});
        }
        SECTION("current_transaction_version()") {
            REQUIRE(barq->current_transaction_version() == VersionID{1, 0});
        }
        SECTION("latest_snapshot_version()") {
            REQUIRE(barq->latest_snapshot_version() == 1);
        }
        SECTION("duplicate()") {
            auto duplicate = barq->duplicate();
            REQUIRE(duplicate->get_table("class_object")->size() == 1);
        }
        SECTION("invalidate()") {
            REQUIRE_NOTHROW(barq->invalidate());
            REQUIRE_FALSE(barq->is_in_read_transaction());
            REQUIRE(barq->read_group().get_table("class_object")->size() == 1);
        }
        SECTION("close()") {
            REQUIRE_NOTHROW(barq->close());
            REQUIRE(barq->is_closed());
        }
        SECTION("has_pending_async_work()") {
            REQUIRE_FALSE(barq->has_pending_async_work());
        }
        SECTION("wait_for_change()") {
            REQUIRE_FALSE(barq->wait_for_change());
        }
    }
}
...
TEST_CASE("KeyPathMapping generation") {
    TestFile config;
    barq::query_parser::KeyPathMapping mapping;
    ...
    SECTION("class aliasing") {
        ...
        auto table = barq->read_group().get_table("class_class_with_policy");
...
TEST_CASE("Concurrent operations") {
    SECTION("Async commits together with online compaction") {
        // This is a reproduction test for issue https://github.com/barq/barq-dart/issues/1396
        // First create a relatively large barq, then delete the content and do some more
        ...
        auto barq_1 = Barq::get_shared_barq(config);
        Results res(barq_1, barq_1->read_group().get_table("class_object")->where());
        auto barq_2 = Barq::get_shared_barq(config);

        {
            // Create a lot of objects
            barq_2->begin_transaction();
            auto table = barq_2->read_group().get_table("class_object");
            for (int i = 0; i < 400000; i++) {
                table->create_object().set("value", i);
            }
            barq_2->commit_transaction();
        }
        ...
        for (int i = 0; i < 4; i++) {
            barq_1->async_begin_transaction([&]() {
                ...
                res.clear();
                barq_1->async_commit_transaction([&](std::exception_ptr) {
                    commit_1++;
                });
            });
            barq_2->async_begin_transaction([&]() {
                // Make sure we will continue to have something to delete
                auto table = barq_2->read_group().get_table("class_object");
                for (int i = 0; i < 100; i++) {
                    table->create_object().set("value", i);
                }
                barq_2->async_commit_transaction([&](std::exception_ptr) {
                    commit_2++;
                });
            });
        }
    ...
    SECTION("No open barqs") {
        // This is just to check that the section above did not leave any barqs open
        _impl::BarqCoordinator::assert_no_open_barqs();
    }
}
...
TEST_CASE("Notification logging") {
    ...
    TestFile config;
    // util::LogCategory::barq.set_default_level_threshold(util::Logger::Level::all);
    config.schema_version = 1;
    config.schema = Schema{{"object", {{"value", PropertyType::Int}}}};
    ...
    auto table = barq->read_group().get_table("class_object");
    int changed = 0;
    Results res(barq, table->query("value == 5"));
