...
void create_dummy_barq(std::string path, std::shared_ptr<Barq>* out)
{
    Barq::Config config;
    config.path = path;
    auto barq = _impl::BarqCoordinator::get_coordinator(path)->get_barq(config, none);
    REQUIRE_BARQ_EXISTS(path);
    if (out) {
        *out = std::move(barq);
    }
}
...
int get_permissions(const std::string& path)
{
    ...
    BARQ_ASSERT_EX(ret == 0, ret, errno);
