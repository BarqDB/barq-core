...
#if BARQ_ENABLE_SYNC
#define BARQ_REGISTER_SYNC_CLIENT_HOOK_EVENT(X) barq::SyncClientHookEvent::X
CATCH_REGISTER_ENUM(barq::SyncClientHookEvent,
                    BARQ_FOR_EACH_SYNC_CLIENT_HOOK_EVENT(BARQ_REGISTER_SYNC_CLIENT_HOOK_EVENT))
#undef BARQ_REGISTER_SYNC_CLIENT_HOOK_EVENT
#endif
...
struct ExpectedBarqPaths {
    ExpectedBarqPaths(const std::string& base_path, const std::string& app_id, const std::string& user_identity,
...
Obj create_object(Barq& barq, StringData object_type, util::Optional<ObjectId> primary_key = util::none,
                  util::Optional<Partition> partition = util::none);

struct TestClientReset {
    using Callback = util::UniqueFunction<void(const SharedBarq&)>;
    using InitialObjectCallback = util::UniqueFunction<ObjectId(const SharedBarq&)>;
    TestClientReset(const Barq::Config& local_config, const Barq::Config& remote_config);
...
protected:
    barq::Barq::Config m_local_config;
    barq::Barq::Config m_remote_config;
