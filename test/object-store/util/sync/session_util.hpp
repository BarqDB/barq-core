...
inline std::string conn_id_for_barq(const SharedBarq& barq)
{
    return SyncSession::OnlyForTesting::get_barq_connection_id(*barq->sync_session());
}
...
std::shared_ptr<SyncSession> sync_session(
    ...
    std::string* on_disk_path = nullptr, util::Optional<Schema> schema = none, Barq::Config* out_config = nullptr)
{
