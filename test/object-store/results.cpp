...
#include <barq/object-store/impl/results_notifier.hpp>
...
class TestHelper {
public:
    static DBRef& get_shared_group(SharedBarq const& shared_barq)
    {
...
struct StringMaker<std::any> {
    ...
    {
        return barq::util::format("Any<%1>", any.type().name());
    }
};
template <>
struct StringMaker<barq::util::Optional<std::any>> {
    static std::string convert(barq::util::Optional<std::any> any)
    {
        return any ? barq::util::format("some(Any<%1>)", any->type().name()) : "none";
    }
};
} // namespace Catch
...
struct TestContext : CppContext {
    ...
    TestContext(TestContext& parent, barq::Obj& obj, barq::Property const& prop)
...
TEST_CASE("notifications: async delivery", "[notifications]") {
    ...
    SECTION("handling of results not ready") {
        ...
        SECTION("begin_transaction() does not run stale notifiers if they are for a different Barq") {
            // this would deadlock if beginning the write on the secondary Barq
            // waited for the primary Barq to be ready
            auto lock = coordinator->block_notifier_execution();
            make_remote_change();
    ...
    SECTION("begin_transaction() from within did_change() does not break delivering collection notification") {
        struct Context : BindingContext {
            _impl::BarqCoordinator& coordinator;
            Barq& barq;
            Context(_impl::BarqCoordinator& coordinator, Barq& barq)
...
TEST_CASE("notifications: skip", "[notifications]") {
    ...
    SECTION("skipping must be done from the Barq's thread") {
        advance_and_notify(*r);
        r->begin_transaction();
        JoiningThread([&] {
            REQUIRE_EXCEPTION(token1.suppress_next(), WrongThread, "Barq accessed from incorrect thread.");
        });
        r->cancel_transaction();
    }
...
TEST_CASE("notifications: sync", "[sync][pbs][notifications]") {
    ...
    SECTION("sync progress commits do not distrupt notifications") {
        auto r = Barq::get_shared_barq(config);
        auto wait_barq = Barq::get_shared_barq(config);

        Results results(r, r->read_group().get_table("class_object"));
        Results wait_results(wait_barq, wait_barq->read_group().get_table("class_object"));
        ...
        // Add an object to the Barq so that notifications are needed
        {
            auto write_barq = Barq::get_shared_barq(config);
            write_barq->begin_transaction();
            write_barq->read_group().get_table("class_object")->create_object_with_primary_key(0);
        ...
        // Start the server and wait for the Barq to be uploaded so that sync
        // makes some writes to the Barq and bumps the version
        server.start();
        wait_for_upload(*r);
...
TEST_CASE("notifications: results", "[notifications][results]") {
    ...
    SECTION("notifier query rerunning") {
        ...
        _impl::BarqCoordinator::register_notifier(notifier);
        advance_and_notify(*r);
        TableView tv;
...
TEST_CASE("results: notifier with no callbacks", "[notifications][results]") {
    ...
    auto coordinator = _impl::BarqCoordinator::get_coordinator(config.path);
    auto r = coordinator->get_barq(std::move(config), none);
    r->update_schema({
        {"object",
         {
             {"value", PropertyType::Int},
    ...
    SECTION("refresh() does not attempt to deliver stale results") {
        ...
        // they're for version 1 and the barq is at 2)
        r->refresh();
    }

    SECTION("should not pin the source version even after the Barq has been closed") {
        auto r2 = coordinator->get_barq();
        REQUIRE(r != r2);
        r->close();
        ...
        // Would now be 3 if the closed Barq is still pinning the version it was at
        REQUIRE(shared_group->get_number_of_versions() == 2);
    }
}
...
TEST_CASE("results: sort", "[results]") {
    ...
    auto table2 = barq->read_group().get_table("class_object 2");
    Results r(barq, table);
...
struct ResultsFromTable {
    static Results call(std::shared_ptr<Barq> r, ConstTableRef table)
    {
...
struct ResultsFromNothing {
    static Results call(std::shared_ptr<Barq>, ConstTableRef)
    {
        return Results();
    }
};
...
struct ResultsFromInvalidTable {
    static Results call(std::shared_ptr<Barq> r, ConstTableRef)
    {
...
TEST_CASE("results: list of primitives indexes", "[results]") {
    ...
    SECTION("index_of(null)") {
        REQUIRE(results.index_of(Mixed{}) == barq::not_found);
    }
    SECTION("index_of() with type checking") {
        SECTION("double does not match") {
            for (size_t i = 0; i < num_items; ++i)
                REQUIRE(results.index_of(Mixed(double(i))) == barq::not_found);
        }
    }
}
...
TEST_CASE("results: dictionary keys", "[results][dictionary]") {
    ...
    SECTION("index_of() non existant key") {
        REQUIRE(results.index_of(Mixed("foo")) == barq::npos);
    }
    SECTION("index_of() wrong key type") {
        REQUIRE(results.index_of(Mixed(int64_t(0))) == barq::npos);
    }
}
...
TEMPLATE_TEST_CASE("results: backed by nothing", "[results]", ResultsFromInvalidTable, ResultsFromNothing)
{
    InMemoryTestFile config;
    auto barq = Barq::get_shared_barq(config);
    Results results = TestType::call(barq, TableRef());
    TestContext ctx(barq);
...
TEST_CASE("results: set property value on all objects", "[results][batch updates]") {
    ...
    auto barq = Barq::get_shared_barq(config);
    auto table = barq->read_group().get_table("class_AllTypes");
    barq->begin_transaction();
    ...
    SECTION("set property values removes object from Results") {
        barq->begin_transaction();
        Results results(barq, table->where().equal(table->get_column_key("int"), 0));
    ...
    SECTION("set property value") {
        ...
        Object linked_obj(barq, "AllTypes", object_key);
        ...
        Object list_object(barq, "AllTypes", list_object_key);
...
TEST_CASE("results: nullable list of primitives", "[results]") {
    ...
    config.schema_version = 0;
    auto barq = Barq::get_shared_barq(config);
    auto table = barq->read_group().get_table("class_ListTypes");
    ...
    List nullable_decimal_list(barq, obj, nullable_decimal_col);
    List non_nullable_decimal_list(barq, obj, non_nullable_decimal_col);
    nullable_decimal_list.add(Decimal128{barq::null()});
    non_nullable_decimal_list.add(Decimal128{});
    List nullable_oid_list(barq, obj, nullable_oid_col);
    List non_nullable_oid_list(barq, obj, non_nullable_oid_col);
    ...
    SECTION("check property values on internal null type") {
        ...
        CHECK(r_nullable.get<Decimal128>(0) == Decimal128(barq::null()));
...
TEST_CASE("notifications: objects with PK recreated", "[results]") {
    ...
    auto coordinator = _impl::BarqCoordinator::get_existing_coordinator(config.path);
