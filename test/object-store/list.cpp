...
TEST_CASE("list", "[list]") {
    ...
    SECTION("add_notification_block()") {
        ...
        SECTION("multiple LinkViews for the same LinkList can get notifications") {
            ...
            for (auto& list : lists)
                advance_and_notify(*list.get_barq());
    ...
    SECTION("add(Context)") {
        ...
        SECTION("adds boxed barq::Object") {
            barq::Object obj(r, list.get_object_schema(), target->get_object(target_keys[5]));
        ...
        SECTION("throws for object in wrong table") {
            REQUIRE_EXCEPTION(list.add(ctx, std::any(origin->get_object(0))), ObjectTypeMismatch,
                              "Object of type (origin) does not match List type (target)");
            barq::Object object(r, *r->schema().find("origin"), origin->get_object(0));
    ...
    SECTION("find(Context)") {
        ...
        SECTION("returns index in list for boxed Object") {
            barq::Object obj(r, *r->schema().find("origin"), target->get_object(target_keys[5]));
...
TEST_CASE("embedded List", "[list]") {
    ...
    SECTION("find(Context)") {
        ...
        SECTION("returns index in list for boxed Object") {
            barq::Object obj(r, *r->schema().find("origin"), list.get(5));
            REQUIRE(list.find(ctx, std::any(obj)) == 5);
        }
...
TEST_CASE("list of embedded objects", "[list]") {
    ...
    auto parent_table = barq->read_group().get_table("class_parent");
    ColKey col_array = parent_table->get_column_key("array");
    auto embedded_table = barq->read_group().get_table("class_embedded");
    ...
    auto list = List(barq, parent, col_array);
