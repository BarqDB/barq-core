...
inline std::ostream& operator<<(std::ostream& out, const barq::object_store::Collection&)
{
    return out;
}
                   ...
                   cf::UnboxedOptional<cf::Binary>, cf::UnboxedOptional<cf::Date>, cf::UnboxedOptional<cf::Decimal>)
{
    ...
    SECTION("get_barq()") {
        REQUIRE(list.get_barq() == r);
        REQUIRE(results.get_barq() == r);
    }
#if 0
    SECTION("get_query()") {
    ...
    SECTION("verify_attached()") {
        ...
        SECTION("invalidate") {
            ...
            REQUIRE_EXCEPTION(list.verify_attached(), InvalidatedObject,
                              "List is no longer valid. Either the parent object was deleted or the containing Barq "
...
#if BARQ_ENABLE_SYNC && BARQ_HAVE_SYNC_STABLE_IDS
    SECTION("sync compatibility") {
        ...
        {
            auto r = Barq::get_shared_barq(sync_config);
            r->begin_transaction();
