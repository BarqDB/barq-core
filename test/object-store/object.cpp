...
public:
    ...
    CreatePolicyRecordingContext(std::shared_ptr<Barq>, const ObjectSchema*) {}
...
TEST_CASE("object") {
    ...
    SECTION("add_notification_callback()") {
        ...
        SECTION("deleting object before first run of notifier") {
            auto token = object.add_notification_callback(
                ...
                {});
            // Delete via a different Barq as begin_transaction() will wait
    ...
    for (auto policy : {CreatePolicy::UpdateAll, CreatePolicy::UpdateModified}) {
        SECTION("set existing fields to null with update "s + (policy.diff ? "(diffed)" : "(all)")) {
            ...
            AnyDict null_values{
                ...
                {"decimal array", AnyVec{Decimal128(barq::null())}},
            ...
            REQUIRE(d.get<List>(obj, "decimal array").get<Decimal>(0) == Decimal128(barq::null()));
    ...
    SECTION("create only requires properties explicitly in the schema") {
        ...
        auto subset_barq = Barq::get_shared_barq(config);
        subset_barq->begin_transaction();
        REQUIRE_NOTHROW(Object::create(d, subset_barq, "all types", std::any(AnyDict{{"_id", INT64_C(123)}})));
        subset_barq->commit_transaction();
    ...
    SECTION("defaults do not override values explicitly passed to create()") {
        ...
        auto r1 = Barq::get_shared_barq(config1);
        auto r2 = Barq::get_shared_barq(config2);
...
TEST_CASE("Embedded Object") {
    ...
    auto create = [&](std::any&& value, CreatePolicy policy = CreatePolicy::UpdateAll) {
        ...
        auto obj = Object::create(ctx, barq, *barq->schema().find("all types"), value, policy);
    ...
    SECTION("set_property_value() on list of embedded objects") {
        ...
        List list(barq, obj.get_obj().get_linklist("array"));
        auto obj2 = create(AnyDict{
            {"_id", INT64_C(2)},
            {"array", AnyVector{AnyDict{{"value", INT64_C(1)}}, AnyDict{{"value", INT64_C(2)}}}},
        });
        List list2(barq, obj2.get_obj().get_linklist("array"));
    ...
    SECTION("create with UpdateModified diffs child objects") {
        ...
        auto array_table = barq->read_group().get_table("class_array target");
        Results result(barq, array_table);
...
TEST_CASE("Asymmetric Object") {
    ...
    auto barq = Barq::get_shared_barq(config);
    {
        ...
        mut_subs.insert_or_assign(Query(barq->read_group().get_table("class_table")));
    ...
    auto create = [&](std::any&& value, std::string table_name, CreatePolicy policy = CreatePolicy::ForceCreate) {
        barq->begin_transaction();
        auto obj = Object::create(ctx, barq, *barq->schema().find(table_name), value, policy);
        barq->commit_transaction();
        return obj;
    };
    ...
    SECTION("Basic object creation") {
        ...
        REQUIRE(barq->is_empty());
    }

    SECTION("Re-open barq") {
        barq->close();
        barq.reset();
    ...
    SECTION("Delete ephemeral object before comitting") {
        barq->begin_transaction();
        auto obj = barq->read_group().get_table("class_asymmetric")->create_object_with_primary_key(1);
        obj.remove();
        barq->commit_transaction();
...
TEST_CASE("KeyPath generation - star notation") {
    ...
    auto kpa = barq->create_key_path_array("Person", {"*.*.*"});
    CHECK(kpa.size() == 8);
    // {class_Person:name}
    // {class_Person:age}
    ...
    // barq->print_key_path_array(kpa);

    kpa = barq->create_key_path_array("Person", {"*.name"});
    CHECK(kpa.size() == 1);
    // {class_Person:children}{class_Child:name}
    // barq->print_key_path_array(kpa);

    kpa = barq->create_key_path_array("Person", {"*.*.breed"});
    CHECK(kpa.size() == 1);
    // {class_Person:children}{class_Child:favoritePet}{class_Pet:breed}
    // barq->print_key_path_array(kpa);

    kpa = barq->create_key_path_array("Child", {"*.name"});
    CHECK(kpa.size() == 2);
    // {class_Child:favoritePet}{class_Pet:name}
    // {class_Child:{class_Person:children}->}{class_Person:name}
    // barq->print_key_path_array(kpa);

    kpa = barq->create_key_path_array("Person", {"children.*.breed"});
    CHECK(kpa.size() == 1);
    // {class_Person:children}{class_Child:favoritePet}{class_Pet:breed}
    // barq->print_key_path_array(kpa);

    CHECK_THROWS_AS(barq->create_key_path_array("Person", {"children.favoritePet.colour"}), InvalidArgument);
    CHECK_THROWS_AS(barq->create_key_path_array("Person", {"*.myPet.breed"}), InvalidArgument);
}
