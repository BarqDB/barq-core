                   ...
                   cf::UnboxedOptional<cf::Binary>, cf::UnboxedOptional<cf::Date>, cf::UnboxedOptional<cf::Decimal>)
{
    ...
    SECTION("get_barq()") {
        REQUIRE(dict.get_barq() == r);
        REQUIRE(values_as_results.get_barq() == r);
    }
    ...
    SECTION("find_any()") {
        for (auto val : values) {
            ...
            REQUIRE(ndx != barq::not_found);
        }
        dict.remove_all();
        for (auto val : values) {
            REQUIRE(dict.find_any(Mixed{val}) == barq::not_found);
        }
    }
    ...
    SECTION("notifications") {
        ...
        SECTION("now with links") {
            ...
            SECTION("with links on frozen Barq") {
...
TEST_CASE("dictionary comparison different barq", "[dictionary]") {
    TestFile config1;
    TestFile config2;
    Schema schema{
