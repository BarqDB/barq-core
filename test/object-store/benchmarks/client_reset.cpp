...
static TableRef get_table(Barq& barq, StringData object_type)
{
    return ObjectStore::table_for_object_type(barq.read_group(), object_type);
}

static Obj create_object(Barq& barq, StringData object_type, util::Optional<int64_t> primary_key = util::none)
{
    auto table = get_table(barq, object_type);
    REQUIRE(table);
    static int64_t pk = 0;
    FieldValues values = {};
...
struct BenchmarkLocalClientReset : public reset_utils::TestClientReset {
    BenchmarkLocalClientReset(barq::Barq::Config local_config, barq::Barq::Config remote_config)
    ...
    void prepare()
    {
        m_local = Barq::get_shared_barq(m_local_config);
        ...
        m_remote = Barq::get_shared_barq(m_remote_config);
    ...
    {
        m_did_run = true;
        BARQ_ASSERT(m_did_setup);
        auto frozen_local_barq = m_local->freeze();
        Transaction& frozen_local = (Transaction&)frozen_local_barq->read_group();
        m_local->begin_transaction();
        m_remote->begin_transaction();
...
TEST_CASE("client reset", "[sync][pbs][benchmark][client reset]") {
    ...
    Property partition_prop = {"barq_id", PropertyType::String | PropertyType::Nullable};
    Schema schema = {
        {"source",
    ...
    auto populate_objects = [&](SharedBarq barq, size_t num_objects) {
        TableRef table = get_table(*barq, "object");
        REQUIRE(table);
    ...
    auto populate_source_objects_with_links = [&](SharedBarq barq) {
        TableRef table = get_table(*barq, "source");
        TableRef dest = get_table(*barq, "object");
        REQUIRE(table);
        REQUIRE(dest);
        ColKey partition_col_key = table->get_column_key(partition_prop.name);
    ...
    SECTION(util::format("%1: populated with %2 simple objects", reset_mode, num_objects)) {
        test_reset.setup([&](SharedBarq barq) {
            populate_objects(barq, num_objects);
        });
        ...
        SECTION("remote removes half the local data") {
            test_reset.make_remote_changes([&](SharedBarq remote) {
        ...
        SECTION("local removes half the objects") {
            test_reset.make_local_changes([&](SharedBarq local) {
    ...
    SECTION(util::format("%1: %2 source objects linked to %2 dest objects", reset_mode, num_objects / 2)) {
        test_reset.setup([&](SharedBarq barq) {
            populate_objects(barq, num_objects / 2);
            populate_source_objects_with_links(barq);
        });
