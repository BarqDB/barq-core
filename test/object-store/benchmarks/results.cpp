...
TEST_CASE("Benchmark results notifier", "[benchmark][results]") {
    ...
    SECTION("100 strongly connected tables") {
        ...
        auto barq = Barq::get_shared_barq(config);
        auto table_0 = barq->read_group().get_table("class_table 0");

        BENCHMARK("create notifier") {
            Results r(barq, table_0->where());
...
TEST_CASE("aggregates", "[benchmark][aggregate]") {
    ...
    auto barq = Barq::get_shared_barq(config);
    auto table = barq->read_group().get_table("class_object");
    auto link_table = barq->read_group().get_table("class_link");
    auto prim_table = barq->read_group().get_table("class_primitive");
    ...
    Results table_results(barq, table);
    Results query_results(barq, table->where());
    Results tv_results(barq, table->where());
    ...
    BENCHMARK("object list") {
        return List(barq, obj_list).sum(col);
    };
    BENCHMARK("object set") {
        return object_store::Set(barq, obj_set).sum(col);
    };
    BENCHMARK("object dictionary") {
        return object_store::Dictionary(barq, obj_dict).sum(col);
    };
    BENCHMARK("int list") {
        return List(barq, int_list).sum();
    };
    BENCHMARK("int set") {
        return object_store::Set(barq, int_set).sum();
    };
    BENCHMARK("int dictionary") {
        return object_store::Dictionary(barq, int_dict).sum();
    };
}
...
TEST_CASE("Benchmark sectioned results", "[benchmark][results]") {
    ...
    auto key_fn = [&](Mixed value, const std::shared_ptr<Barq>&) -> Mixed {
    ...
    BENCHMARK("create and get section count") {
        auto size = Results(barq, table).sectioned_results(key_fn).size();
        REQUIRE(size == section_count);
    };

    BENCHMARK_ADVANCED("iterate directly")(Catch::Benchmark::Chronometer meter)
    {
        auto results = Results(barq, table).sectioned_results(key_fn);
