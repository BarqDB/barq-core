...
#define BARQ_SC_PRINT(type, ...)                                                                                    \
    ...
    BARQ_SC_PRINT(AddIndex, v.object, v.property, v.type)
    BARQ_SC_PRINT(AddProperty, v.object, v.property)
    BARQ_SC_PRINT(AddTable, v.object)
    BARQ_SC_PRINT(RemoveTable, v.object)
    BARQ_SC_PRINT(ChangeTableType, v.object, v.old_table_type, v.new_table_type)
    BARQ_SC_PRINT(AddInitialProperties, v.object)
    BARQ_SC_PRINT(ChangePrimaryKey, v.object, v.property)
    BARQ_SC_PRINT(ChangePropertyType, v.object, v.old_property, v.new_property)
    BARQ_SC_PRINT(MakePropertyNullable, v.object, v.property)
    BARQ_SC_PRINT(MakePropertyRequired, v.object, v.property)
    BARQ_SC_PRINT(RemoveIndex, v.object, v.property)
    BARQ_SC_PRINT(RemoveProperty, v.object, v.property)
...
TEST_CASE("ObjectSchema") {
    SECTION("Aliases are still present in schema returned from the Barq") {
        TestFile config;
        ...
        REQUIRE(barq->schema().find("object")->property_for_name("value")->public_name == "alias");
    }
...
TEST_CASE("Schema") {
    SECTION("validate()") {
        ...
        SECTION("asymmetric tables not allowed in local barq") {
            Schema schema = {
                {"location",
                 ObjectSchema::ObjectType::TopLevelAsymmetric,
                 {{"_id", PropertyType::Int, Property::IsPrimary{true}}, {"street", PropertyType::String}}},
            ...
            REQUIRE_EXCEPTION(schema.validate(), SchemaValidationFailed,
                              ContainsSubstring("Asymmetric table 'location' not allowed in a local Barq"));
        }
