...
static TableRef get_table(Barq& barq, StringData object_name)
{
    return ObjectStore::table_for_object_type(barq.read_group(), object_name);
}

static Object create_object(SharedBarq const& barq, StringData object_type, AnyDict value)
{
    CppContext ctx(barq);
    return Object::create(ctx, barq, object_type, std::any(value));
}
...
TEST_CASE("thread safe reference") {
    ...
    SECTION("version pinning") {
        ...
        auto history = make_in_barq_history();
    ...
    SECTION("coordinator pinning") {
        ...
        SECTION("Results retains the source BarqCoordinator") {
            auto coordinator = _impl::BarqCoordinator::get_existing_coordinator(config.path).get();
            auto table = r->read_group().get_table("class_int object");
            auto results = Results(r, table->where());
            auto ref = util::make_optional(ThreadSafeReference(results));

            r->close();
            REQUIRE(coordinator == _impl::BarqCoordinator::get_existing_coordinator(config.path).get());
            ref = {};
            REQUIRE_FALSE(_impl::BarqCoordinator::get_existing_coordinator(config.path).get());
        }
    ...
    SECTION("version mismatch") {
        ...
        SECTION("resolves at older version") {
            ...
            {
                SharedBarq r2 = Barq::get_shared_barq(config);
    ...
    SECTION("same thread") {
        ...
        SECTION("different barq") {
            did_run_section = true;
            {
                SharedBarq r = Barq::get_shared_barq(config);
                Object num = ref.resolve<Object>(r);
                REQUIRE(num.get_obj().get<Int>(col) == 7);
                r->begin_transaction();
                num.get_obj().set(col, 9);
    ...
    SECTION("passing over") {
        SECTION("read-only `ThreadSafeReference`") {
            ...
            create_object(barq, "int object", {{"value", INT64_C(42)}});
            barq->commit_transaction();
            barq->close();
            config.schema_mode = SchemaMode::Immutable;
            SharedBarq read_only_barq = Barq::get_shared_barq(config);
            auto table = read_only_barq->read_group().get_table("class_int object");
            Results results(read_only_barq, table);
            ...
            SECTION("read-only `ThreadSafeReference` to `Results`") {
                ...
                JoiningThread([&thread_safe_results, &config, int_obj_col]() mutable {
                    ...
                    SharedBarq barq_in_thread = Barq::get_shared_barq(config);
                    Results resolved_results = thread_safe_results.resolve<Results>(barq_in_thread);
            ...
            SECTION("read-only `ThreadSafeReference` to an `Object`") {
                Object object(read_only_barq, results.get(0));
                auto thread_safe_object = ThreadSafeReference(object);
                JoiningThread([&thread_safe_object, &config, int_obj_col]() mutable {
                    config.scheduler = util::Scheduler::make_dummy();
                    SharedBarq barq_in_thread = Barq::get_shared_barq(config);
                    auto resolved_object = thread_safe_object.resolve<Object>(barq_in_thread);
    ...
    SECTION("resolve after source version has been cleaned up") {
        ...
        SECTION("object") {
            ...
            SECTION("target Barq already in a read transaction") {
                r->read_group();
            }
            SECTION("target Barq not in a read transaction") {
            }
