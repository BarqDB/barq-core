...
struct CreateNewSet {
    using Test = T;
    static auto get_set(const std::shared_ptr<Barq>& r, Obj& obj, ColKey col)
    {
        return [&, col] {
            return object_store::Set{r, obj, col};
        };
    }
    static auto get_results(const std::shared_ptr<Barq>& r, Obj& obj, ColKey col)
    {
        return [&, col] {
                            ...
                            cf::UnboxedOptional<cf::Decimal>))
{
    ...
    SECTION("basic value operations") {
        ...
        auto check_empty = [&]() {
            REQUIRE(set().size() == 0);
            for (size_t i = 0; i < values.size(); ++i) {
                REQUIRE(set().find(T(values[i])) == barq::not_found);
            }
        };
        SECTION("remove()") {
            write([&] {
                            ...
                            cf::UnboxedOptional<cf::Decimal>))
{
    ...
    SECTION("basic value operations") {
        ...
        auto check_empty = [&] {
            REQUIRE(set().size() == 0);
            for (auto key : keys) {
                REQUIRE(set().find(key) == barq::not_found);
            }
        };
        SECTION("remove()") {
            write([&] {
...
TEMPLATE_TEST_CASE("set", "[set]", CreateNewSet<void>, ReuseSet<void>)
{
    ...
    SECTION("nullable decimal") {
        ...
        write([&] {
            ...
            CHECK(set().insert(Decimal128(barq::null())).second);
            CHECK(set().insert(Decimal128(7)).second);
        });

        REQUIRE(set().is_valid());
        CHECK(set().size() == 3);
        CHECK(results.index_of(Decimal128(barq::null())) == 0);
        auto sorted = results.sort({{"self", false}});
        CHECK(sorted.index_of(Decimal128(barq::null())) == 2);
    }
