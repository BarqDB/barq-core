...
TEST_CASE("Construct frozen Barq", "[frozen]") {
    TestFile config;
    ...
    SECTION("Create frozen Barq directly") {
        auto barq = Barq::get_shared_barq(config);
        barq->read_group();
        auto frozen_barq = Barq::get_frozen_barq(config, barq->read_transaction_version());
        REQUIRE(frozen_barq->is_frozen());
        REQUIRE(barq->read_transaction_version() == *frozen_barq->current_transaction_version());
    }
}

TEST_CASE("Freeze Barq", "[frozen]") {
    TestFile config;
    config.schema_version = 1;
    config.schema = Schema{
        {"object", {{"value", PropertyType::Int}}},
    ...
    SECTION("refresh() returns false") {
        REQUIRE(!frozen_barq->refresh());
    }

    SECTION("wait_for_change() returns false") {
        REQUIRE(!frozen_barq->wait_for_change());
    }

    SECTION("auto_refresh") {
        REQUIRE(!frozen_barq->auto_refresh());
        REQUIRE_EXCEPTION(frozen_barq->set_auto_refresh(true), WrongTransactionState,
                          "Auto-refresh cannot be enabled for frozen Barqs.");
        REQUIRE(!frozen_barq->auto_refresh());
    }

    SECTION("begin_transaction() throws") {
        REQUIRE_EXCEPTION(frozen_barq->begin_transaction(), WrongTransactionState,
                          "Can't perform transactions on a frozen Barq");
    }

    SECTION("can call methods on another thread") {
        JoiningThread thread([&] {
            // Smoke-test
            REQUIRE_NOTHROW(frozen_barq->write_copy());
            REQUIRE_NOTHROW(frozen_barq->read_transaction_version());
        });
    }
...
TEST_CASE("Freeze Results", "[frozen]") {
    ...
    auto linked_table = barq->read_group().get_table("class_linked to object");
    ...
    auto create_object = [&](int value, bool with_links = false) {
        ...
        auto int_list = List(barq, obj, int_list_col);
        object_store::Dictionary int_dict(barq, obj, int_dict_col);
...
#define VERIFY_VALID_RESULTS(results, barq)                                                                         \
    ...
    REQUIRE(results.freeze(barq).is_valid());

#define VERIFY_INVALID_RESULTS(results, barq, exception, message)                                                   \
    REQUIRE_EXCEPTION(results.freeze(barq), exception, message);                                                    \
...
#define VERIFY_STALE_RESULTS(results, barq)                                                                         \
    VERIFY_INVALID_RESULTS(results, barq, StaleAccessor, "Access to invalidated Results objects")

    SECTION("is_frozen") {
        Results results(barq, table);
        Results frozen_results = results.freeze(frozen_barq);
    ...
    SECTION("Result constructor - Empty") {
        ...
        Results frozen_res = res.freeze(frozen_barq);
        JoiningThread thread([&] {
            REQUIRE(frozen_res.is_frozen());
            REQUIRE(frozen_res.size() == 0);
            VERIFY_VALID_RESULTS(frozen_res, frozen_barq);
        });
    }

    SECTION("Result constructor - Table") {
        Results results = Results(frozen_barq, frozen_barq->read_group().get_table("class_object"));
        Results frozen_res = results.freeze(frozen_barq);
        JoiningThread thread([&] {
            auto obj = frozen_res.get(0);
            auto any = frozen_res.get_any(0);
            REQUIRE(obj.is_valid());
            REQUIRE(any.get_link() == obj.get_link());
            REQUIRE(Object(frozen_barq, obj).is_frozen());
            REQUIRE(frozen_res.get(0).get<int64_t>(value_col) == 2);
            REQUIRE(frozen_res.first()->get<int64_t>(value_col) == 2);

            VERIFY_VALID_RESULTS(results, frozen_barq);
            VERIFY_VALID_RESULTS(frozen_res, frozen_barq);
        });
    }

    SECTION("Result constructor - Primitive list") {
        const List list = List(frozen_barq, table->get_object(0), int_list_col);
        auto list_results = list.as_results();

        SECTION("unsorted") {
            Results frozen_res = list_results.freeze(frozen_barq);
        ...
        SECTION("sorted") {
            Results sorted_frozen_res = list.sort({{"self", false}}).freeze(frozen_barq);
    ...
    SECTION("Result constructor - Dictionary") {
        const object_store::Dictionary dict(frozen_barq, table->get_object(0), int_dict_col);
        auto dict_results = dict.as_results();

        Results frozen_res = dict_results.freeze(frozen_barq);
        JoiningThread thread1([&] {
            REQUIRE(frozen_res.is_frozen());
            REQUIRE(frozen_res.size() == 5);
        ...
        VERIFY_STALE_RESULTS(dict_results, barq);
        */
    }
    ...
    SECTION("Result constructor - Query") {
        ...
        Results query_results(barq, std::move(q), ordering);
        Results frozen_res = query_results.freeze(frozen_barq);
        JoiningThread thread([&] {
            auto obj = frozen_res.get(0);
            auto any = frozen_res.get_any(0);
            REQUIRE(obj.is_valid());
        ...
        /* FIXME causes ThreadSanitizer error in Catch2?
        write([&] {
            barq->read_group().remove_table(table->get_name());
        });
        VERIFY_STALE_RESULTS(query_results, barq);
        */
    }
    ...
    SECTION("Result constructor - TableView") {
        ...
        Results query_results(barq, tv, ordering);
        query_results.get(0);
        Results frozen_res = query_results.freeze(frozen_barq);
        JoiningThread thread([&] {
            auto obj = frozen_res.get(0);
    ...
    SECTION("Result constructor - LinkList") {
        ...
        Results res = Results(barq, link_list);
        Results frozen_res = res.freeze(frozen_barq);
        JoiningThread thread([&] {
            REQUIRE(frozen_res.is_frozen());
            REQUIRE(frozen_res.size() == 5);
            auto any = frozen_res.get_any(0);
            Object o = Object(frozen_barq, any.get_link());
            REQUIRE(o.is_frozen());
    ...
    SECTION("Results after source remove") {
        ...
        SECTION("Results on collection") {
            ...
            SECTION("Dictionary") {
                results = Results(barq, obj.get_dictionary_ptr(int_dict_col));
                write([&]() {
                ...
                VERIFY_STALE_RESULTS(results, barq);
            }

            SECTION("Links") {
                results = Results(barq, obj.get_linklist_ptr(object_link_col));
                ...
                auto frozen = results.freeze(barq);
        ...
        //       with barq::NoSuchTable on Query constructor through import_copy_of
        /* SECTION("Results on query") {
            results = Results(barq, table->column<Int>(value_col) > 0, DescriptorOrdering());
            do_remove = [&] {
                barq->read_group().remove_table(table->get_key());
            };
        } */
...
TEST_CASE("Freeze List", "[frozen]") {
    ...
    std::shared_ptr<LnkLst> object_link_view = obj.get_linklist_ptr(object_link_col);
    auto int_list = List(barq, obj, int_link_col);
    for (int j = 0; j < 5; ++j) {
        auto child_obj = linked_table->create_object();
        child_obj.set(linked_object_value_col, j + 10);
        object_link_view->add(child_obj.get_key());
    ...
    List frozen_link_list = List(barq, *link_list).freeze(frozen_barq);
    List frozen_primitive_list = List(barq, table->get_object(0), int_link_col).freeze(frozen_barq);
...
TEST_CASE("Reclaim Frozen", "[frozen]") {
    ...
    auto barq = Barq::get_shared_barq(config);
    auto table = barq->read_group().get_table("class_table");
    ...
    for (int j = 0; j < num_iterations; ++j) {
        ...
        if (entry.barq && !entry.barq->is_frozen()) {
            auto& r = entry.barq;
            BARQ_ASSERT(r->is_in_read_transaction());
            auto before = r->current_transaction_version();
            r->refresh();
            auto after = r->current_transaction_version();
            BARQ_ASSERT(before != after);
        }

        // set up and save a new barq for later refresh, replacing the old one
        // which we refreshed above
        int should_freeze = (random_int() % 5) != 0; // freeze 80%
        auto barq2 = should_freeze ? barq->freeze() : Barq::get_shared_barq(config);
        entry.barq = barq2;
        int key = (unsigned)random_int() % num_objects;
        auto table2 = barq2->read_group().get_table(table_key);
        auto& o = entry.o;
        o = Object(barq2, table2->get_object(key));
        ...
        for (int k = 0; k < num_checks_pr_trans; ++k) {
            ...
            if (entry.barq) {
                ...
                auto table = entry.barq->read_group().get_table(table_key);
...
TEST_CASE("Freeze Object", "[frozen]") {
    ...
    Results results(barq, table);
    auto frozen_barq = Barq::get_frozen_barq(config, barq->read_transaction_version());
    Object frozen_obj = Object(barq, table->get_object(0)).freeze(frozen_barq);
    CppContext ctx(frozen_barq);
...
TEST_CASE("Freeze dictionary", "[frozen]") {
    ...
    auto frozen_obj_dict = object_store::Dictionary(barq, obj_dict).freeze(frozen_barq);
    auto frozen_int_dict = object_store::Dictionary(barq, table->get_object(0), int_col).freeze(frozen_barq);
