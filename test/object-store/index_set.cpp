...
TEST_CASE("index_set: shift_for_insert_at()", "[index set]") {
    ...
    SECTION("does nothing given an empty set of insertion points") {
        set = {5, 8};
        set.shift_for_insert_at(barq::IndexSet{});
...
TEST_CASE("index_set: erase_or_unshift()", "[index set]") {
    ...
    SECTION("returns npos for indices in the set") {
        set = {1, 3, 5};
        REQUIRE(barq::IndexSet(set).erase_or_unshift(1) == barq::IndexSet::npos);
        REQUIRE(barq::IndexSet(set).erase_or_unshift(3) == barq::IndexSet::npos);
        REQUIRE(barq::IndexSet(set).erase_or_unshift(5) == barq::IndexSet::npos);
    }

    SECTION("returns the number of indices in the set before the index for ones not in the set") {
        set = {1, 3, 5, 6};
        REQUIRE(barq::IndexSet(set).erase_or_unshift(0) == 0);
        REQUIRE(barq::IndexSet(set).erase_or_unshift(2) == 1);
        REQUIRE(barq::IndexSet(set).erase_or_unshift(4) == 2);
        REQUIRE(barq::IndexSet(set).erase_or_unshift(7) == 3);
    }
}
