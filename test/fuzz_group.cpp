...
#define BARQ_VERIFY true

#if BARQ_VERIFY
#define BARQ_DO_IF_VERIFY(log, op)                                                                                  \
    do {                                                                                                             \
        if (log)                                                                                                     \
            *log << #op << ";\n";                                                                                    \
        op;                                                                                                          \
...
#else
#define BARQ_DO_IF_VERIFY(log, owner)                                                                               \
    do {                                                                                                             \
    } while (false)
#endif
...
void parse_and_apply_instructions(std::string& in, const std::string& path, std::ostream* log)
{
    ...
    const size_t max_tables = BARQ_MAX_BPNODE_SIZE * 10;
    ...
    if (log) {
        *log << "// Test case generated in " BARQ_VER_CHUNK " on " << get_current_time_stamp() << ".\n";
        *log << "// BARQ_MAX_BPNODE_SIZE is " << BARQ_MAX_BPNODE_SIZE << "\n";
        *log << "// ----------------------------------------------------------------------\n";
        ...
        *log << "std::unique_ptr<Replication> hist(make_in_barq_history());\n";
        ...
        for (;;) {
            ...
            */
            ...
            if (instr == ADD_TABLE && wt->size() < max_tables) {
                ...
                BARQ_DO_IF_VERIFY(log, wt->verify());
                if (log) {
                ...
                rt->advance_read();
                BARQ_DO_IF_VERIFY(log, rt->verify());
            }
                ...
                // try to fuzz find this: https://github.com/barq/barq-core/issues/1769
                ...
                if (log) {
                    *log << "BARQ_ASSERT_RELEASE(db_w.compact());\n";
                }
                BARQ_ASSERT_RELEASE(db_w.compact());
...
static void usage(const char* argv[])
{
    fprintf(stderr,
            ...
            "                 to make sure the test don't use the same Barq file\n"
    ...
    if (file_names_from_stdin) {
        ...
        while (std::cin) {
            ...
            if (!in.is_open()) {
                ...
                barq::test_util::BarqPathInfo test_context{name};
