...
using barq::test_util::crypt_key;
...
TEST_IF(Transactions_LargeMappingChange, BARQ_ANDROID == 0 && TEST_DURATION > 0)
{
...
TEST(Transactions_StateChanges)
{
    ...
    CHECK_THROW(writer->freeze(), barq::LogicError);
    writer->commit_and_continue_as_read();
    // verify that we cannot modify data in a read transaction
    CHECK_THROW(writer->add_table("gylle"), barq::LogicError);
    CHECK_THROW(obj.set(col, 100), barq::LogicError);
    ...
    CHECK_THROW(frozen_obj.set<int64_t>(col, 47), barq::LogicError);
    ...
    CHECK_THROW(tr->create_object(), barq::LogicError);
...
#if defined(_WIN32) || BARQ_ANDROID || BARQ_PLATFORM_APPLE
