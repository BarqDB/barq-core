...
#if !BARQ_MOBILE
TEST_IF(Sync_HistoryMigration, BARQ_MAX_BPNODE_SIZE == 1000)
{
    ...
    if (bootstrapped) {
        ...
        BARQ_ASSERT(latest_client_schema_version <= sync::get_client_history_schema_version());
    }
    ...
    if (bootstrapped) {
        ...
        BARQ_ASSERT(latest_server_schema_version <= _impl::get_server_history_schema_version());
    }
    ...
    auto modify = [&](const std::string& client_path, StringData label, int old_value, int new_value) {
        ...
        BARQ_ASSERT(col_key_label);
        BARQ_ASSERT(col_key_value);
    ...
    auto stash_client_file_if_new = [&](const std::string& client_path) {
        ...
        int history_type = 0, history_schema_version = 0;
        get_history_info(client_path, history_type, history_schema_version);
        BARQ_ASSERT(history_type == Replication::hist_SyncClient);
        if (bootstrapped) {
    ...
    auto stash_server_file_if_new = [&](const std::string& server_path) {
        ...
        int history_type = 0, history_schema_version = 0;
        get_history_info(server_path, history_type, history_schema_version);
        BARQ_ASSERT(history_type == Replication::hist_SyncServer);
        if (bootstrapped) {
