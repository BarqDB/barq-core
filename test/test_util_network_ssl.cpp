...
void connect_ssl_streams(network::ssl::Stream& server_stream, network::ssl::Stream& client_stream)
{
    ...
    BARQ_ASSERT(server_handshake_occurred);
    BARQ_ASSERT(client_handshake_occurred);
}
...
TEST(Util_Network_SSL_PrematureEndOfInputOnHandshakeRead)
{
    ...
    auto consumer = [&] {
        ...
        BARQ_ASSERT(ec == MiscExtErrors::end_of_input);
    };
...
TEST(Util_Network_SSL_BrokenPipeOnHandshakeWrite)
{
    ...
    // released.
    BARQ_ASSERT(ec == error::broken_pipe || ec == error::connection_reset || ec.value() == EHOSTDOWN);
#else
    BARQ_ASSERT(ec == error::broken_pipe);
#endif
