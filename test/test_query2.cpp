...
TEST(Query_OfByOne)
{
    ...
    size_t cluster_size = (BARQ_MAX_BPNODE_SIZE > 256) ? 256 : 4;
...
TEST(Query_AggregateSortedView)
{
    ...
    const int count = BARQ_MAX_BPNODE_SIZE * 2;
...
TEST(Query_NullStrings)
{
    ...
    auto k1 = table.create_object().set<String>(col, barq::null()).get_key(); // NULL string
    auto k2 = table.create_object().set<String>(col, "").get_key();            // Empty string

    q = table.column<StringData>(col) == barq::null();
    v = q.find_all();
    ...
    // contrary to SQL, comparisons with barq::null() can be true in Barq (todo, discuss if we want this behaviour)
        ...
        for (size_t t = 0; t < 10; t++) {
            ...
            for (size_t i = 0; i < 100; i++) {
                ...
                if (action > 48 && table.size() < 10) {
                    ...
                    char buf[] =
                        "This string is around 90 bytes long, which falls in the long-string type of Barq strings";
...
TEST(Query_NullShowcase)
{
    ...
    */
    ...
    // for internal Barq representation, because that's a NaN, hence making it harder for the end user
    ...
    // Barq may return a signalling/quiet NaN that is different from the signalling/quiet NaN you stored
    // (the IEEE standard defines a sequence of bits in the NaN that can have custom contents). Barq does
...
#if !defined(_WIN32) && !BARQ_ARCHITECTURE_X86_32
...
TEST(Query_Null_ManyRows)
{
    ...
    // Do that for all Barq data types
    tv = (price == null()).find_all();
...
TEST_TYPES(Query_EqualityInts, int64_t, util::Optional<int64_t>)
{
    ...
    int64_t id = -1;
    int64_t sum = 0;
    constexpr static size_t num_rows = BARQ_MAX_BPNODE_SIZE + 10;
...
TEST(Query_ArrayLeafRelocate)
{
    for (size_t iter = 0; iter < 10; iter++) {
        ...
        // that was relocated. https://github.com/barq/barq-core/issues/2269
        ...
        for (size_t t = 0; t < BARQ_MAX_BPNODE_SIZE + 1; t++) {
...
TEST_IF(Query_IntOrQueryPerformance, TEST_DURATION > 0)
{
    ...
    auto run_queries = [&](int num_matches) {
        ...
        Query q_nullables =
            (table->column<Int>(nullable_ints_col_key) == -1).Or().equal(nullable_ints_col_key, barq::null());
...
TEST(Query_LinkListIntPastOneIsNull)
{
    ...
    Query q = table_bar->link(col_list).column<Int>(col_int) == barq::null();
...
TEST(Query_links_to_with_bpnode_split)
{
    ...
    // instead of a BPTree. So this only worked when the number of items < BARQ_MAX_BPNODE_SIZE
    Group g;
    auto table = g.add_table("Foo");
    auto origin = g.add_table("Origin");
    auto col_int = table->add_column(type_Int, "int");
    auto col_link = origin->add_column(*table, "link");
    auto col_links = origin->add_column_list(*table, "links");
    constexpr size_t num_items = BARQ_MAX_BPNODE_SIZE + 1;
