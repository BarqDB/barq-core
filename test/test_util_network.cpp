...
void connect_sockets(network::Socket& socket_1, network::Socket& socket_2)
{
    ...
    BARQ_ASSERT(accept_occurred);
    BARQ_ASSERT(connect_occurred);
...
TEST(Network_AsyncReadWriteLargeAmount)
{
    ...
    auto reader = [&] {
        ...
        size_t offset_in_chunk = 0;
        int chunk_index = 0;
        barq::util::UniqueFunction<void()> read_chunk = [&] {
...
TEST(Network_RepeatedCancelAndRestartRead)
{
    ...
    for (int i = 0; i < 1; ++i) {
        ...
        size_t num_bytes_read = 0;
        bool end_of_input_seen = false;
        barq::util::UniqueFunction<void()> initiate_read = [&] {
            auto handler = [&](std::error_code ec, size_t n) {
                num_bytes_read += n;
                if (ec == MiscExtErrors::end_of_input) {
                    end_of_input_seen = true;
...
TEST(Network_StressTest)
{
    ...
    auto thread = [&](int id, network::Socket& socket, const char* read_original, const char* write_original,
        ...
        barq::util::UniqueFunction<void()> shedule_cancellation = [&] {
            if (progress) {
            ...
            cancellation_timer.async_wait(std::chrono::microseconds(microseconds_per_cancellation),
                                          [&](Status status) {
                                              BARQ_ASSERT(status.is_ok() || status == ErrorCodes::OperationAborted);
                                              if (status == ErrorCodes::OperationAborted)
                                                  return;
        ...
        barq::util::UniqueFunction<void()> read = [&] {
            ...
            auto handler = [&](std::error_code ec, size_t n) {
                BARQ_ASSERT(!ec || ec == error::operation_aborted);
                ++stats.num_reads;
                if (ec == error::operation_aborted) {
        ...
        barq::util::UniqueFunction<void()> write = [&] {
...
TEST(Sync_Trigger_Basics)
{
    ...
    barq::util::UniqueFunction<void()> func_2;
    Trigger<network::Service> trigger_2(&service, [&](barq::Status) {
        func_2();
    });
    was_triggered = false;
    ...
    auto func_4 = [&](barq::Status) {
        was_triggered_4 = true;
    };
    auto func_5 = [&](barq::Status) {
