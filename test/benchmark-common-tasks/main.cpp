...
/**
  ...
  - Cases have been derived from:
    https://github.com/barq/barq-java/blob/bp-performance-test/barq/src/androidTest/java/io/barq/BarqPerformanceTest.java
...
struct BenchmarkInsertPKToIndexForType : public BenchmarkWithType<Type> {
    ...
    void before_all(DBRef group) override
    {
        ...
        BARQ_ASSERT(t->has_search_index(Base::m_col));
        tr.commit();
...
struct BenchmarkQueryTimestampGreater : BenchmarkWithTimestamps {
    ...
    void operator()(DBRef)
    {
        ...
        TableView results = query.find_all();
        BARQ_ASSERT_EX(results.size() == values.size() - num_results_to_needle - 1, results.size(),
...
struct BenchmarkQueryTimestampGreaterEqual : BenchmarkWithTimestamps {
    ...
    void operator()(DBRef)
    {
        ...
        TableView results = query.find_all();
        BARQ_ASSERT_EX(results.size() == values.size() - num_results_to_needle, results.size(),
                        num_results_to_needle, values.size());
        static_cast<void>(results);
    }
};
...
struct BenchmarkQueryTimestampLess : BenchmarkWithTimestamps {
    ...
    void operator()(DBRef)
    {
        ...
        TableView results = query.find_all();
        BARQ_ASSERT_EX(results.size() == num_results_to_needle, results.size(), num_results_to_needle,
                        values.size());
        static_cast<void>(results);
    }
};
...
struct BenchmarkQueryTimestampLessEqual : BenchmarkWithTimestamps {
    ...
    void operator()(DBRef)
    {
        ...
        TableView results = query.find_all();
        BARQ_ASSERT_EX(results.size() == num_results_to_needle + 1, results.size(), num_results_to_needle,
                        values.size());
        static_cast<void>(results);
    }
};
...
struct BenchmarkQueryTimestampEqual : BenchmarkWithTimestamps {
    ...
    void operator()(DBRef)
    {
        ...
        TableView results = query.find_all();
        BARQ_ASSERT_EX(results.size() == values.count(needle), results.size(), num_results_to_needle,
...
struct BenchmarkQueryTimestampNotEqual : BenchmarkWithTimestamps {
    ...
    void operator()(DBRef)
    {
        ...
        TableView results = query.find_all();
        BARQ_ASSERT_EX(results.size() == values.size() - values.count(needle) + num_nulls_added, results.size(),
...
struct BenchmarkQueryTimestampNotNull : BenchmarkWithTimestamps {
    ...
    {
        ConstTableRef table = m_table;
        Query query = table->where().not_equal(m_col, barq::null());
        TableView results = query.find_all();
        BARQ_ASSERT_EX(results.size() == values.size(), results.size(), num_nulls_added, num_results_to_needle,
                        values.size());
        static_cast<void>(results);
    }
};
...
struct BenchmarkQueryTimestampEqualNull : BenchmarkWithTimestamps {
    ...
    {
        ConstTableRef table = m_table;
        Query query = table->where().equal(m_col, barq::null());
        TableView results = query.find_all();
        BARQ_ASSERT_EX(results.size() == num_nulls_added, results.size(), num_nulls_added, values.size());
        static_cast<void>(results);
    }
};
...
struct BenchmarkIntVsDoubleColumns : Benchmark {
    ...
    void operator()(DBRef)
    {
        ...
        BARQ_ASSERT_3(q.count(), ==, ((num_rows / 2) - 1));
    }
...
struct BenchmarkQueryChainedOrInts : BenchmarkWithIntsTable {
    ...
    void before_all(DBRef group)
    {
        ...
        BARQ_ASSERT(num_rows > num_queried_matches);
        Random r;
        size_t i = 0;
        for (auto e : *t) {
            e.set<Int>(m_col, i);
    ...
    void operator()(DBRef)
    {
        ...
        TableView results = query.find_all();
        BARQ_ASSERT_EX(results.size() == num_queried_matches, results.size(), num_queried_matches,
...
struct BenchmarkQueryChainedOrIntsCount : BenchmarkQueryChainedOrInts {
    ...
    void operator()(DBRef)
    {
        ...
        BARQ_ASSERT_EX(matches == num_queried_matches, matches, num_queried_matches, values_to_query.size());
    }
};
...
struct BenchmarkQueryIntEquality : BenchmarkQueryChainedOrInts {
    ...
    void operator()(DBRef)
    {
        ...
        for (int k = 0; k < 1000; k++) {
            ...
            TableView results = query.find_all();
            BARQ_ASSERT_EX(results.size() == 1, results.size(), 1);
            static_cast<void>(results);
        }
    }
};
...
struct BenchmarkQueryNotChainedOrStrings : BenchmarkWithStringsTableForIn {
    ...
    void operator()(DBRef)
    {
        ...
        query.end_group();
        TableView results = query.find_all();
        BARQ_ASSERT_EX(results.size() == num_rows - num_queried_matches, results.size(),
...
struct BenchmarkNonInitiatorOpen : Benchmark {
    ...
    // the shared barq will be removed after the benchmark finishes
    std::unique_ptr<barq::test_util::DBTestPathGuard> path;
    DBRef initiator;
    ...
    void before_all(DBRef)
    {
        ...
        path = std::make_unique<DBTestPathGuard>(get_test_path(ident, ".barq"));
...
void run_benchmark(BenchmarkResults& results, bool force_full = false)
{
    ...
    for (auto it = configs.begin(); it != configs.end(); ++it) {
        ...
        barq::test_util::unit_test::TestDetails test_details;
        test_details.suite_name = "BenchmarkCommonTasks";
        test_details.test_name = ident.c_str();
        test_details.file_name = __FILE__;
        test_details.line_number = __LINE__;
        ...
        // Open a SharedGroup:
        barq::test_util::DBTestPathGuard barq_path(
            test_util::get_test_path("benchmark_common_tasks_" + ident, ".barq"));
        DBRef group;
        group = DB::create(barq_path, DBOptions(level, key));
...
{
    std::string results_file_stem = barq::test_util::get_test_path_prefix();
