...
TEST(Column_FindLeafs)
{
    ...
    // when testing with BARQ_MAX_BPNODE_SIZE=4
    for (size_t i = 0; i < BARQ_MAX_BPNODE_SIZE * 5; ++i)
        a.add(0);

    // Set sentinel values at before and after each break
    a.set(0, 1);
    a.set(BARQ_MAX_BPNODE_SIZE - 1, 2);
    a.set(BARQ_MAX_BPNODE_SIZE, 3);
    a.set(BARQ_MAX_BPNODE_SIZE * 2 - 1, 4);
    a.set(BARQ_MAX_BPNODE_SIZE * 2, 5);
    a.set(BARQ_MAX_BPNODE_SIZE * 3 - 1, 6);
    a.set(BARQ_MAX_BPNODE_SIZE * 3, 7);
    a.set(BARQ_MAX_BPNODE_SIZE * 4 - 1, 8);
    a.set(BARQ_MAX_BPNODE_SIZE * 4, 9);
    a.set(BARQ_MAX_BPNODE_SIZE * 5 - 1, 10);
    ...
    CHECK_EQUAL(0, res1);
    CHECK_EQUAL(BARQ_MAX_BPNODE_SIZE - 1, res2);
    CHECK_EQUAL(BARQ_MAX_BPNODE_SIZE, res3);
    CHECK_EQUAL(BARQ_MAX_BPNODE_SIZE * 2 - 1, res4);
    CHECK_EQUAL(BARQ_MAX_BPNODE_SIZE * 2, res5);
    CHECK_EQUAL(BARQ_MAX_BPNODE_SIZE * 3 - 1, res6);
    CHECK_EQUAL(BARQ_MAX_BPNODE_SIZE * 3, res7);
    CHECK_EQUAL(BARQ_MAX_BPNODE_SIZE * 4 - 1, res8);
    CHECK_EQUAL(BARQ_MAX_BPNODE_SIZE * 4, res9);
    CHECK_EQUAL(BARQ_MAX_BPNODE_SIZE * 5 - 1, res10);
...
TEST(Column_Iterators)
{
    ...
    auto barq_begin = c.cbegin();
    auto barq_it = c.cbegin();
    auto barq_end = c.cend();

    CHECK_EQUAL(std_end - std_it, barq_end - barq_it);

    while (std_it != std_end) {
        CHECK_EQUAL(*std_it, *barq_it);
        auto std_found = std::find(std_begin, std_end, *std_it);
        auto barq_found = std::find(barq_begin, barq_end, *barq_it);
        CHECK_EQUAL(std_end - std_found, barq_end - barq_found);
        CHECK_EQUAL(std_found - std_begin, barq_found - barq_begin);
        size_t ndx = c.find_first(*barq_it);
        if (ndx == npos) {
            CHECK_EQUAL(barq_found, barq_end);
        }
        else {
            CHECK_EQUAL(ndx, barq_found - barq_begin);
            CHECK_EQUAL(ndx, barq_found.get_position());
        }
    ...
    // operator +
    std_it = std_begin + 5;
    barq_it = barq_begin + 5;
    CHECK_EQUAL(std_end - std_it, barq_end - barq_it);
    CHECK_EQUAL(std_it - std_begin, barq_it - barq_begin);
    CHECK_EQUAL(barq_it.get_position(), 5);
    // operator +=
    std_it += 10;
    barq_it += 10;
    CHECK_EQUAL(std_end - std_it, barq_end - barq_it);
    CHECK_EQUAL(std_it - std_begin, barq_it - barq_begin);
    CHECK_EQUAL(barq_it.get_position(), 15);
    // operator -=
    std_it -= 10;
    barq_it -= 10;
    CHECK_EQUAL(std_end - std_it, barq_end - barq_it);
    ...
    CHECK_EQUAL(std_end - std_it, barq_end - barq_it);
    CHECK_EQUAL(std_it - std_begin, barq_it - barq_begin);
    CHECK_EQUAL(barq_it.get_position(), 0);
    // operator ++
    std_it++;
    ++std_it;
    barq_it++;
    ...
    CHECK_EQUAL(std_end - std_it, barq_end - barq_it);
    CHECK_EQUAL(std_it - std_begin, barq_it - barq_begin);
    CHECK_EQUAL(barq_it.get_position(), 2);
    // operator --
    std_it--;
    --std_it;
    barq_it--;
    ...
    int64_t barq_value = (barq_it + 10)[5];
    CHECK_EQUAL(std_value, barq_value);
    std_value = (std_it + 10)[-5];
    barq_value = (barq_it + 10)[-5];
    CHECK_EQUAL(std_value, barq_value);
    // operator equality
    auto barq_next = barq_it + 1;
    CHECK(barq_next == barq_next);
    CHECK(barq_it == barq_it);
    CHECK(barq_it != barq_next);
    CHECK(barq_next != barq_it);
    CHECK_EQUAL(barq_next, barq_it + 1);
    // operator <
    CHECK(barq_it < barq_next);
    CHECK(!(barq_it < barq_it));
    CHECK(!(barq_next < barq_it));
