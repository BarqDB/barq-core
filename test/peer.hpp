...
#ifndef BARQ_TEST_PEER_HPP
#define BARQ_TEST_PEER_HPP
...
using barq::sync::HistoryEntry;
using barq::sync::RemoteChangeset;
using barq::sync::SyncReplication;
using barq::sync::TransformHistory;
...
public:
    ...
    {
        BARQ_ASSERT(orig_version == s_initial_version + m_core_entries.size());
                                    ...
                                    HistoryEntry& entry) const noexcept
    {
        ...
        for (size_t i = 0; i < size_t(end_version - begin_version); ++i) {
            ...
            if (entry.changeset.size() > 0 || !only_nonempty) {
                ...
                bool from_remote;
                if (is_server) {
                    BARQ_ASSERT(remote_file_ident != servers_file_ident());
    ...
    -> version_type
{
    ...
    BARQ_ASSERT(local_version == s_initial_version + m_entries.size());

    std::vector<Changeset> changesets;
    changesets.resize(num_changesets);

    for (size_t i = 0; i < num_changesets; ++i) {
        BARQ_ASSERT(incoming_changesets[i].last_integrated_local_version <= local_version);
    ...
    BARQ_ASSERT(m_entries.size() == m_core_entries.size() + 1);
...
public:
                                               ...
                                               const std::string path_add_on = "")
    {
        ...
        BARQ_ASSERT(client_file_ident != ShortCircuitHistory::servers_file_ident());
        std::ostringstream out;
        out << ".client_" << client_file_ident << path_add_on << ".barq";
        std::string suffix = out.str();
        std::string test_path = get_test_path(test_context.get_test_name(), suffix);
        return std::unique_ptr<Peer>(
            new Peer(client_file_ident, test_path, changeset_dump_dir_gen, *test_context.logger));
    ...
    {
        BARQ_ASSERT(group); // Must be in transaction
    ...
    bool integrate_next_changesets_from(const Peer& remote, size_t num_changesets)
    {
        ...
        BARQ_ASSERT(local_file_ident != remote.local_file_ident);
        // Star shaped topology required
        BARQ_ASSERT((local_file_ident == ShortCircuitHistory::servers_file_ident()) !=
                     (remote.local_file_ident == ShortCircuitHistory::servers_file_ident()));
        BARQ_ASSERT(!group); // A transaction must not be in progress
...
struct Associativity {
    ...
    struct Iteration {
        ...
        {
            BARQ_ASSERT(this->sync_order.size() == num_clients);
