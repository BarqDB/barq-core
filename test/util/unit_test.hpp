...
#ifndef BARQ_TEST_UTIL_UNIT_TEST_HPP
#define BARQ_TEST_UTIL_UNIT_TEST_HPP
...
/// TestList::run().
#define TEST_IF(name, enabled) TEST_EX(name, barq::test_util::unit_test::get_default_test_list(), enabled, true)
...
#define NONCONCURRENT_TEST_IF(name, enabled)                                                                         \
    TEST_EX(name, barq::test_util::unit_test::get_default_test_list(), enabled, false)

#define TEST_EX(name, list, enabled, allow_concur)                                                                   \
    struct Barq_UnitTest__##name : barq::test_util::unit_test::TestBase {                                          \
        static bool test_enabled()                                                                                   \
        {                                                                                                            \
            return bool(enabled);                                                                                    \
        }                                                                                                            \
        Barq_UnitTest__##name(barq::test_util::unit_test::TestContext& c)                                          \
            : TestBase(c)                                                                                            \
        {                                                                                                            \
        }                                                                                                            \
        void test_run();                                                                                             \
    };                                                                                                               \
    barq::test_util::unit_test::RegisterTest<Barq_UnitTest__##name> barq_unit_test_reg__##name(                   \
...
template <typename T>
constexpr static bool barq_serializable_types =
    is_any_v<T, StringData, BinaryData, Timestamp, ObjectId, std::optional<ObjectId>, ObjKey, ObjLink, UUID,
             std::optional<UUID>, bool, float, std::optional<float>, double, std::optional<double>, barq::null>;

template <class T>
void to_string(const T& value, std::string& str)
{
    if constexpr (barq_serializable_types<T>) {
...
#endif // BARQ_TEST_UTIL_UNIT_TEST_HPP
