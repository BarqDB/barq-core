...
#if BARQ_APPLE_DEVICE || TARGET_OS_SIMULATOR
...
std::shared_ptr<DB> get_test_db(const std::string& path, const char* crypt_key)
{
    ...
    barq::util::Logger::Level core_log_level = barq::util::Logger::Level::off;
    if (str && strlen(str) != 0) {
    ...
    return DB::create(make_in_barq_history(), path, options);
}
