...
#include <barq/object-store/impl/object_notifier.hpp>
...
// TODO look into moving some of this to barq-core
struct Helpers {
    static TableRef get_table(const SharedBarq& barq, StringData name)
    {
        return barq->read_group().get_table(name);
    }
    static TableRef get_table(const SharedBarq& barq, TableKey key)
    {
        return barq->read_group().get_table(key);
    }
    static query_parser::KeyPathMapping get_keypath_mapping(const SharedBarq& barq)
    {
        query_parser::KeyPathMapping mapping;
        populate_keypath_mapping(mapping, *barq);
        return mapping;
    }
    static Results results_from_query(const SharedBarq& barq, Query q)
    {
        auto ordering = q.get_ordering();
        return Results(barq, std::move(q), ordering ? *ordering : DescriptorOrdering());
    }
    ...
    static std::shared_ptr<_impl::ObjectNotifier> make_object_notifier(const SharedBarq& barq, const Obj& obj)
    {
        barq->verify_thread();
        barq->verify_notifications_available();
        auto notifier = std::make_shared<_impl::ObjectNotifier>(barq, obj);
    ...
    struct BindingContextMethods {
        util::UniqueFunction<void(SharedBarq)> did_change;
        util::UniqueFunction<void(SharedBarq)> before_notify;
        util::UniqueFunction<void(SharedBarq)> schema_did_change;
    };

    static bool has_binding_context(const Barq& barq)
    {
        return bool(barq.m_binding_context);
    }
    static void set_binding_context(const SharedBarq& barq, BindingContextMethods methods)
    {
        struct TheBindingContext final : BindingContext {
            TheBindingContext(const SharedBarq& r, BindingContextMethods&& methods)
                : methods(std::move(methods))
            {
                // barq is a weak_ptr on the base.
                barq = r;
            }
            ...
            void did_change(std::vector<ObserverState> const&, std::vector<void*> const&, bool) override
            {
                if (methods.did_change)
                    methods.did_change(get_barq());
            }
            void before_notify() override
            {
                if (methods.before_notify)
                    methods.before_notify(get_barq());
            }
            void schema_did_change(barq::Schema const&) override
            {
                if (methods.schema_did_change)
                    methods.schema_did_change(get_barq());
            }
        ...
        private:
            SharedBarq get_barq()
            {
                if (auto ptr = barq.lock())
                    return ptr;
                throw std::runtime_error("Barq no longer exists");
            }

            BindingContextMethods methods;
        };

        barq->m_binding_context = std::make_unique<TheBindingContext>(barq, std::move(methods));
    }

    // This requires the ability to implement interfaces.
    // This is planned, but for now, providing a helper unlocks sync.
    template <typename F>
    static std::shared_ptr<networking::GenericNetworkTransport> make_network_transport(F&& runRequest)
    {

        class Impl final : public networking::GenericNetworkTransport {
        public:
            Impl(F&& runRequest)
            ...
            void send_request_to_server(const networking::Request& request,
                                        util::UniqueFunction<void(const networking::Response&)>&& completionBlock) override
            {
    ...
    static void delete_data_for_object(const SharedBarq& barq, StringData object_type)
    {
    ...
    static std::shared_ptr<util::Logger> make_logger(LogCallback&& logger)
    {
        ...
        private:
            void do_log(const barq::util::LogCategory& category, Level level, const std::string& message) final
            {
    ...
    {
        sync::SessionErrorInfo error(Status{type == "barq::sync::ProtocolError" ? ErrorCodes::SyncClientResetRequired
    ...
    static SharedBarq consume_thread_safe_reference_to_shared_barq(ThreadSafeReference& tsr)
    {
        return Barq::get_shared_barq(std::move(tsr));
    }

    static bool file_exists(const StringData& path)
    {
        return barq::util::File::exists(path);
    }
...
public:
    ...
    {
        BARQ_ASSERT_RELEASE(std::this_thread::get_id() == m_constructed_on);
    }
