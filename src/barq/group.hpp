...
#ifndef BARQ_GROUP_HPP
#define BARQ_GROUP_HPP
...
#include <barq/impl/output_stream.hpp>
...
public:
    ...
    /// specified name, or `barq::not_found` if this group does not contain a
    ...
    {
        BARQ_ASSERT(class_name.size() <= max_class_name_length);
...
{
    BARQ_ASSERT(table_type != Table::Type::Embedded);
    check_attached();
...
{
    BARQ_ASSERT_DEBUG(m_notify_handler);
...
inline ref_type Group::get_history_ref(const Array& top) noexcept
{
    ...
    if (has_history) {
        ...
        BARQ_ASSERT(top.size() > s_hist_version_ndx);
...
public:
    ...
    {
        BARQ_ASSERT_DEBUG(notification_handler());
