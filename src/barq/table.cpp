...
#include <barq/array_decimal128.hpp>
...
/// Barq accessors rooted in a common group accessor (tables, columns, rows,
/// descriptors, arrays):
///
...
///    accessor whose type agrees with the data type (barq::DataType) of the
...
bool TableVersions::operator==(const TableVersions& other) const
{
    ...
    for (size_t i = 0; i < sz; i++) {
        BARQ_ASSERT_DEBUG(this->at(i).first == other.at(i).first);
...
bool LinkChain::add(ColKey ck)
{
    ...
    BARQ_ASSERT(m_current_table->valid_column(ck));
...
ColKey Table::add_column(DataType type, StringData name, bool nullable, std::optional<CollectionType> collection_type,
                         DataType key_type)
{
    BARQ_ASSERT(!is_link_type(ColumnType(type)));
                         ...
                         DataType key_type)
{
    ...
    BARQ_ASSERT_RELEASE(origin_group && target_group);
    BARQ_ASSERT_RELEASE(origin_group == target_group);
...
void Table::remove_recursive(CascadeState& cascade_state)
{
    ...
    do {
        ...
        for (auto obj : to_delete) {
            ...
            BARQ_ASSERT(!obj.second.is_unresolved());
...
CollectionType Table::get_collection_type(ColKey col_key) const
{
    ...
    BARQ_ASSERT(col_key.is_dictionary());
...
void Table::remove_column(ColKey col_key)
{
    ...
    if (col_key == m_primary_key_col) {
        ...
        BARQ_ASSERT_RELEASE(m_primary_key_col.get_index().val != col_key.get_index().val);
    }
...
{
    BARQ_ASSERT(!(is_writable && is_frzn));
    m_is_frozen = is_frzn;
...
void Table::populate_search_index(ColKey col_key)
{
    ...
    if (type == type_Int) {
        ...
        BARQ_ASSERT_RELEASE(false && "Data type does not support search index");
    }
}
...
void Table::do_add_search_index(ColKey col_key, IndexType type)
{
    ...
    BARQ_ASSERT(m_index_accessors.size() == m_leaf_ndx2colkey.size());
    BARQ_ASSERT(m_index_accessors[column_ndx] == nullptr);
    ...
    switch (type) {
        ...
        case IndexType::Fulltext:
            ...
            if (attr.test(col_attr_FullText_Indexed)) {
                BARQ_ASSERT(search_index_type(col_key) == IndexType::Fulltext);
                return;
            }
        ...
        case IndexType::General:
            if (attr.test(col_attr_Indexed)) {
                BARQ_ASSERT(search_index_type(col_key) == IndexType::General);
                return;
            }
            if (attr.test(col_attr_FullText_Indexed)) {
                this->remove_search_index(col_key);
...
ColKey Table::do_insert_root_column(ColKey col_key, ColumnType type, StringData name, DataType key_type)
{
    ...
    BARQ_ASSERT(!col_key || !valid_column(col_key));
    ...
    auto col_ndx = col_key.get_index().val;
    build_column_mapping();
    BARQ_ASSERT(col_ndx <= m_index_refs.size());
...
void Table::set_table_type(Type table_type, bool handle_backlinks)
{
    ...
    BARQ_ASSERT_EX(table_type == Type::TopLevel || table_type == Type::Embedded, table_type);
...
Table::~Table() noexcept
{
    if (m_top.is_attached()) {
        ...
        BARQ_ASSERT(m_index_accessors.size() == 0);
    }
        ...
        for (auto obj : *this) {
            for (auto col : to_migrate) {
                if (col.get_type() == col_type_Mixed) {
                    ...
                    BARQ_ASSERT_3(col.get_type(), ==, col_type_String);
...
void Table::migrate_col_keys()
{
    ...
    for (size_t n = 0; n < sz; n++) {
        ...
        if (col_key.get_type() == col_type_LinkList) {
            ...
            BARQ_ASSERT(attrs.test(col_attr_List));
...
StringData Table::get_name() const noexcept
{
    ...
    if (!parent)
        return StringData("");
    BARQ_ASSERT(dynamic_cast<Group*>(parent));
...
{
    BARQ_ASSERT_DEBUG(valid_column(col_key));
...
ref_type Table::create_empty_table(Allocator& alloc, TableKey key)
{
    ...
    BARQ_ASSERT(top.size() == top_array_size);
...
void Table::ensure_graveyard()
{
    if (!m_tombstones) {
        while (m_top.size() < top_position_for_tombstones)
            m_top.add(0);
        BARQ_ASSERT(!m_top.get(top_position_for_tombstones));
...
{
    if (!m_top.is_attached())
        return barq::npos;                   // Subtable with shared descriptor
    ArrayParent* parent = m_top.get_parent(); // ArrayParent guaranteed to be Table::Parent
    if (!parent)
        return barq::npos; // Free-standing table
...
void Table::refresh_content_version()
{
    ...
    if (m_top.size() >= top_position_for_version) {
        ...
        auto rot_version = m_top.get_as_ref_or_tagged(top_position_for_version);
        BARQ_ASSERT(rot_version.is_tagged());
...
{
    BARQ_ASSERT(m_cookie == cookie_initialized);
    BARQ_ASSERT(m_top.is_attached());
    m_top.init_from_parent();
    m_spec.init_from_parent();
    BARQ_ASSERT(m_top.size() > top_position_for_pk_col);
    m_clusters.init_from_parent();
    m_index_refs.init_from_parent();
    m_opposite_table.init_from_parent();
    m_opposite_column.init_from_parent();
                                          ...
                                          bool* did_create)
{
    ...
    BARQ_ASSERT(type == type_String || type == type_ObjectId || type == type_Int || type == type_UUID);
...
{
    auto primary_key_col = get_primary_key_column();
    BARQ_ASSERT(primary_key_col);
    DataType type = DataType(primary_key_col.get_type());
    BARQ_ASSERT((primary_key.is_null() && primary_key_col.get_attrs().test(col_attr_Nullable)) ||
                 primary_key.get_type() == type);
...
{
#if BARQ_EXERCISE_OBJECT_ID_COLLISION
...
{
    BARQ_ASSERT(!(sequence_number >> 62));
                                                     ...
                                                     ObjKey colliding_local_id)
{
    ...
    BARQ_ASSERT(lo.size() == num_entries);
    BARQ_ASSERT(local_id.size() == num_entries);
    ...
    auto insert_collision = [&](GlobalKey object_id, ObjKey new_local_id) {
        ...
        if (i != num_entries) {
            ...
            if (existing == object_id) {
                BARQ_ASSERT(new_local_id.value == local_id.get(i));
                return;
            }
        }
...
{
    BARQ_ASSERT(!attr.test(col_attr_Indexed));
    BARQ_ASSERT(!attr.test(col_attr_Unique)); // Must not be encoded into col_key
    ...
    if (Replication* repl = get_repl()) {
        if (repl->get_history_type() == Replication::HistoryType::hist_SyncClient) {
            throw RuntimeError(
                ...
                util::format("Cannot change primary key property in '%1' when barq is synchronized", get_name()));
        }
    }

    BARQ_ASSERT_RELEASE(col_key.value >= 0); // Just to be sure. We have an issue where value seems to be -1
...
{
    barq::DataType type_id = get_column_type(from);
