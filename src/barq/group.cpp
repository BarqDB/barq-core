...
void Group::set_size() const noexcept
{
    ...
    if (is_attached() && m_table_names.is_attached()) {
        ...
        BARQ_ASSERT_EX(max_index < (1 << 16), max_index);
...
void Group::open(ref_type top_ref, const std::string& file_path)
{
    ...
    if (m_file_format_version == 0) {
        ...
        BARQ_ASSERT(target_file_format_version == m_file_format_version);
    }
...
{
    BARQ_ASSERT_3(new_top_ref, <, new_file_size);
    BARQ_ASSERT(!is_attached());
...
{
    BARQ_ASSERT(m_table_accessors.size() == m_tables.size());
...
Table* Group::do_add_table(StringData name, Table::Type table_type, bool do_repl)
{
    ...
    if (BARQ_UNLIKELY(name.size() > max_table_name_length))
    ...
    BARQ_ASSERT_3(m_tables.size(), ==, m_table_names.size());
...
{
    BARQ_ASSERT(m_tables.size() == m_table_accessors.size());
    BARQ_ASSERT(table_ndx < m_table_accessors.size());
...
void Group::remove_table(size_t table_ndx, TableKey key)
{
    if (!m_is_writable)
        throw LogicError(ErrorCodes::ReadOnlyDB, "Database not writable");
    BARQ_ASSERT_3(m_tables.size(), ==, m_table_names.size());
    BARQ_ASSERT(table_ndx < m_tables.size());
    ...
    BARQ_ASSERT(!int_cast_has_overflow<ref_type>(ref_64));
...
auto Group::DefaultTableWriter::write_history(_impl::OutputStream& out) -> HistoryInfo
{
    ...
    if (history_ref) {
        ...
        BARQ_ASSERT(history_type != Replication::hist_None);
                  ...
                  bool pad_for_encryption, uint_fast64_t version_number)
{
    ...
    if (no_top_array) {
        ...
        BARQ_ASSERT(version_number == 0 || version_number == 1);
    }
    else {
        ...
        BARQ_ASSERT_3(size_t(out_2.get_ref_of_next_array()), ==, final_file_size);
...
{
    BARQ_ASSERT(m_file_format_version >= 7);
    ...
    if (m_top.size() > s_hist_version_ndx) {
        ...
        if (stored_history_type != Replication::hist_None) {
            BARQ_ASSERT(stored_history_type == history_type);
            BARQ_ASSERT(stored_history_schema_version == history_schema_version);
        }
        ...
        // No history yet
        BARQ_ASSERT(m_top.size() == s_hist_type_ndx);
...
#ifdef BARQ_DEBUG // LCOV_EXCL_START ignore debug functions
...
public:
    ...
    {
        BARQ_ASSERT_3(ref % 8, ==, 0);  // 8-byte alignment
        BARQ_ASSERT_3(size % 8, ==, 0); // 8-byte alignment
        BARQ_ASSERT_3(size, >, 0);
        BARQ_ASSERT_3(ref, >=, m_ref_begin);
        BARQ_ASSERT_3(size, <=, m_immutable_ref_end - ref);
        Chunk chunk;
        chunk.ref = ref;
        chunk.size = size;
    ...
    void add_mutable(ref_type ref, size_t size)
    {
        ...
        BARQ_ASSERT_3(size % 8, ==, 0); // 8-byte alignment
        BARQ_ASSERT_3(size, >, 0);
        BARQ_ASSERT_3(ref, >=, m_immutable_ref_end);
        BARQ_ASSERT_3(size, <=, m_mutable_ref_end - ref);
        Chunk chunk;
        chunk.ref = ref;
        chunk.size = size;
        m_chunks.push_back(chunk);
    ...
    void add(ref_type ref, size_t size)
    {
        ...
        BARQ_ASSERT_3(size, >, 0);
        BARQ_ASSERT_3(ref, >=, m_ref_begin);
        BARQ_ASSERT(size <= (ref < m_baseline ? m_immutable_ref_end : m_mutable_ref_end) - ref);
        Chunk chunk;
        chunk.ref = ref;
        chunk.size = size;
        m_chunks.push_back(chunk);
    ...
    void canonicalize()
    {
        ...
        if (i_1 != end) {
            while (++i_2 != end) {
                ...
                BARQ_ASSERT_3(prev_ref_end, <=, i_2->ref);
    ...
    {
        BARQ_ASSERT_3(m_chunks.size(), ==, 1);
        BARQ_ASSERT_3(m_chunks.front().ref, ==, m_ref_begin);
        BARQ_ASSERT_3(m_chunks.front().size, ==, m_mutable_ref_end - m_ref_begin);
    }
...
void Group::verify() const
{
    ...
    // Verify tables
    {
        ...
        for (auto key : keys) {
            ...
            BARQ_ASSERT_3(table->get_key().value, ==, key.value);
            table->verify();
        }
    }
    ...
    if (Replication* repl = *get_repl()) {
        if (auto hist = repl->_create_history_read()) {
            ...
            BARQ_ASSERT(history_type != Replication::hist_None || history_schema_version == 0);
    ...
    {
        BARQ_ASSERT_EX(m_top.size() == 3 || m_top.size() == 5 || m_top.size() == 7 || m_top.size() >= 10,
                        m_top.size());
        ...
        BARQ_ASSERT(pos.is_attached() == len.is_attached());
        BARQ_ASSERT(pos.is_attached() || !ver.is_attached()); // pos.is_attached() <== ver.is_attached()
        if (pos.is_attached()) {
            size_t n = pos.size();
            BARQ_ASSERT_3(n, ==, len.size());
            if (ver.is_attached())
                BARQ_ASSERT_3(n, ==, ver.size());
    ...
    BARQ_ASSERT_3(real_immutable_ref_end, <=, real_baseline);
