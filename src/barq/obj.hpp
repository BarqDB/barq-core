...
public:
    ...
    // By default this alogirithm is disabled. BarqConfig contains a boolean flag
    // to enable it.
...
inline Obj& Obj::set(ColKey col_key, uint_fast64_t value, bool is_default)
{
    ...
    if (BARQ_UNLIKELY(util::int_cast_with_overflow_detect(value, value_2))) {
        BARQ_TERMINATE("Unsigned integer too big.");
    }
...
template <>
inline Obj& Obj::set(ColKey col_key, barq::null, bool is_default)
{
...
inline Obj& Obj::set_all(Head v, Tail... tail)
{
    ...
    if (get_primary_key_column()) {
        BARQ_ASSERT(colkey2spec_ndx(get_primary_key_column()) == 0);
        start_index = 1;
    }
