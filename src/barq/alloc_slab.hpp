...
#ifndef BARQ_ALLOC_SLAB_HPP
#define BARQ_ALLOC_SLAB_HPP
...
{
    BARQ_ASSERT_3(m_attach_mode, ==, attach_UsersBuffer);
    BARQ_ASSERT(m_data);
...
void SlabAlloc::for_all_free_entries(Func f) const
{
    ...
    for (const auto& e : m_slabs) {
        ...
        BARQ_ASSERT(bb->block_before_size == 0);
        while (1) {
