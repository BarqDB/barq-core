...
void Spec::init(MemRef mem) noexcept
{
    ...
    BARQ_ASSERT(top_size == s_spec_max_size);
...
{
    BARQ_ASSERT(column_ndx <= m_types.size());

    if (BARQ_UNLIKELY(name.size() > Table::max_column_name_length)) {
...
{
    BARQ_ASSERT(get_column_type(column_ndx) == col_type_String);
    ...
    for (size_t col_ndx = 0; col_ndx < column_count; ++col_ndx) {
        ...
        switch (col_type) {
            ...
            case col_type_TypedLink: {
                ...
                BARQ_ASSERT(false); // We can no longer compare specs - in fact we don't want to
                break;
            }
            case col_type_Int:
            case col_type_Bool:
...
{
#ifdef BARQ_DEBUG
    BARQ_ASSERT(m_names.size() == get_public_column_count());
    BARQ_ASSERT(m_types.size() == get_column_count());
    BARQ_ASSERT(m_attr.size() == get_column_count());

    BARQ_ASSERT(m_types.get_ref() == m_top.get_as_ref(0));
    BARQ_ASSERT(m_names.get_ref() == m_top.get_as_ref(1));
    BARQ_ASSERT(m_attr.get_ref() == m_top.get_as_ref(2));
#endif
}
