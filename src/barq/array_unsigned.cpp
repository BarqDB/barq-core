...
{
    BARQ_ASSERT_DEBUG(width > 0 || m_size == 0);
...
size_t ArrayUnsigned::lower_bound(uint64_t value) const noexcept
{
    if (m_width == 8) {
        ...
        switch (m_width) {
            case 0:
                return barq::lower_bound<0>(m_data, m_size, value);
            case 1:
                return barq::lower_bound<1>(m_data, m_size, value);
            case 2:
                return barq::lower_bound<2>(m_data, m_size, value);
            case 4:
                return barq::lower_bound<4>(m_data, m_size, value);
            default:
                BARQ_UNREACHABLE();
                break;
        }
...
size_t ArrayUnsigned::upper_bound(uint64_t value) const noexcept
{
    if (m_width == 8) {
        ...
        switch (m_width) {
            case 0:
                return barq::upper_bound<0>(m_data, m_size, value);
            case 1:
                return barq::upper_bound<1>(m_data, m_size, value);
            case 2:
                return barq::upper_bound<2>(m_data, m_size, value);
            case 4:
                return barq::upper_bound<4>(m_data, m_size, value);
            default:
                BARQ_UNREACHABLE();
                break;
        }
...
void ArrayUnsigned::insert(size_t ndx, uint64_t value)
{
    ...
    BARQ_ASSERT_DEBUG(!do_expand || new_width > m_width);
