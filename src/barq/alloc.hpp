...
#ifndef BARQ_ALLOC_HPP
#define BARQ_ALLOC_HPP
...
#if defined BARQ_ARCHITECTURE_X86_32 && defined BARQ_WINDOWS
#define BARQ_WORKAROUND_MSVC_BUG BARQ_NOINLINE
#else
#define BARQ_WORKAROUND_MSVC_BUG
#endif
...
public:
    BARQ_WORKAROUND_MSVC_BUG inline uint_fast64_t get_content_version() noexcept
    {
...
protected:
    ...
    BARQ_WORKAROUND_MSVC_BUG inline uint_fast64_t get_instance_version() noexcept
    {
...
{
    if (m_is_read_only)
        throw barq::LogicError(ErrorCodes::WrongTransactionState,
...
inline MemRef Allocator::realloc_(ref_type ref, const char* addr, size_t old_size, size_t new_size)
{
    ...
    if (ref == m_debug_watch)
        BARQ_TERMINATE("Allocator watch: Ref was reallocated");
#endif
    if (m_is_read_only)
        throw barq::LogicError(ErrorCodes::WrongTransactionState,
                                "Trying to modify database while in read transaction");
...
#ifdef BARQ_DEBUG
    if (ref == m_debug_watch)
        BARQ_TERMINATE("Allocator watch: Ref was freed");
#endif
    BARQ_ASSERT(!m_is_read_only);
...
{
    BARQ_ASSERT_DEBUG(ref != 0);
    // BARQ_ASSERT_DEBUG(m_baseline != 0); // Attached SlabAlloc
...
inline char* Allocator::translate_critical(RefTranslation* ref_translation_ptr, ref_type ref) const noexcept
{
    ...
    if (BARQ_LIKELY(txl.cookie == 0x1234567890)) {
        ...
        if (BARQ_LIKELY(offset < lowest_possible_xover_offset)) {
    ...
    barq::util::terminate("Invalid ref translation entry", __FILE__, __LINE__, txl.cookie, 0x1234567890, ref, idx);
}

inline char* Allocator::translate(ref_type ref) const noexcept
{
    if (auto ptr = m_ref_translation_ptr.load(std::memory_order_acquire); BARQ_LIKELY(ptr)) {
