...
// Linux epoll
#if defined(BARQ_USE_EPOLL) && !BARQ_ANDROID
#define BARQ_NETWORK_USE_EPOLL 1
#else
#define BARQ_NETWORK_USE_EPOLL 0
#endif
...
#if !defined(BARQ_HAVE_KQUEUE)
#if !defined(BARQ_DISABLE_UTIL_NETWORK_KQUEUE)
#define BARQ_HAVE_KQUEUE 1
#endif
#endif
#endif
#if !defined(BARQ_HAVE_KQUEUE)
#define BARQ_HAVE_KQUEUE 0
#endif
...
/// operations. The Barq networking library (this library) considers an
...
public:
    ...
    /// `BARQ_UTIL_NETWORK_EVENT_LOOP_METRICS` was defined during
...
template <>
class is_error_code_enum<barq::sync::network::ResolveErrors> {
public:
...
inline void Service::Descriptor::ensure_blocking_mode()
{
    ...
    if (BARQ_UNLIKELY(!m_in_blocking_mode)) {
        bool value = false;
...
public:
    ...
    void orphan() noexcept override final
    {
        BARQ_ASSERT(false); // Never called
    }
...
public:
    ...
    {
        BARQ_ASSERT(!stream.lowest_layer().m_read_oper || !stream.lowest_layer().m_read_oper->in_use());
    ...
    {
        BARQ_ASSERT(!stream.lowest_layer().m_write_oper || !stream.lowest_layer().m_write_oper->in_use());
        stream.lowest_layer().m_desc.ensure_blocking_mode(); // Throws
...
public:
    ...
    {
        auto& s = *this;
        BARQ_ASSERT(this == s.m_stream->lowest_layer().m_read_oper.get());
        BARQ_ASSERT(!s.is_complete());
        BARQ_ASSERT(s.m_curr <= s.m_end);
        Want want = Want::nothing;
        if (BARQ_UNLIKELY(s.m_curr == s.m_end)) {
            ...
            if (want == Want::nothing) {
                if (BARQ_UNLIKELY(s.m_error_code)) {
    ...
    Want advance() noexcept override final
    {
        auto& s = *this;
        BARQ_ASSERT(!s.is_complete());
        BARQ_ASSERT(!s.is_canceled());
        BARQ_ASSERT(!s.m_error_code);
        BARQ_ASSERT(s.m_curr < s.m_end);
        BARQ_ASSERT(!s.m_is_read_some || s.m_curr == m_begin);
        for (;;) {
            ...
            BARQ_ASSERT(n > 0 || s.m_error_code || want != Want::nothing); // No busy loop, please
...
public:
    ...
    {
        auto& s = *this;
        BARQ_ASSERT(this == s.m_stream->lowest_layer().m_write_oper.get());
        BARQ_ASSERT(!s.is_complete());
        BARQ_ASSERT(s.m_curr <= s.m_end);
        Want want = Want::nothing;
        if (BARQ_UNLIKELY(s.m_curr == s.m_end)) {
    ...
    Want advance() noexcept override final
    {
        ...
        BARQ_ASSERT(!s.m_error_code);
        BARQ_ASSERT(s.m_curr < s.m_end);
        BARQ_ASSERT(!s.m_is_write_some || s.m_curr == s.m_begin);
        for (;;) {
...
public:
    ...
    Want advance() noexcept override final
    {
        ...
        BARQ_ASSERT(!s.is_canceled());
        BARQ_ASSERT(!s.m_error_code);
        BARQ_ASSERT(s.m_read_ahead_buffer.empty());
        BARQ_ASSERT(s.m_curr < s.m_end);
        for (;;) {
            ...
            BARQ_ASSERT(nonempty || s.m_error_code || want != Want::nothing); // No busy loop, please
...
public:
    ...
    {
        auto& s = *this;
        BARQ_ASSERT(s.is_complete() || s.is_canceled());
        BARQ_ASSERT(s.is_complete() ==
                     (s.m_error_code || s.m_curr == s.m_end || (s.m_is_read_some && s.m_curr != s.m_begin)));
        BARQ_ASSERT(s.m_curr >= s.m_begin);
...
public:
    ...
    {
        auto& s = *this;
        BARQ_ASSERT(s.is_complete() || (s.is_canceled() && !s.m_error_code));
        BARQ_ASSERT(s.is_canceled() || s.m_error_code ||
...
std::unique_ptr<Oper, Service::LendersOperDeleter> Service::alloc(OwnersOperPtr& owners_ptr, Args&&... args)
{
    ...
    if (BARQ_LIKELY(addr)) {
        ...
        BARQ_ASSERT(dynamic_cast<UnusedOper*>(owners_ptr.get()));
        static_cast<UnusedOper*>(owners_ptr.get())->UnusedOper::~UnusedOper();
        if (BARQ_UNLIKELY(size < sizeof(Oper))) {
            owners_ptr.release();
            delete[] static_cast<char*>(addr);
            goto no_object;
        }
...
{
    BARQ_ASSERT(m_in_use);
    BARQ_ASSERT(!m_canceled);
    m_canceled = true;
}
...
{
    BARQ_ASSERT(!m_complete);
    BARQ_ASSERT(!value || m_in_use);
    m_complete = value;
}
...
public:
    ...
    void recycle_and_execute() override final
    {
        BARQ_ASSERT(is_complete() || (is_canceled() && !m_error_code));
        BARQ_ASSERT(is_canceled() || m_error_code || !m_endpoints.empty());
        bool orphaned = !m_resolver;
...
inline void SocketBase::Option<T, opt, U>::get(const SocketBase& sock, std::error_code& ec)
{
    ...
    if (!ec) {
        BARQ_ASSERT(value_size == sizeof value);
        m_value = T(value);
    }
}
...
public:
    ...
    {
        BARQ_ASSERT(this == m_socket->m_write_oper.get());
    ...
    Want advance() noexcept override final
    {
        BARQ_ASSERT(!is_complete());
        BARQ_ASSERT(!is_canceled());
        BARQ_ASSERT(!m_error_code);
...
public:
    ...
    Want initiate()
    {
        BARQ_ASSERT(this == m_acceptor->m_read_oper.get());
        BARQ_ASSERT(!is_complete());
...
public:
    ...
    {
        BARQ_ASSERT(is_complete() || (is_canceled() && !m_error_code));
        BARQ_ASSERT(is_canceled() || m_error_code || m_socket.is_open());
        bool orphaned = !m_acceptor;
        std::error_code ec = m_error_code;
        if (is_canceled())
            ec = util::error::operation_aborted;
...
{
    BARQ_ASSERT(!m_read_oper || !m_read_oper->in_use());
    if (BARQ_UNLIKELY(socket.is_open()))
...
inline bool ReadAheadBuffer::refill_async(S& stream, std::error_code& ec, Want& want) noexcept
{
    ...
    if (n == 0)
        return false;
    BARQ_ASSERT(!ec || ec == util::MiscExtErrors::end_of_input);
    BARQ_ASSERT(n <= size);
    m_begin = m_buffer.get();
    m_end = m_begin + n;
    return true;
