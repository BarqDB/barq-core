...
#include <barq/util/cf_ptr.hpp>
#include <Security/Security.h>
#include <Security/SecureTransport.h>

#define BARQ_HAVE_KEYCHAIN_APIS (TARGET_OS_MAC && !TARGET_OS_IPHONE)
...
namespace barq::sync::network::ssl {
...
template <>
class is_error_condition_enum<barq::sync::network::ssl::Errors> {
public:
...
public:
    ...
    Want initiate()
    {
        BARQ_ASSERT(this == m_stream->m_tcp_socket.m_read_oper.get());
...
public:
    ...
    {
        BARQ_ASSERT(is_complete() || is_canceled());
        bool orphaned = !m_stream;
...
public:
    ...
    Want initiate()
    {
        BARQ_ASSERT(this == m_stream->m_tcp_socket.m_write_oper.get());
        BARQ_ASSERT(!is_complete());
        m_stream->m_tcp_socket.m_desc.ensure_nonblocking_mode(); // Throws
        return advance();
    }
...
std::size_t Stream::ssl_perform(Oper oper, std::error_code& ec, Want& want) noexcept
{
    ...
    BARQ_ASSERT((ret > 0) == (ssl_error == SSL_ERROR_NONE));

    BARQ_ASSERT(!m_bio_error_code || ssl_error == SSL_ERROR_SYSCALL);
    ...
    switch (ssl_error) {
        ...
        case SSL_ERROR_SYSCALL:
            if (BARQ_UNLIKELY(sys_error != 0)) {
                ec = util::make_basic_system_error_code(sys_error);
            }
            else if (BARQ_UNLIKELY(m_bio_error_code)) {
                ec = m_bio_error_code;
            }
            else if (ret == 0) {
...
std::size_t Stream::ssl_perform(Oper oper, std::error_code& ec, Want& want) noexcept
{
    ...
    if (BARQ_UNLIKELY(m_mock_ssl_perform_error)) {
    ...
    if (result == errSSLWouldBlock) {
        BARQ_ASSERT(blocking_want != Want::nothing);
        ec = std::error_code();
        want = blocking_want;
        return n;
    }
...
#endif // BARQ_HAVE_OPENSSL / BARQ_HAVE_SECURE_TRANSPORT
