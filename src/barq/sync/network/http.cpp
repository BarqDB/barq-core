...
struct HTTPParserErrorCategory : std::error_category {
    ...
    std::string message(int condition) const override
    {
        ...
        switch (HTTPParserError(condition)) {
            ...
            default:
                BARQ_TERMINATE("Invalid HTTP Parser Error");
        }
    }
};
...
std::ostream& operator<<(std::ostream& os, HTTPMethod method)
{
    ...
    BARQ_TERMINATE("Invalid HTTPRequest object.");
}
...
std::ostream& operator<<(std::ostream& os, const HTTPRequest& request)
{
    ...
    if (request.method == HTTPMethod::Connect) {
        BARQ_ASSERT_RELEASE(host != request.headers.end());
        os << host->second;
    }
    ...
    BARQ_ASSERT(content_length_exists == body_exists);
