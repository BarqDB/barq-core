...
public:
    ...
    std::string_view get_barq_request_id() const noexcept override
    {
        return m_barq_request_id;
    }
...
private:
    ...
    void websocket_handshake_completion_handler(const HTTPHeaders& headers) override
    {
        ...
        if (auto it = headers.find("X-Barq-Request-Id"); it != headers.end()) {
            m_barq_request_id = it->second;
        }
...
{
    BARQ_ASSERT(m_logger_ptr);                     // Make sure the logger is valid
...
DefaultSocketProvider::~DefaultSocketProvider()
{
    ...
    BARQ_ASSERT(m_state == State::Stopped);
}
...
void DefaultSocketProvider::event_loop()
{
    ...
    {
        util::CheckedLockGuard lock(m_mutex);
        BARQ_ASSERT(m_state == State::Starting);
    }
