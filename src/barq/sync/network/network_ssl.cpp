...
#include <pthread.h>
#endif
#elif BARQ_HAVE_SECURE_TRANSPORT
#include <fstream>
#include <vector>
#endif
...
#include <barq/sync/noinst/root_certs.hpp>
};
...
#endif // BARQ_INCLUDE_CERTS

#if BARQ_HAVE_OPENSSL && _WIN32
...
private:
    ...
    {
        BARQ_ASSERT_RELEASE(type == X509_LU_X509);
...
#endif // BARQ_HAVE_OPENSSL && _WIN32

#if BARQ_HAVE_OPENSSL && (OPENSSL_VERSION_NUMBER < 0x10100000L || defined(LIBRESSL_VERSION_NUMBER))
...
#endif // BARQ_HAVE_OPENSSL && (OPENSSL_VERSION_NUMBER < 0x10100000L || defined(LIBRESSL_VERSION_NUMBER))
...
#endif // BARQ_HAVE_SECURE_TRANSPORT
...
{
    BARQ_ASSERT(!m_tcp_socket.m_read_oper || !m_tcp_socket.m_read_oper->in_use());
    BARQ_ASSERT(!m_tcp_socket.m_write_oper || !m_tcp_socket.m_write_oper->in_use());
    m_tcp_socket.m_desc.ensure_blocking_mode(); // Throws
    Want want = Want::nothing;
    ssl_handshake(ec, want);
    BARQ_ASSERT(want == Want::nothing);
    return ec;
}
...
{
#if BARQ_USE_SYSTEM_OPENSSL_PATHS
    ERR_clear_error();
...
public:
    ...
    BioMethod()
    {
        const char* name = "barq::util::Stream::BioMethod";
...
int Stream::verify_callback_using_delegate(int preverify_ok, X509_STORE_CTX* ctx) noexcept
{
    ...
    BARQ_ASSERT(stream->m_ssl_verify_callback);
...
int Stream::bio_write(BIO* bio, const char* data, int size) noexcept
{
    ...
    if (ec) {
        if (BARQ_UNLIKELY(ec != error::resource_unavailable_try_again)) {
...
int Stream::bio_read(BIO* bio, char* buffer, int size) noexcept
{
    ...
    BIO_clear_retry_flags(bio);
    if (ec) {
        if (BARQ_UNLIKELY(ec == MiscExtErrors::end_of_input)) {
...
#endif
        default:
            BARQ_ASSERT_EX(false, "Got BIO_ctrl with unknown command %d", cmd);
    }
    return 0;
}
...
std::error_code Context::open_temporary_keychain_if_needed()
{
    ...
    std::string path = temporary_directory() + "barq-sync-ssl-XXXXXXXX.keychain";
...
std::pair<OSStatus, std::size_t> Stream::do_ssl_write(const char* data, std::size_t size) noexcept
{
    ...
    BARQ_ASSERT(size >= m_num_partially_written_bytes);
    ...
    if (result != noErr) {
        ...
        if (BARQ_LIKELY(result == errSSLWouldBlock)) {
...
#else // !BARQ_HAVE_OPENSSL && !BARQ_HAVE_SECURE_TRANSPORT
