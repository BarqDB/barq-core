...
#include <barq/util/priority_queue.hpp>
#include <barq/sync/network/network.hpp>

#if defined _GNU_SOURCE && !BARQ_ANDROID
...
#if defined _GNU_SOURCE && defined SOCK_NONBLOCK && defined SOCK_CLOEXEC && !BARQ_ANDROID
...
#if BARQ_NETWORK_USE_EPOLL
#include <linux/version.h>
#include <sys/epoll.h>
#elif BARQ_HAVE_KQUEUE
#include <sys/types.h>
#include <sys/event.h>
#include <sys/time.h>
#else
...
using namespace barq::sync::network;
...
{
#ifdef _WIN32
    if (BARQ_UNLIKELY(ret == SOCKET_ERROR)) {
        ec = make_winsock_error_code(WSAGetLastError());
        return true;
    }
#else
    if (BARQ_UNLIKELY(ret == -1)) {
...
// std::error_code&) for details. Throws std::system_error on failure.
BARQ_UNUSED inline void set_cloexec_flag(native_handle_type fd, bool value = true)
{
    std::error_code ec;
...
inline void checked_close(native_handle_type fd) noexcept
{
    ...
    if (status == -1) {
        ...
        BARQ_ASSERT(b || GetLastError() != ERROR_INVALID_HANDLE);
    }
#else
    int ret = ::close(fd);
...
#if BARQ_NETWORK_USE_EPOLL || BARQ_HAVE_KQUEUE
    void register_desc(Descriptor&);
    void deregister_desc(Descriptor&) noexcept;
#endif

#ifdef BARQ_UTIL_NETWORK_EVENT_LOOP_METRICS
...
#elif BARQ_HAVE_KQUEUE // !BARQ_NETWORK_USE_EPOLL && BARQ_HAVE_KQUEUE
...
#endif // !BARQ_NETWORK_USE_EPOLL && BARQ_HAVE_KQUEUE
...
#else // !(BARQ_NETWORK_USE_EPOLL || BARQ_HAVE_KQUEUE)
...
bool Service::IoReactor::wait_and_activate(clock::time_point timeout, clock::time_point now)
{
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        for (int j = 0; j < n; ++j) {
            ...
            if (BARQ_UNLIKELY(is_wakeup_pipe_signal)) {
...
bool Service::IoReactor::wait_and_activate(clock::time_point timeout, clock::time_point now)
{
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        for (int j = 0; j < n; ++j) {
            ...
            if (BARQ_UNLIKELY(is_wakeup_pipe_signal)) {
                BARQ_ASSERT(m_wakeup_pipe.wait_fd() == int(event.ident));
                m_wakeup_pipe.acknowledge_signal();
                got_wakeup_pipe_signal = true;
                continue;
            }
            Descriptor& desc = *static_cast<Descriptor*>(event.udata);
            BARQ_ASSERT(desc.m_fd == int(event.ident));
...
{
    if (BARQ_UNLIKELY(!desc.m_is_registered)) {
    ...
    switch (want) {
        case Want::read:
            if (BARQ_UNLIKELY(desc.m_read_ready))
                goto active;
            desc.m_suspended_read_ops.push_back(std::move(op));
            goto proceed;
        case Want::write:
            if (BARQ_UNLIKELY(desc.m_write_ready))
                goto active;
                                          ...
                                          OperQueue<AsyncOper>& completed_ops)
{
    ...
    for (;;) {
        ...
        if (BARQ_UNLIKELY(wakeup_pipe_signal)) {
            interrupted = true;
            return false;
        }
...
inline Service::IoReactor::~IoReactor() noexcept
{
    ...
    BARQ_ASSERT(n == m_num_operations);
#endif
}
...
void Service::IoReactor::add_oper(Descriptor& desc, LendersIoOperPtr op, Want want)
{
    ...
    BARQ_ASSERT(pollfd_slot.fd == fd);
    BARQ_ASSERT(((pollfd_slot.events & POLLRDNORM) != 0) == !oper_slot.read_ops.empty());
    BARQ_ASSERT(((pollfd_slot.events & POLLWRNORM) != 0) == !oper_slot.write_ops.empty());
    BARQ_ASSERT((pollfd_slot.events & ~(POLLRDNORM | POLLWRNORM)) == 0);
    switch (want) {
        case Want::nothing:
            break;
        case Want::read:
...
void Service::IoReactor::remove_canceled_ops(Descriptor& desc, OperQueue<AsyncOper>& completed_ops) noexcept
{
    native_handle_type fd = desc.m_fd;
    BARQ_ASSERT(fd >= 0);
    BARQ_ASSERT(std::size_t(fd) < m_operations.size());
    OperSlot& oper_slot = m_operations[fd];
    BARQ_ASSERT(oper_slot.pollfd_slot_ndx > 0);
    BARQ_ASSERT(!oper_slot.read_ops.empty() || !oper_slot.write_ops.empty());
    pollfd& pollfd_slot = m_pollfd_slots[oper_slot.pollfd_slot_ndx];
    BARQ_ASSERT(pollfd_slot.fd == fd);
                                          ...
                                          OperQueue<AsyncOper>& completed_ops)
{
    ...
    if (m_pollfd_slots[0].revents != 0) {
        BARQ_ASSERT((m_pollfd_slots[0].revents & POLLNVAL) == 0);
        m_wakeup_pipe.acknowledge_signal();
        interrupted = true;
        return false;
    }
    ...
    while (pollfd_slot_ndx < num_pollfd_slots && num_ready_descriptors > 0) {
        ...
        BARQ_ASSERT(pollfd_slot.fd >= 0);
        if (BARQ_LIKELY(pollfd_slot.revents == 0)) {
            ++pollfd_slot_ndx;
            continue;
        }
        --num_ready_descriptors;

        BARQ_ASSERT((pollfd_slot.revents & POLLNVAL) == 0);
        ...
        BARQ_ASSERT(oper_slot.pollfd_slot_ndx == pollfd_slot_ndx);
        ...
        if ((pollfd_slot.revents & POLLRDNORM) != 0) {
            BARQ_ASSERT(!oper_slot.read_ops.empty());
        ...
        if ((pollfd_slot.revents & POLLWRNORM) != 0) {
            BARQ_ASSERT(!oper_slot.write_ops.empty());
    ...
    BARQ_ASSERT(num_ready_descriptors == 0);
...
#endif // BARQ_UTIL_NETWORK_EVENT_LOOP_METRICS
    ...
    {
        {
            ...
            if (m_post_oper && m_post_oper->m_size >= size) {
                ...
                BARQ_ASSERT(dynamic_cast<UnusedOper*>(op));
...
Endpoint::List Service::Impl::resolve(const Resolver::Query& query, std::error_code& ec)
{
    ...
    while (curr) {
        ...
        bool ip_v6 = curr->ai_family == AF_INET6;
        if (ip_v4 || ip_v6) {
            BARQ_ASSERT((ip_v4 && curr->ai_addrlen == sizeof(Endpoint::sockaddr_ip_v4_type)) ||
                                 ...
                                 std::error_code& ec) noexcept
{
    ...
    if (BARQ_UNLIKELY(addr_len != expected_addr_len))
        BARQ_TERMINATE("Unexpected peer address length");

#if !HAVE_LINUX_ACCEPT4
    {
        bool value = true;
        if (BARQ_UNLIKELY(set_cloexec_flag(new_sock_fd, value, ec)))
            return;
    }
#endif
...
#if !BARQ_PLATFORM_APPLE && !HAVE_LINUX_ACCEPT4
...
{
    if (BARQ_UNLIKELY(assume_read_would_block())) {
...
{
    if (BARQ_UNLIKELY(assume_write_would_block())) {
        ec = error::resource_unavailable_try_again; // Failure
        return 0;
    }
    for (;;) {
        ...
        if (ret == -1) {
            ...
            if (BARQ_UNLIKELY(err == EPROTOTYPE))
                err = EPIPE;
#endif
            if (err == EWOULDBLOCK)
                err = EAGAIN;
...
{
    if (BARQ_UNLIKELY(want == Want::nothing)) {
        BARQ_ASSERT(op->is_complete());
...
{
    if (!is_open()) {
        if (BARQ_UNLIKELY(open(ep.protocol(), ec)))
            return ec;
    }
    ...
    if (BARQ_UNLIKELY(check_socket_error(ret, ec)))
        return ec;
    ec = std::error_code(); // Success
    return ec;
}
...
std::error_code SocketBase::open(const StreamProtocol& prot, std::error_code& ec)
{
...
#ifdef _WIN32
    if (BARQ_UNLIKELY(ret == INVALID_SOCKET)) {
        ec = make_winsock_error_code(WSAGetLastError());
        return ec;
    }
#else
    ...
    {
        bool value = true;
        if (BARQ_UNLIKELY(set_cloexec_flag(sock_fd, value, ec)))
            return ec;
    }
#endif
...
{
    BARQ_ASSERT(!m_write_oper || !m_write_oper->in_use());
...
#else
    int err = errno;
    if (BARQ_UNLIKELY(err != EINPROGRESS && err != EALREADY)) {
        ec = make_basic_system_error_code(err);
        return true; // Failure
    }
#endif
...
std::error_code Socket::finalize_async_connect(std::error_code& ec) noexcept
{
    ...
    if (BARQ_UNLIKELY(connect_errno)) {
...
public:
    ...
    {
        return "barq.sync.network.resolve";
    }
