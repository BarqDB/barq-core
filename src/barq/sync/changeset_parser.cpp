...
struct State {
    ...
    BARQ_NORETURN void parser_error(std::string_view complaint); // Throws
    BARQ_NORETURN void parser_error()
    {
...
struct InstructionBuilder : InstructionHandler {
    ...
    void set_intern_string(uint32_t index, StringBufferRange range) final
    {
        ...
        BARQ_ASSERT(index == strings.size());
...
{
    T value = 0;
    if (BARQ_LIKELY(_impl::decode_int(*this, value)))
        return value;
...
StringData State::read_string()
{
    ...
    if (size > barq::Table::max_string_size)
