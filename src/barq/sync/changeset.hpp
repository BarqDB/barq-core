...
#ifndef BARQ_SYNC_CHANGESET_HPP
#define BARQ_SYNC_CHANGESET_HPP
...
#define BARQ_DEFINE_REFLECTOR_VISITOR(X) void operator()(const Instruction::X&) const;
    BARQ_FOR_EACH_INSTRUCTION_TYPE(BARQ_DEFINE_REFLECTOR_VISITOR)
#undef BARQ_DEFINE_REFLECTOR_VISITOR
};
...
{
    BARQ_ASSERT(string.value < m_strings.size());
    return m_strings[string.value];
}
...
inline Changeset::iterator Changeset::erase_stable(const_iterator cpos)
{
    ...
    BARQ_ASSERT(pos.m_inner >= begin);
    BARQ_ASSERT(pos.m_inner < end);
...
template <bool is_const>
struct iterator_traits<barq::sync::Changeset::IteratorImpl<is_const>> {
    using difference_type = std::ptrdiff_t;
