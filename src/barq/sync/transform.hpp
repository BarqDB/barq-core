...
#ifndef BARQ_SYNC_TRANSFORM_HPP
#define BARQ_SYNC_TRANSFORM_HPP
...
/// Represents an entry in the history of changes in a sync-enabled Barq
...
/// to a version of the Barq state. For server and client-side histories, these
/// versions are referred to as *server versions* and *client versions*
/// respectively. These versions may, or may not correspond to Barq snapshot
/// numbers (on the server-side they currently do not).
///
/// FIXME: Move this class into a separate header
/// (`<barq/sync/history_entry.hpp>`).
class HistoryEntry {
public:
    ...
    /// remote-side Barq file by the change that gave rise to this history
    /// entry. The remote-side Barq file is not always the same Barq file from
...
public:
    ...
    ///
    /// If \a buffer is null, the changeset is not copied out of the Barq,
    /// and entry.changeset.data() does not point to the changeset.
    /// The changeset in the Barq could be chunked, hence it is not possible
...
public:
    ...
    /// version.
    ///
    /// \param local_file_ident The identifier of the local Barq file. The
