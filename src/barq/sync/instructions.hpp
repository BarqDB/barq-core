...
#ifndef BARQ_IMPL_INSTRUCTIONS_HPP
#define BARQ_IMPL_INSTRUCTIONS_HPP
...
#define BARQ_FOR_EACH_INSTRUCTION_TYPE(X)                                                                           \
    X(AddTable)                                                                                                      \
    X(EraseTable)                                                                                                    \
    X(AddColumn)                                                                                                     \
    X(EraseColumn)                                                                                                   \
...
struct Payload {
    ...
    explicit Payload(barq::util::None) noexcept
        : type(Type::Null)
    {
    }
...
struct Instruction {
#define BARQ_DECLARE_INSTRUCTION_STRUCT(X) using X = instr::X;
    BARQ_FOR_EACH_INSTRUCTION_TYPE(BARQ_DECLARE_INSTRUCTION_STRUCT)
#undef BARQ_DECLARE_INSTRUCTION_STRUCT
...
#define BARQ_INSTRUCTION_VARIANT_ALTERNATIVE(X) , X
                       BARQ_FOR_EACH_INSTRUCTION_TYPE(BARQ_INSTRUCTION_VARIANT_ALTERNATIVE)
#undef BARQ_INSTRUCTION_VARIANT_ALTERNATIVE
                   >
        m_instr;
...
{
    switch (type) {
#define BARQ_INSTRUCTION_TYPE_TO_STRING(X)                                                                          \
    case Instruction::Type::X:                                                                                       \
        return #X;
        BARQ_FOR_EACH_INSTRUCTION_TYPE(BARQ_INSTRUCTION_TYPE_TO_STRING)
#undef BARQ_INSTRUCTION_TYPE_TO_STRING
    }
    return "(invalid)";
}
...
inline DataType get_data_type(Instruction::Payload::Type type) noexcept
{
    ...
    switch (type) {
        ...
        case Type::ObjectValue:
            [[fallthrough]];
        case Type::GlobalKey:
            BARQ_TERMINATE(util::format("Invalid data type: %1", int8_t(type)).c_str());
    }
...
#define BARQ_DEFINE_INSTRUCTION_GET_TYPE(X)                                                                         \
    template <>                                                                                                      \
...
BARQ_FOR_EACH_INSTRUCTION_TYPE(BARQ_DEFINE_INSTRUCTION_GET_TYPE)
#undef BARQ_DEFINE_INSTRUCTION_GET_TYPE
...
inline decltype(auto) Instruction::visit(F&& lambda, V&& instr)
{
    ...
    if (mpark::holds_alternative<Vector>(instr)) {
        BARQ_TERMINATE("visiting instruction vector");
    }
#define BARQ_VISIT_VARIANT(X)                                                                                       \
    else if (auto ptr = mpark::get_if<Instruction::X>(&instr))                                                       \
    {                                                                                                                \
        return lambda(*ptr);                                                                                         \
    }
    BARQ_FOR_EACH_INSTRUCTION_TYPE(BARQ_VISIT_VARIANT)
#undef BARQ_VISIT_VARIANT
    else
    {
        BARQ_TERMINATE("Unhandled instruction variant entry");
    }
}
...
template <class T>
BARQ_NOINLINE T* Instruction::get_if() noexcept
{
...
inline size_t Instruction::path_length() const noexcept
{
    ...
    switch (type()) {
        ...
        default:
            BARQ_TERMINATE("Unhandled instruction type in Instruction::path_len()");
    }
}
...
#endif // BARQ_IMPL_INSTRUCTIONS_HPP
