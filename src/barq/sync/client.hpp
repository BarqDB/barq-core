#ifndef BARQ_SYNC_CLIENT_HPP
#define BARQ_SYNC_CLIENT_HPP
...
public:
    ...
    /// and the client is guaranteed to no longer have a Barq file open on
    ...
    /// called, and the associated Barq files may still not have been closed.
    ///
...
/// \brief Client-side representation of a Barq file synchronization session.
///
/// A synchronization session deals with precisely one local Barq file. To
/// synchronize multiple local Barq files, you need multiple sessions.
///
...
///
/// At most one session is allowed to exist for a particular local Barq file
...
/// local Barq file, if they are associated with two different client objects
...
public:
    ...
    struct Config {
        ...
        /// barq_identifier is  the virtual path by which the server identifies the
        /// Barq.
        ...
        /// ProtocolEnvelope.
        ProtocolEnvelope protocol_envelope = ProtocolEnvelope::barq;

        /// service_identifier is a prefix that is prepended to the barq_identifier
        ...
        /// the Barq access token. The value of the HTTP header is "Bearer <token>".
        ...
        /// Is this session being opened for a barq whose path ends in ".fresh"? If so,
        /// it will be downloading a fresh copy of the barq data from the server.
        bool fresh_barq_download = false;
    ...
    /// \brief Start a new session for the specified client-side Barq.
    ...
    /// local Barq state. To know what is currently outstanding on the server,
    ...
    /// Returns the Barq connection id if the session is connected, otherwise
