...
BARQ_NORETURN void throw_bad_transaction_log(std::string msg)
{
...
BARQ_NORETURN void InstructionApplier::bad_transaction_log(const std::string& msg) const
{
    if (m_last_object_key) {
        // If the last_object_key is valid then we should have a changeset and a current table
        BARQ_ASSERT(m_log);
        BARQ_ASSERT(m_last_table_name);
        std::stringstream ss;
...
BARQ_NORETURN void InstructionApplier::bad_transaction_log(const char* msg, Params&&... params) const
{
...
void InstructionApplier::operator()(const Instruction::EraseTable& instr)
{
    ...
    if (BARQ_UNLIKELY(BARQ_COVER_NEVER(!m_transaction.has_table(table_name)))) {
...
void InstructionApplier::visit_payload(const Instruction::Payload& payload, F&& visitor)
{
    ...
    switch (payload.type) {
        ...
        case Type::GlobalKey:
            return visitor(barq::util::none); // FIXME: Not sure about this
        case Type::Null:
            return visitor(barq::util::none);
        case Type::Int:
    ...
    if (instr.type != Type::Link) {
        ...
        if (target_table_name.size() != 0) {
            ...
            if (instr.collection_type == CollectionType::List) {
                ...
                BARQ_ASSERT(instr.collection_type == CollectionType::Single);
            ...
            auto inserter = util::overload{
                ...
                [&](const Instruction::Payload::ObjectValue&) {
                    if (data_type == type_Link) {
                        ...
                        BARQ_ASSERT(dynamic_cast<LnkLst*>(&list));
...
void InstructionApplier::operator()(const Instruction::SetInsert& instr)
{
    struct SetInsertResolver : public PathResolver {
        ...
        Status on_property(Obj& obj, ColKey col) override
        {
            ...
            BARQ_ASSERT(col.get_type() == col_type_Mixed);
    ...
    return mpark::visit(
        util::overload{
            [&](mpark::monostate) {
                ...
                ObjKey key = table.get_objkey_from_primary_key(barq::util::none);
                return key;
            },
            [&](int64_t pk) {
                if (!pk_col) {
