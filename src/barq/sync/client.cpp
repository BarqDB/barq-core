...
#include <barq/sync/noinst/pending_bootstrap_store.hpp>
#include <barq/sync/noinst/pending_reset_store.hpp>
...
public:
    ...
    // Can be called from any thread.
    std::string get_barq_connection_id();
...
ClientImpl::~ClientImpl()
{
    ...
    // are abandoned.
    BARQ_ASSERT(m_stopped);
    BARQ_ASSERT(m_unactualized_session_wrappers.empty());
    BARQ_ASSERT(m_abandoned_session_wrappers.empty());
}
    ...
    post([this] {
        for (auto& p : m_server_slots) {
            ...
            if (m_one_connection_per_session) {
                ...
                BARQ_ASSERT(slot.alt_connections.empty());
                if (slot.connection) {
...
void ClientImpl::register_unactualized_session_wrapper(SessionWrapper* wrapper)
{
    // Thread safety required.
    {
        ...
        BARQ_ASSERT(m_actualize_and_finalize);
...
void ClientImpl::actualize_and_finalize_session_wrappers()
{
    ...
    // finalizing and actualizing sessions for the same Barq file, and
                                                   ...
                                                   Optional<ProxyConfig> proxy_config, bool& was_created)
{
    ...
    if (server_slot.connection && !m_one_connection_per_session && !proxy_config) {
        ...
        BARQ_ASSERT(server_slot.alt_connections.empty());
        return *server_slot.connection;
    }

    // Create a new connection
    BARQ_ASSERT(!server_slot.connection);
...
void ClientImpl::remove_connection(ClientImpl::Connection& conn) noexcept
{
    ...
    BARQ_ASSERT(i != m_server_slots.end()); // Must be found
    ServerSlot& server_slot = i->second;
    if (!m_one_connection_per_session) {
        BARQ_ASSERT(server_slot.alt_connections.empty());
        BARQ_ASSERT(&*server_slot.connection == &conn);
        ...
        BARQ_ASSERT(j != server_slot.alt_connections.end()); // Must be found
        BARQ_ASSERT(&*j->second == &conn);
...
const std::string& SessionImpl::get_virt_path() const noexcept
{
    ...
    BARQ_ASSERT_EX(m_state == State::Active || m_state == State::Unactivated, m_state);
    return m_wrapper.m_virt_path;
}

const std::string& SessionImpl::get_barq_path() const noexcept
{
    // Can only be called if the session is active or being activated
    BARQ_ASSERT_EX(m_state == State::Active || m_state == State::Unactivated, m_state);
                                                ...
                                                const SyncProgress& progress, const ReceivedChangesets& changesets)
{
    ...
    try {
        ...
        if (BARQ_LIKELY(!get_client().is_dry_run())) {
            ...
            client_version = version_info.barq_version;
        }
        else {
...
void SessionImpl::process_pending_flx_bootstrap()
{
    ...
    while (bootstrap_store->has_pending()) {
        ...
        history.integrate_server_changesets(
            *pending_batch.progress, 1.0, pending_batch.changesets, new_version, batch_state, logger, transact,
            [&](const Transaction& tr, util::Span<Changeset> changesets_applied) {
                BARQ_ASSERT_3(changesets_applied.size(), <=, pending_batch.changesets.size());
        ...
        auto action = call_debug_hook(SyncClientHookEvent::DownloadMessageIntegrated, progress, query_version,
                                      batch_state, pending_batch.changesets.size());
        BARQ_ASSERT_EX(action == SyncClientHookAction::NoAction, action);
        ...
        logger.info("Integrated %1 changesets from pending bootstrap for query version %2, producing client version "
                    ...
                    pending_batch.changesets.size(), pending_batch.query_version, new_version.barq_version,
                    std::chrono::duration_cast<std::chrono::milliseconds>(duration).count(),
                    pending_batch.remaining_changesets);
    }

    BARQ_ASSERT_3(query_version, !=, -1);

    on_changesets_integrated(new_version.barq_version, progress);
    ...
    BARQ_ASSERT_EX(action == SyncClientHookAction::NoAction || action == SyncClientHookAction::EarlyReturn, action);
}
...
SubscriptionStore* SessionImpl::get_flx_subscription_store()
{
    ...
    BARQ_ASSERT_EX(m_state == State::Active, m_state);
...
SyncClientHookAction SessionImpl::call_debug_hook(const SyncClientHookData& data)
{
    ...
    switch (action) {
        case barq::SyncClientHookAction::SuspendWithRetryableError: {
            ...
            BARQ_ASSERT_EX(err_processing_err.is_ok(), err_processing_err);
            return SyncClientHookAction::EarlyReturn;
        }
        case barq::SyncClientHookAction::TriggerReconnect: {
...
{
    if (BARQ_LIKELY(!m_wrapper.m_debug_hook)) {
        return SyncClientHookAction::NoAction;
    }
    if (BARQ_UNLIKELY(m_state != State::Active)) {
        return SyncClientHookAction::NoAction;
    }
...
{
    BARQ_ASSERT_EX(m_state == State::Unactivated || m_state == State::Active, m_state);
...
SessionWrapper::SessionWrapper(ClientImpl& client, DBRef db, std::shared_ptr<SubscriptionStore> flx_sub_store,
    ...
    , m_virt_path{std::move(config.barq_identifier)}
    ...
    , m_session_reason(m_client_reset_config || config.fresh_barq_download ? SessionReason::ClientReset
                                                                            : SessionReason::Sync)
    , m_allow_upload_messages(!config.fresh_barq_download)
    , m_schema_version(config.schema_version)
    , m_flx_subscription_store(std::move(flx_sub_store))
    , m_migration_store(std::move(migration_store))
{
    BARQ_ASSERT(m_db);
    BARQ_ASSERT(m_db->get_replication());
    BARQ_ASSERT(dynamic_cast<ClientReplication*>(m_db->get_replication()));
...
SessionWrapper::~SessionWrapper() noexcept
{
    ...
    BARQ_ASSERT(m_actualized);
    BARQ_ASSERT(m_abandoned);
    BARQ_ASSERT(m_finalized);
    BARQ_ASSERT(m_closed);
    BARQ_ASSERT(!m_db);
}
...
void SessionWrapper::on_commit(version_type new_version)
{
    ...
    m_client.post([self = util::bind_ptr{this}, new_version] {
        BARQ_ASSERT(self->m_actualized);
        if (BARQ_UNLIKELY(!self->m_sess))
...
void SessionWrapper::cancel_reconnect_delay()
{
    ...
    m_client.post([self = util::bind_ptr{this}] {
        BARQ_ASSERT(self->m_actualized);
        if (BARQ_UNLIKELY(self->m_closed)) {
            return;
        }
...
{
    BARQ_ASSERT(upload_completion || download_completion);
...
void SessionWrapper::finalize()
{
    ...
    // The Barq file can be closed now, as no access to the Barq file is
...
void SessionWrapper::report_progress(ReportedProgress& p, DownloadableProgress downloadable)
{
    ...
    auto calculate_progress = [](uint64_t transferred, uint64_t transferable, uint64_t final_transferred) {
        BARQ_ASSERT_DEBUG_EX(final_transferred <= transferred, final_transferred, transferred, transferable);
        BARQ_ASSERT_DEBUG_EX(transferred <= transferable, final_transferred, transferred, transferable);
...
std::string SessionWrapper::get_barq_connection_id()
{
    auto pf = util::make_promise_future<std::string>();
    ...
    m_client.post([self = util::bind_ptr{this}, promise = std::move(pf.promise)](Status status) mutable {
        ...
        promise.emplace_value(self->m_sess->get_connection().get_active_barq_connection_id());
    });
...
std::string ClientImpl::Connection::get_http_request_path() const
{
    ...
    const auto param = m_http_request_path_prefix.find('?') == std::string::npos ? "?barq_at="sv : "&barq_at="sv;
...
std::string Session::get_barq_connection_id()
{
    return m_impl->get_barq_connection_id();
}
...
std::ostream& operator<<(std::ostream& os, ProxyConfig::Type proxyType)
{
    ...
    BARQ_TERMINATE("Invalid Proxy Type object.");
}
