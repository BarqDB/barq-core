...
#ifndef BARQ_SYNC_CONFIG_HPP
#define BARQ_SYNC_CONFIG_HPP
...
enum class SyncSessionStopPolicy {
    Immediately,          // Immediately stop the session as soon as all Barqs/Sessions go out of scope.
    LiveIndefinitely,     // Never stop the session.
    AfterChangesUploaded, // Once all Barqs/Sessions go out of scope, wait for uploads to complete and stop.
};

enum class ClientResyncMode : unsigned char {
    // Fire a client reset error
    Manual,
    // Discard local changes, without disrupting accessors or closing the Barq
    DiscardLocal,
...
// clang-format off
#define BARQ_FOR_EACH_SYNC_CLIENT_HOOK_EVENT(X) \
...
#define BARQ_DECLARE_SYNC_CLIENT_HOOK_EVENT(X) X
    BARQ_FOR_EACH_SYNC_CLIENT_HOOK_EVENT(BARQ_DECLARE_SYNC_CLIENT_HOOK_EVENT)
#undef BARQ_DECLARE_SYNC_CLIENT_HOOK_EVENT
};
...
inline std::ostream& operator<<(std::ostream& os, SyncClientHookAction action)
{
    ...
    BARQ_TERMINATE("Invalid SyncClientHookAction value");
}
...
struct SyncConfig {
    ...
    // applying them to the Barq file. This is required when writing objects
    // directly to replication, and will break horribly otherwise
    bool apply_server_changes = true;

    // The name of the directory which Barqs should be backed up to following
    ...
    std::function<void(std::shared_ptr<Barq> before)> notify_before_client_reset;
    std::function<void(std::shared_ptr<Barq> frozen_before, ThreadSafeReference after, bool did_recover)>
        notify_after_client_reset;
    // If true, the Barq passed as the `before` argument to the before reset
    // callbacks will be frozen
    bool freeze_before_reset_barq = true;
    ...
    using SubscriptionInitializerCallback = std::function<void(std::shared_ptr<Barq>)>;
    ...
    // the query to be run again every time the barq is opened. This flag should be set to true
    // in this case.
