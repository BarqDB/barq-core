...
#include <barq/sync/noinst/server/server_impl_base.hpp>
#include <barq/sync/transform.hpp>
#include <barq/util/base64.hpp>
#include <barq/util/bind_ptr.hpp>
#include <barq/util/buffer_stream.hpp>
#include <barq/util/circular_buffer.hpp>
#include <barq/util/compression.hpp>
#include <barq/util/file.hpp>
#include <barq/util/json_parser.hpp>
#include <barq/util/memory_stream.hpp>
...
public:
    ...
    bool produced_new_barq_version = false;
    ...
    void reset() noexcept
    {
        ...
        produced_new_barq_version = false;
...
public:
    ...
    version_type get_barq_version() const noexcept
    {
        return m_version_info.barq_version;
    }
...
private:
    ...
    // In general, `m_version_info` refers to the last snapshot of the Barq
    // file that is supposed to be visible to remote peers engaging in regular
    // Barq file synchronization.
...
// All write transaction on server-side Barq files performed on behalf of the
...
public:
    ...
    const std::set<std::string>& get_barq_names() const noexcept
    {
        return m_barq_names;
    }
    ...
    {
        if (BARQ_UNLIKELY(m_config.token_expiration_clock))
...
private:
    ...
    // The set of all Barq files known to this server, represented by their
    // virtual path.
    //
    // INVARIANT: If a Barq file is in the servers directory (i.e., it would be
    // reported by an invocation of _impl::get_barq_names()), then the
...
public:
    ...
    void websocket_handshake_completion_handler(const HTTPHeaders&) final override
    {
        ...
        BARQ_TERMINATE("websocket_handshake_completion_handler should not have been called");
    }
...
{
    BARQ_ASSERT(ec != util::error::operation_aborted);
...
public:
    ...
    std::string get_barq_request_id() const
    {
        return m_barq_request_id.to_string();
    }

private:
    ServerImpl& m_server;
    const int_fast64_t m_id;
    const ObjectId m_barq_request_id = ObjectId::gen();
    std::unique_ptr<network::Socket> m_socket;
    std::unique_ptr<network::ssl::Stream> m_ssl_stream;
    std::unique_ptr<network::ReadAheadBuffer> m_read_ahead_buffer;
    ...
    void initiate_http()
    {
        ...
        auto handler = [this](HTTPRequest request, std::error_code ec) {
            if (BARQ_UNLIKELY(ec == util::error::operation_aborted))
                return;
            if (BARQ_UNLIKELY(ec == HTTPParserError::MalformedRequest)) {
                logger.error("Malformed HTTP request");
                close_due_to_error(ec); // Throws
                return;
            }
            if (BARQ_UNLIKELY(ec == HTTPParserError::BadRequest)) {
    ...
    void handle_http_request(const HTTPRequest& request)
    {
        ...
        // wrong that it requires a `%2F` after `/barq-sync/`. If `%2F` is
            ...
            while (parser.next(elem)) {
                ...
                if (!prefix.empty()) {
                    auto parse_version = [&](std::string_view str) {
                        ...
                        int version = 0;
                        in >> version;
                        if (BARQ_LIKELY(in && in.eof() && version >= 0))
                            return version;
                        return -1;
                    };
                    int min, max;
                    ...
                    if (BARQ_LIKELY(min >= 0 && max >= 0 && min <= max)) {
    ...
    {
        response.headers["Server"] = "BarqSync/" BARQ_VERSION_STRING; // Throws
        if (m_negotiated_protocol_version < SyncConnection::SERVER_LOG_PROTOCOL_VERSION) {
            // This isn't a real X-Barq-Request-Id, but it should be enough to test with.
            response.headers["X-Barq-Request-Id"] = get_barq_request_id();
        }
    }
...
public:
    ...
    ~Session() noexcept
    {
        BARQ_ASSERT(!is_enlisted_to_send());
    ...
    {
        BARQ_ASSERT(is_session_level_error(error_code));
        BARQ_ASSERT(!error_occurred()); // Must only be called once
    ...
    void receive_file_ident(SaltedFileIdent file_ident) override final
    {
        ...
        if (!ident_message_received()) {
            BARQ_ASSERT(need_client_file_ident());
        ...
        BARQ_ASSERT(!error_occurred());
        BARQ_ASSERT(!m_error_message_sent);
                              ...
                              bool is_subserver, ProtocolError& error)
    {
        ...
        if (BARQ_LIKELY(!need_client_file_ident)) {
                               ...
                               ProtocolError& error)
    {
        ...
        if (BARQ_UNLIKELY(config.session_bootstrap_callback)) {
                                ...
                                ProtocolError& error)
    {
        ...
        if (BARQ_UNLIKELY(!is_consistent(upload_progress))) {
        ...
        // consistent.
        if (BARQ_UNLIKELY(!are_mutually_consistent(upload_progress, m_upload_threshold))) {
        ...
        // consistent.
        if (BARQ_UNLIKELY(!are_mutually_consistent(upload_progress, m_upload_progress))) {
        ...
        if (BARQ_UNLIKELY(locked_server_version_2 < m_locked_server_version)) {
        ...
        // heard about.
        if (BARQ_UNLIKELY(locked_server_version_2 > m_download_progress.server_version)) {
        ...
        if (!upload_changesets.empty()) {
            ...
            for (const ServerProtocol::UploadChangeset& uc : upload_changesets) {
                ...
                // UPLOAD message.
                if (BARQ_UNLIKELY(uc.upload_cursor.client_version <= up.client_version)) {
                ...
                if (BARQ_UNLIKELY(!is_consistent(uc.upload_cursor))) {
                ...
                if (BARQ_UNLIKELY(!are_mutually_consistent(uc.upload_cursor, up))) {
                ...
                if (BARQ_UNLIKELY(!consistent_with_threshold)) {
                    logger.error("Upload cursor of uploaded changeset (%1, %2) is mutually "
            ...
            // UPLOAD message.
            if (BARQ_UNLIKELY(up.client_version > upload_progress.client_version)) {
            ...
            if (BARQ_UNLIKELY(!are_mutually_consistent(up, upload_progress))) {
        ...
        // scheme.
        if (BARQ_UNLIKELY(!m_server_file->can_add_changesets_from_downstream())) {
        ...
        BARQ_ASSERT(have_real_upload_progress || num_changesets_to_integrate == 0);
    ...
    void receive_unbind_message()
    {
        ...
        BARQ_ASSERT(!m_unbind_message_received);
...
private:
    ...
    void continue_history_scan()
    {
        ...
        BARQ_ASSERT(last_server_version.version >= m_download_progress.server_version);
        ...
        if (have_more_to_scan) {
            ...
            if (fetch_from_cache) {
                ...
                auto fetch_and_compress = [&](std::size_t max_download_size) {
                    ...
                    BARQ_ASSERT(upload_progress.client_version >= download_progress.last_integrated_client_version);
                ...
                if (enable_cache) {
                    ...
                    BARQ_ASSERT(upload_progress.client_version == 0);
    ...
    void send_ident_message()
    {
        ...
        BARQ_ASSERT(m_allocated_file_ident.ident != 0);
    ...
    void send_error_message()
    {
        ...
        BARQ_ASSERT(is_session_level_error(m_error_code));
...
{
    BARQ_ASSERT(m_unidentified_sessions.empty());
    BARQ_ASSERT(m_identified_sessions.empty());
    BARQ_ASSERT(m_file_ident_request == 0);
}
...
void ServerFile::cancel_file_ident_request(file_ident_request_type request) noexcept
{
    ...
    BARQ_ASSERT(i != m_file_ident_requests.end());
    FileIdentRequestInfo& info = i->second;
    BARQ_ASSERT(info.receiver);
...
{
    BARQ_ASSERT(m_unidentified_sessions.count(sess) == 0);
...
{
    BARQ_ASSERT(m_unidentified_sessions.count(sess) == 1);
    BARQ_ASSERT(m_identified_sessions.count(client_file_ident) == 0);
...
{
    BARQ_ASSERT(m_identified_sessions.count(client_file_ident) == 1);
                                                ...
                                                std::size_t num_changesets)
{
    ...
    BARQ_ASSERT(upload_progress.client_version >= list.upload_progress.client_version);
    BARQ_ASSERT(are_mutually_consistent(upload_progress, list.upload_progress));
    if (upload_progress.client_version > list.upload_progress.client_version) {
        list.upload_progress = upload_progress;
        dirty = true;
    }

    BARQ_ASSERT(locked_server_version >= list.locked_server_version);
    if (locked_server_version > list.locked_server_version) {
        list.locked_server_version = locked_server_version;
        dirty = true;
    }

    if (BARQ_LIKELY(dirty)) {
        if (num_changesets > 0) {
...
{
    // The Barq file may contain a later snapshot than the one reflected by
    ...
    if (error == BootstrapError::no_error) {
        ...
        if (i != m_work.changesets_from_downstream.end()) {
            ...
            BARQ_ASSERT(list.upload_progress.client_version >= upload_progress.client_version);
            upload_progress = list.upload_progress;
            BARQ_ASSERT(list.locked_server_version >= locked_server_version);
...
void ServerFile::worker_process_work_unit(WorkerState& state)
{
    ...
    if (work.has_primary_work) {
        if (BARQ_UNLIKELY(!m_work.file_ident_alloc_slots.empty()))
...
{
    BARQ_ASSERT(!m_has_work_in_progress);
    if (BARQ_LIKELY(!m_server.is_sync_stopped())) {
        unblock_work(); // Throws
        const Work& work = m_work;
        if (BARQ_LIKELY(work.has_primary_work)) {
...
{
    BARQ_ASSERT(m_has_blocked_work);
    ...
    BARQ_ASSERT(m_unblocked_changesets_from_downstream_byte_size == 0);
...
void ServerFile::recognize_external_change()
{
    ...
    BARQ_ASSERT(m_version_info.barq_version >= prev_version_info.barq_version);
    BARQ_ASSERT(m_version_info.sync_version.version >= prev_version_info.sync_version.version);
...
{
    Work& work = m_work;
    BARQ_ASSERT(!work.file_ident_alloc_slots.empty());
...
{
    BARQ_ASSERT(!m_work.changesets_from_downstream.empty());
                                                   ...
                                                   DBRef& sg_ptr)
{
    ...
    const std::string& path = m_worker_file.barq_path;
...
void ServerFile::finalize_work_stage_1()
{
    ...
    BARQ_ASSERT(num_changesets_removed <= m_num_changesets_from_downstream);
    BARQ_ASSERT(num_bytes_removed <= m_blocked_changesets_from_downstream_byte_size);
...
void ServerFile::finalize_work_stage_2()
{
    ...
    BARQ_ASSERT(m_work.produced_new_barq_version || m_work.version_info.barq_version == 0);
    if (m_work.version_info.barq_version > m_version_info.barq_version) {
        BARQ_ASSERT(m_work.version_info.sync_version.version >= m_version_info.sync_version.version);
    ...
    BARQ_ASSERT(m_file_ident_requests.size() >= m_work.file_ident_alloc_slots.size());
    auto begin = m_file_ident_requests.begin();
    auto i = begin;
    for (const FileIdentAllocSlot& slot : m_work.file_ident_alloc_slots) {
        FileIdentRequestInfo& info = i->second;
        BARQ_ASSERT(info.proxy_file == slot.proxy_file);
        BARQ_ASSERT(info.client_type == slot.client_type);
...
ServerImpl::~ServerImpl() noexcept
{
    ...
    BARQ_ASSERT_RELEASE(!server_destroyed_while_still_running);
}


void ServerImpl::start()
{
    logger.info("Barq sync server started (%1)", BARQ_VER_CHUNK); // Throws
...
void ServerImpl::run()
{
    ...
    logger.info("Barq sync server stopped");
}
...
{
    BARQ_ASSERT(byte_size <= m_pending_changesets_from_downstream_byte_size);
    m_pending_changesets_from_downstream_byte_size -= byte_size;
...
{
    return _impl::map_virt_to_real_barq_path(m_root_dir, virt_path, real_path); // Throws
}
...
{
    BARQ_ASSERT(m_send_trigger);
    BARQ_ASSERT(!m_is_closing);
    BARQ_ASSERT(!sess->is_enlisted_to_send());
...
{
    BARQ_ASSERT(!m_is_sending);
    BARQ_ASSERT(!m_sending_pong);
    if (m_send_pong) {
...
void SyncConnection::protocol_error(ProtocolError error_code, Session* sess)
{
    ...
    BARQ_ASSERT(!session_level || sess);
    BARQ_ASSERT(!sess || m_sessions.count(sess->get_session_ident()) == 1);
...
{
    BARQ_ASSERT(get_protocol_error_message(int(error_code)));
    ...
    BARQ_ASSERT(is_session_level_error(error_code) == (session_ident != 0));
    BARQ_ASSERT(!is_session_level_error(error_code));
