...
#ifndef BARQ_NOINST_SERVER_FILE_ACCESS_CACHE_HPP
...
public:
    ...
    /// \param max_open_files The maximum number of Barq files to keep open
...
private:
    ...
    /// recently accessed open Barq file. `m_first_open_file->m_next_open_file`
    /// is the next most recently accessed open Barq
    /// file. `m_first__open_file->m_prev_open_file` is the least recently
    /// accessed open Barq file.
    Slot* m_first_open_file = nullptr;

    /// Current number of open Barq files.
...
public:
    ...
    /// Returns true if the associated Barq file is currently open.
    bool is_open() const noexcept;

    /// Open the Barq file at `barq_path` if it is not already open. The
    ...
    ///
    /// Calling this function may cause Barq files associated with other Slot
    ...
    /// Close the Barq file now if it is open (idempotency).
    void close() noexcept;
...
{
    BARQ_ASSERT(!slot.m_next_open_file);
    BARQ_ASSERT(!slot.m_prev_open_file);
...
{
    if (m_file) {
        BARQ_ASSERT(m_prev_open_file);
        BARQ_ASSERT(m_next_open_file);
        return true;
    }
...
inline ServerFileAccessCache::File::File(const Slot& slot)
    ...
    , shared_group{DB::create(history, slot.barq_path, slot.make_shared_group_options())} // Throws
{
...
#endif // BARQ_NOINST_SERVER_FILE_ACCESS_CACHE_HPP
