...
#ifndef BARQ_NOINST_SERVER_HISTORY_HPP
#define BARQ_NOINST_SERVER_HISTORY_HPP
...
#include <barq/sync/noinst/server/permissions.hpp>
...
public:
    ...
    /// Get the current Barq version and server version.
    ///
    ...
    /// this file, and \a version_info is set to reflect the produced Barq
    /// snapshot.
    ///
    ...
    ///
    /// \param backup_whole_barq Will be set to true when a change was made
    /// that necessitates a full Barq backup (nonincremental). Otherwise it is
    /// left unmodified.
    ///
    ...
    ///
    /// This function must be called at most once for each Barq file. Use
    ...
    // The contents of the entire Barq is returned in a HistoryContents object.
...
private:
    ...
    // The identifier of the local Barq file. If this file is used on a subtier
    // node of a star topology server cluster, the identifier is allocated in
    // the context of a different Barq file.
    //
    ...
    /// Barq version (snapshot number) on which the changeset associated with
    ...
    ///
    /// \return True if, and only if changes were made to the Barq file (state
...
inline void ServerHistory::prepare_for_write()
{
    ...
    BARQ_ASSERT(m_acc->sh_changesets.is_attached());
    BARQ_ASSERT(m_acc->root.size() == s_root_size);
}
...
#endif // BARQ_NOINST_SERVER_HISTORY_HPP
