...
/// \brief Server of the Barq synchronization protocol.
///
/// Instances of this class are servers of the WebSocket-based Barq
/// synchronization protocol (`/doc/protocol.md`), and are generally referred to
/// simply as *sync servers*.
///
/// No agent external to a sync server is allowed to open Barq files belonging
...
public:
    ...
    struct Config {
        ...
        /// The maximum number of Barq files that will be kept open
        ...
        /// server keeps a cache of open Barq files for efficiency reasons (one
        ...
        /// receive the Barq access token. The value of the HTTP header is
        /// "Bearer <token>"
    ...
    /// when a backup slave has a full copy of all Barqs or after timing out.
    ...
    /// Map the specified virtual Barq path to a real file system path. The
    ...
    /// Inform the server about an external change to one of the Barq files
    /// managed by the server.
    ///
    /// CAUTION: On a server where backup is enabled, Barq files are not
