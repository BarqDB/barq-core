...
`client_version`)
  ...
  // Continuous transactions history (one entry per Barq version (snapshot number), trimmed)
  //
  // `ct_history_entry_index = barq_version - ct_history_base_version - 1`
  table ct_history:
...
looks wrong!
  ...
  optional array partial_sync:
    ...
    // partial Barq, and with the purpose of identifying changesets in the
    // history of the reference Barq, that originate from, or via the partial
    // Barq.
    //
    // At the same time, it is used to identify outgoing changesets in the
    // history of the partial Barq that originate from, or via the reference
    // Barq.
...
void ServerHistory::allocate_file_identifiers(FileIdentAllocSlots& slots, VersionInfo& version_info)
{
    ...
    if (BARQ_UNLIKELY(m_acc->upstream_status.is_attached())) {
    ...
    version_type new_barq_version = tr->commit(); // Throws
    version_info.barq_version = new_barq_version;
    version_info.sync_version = get_salted_server_version();
}
                                                ...
                                                IntegrationResult& result, util::Logger& logger)
{
    ...
    for (;;) {
        ...
        if (BARQ_UNLIKELY(!anything_to_do))
            return false;
        ...
        try {
            ...
            for (file_ident_type client_file_ident : client_file_order) {
                BARQ_ASSERT(client_file_ident > 0);
                BARQ_ASSERT(client_file_ident != g_root_node_file_ident);
                BARQ_ASSERT(client_file_ident != m_local_file_ident);
                bool excluded =
                    (result.excluded_client_files.find(client_file_ident) != result.excluded_client_files.end());
                if (BARQ_UNLIKELY(excluded))
                    continue;
                ...
                for (const IntegratableChangeset& ic : list.changesets) {
                    BARQ_ASSERT(ic.client_file_ident == client_file_ident);
                    ...
                    if (origin_file_ident != 0) {
                        ...
                        if (BARQ_UNLIKELY(origin_file_ident <= g_root_node_file_ident))
                            goto error;
                        if (BARQ_UNLIKELY(std::uint_fast64_t(origin_file_ident) >= m_num_client_files))
                            goto error;
                        ...
                        if (BARQ_UNLIKELY(proxy_file_ident != ic.client_file_ident))
                            goto error;
                    }
                ...
                auto integrate = [&](std::size_t begin, std::size_t end, UploadCursor upload_progress) {
                    ...
                    if (dirty_2) {
                        ...
                        if (backup_whole_barq_3)
                            backup_whole_barq_2 = true;
                    }
                };
    ...
    error:
        BARQ_ASSERT(current_client_file_ident != 0);
...
auto ServerHistory::integrate_backup_idents_and_changeset(
    version_type expected_barq_version, salt_type server_version_salt,
    ...
    const std::vector<IntegratableChangeset>& integratable_changesets, util::Logger& logger) -> IntegratedBackup
{
    ...
    try {
        ...
        result.version_info.barq_version = barq_version;

        if (barq_version + 1 != expected_barq_version)
            return result;

        // To ensure identity of a server Barq and its backup, it is necessary
        // to set the server_version_salt of the backup Barq to the same value
        // as that of the original Barq.
        ...
        auto ta = util::make_temp_assign(m_is_local_changeset, false, true);
        result.version_info.barq_version = tr->commit(); // Throws
...
{
    BARQ_ASSERT(!m_acc->upstream_status.is_attached());
                                            ...
                                            ClientType client_type, salt_type& file_ident_salt)
{
    BARQ_ASSERT(m_acc->upstream_status.is_attached());
    static_assert(g_root_node_file_ident == 1, "");
    if (BARQ_UNLIKELY(file_ident < 2))
        return false;
    std::size_t max = std::numeric_limits<std::size_t>::max();
    if (BARQ_UNLIKELY(std::uint_fast64_t(file_ident) > max))
...
{
    BARQ_ASSERT(file_index >= m_num_client_files);
    BARQ_ASSERT(proxy_file_ident == 0 || is_valid_proxy_file_ident(proxy_file_ident));
...
bool ServerHistory::ensure_upstream_file_ident(file_ident_type file_ident)
{
    ...
    if (BARQ_UNLIKELY(std::uint_fast64_t(file_ident) > max))
        throw util::overflow_error{"Client file index"};
    std::size_t file_index = std::size_t(file_ident);
    if (BARQ_LIKELY(file_index < m_num_client_files)) {
        auto client_type = m_acc->cf_client_types.get(file_index);
        if (BARQ_UNLIKELY(client_type != int(ClientType::upstream)))
            return false;
        BARQ_ASSERT(m_acc->cf_ident_salts.get(file_index) == 0);
        BARQ_ASSERT(m_acc->cf_proxy_files.get(file_index) == 0);
        return true;
    }
    do
        add_client_file(0, 0, ClientType::upstream); // Throws
                                    ...
                                    ClientType client_type)
{
    switch (client_type) {
        ...
        case ClientType::self:
            BARQ_ASSERT(file_ident_salt == 0);
            BARQ_ASSERT(proxy_file_ident == 0);
            break;
    ...
    m_acc->cf_ident_salts.insert(barq::npos, std::int_fast64_t(file_ident_salt));  // Throws
    m_acc->cf_client_versions.insert(barq::npos, client_version);                  // Throws
    m_acc->cf_rh_base_versions.insert(barq::npos, recip_hist_base_version);        // Throws
    m_acc->cf_recip_hist_refs.insert(barq::npos, recip_hist_ref);                  // Throws
    m_acc->cf_proxy_files.insert(barq::npos, std::int_fast64_t(proxy_file_ident)); // Throws
    m_acc->cf_client_types.insert(barq::npos, std::int_fast64_t(client_type));     // Throws
    m_acc->cf_last_seen_timestamps.insert(barq::npos, last_seen_timestamp);        // Throws
    m_acc->cf_locked_server_versions.insert(barq::npos, locked_server_version);    // Throws
...
{
    BARQ_ASSERT(is_direct_client(client_type));
    BARQ_ASSERT(client_type != ClientType::legacy);
    ...
    // `/doc/history_compaction.md` in the `barq-sync` Git repository.
    std::int_fast64_t last_seen_timestamp = m_acc->cf_last_seen_timestamps.get(client_file_index);
    bool expired_due_to_compaction = (last_seen_timestamp == 0);
    if (BARQ_UNLIKELY(expired_due_to_compaction)) {
        logger.debug("Client expired because history has been compacted");
        return BootstrapError::client_file_expired;
    }

    BARQ_ASSERT_RELEASE(recip_hist_base_version >= m_history_base_version);
                                             ...
                                             Logger& logger) const -> BootstrapError
{
    ...
    auto barq_version = tr->get_version();
...
{
    BARQ_ASSERT(client_file_ident != 0);
    BARQ_ASSERT(download_progress.server_version <= end_version);
    ...
    BARQ_ASSERT(download_progress.server_version >= m_history_base_version);

    std::size_t client_file_index = std::size_t(client_file_ident);
    {
        auto client_type = ClientType(m_acc->cf_client_types.get(client_file_index));
        BARQ_ASSERT_RELEASE(is_direct_client(client_type));
        std::int_fast64_t last_seen_timestamp = m_acc->cf_last_seen_timestamps.get(client_file_index);
        bool expired = (last_seen_timestamp == 0);
        if (BARQ_UNLIKELY(expired))
            return false;
    ...
    if (download_progress_2.server_version > m_history_base_version) {
        ...
        BARQ_ASSERT(cumulative_byte_size_current_2 >= 0);
    }
    if (m_history_size > 0) {
        std::size_t end_ndx = m_history_size - 1;
        cumulative_byte_size_total_2 = m_acc->sh_cumul_byte_sizes.get(end_ndx);
    }
    BARQ_ASSERT(cumulative_byte_size_current_2 <= cumulative_byte_size_total_2);
...
void ServerHistory::add_upstream_sync_status()
{
    ...
    BARQ_ASSERT(!m_acc->upstream_status.is_attached());
    BARQ_ASSERT(m_local_file_ident == g_root_node_file_ident);
    ...
    static_assert(g_root_node_file_ident == 1, "");
    if (BARQ_UNLIKELY(m_num_client_files > 2)) {
        throw util::runtime_error("Barq file has registered client file identifiers, "
...
std::vector<sync::Changeset> ServerHistory::get_parsed_changesets(version_type begin, version_type end) const
{
    ...
    version_type barq_version = rt->get_version();
    const_cast<ServerHistory*>(this)->set_group(rt.get());
    ensure_updated(barq_version);

    BARQ_ASSERT(begin > m_history_base_version);
    if (end == version_type(-1)) {
        end = m_history_base_version + m_history_size + 1;
    }
    BARQ_ASSERT(begin <= end);
...
public:
    ...
    {
        BARQ_ASSERT(m_changesets);
        BARQ_ASSERT(server_version > m_base_version);
    ...
    {
        BARQ_ASSERT(new_base_version > m_base_version);
                                                ...
                                                std::size_t num_changesets, util::Logger& logger)
{
    ...
    BARQ_ASSERT(remote_file_index < m_num_client_files);
...

    {
        ...
        for (std::size_t i = 0; i < num_changesets; ++i) {
            ...
            BARQ_ASSERT(changesets[i].origin_file_ident > 0);
            UploadCursor upload_cursor = {changesets[i].remote_version, changesets[i].last_integrated_local_version};
            BARQ_ASSERT(upload_cursor.client_version > prev_upload_cursor.client_version);
            BARQ_ASSERT(are_mutually_consistent(upload_cursor, prev_upload_cursor));
    ...
    if (num_changesets > 0) {
        ...
        for (std::size_t i = 0; i < num_changesets; ++i) {
            BARQ_ASSERT(get_instruction_encoder().buffer().size() == 0);
                                           ...
                                           UploadCursor upload_progress)
{
    ...
    BARQ_ASSERT(upload_progress.client_version >= orig_upload_progress.client_version);
    BARQ_ASSERT(are_mutually_consistent(upload_progress, orig_upload_progress));
...
void ServerHistory::upgrade_history_schema(int stored_schema_version)
{
    ...
    // >= 1`).
    BARQ_ASSERT(stored_schema_version < get_server_history_schema_version());
    BARQ_ASSERT(stored_schema_version >= 1);
    int orig_schema_version = stored_schema_version;
    int schema_version = orig_schema_version;
    // NOTE: Future migration steps go here.

    BARQ_ASSERT(schema_version == get_server_history_schema_version());
...
// Overriding member in Replication
auto ServerHistory::prepare_changeset(const char* data, std::size_t size, version_type barq_version) -> version_type
{
    ensure_updated(barq_version);
    prepare_for_write(); // Throws
...
void ServerHistory::update_from_parent(version_type barq_version)
{
    using gf = _impl::GroupFriend;
    ref_type ref = gf::get_history_ref(*m_group);
    update_from_ref_and_version(ref, barq_version); // Throws
}
...
{
    BARQ_ASSERT(begin_version <= end_version);
    BARQ_ASSERT(begin_version >= m_ct_base_version);
    BARQ_ASSERT(end_version <= m_ct_base_version + m_ct_history_size);
    std::size_t n = to_size_t(end_version - begin_version);
    BARQ_ASSERT(n == 0 || m_acc);
...
// Overriding member in _impl::History
void ServerHistory::set_oldest_bound_version(version_type barq_version)
{
    BARQ_ASSERT(barq_version >= m_version_of_oldest_bound_snapshot);
    if (barq_version > m_version_of_oldest_bound_snapshot) {
        m_version_of_oldest_bound_snapshot = barq_version;
...
void ServerHistory::verify() const
{
    ...
    // Barq is in the initial empty state where top-ref is null.
    version_type initial_barq_version = 1;
    BARQ_ASSERT(m_ct_history_size != 0 || m_ct_base_version == initial_barq_version);

    if (!m_acc) {
        BARQ_ASSERT(m_local_file_ident == g_root_node_file_ident);
        BARQ_ASSERT(m_num_client_files == 0);
        BARQ_ASSERT(m_history_size == 0);
        BARQ_ASSERT(m_server_version_salt == 0);
        BARQ_ASSERT(m_history_base_version == 0);
        BARQ_ASSERT(m_ct_history_size == 0);
        return;
    }
    ...
    BARQ_ASSERT(m_history_base_version == m_acc->root.get_as_ref_or_tagged(s_history_base_version_iip).get_as_int());
    salt_type base_version_salt = m_acc->root.get_as_ref_or_tagged(s_base_version_salt_iip).get_as_int();
    BARQ_ASSERT((m_history_base_version == 0) == (base_version_salt == 0));

    BARQ_ASSERT(m_acc->cf_ident_salts.size() == m_num_client_files);
    BARQ_ASSERT(m_acc->cf_client_versions.size() == m_num_client_files);
    BARQ_ASSERT(m_acc->cf_rh_base_versions.size() == m_num_client_files);
    BARQ_ASSERT(m_acc->cf_recip_hist_refs.size() == m_num_client_files);
    BARQ_ASSERT(m_acc->cf_proxy_files.size() == m_num_client_files);
    BARQ_ASSERT(m_acc->cf_client_types.size() == m_num_client_files);
    BARQ_ASSERT(m_acc->cf_last_seen_timestamps.size() == m_num_client_files);
    BARQ_ASSERT(m_acc->cf_locked_server_versions.size() == m_num_client_files);

    BARQ_ASSERT(m_acc->sh_version_salts.size() == m_history_size);
    BARQ_ASSERT(m_acc->sh_origin_files.size() == m_history_size);
    BARQ_ASSERT(m_acc->sh_client_versions.size() == m_history_size);
    BARQ_ASSERT(m_acc->sh_timestamps.size() == m_history_size);
    BARQ_ASSERT(m_acc->sh_changesets.size() == m_history_size);
    BARQ_ASSERT(m_acc->sh_cumul_byte_sizes.size() == m_history_size);

    salt_type server_version_salt =
        (m_history_size == 0 ? base_version_salt : salt_type(m_acc->sh_version_salts.get(m_history_size - 1)));
    BARQ_ASSERT(m_server_version_salt == server_version_salt);

    BARQ_ASSERT(m_local_file_ident > 0 && std::uint_fast64_t(m_local_file_ident) < m_num_client_files);
    ...
    for (std::size_t i = 0; i < m_history_size; ++i) {
        ...
        BARQ_ASSERT(salt > 0 && salt <= 0x0'7FFF'FFFF'FFFF'FFFF);
        file_ident_type origin_file_ident = 0;
        BARQ_ASSERT(!util::int_cast_with_overflow_detect(m_acc->sh_origin_files.get(i), origin_file_ident));
        BARQ_ASSERT(origin_file_ident != m_local_file_ident);
        std::size_t origin_file_index = 0;
        BARQ_ASSERT(!util::int_cast_with_overflow_detect(origin_file_ident, origin_file_index));
        BARQ_ASSERT(origin_file_index < m_num_client_files);
        version_type client_version = 0;
        BARQ_ASSERT(!util::int_cast_with_overflow_detect(m_acc->sh_client_versions.get(i), client_version));
        bool of_local_origin = (origin_file_ident == 0);
        if (of_local_origin) {
            BARQ_ASSERT(client_version == 0);
        }
        else {
            ...
            if (!from_reference_file) {
                ...
                switch (ClientType(client_type)) {
                    ...
                    case ClientType::indirect: {
                        ...
                        BARQ_ASSERT(!util::int_cast_with_overflow_detect(proxy_file, client_file_ident));
                        good_client_type = true;
                        break;
                    }
                    case ClientType::self:
                ...
                BARQ_ASSERT(good_client_type);
            }
            ClientFile& client_file = client_files[client_file_ident];
            if (from_reference_file) {
                BARQ_ASSERT(client_version >= client_file.last_integrated_client_version);
            }
            else {
                BARQ_ASSERT(client_version > client_file.last_integrated_client_version);
            }
    ...
    for (std::size_t i = 0; i < m_num_client_files; ++i) {
        ...
        BARQ_ASSERT(!util::int_cast_with_overflow_detect(client_version, client_version_2));
        file_ident_type proxy_file_2 = 0;
        BARQ_ASSERT(!util::int_cast_with_overflow_detect(proxy_file, proxy_file_2));
        version_type locked_server_version_2 = 0;
        BARQ_ASSERT(!util::int_cast_with_overflow_detect(locked_server_version, locked_server_version_2));
        if (client_file_ident == 0) {
            // Special entry
            BARQ_ASSERT(ident_salt == 0);
            BARQ_ASSERT(proxy_file_2 == 0);
            BARQ_ASSERT(client_type == 0);
            BARQ_ASSERT(last_seen_timestamp == 0);
            BARQ_ASSERT(locked_server_version_2 == 0);
            // Upstream server
            BARQ_ASSERT(client_version_2 >= last_integrated_client_version);
        }
        else if (client_file_ident == g_root_node_file_ident) {
            // Root node's entry
            BARQ_ASSERT(ident_salt == 0);
            BARQ_ASSERT(client_version_2 == 0);
            BARQ_ASSERT(rh_base_version == 0);
            BARQ_ASSERT(recip_hist_ref == 0);
            BARQ_ASSERT(proxy_file_2 == 0);
            BARQ_ASSERT(client_type == 0);
            BARQ_ASSERT(last_seen_timestamp == 0);
            BARQ_ASSERT(locked_server_version_2 == 0);
            BARQ_ASSERT(!client_file);
            if (m_local_file_ident == g_root_node_file_ident)
                found_self = true;
        }
        else if (client_file_ident == m_local_file_ident) {
            // Entry representing the Barq file itself
            BARQ_ASSERT(ident_salt == 0);
            BARQ_ASSERT(client_version_2 == 0);
            BARQ_ASSERT(rh_base_version == 0);
            BARQ_ASSERT(recip_hist_ref == 0);
            BARQ_ASSERT(proxy_file_2 == 0);
            BARQ_ASSERT(client_type == int(ClientType::self));
            BARQ_ASSERT(last_seen_timestamp == 0);
            BARQ_ASSERT(locked_server_version_2 == 0);
            BARQ_ASSERT(!client_file);
            found_self = true;
            ...
            if (proxy_file_2 == 0) {
                ...
                BARQ_ASSERT(rh_base_version == 0);
                BARQ_ASSERT(recip_hist_ref == 0);
                BARQ_ASSERT(client_type == int(ClientType::upstream));
                BARQ_ASSERT(last_seen_timestamp == 0);
                BARQ_ASSERT(locked_server_version_2 == 0);
                BARQ_ASSERT(!client_file);
            }
                ...
                BARQ_ASSERT(rh_base_version == 0);
                BARQ_ASSERT(recip_hist_ref == 0);
                BARQ_ASSERT(client_type == int(ClientType::indirect));
                BARQ_ASSERT(last_seen_timestamp == 0);
                BARQ_ASSERT(locked_server_version_2 == 0);
                BARQ_ASSERT(is_valid_proxy_file_ident(proxy_file_2));
                BARQ_ASSERT(!client_file);
            }
        }
        else {
            ...
            bool expired = (last_seen_timestamp == 0);
            BARQ_ASSERT(ident_salt > 0 && ident_salt <= 0x0'7FFF'FFFF'FFFF'FFFF);
            BARQ_ASSERT(client_version_2 >= last_integrated_client_version);
            BARQ_ASSERT(!expired || (recip_hist_ref == 0));
            BARQ_ASSERT(proxy_file_2 == 0);
            BARQ_ASSERT(is_direct_client(ClientType(client_type)));
            BARQ_ASSERT(locked_server_version_2 <= current_server_version);
        }
    }
    BARQ_ASSERT(found_self);

    BARQ_ASSERT(m_ct_history_size >= 1); // See comment above
    BARQ_ASSERT(m_acc->ct_history.size() == m_ct_history_size);
#endif // BARQ_DEBUG
}
...
public:
    ...
    {
        if (BARQ_UNLIKELY(m_server_history))
...
// Overriding member in _impl::History
void ServerHistory::update_from_ref_and_version(ref_type ref, version_type barq_version)
{
    if (ref == 0) {
        ...
        m_ct_base_version = barq_version;
        m_ct_history_size = 0;
        discard_accessors();
        return;
    }
    if (BARQ_LIKELY(m_acc)) {
        ...
        if (m_acc->upstream_status.is_attached()) {
            BARQ_ASSERT(m_acc->upstream_status.size() == s_upstream_status_size);
        }
        if (m_acc->partial_sync.is_attached()) {
            BARQ_ASSERT(m_acc->partial_sync.size() == s_partial_sync_size);
        }
        dag.release();
    }
    ...
    m_ct_base_version = barq_version - m_ct_history_size;
}
...
void ServerHistory::create_empty_history()
{
    ...
    BARQ_ASSERT(m_num_client_files == 0);
    for (int i = 0; i < 2; ++i) {
        m_acc->cf_ident_salts.insert(barq::npos, 0);            // Throws
        m_acc->cf_client_versions.insert(barq::npos, 0);        // Throws
        m_acc->cf_rh_base_versions.insert(barq::npos, 0);       // Throws
        m_acc->cf_recip_hist_refs.insert(barq::npos, 0);        // Throws
        m_acc->cf_proxy_files.insert(barq::npos, 0);            // Throws
        m_acc->cf_client_types.insert(barq::npos, 0);           // Throws
        m_acc->cf_last_seen_timestamps.insert(barq::npos, 0);   // Throws
        m_acc->cf_locked_server_versions.insert(barq::npos, 0); // Throws
        ++m_num_client_files;
    }
}
...
{
    BARQ_ASSERT(server_version >= m_history_base_version);
    ...
    BARQ_ASSERT(history_entry_index < m_history_size);
...
{
    static_assert(g_root_node_file_ident == 1, "");
    BARQ_ASSERT(file_ident >= 2);
    BARQ_ASSERT(std::uint_fast64_t(file_ident) < m_num_client_files);
...
void ServerHistory::add_sync_history_entry(const HistoryEntry& entry)
{
    ...
    m_acc->sh_version_salts.insert(barq::npos, m_salt_for_new_server_versions); // Throws
    m_acc->sh_origin_files.insert(barq::npos, client_file);                     // Throws
    m_acc->sh_client_versions.insert(barq::npos, client_version);               // Throws
    m_acc->sh_timestamps.insert(barq::npos, timestamp);                         // Throws
    ...
    m_acc->sh_cumul_byte_sizes.insert(barq::npos, history_byte_size);
...
void ServerHistory::trim_cont_transact_history()
{
    ...
    if (m_version_of_oldest_bound_snapshot > m_ct_base_version) {
        ...
        BARQ_ASSERT(num_entries_to_erase < m_ct_history_size);
...
{
    BARQ_ASSERT(server_version > m_history_base_version && server_version <= get_server_version());
...
{
    BARQ_ASSERT(remote_file_ident != g_root_node_file_ident);
    BARQ_ASSERT(begin_version >= m_history_base_version);
    BARQ_ASSERT(begin_version <= end_version);
...
bool ServerHistory::received_from(const HistoryEntry& entry, file_ident_type remote_file_ident) const noexcept
{
    ...
    if (!from_upstream_server) {
        std::size_t remote_file_index = std::size_t(remote_file_ident);
        BARQ_ASSERT(is_direct_client(ClientType(m_acc->cf_client_types.get(remote_file_index))));
...
void ServerHistory::fixup_state_and_changesets_for_assigned_file_ident(Transaction& group, file_ident_type file_ident)
{
    ...
    BARQ_ASSERT(file_ident != 0);
    BARQ_ASSERT(file_ident != g_root_node_file_ident);
    BARQ_ASSERT(m_acc->upstream_status.is_attached());
    BARQ_ASSERT(m_local_file_ident == g_root_node_file_ident);
    using Instruction = barq::sync::Instruction;

    auto promote_global_key = [&](GlobalKey& oid) {
        BARQ_ASSERT(oid.hi() == 0); // client_file_ident == 0
...
void ServerHistory::record_current_schema_version(Array& schema_versions, version_type snapshot_version)
{
    ...
    BARQ_ASSERT(schema_versions.size() == s_schema_versions_size);
    ...
    }
    {
        ...
        const char* library_version = BARQ_VERSION_STRING;
