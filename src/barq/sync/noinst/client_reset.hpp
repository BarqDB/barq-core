...
#ifndef BARQ_NOINST_CLIENT_RESET_HPP
#define BARQ_NOINST_CLIENT_RESET_HPP
...
// preform_client_reset_diff() takes the Barq performs a client reset on
// the Barq in 'path_local' given the Barq 'path_fresh' as the source of truth.
// If the fresh path is not provided, discard mode is assumed and all data in the local
// Barq is removed.
// If the fresh path is provided, the local Barq is changed such that its state is equal
// to the fresh Barq. Then the local Barq will have its client file ident set to
// the file ident from the fresh barq
...
#endif // BARQ_NOINST_CLIENT_RESET_HPP
