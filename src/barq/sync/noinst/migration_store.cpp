...
#include <barq/sync/noinst/sync_metadata_schema.hpp>
...
void MigrationStore::complete_migration_or_rollback()
{
    ...
    BARQ_ASSERT(!migration_table->is_empty());
    auto migration_store_obj = migration_table->get_object(0);
...
std::shared_ptr<barq::SyncConfig> MigrationStore::convert_sync_config(std::shared_ptr<barq::SyncConfig> config)
{
    BARQ_ASSERT(config);
    ...
    // Once in the migrated state, the partition value cannot change for the same barq file
    if (m_state == MigrationState::Migrated && m_migrated_partition &&
        m_migrated_partition != config->partition_value) {
        throw LogicError(
            ErrorCodes::IllegalOperation,
            util::format("Partition value cannot be changed for migrated barqs\n - original: %1\n -   config: %2",
                         m_migrated_partition, config->partition_value));
    }
...
std::shared_ptr<barq::SyncConfig>
MigrationStore::convert_sync_config_to_flx(std::shared_ptr<barq::SyncConfig> config)
{
    if (config->flx_sync_requested) {
        return config;
    }

    auto flx_config = std::make_shared<barq::SyncConfig>(*config); // deep copy
...
{
    BARQ_ASSERT(!rql_query_string.empty());
    ...
    BARQ_ASSERT(m_state != MigrationState::Migrated);
    ...
    if (migration_table->is_empty()) {
        ...
        BARQ_ASSERT(m_state == state);
        BARQ_ASSERT(m_query_string == query_string);
        BARQ_ASSERT(m_migrated_partition == migrated_partition);
    }
}
...
void MigrationStore::rollback_to_pbs()
{
    ...
    BARQ_ASSERT(m_state != MigrationState::NotMigrated);
    m_state = MigrationState::RollbackInProgress;
...
{
    BARQ_ASSERT(!object_class_name.empty());
