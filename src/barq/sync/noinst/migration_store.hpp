...
public:
    ...
    std::shared_ptr<barq::SyncConfig> convert_sync_config(std::shared_ptr<barq::SyncConfig> config);
    // Convert a configuration from PBS to FLX. No-op if already an FLX configuration.
    static std::shared_ptr<barq::SyncConfig> convert_sync_config_to_flx(std::shared_ptr<barq::SyncConfig> config);
