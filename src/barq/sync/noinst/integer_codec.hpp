#ifndef BARQ_NOINST_INTEGER_CODEC_HPP
#define BARQ_NOINST_INTEGER_CODEC_HPP
...
std::size_t decode_int(const char* buffer, std::size_t size, T& value) noexcept
{
    ...
    if (BARQ_LIKELY(decode_int(input, value))) {
        BARQ_ASSERT(input.ptr > buffer);
...
#endif // BARQ_NOINST_INTEGER_CODEC_HPP
