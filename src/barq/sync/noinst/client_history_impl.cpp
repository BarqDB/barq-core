...
#include <barq/sync/instruction_replication.hpp>
#include <barq/sync/noinst/client_reset.hpp>
#include <barq/sync/noinst/client_reset_recovery.hpp>
    ...
    SaltedVersion server_version, const std::vector<_impl::client_reset::RecoveredChange>& recovered_changesets)
{
    ...
    BARQ_ASSERT(client_version == current_version); // For now
    ...
    if (recovered_changesets.empty()) {
        ...
        BARQ_ASSERT(first_version >= m_sync_history_base_version);
    ...
    // when downloading the fresh barq.
...
// Overriding member function in barq::Replication
...
void ClientReplication::upgrade_history_schema(int stored_schema_version)
{
    ...
    // >= 1`).
    BARQ_ASSERT(stored_schema_version < get_client_history_schema_version());
    BARQ_ASSERT(stored_schema_version >= 11);
    ...
    BARQ_ASSERT(schema_version == get_client_history_schema_version());
                               ...
                               SyncProgress& progress) const
{
    ...
    BARQ_ASSERT(current_client_version >= s_initial_version + 0);
...
{
    BARQ_ASSERT(client_file_ident.ident != 0);
    ...
    Array& root = m_arrays->root;
    BARQ_ASSERT(wt->get_sync_file_id() == 0);
...
// Overriding member function in barq::sync::ClientHistoryBase
                                      ...
                                      VersionInfo& version_info)
{
    ...
    version_info.barq_version = new_version;
                                               ...
                                               version_type& locked_server_version) const
{
    ...
    while (accum_byte_size < accum_byte_size_soft_limit) {
        ...
        if (ec == util::compression::error::decompress_unsupported) {
            BARQ_TERMINATE(
                "Synchronized Barq files with unuploaded local changes cannot be copied between platforms.");
        }
        BARQ_ASSERT_3(ec, ==, std::error_code{});
...
{
    BARQ_ASSERT(incoming_changesets.size() != 0);
    BARQ_ASSERT(
    ...
    while (!changesets_to_integrate.empty()) {
        ...
        // from the Barq. downloaded_bytes must be persisted in the Barq, since
        ...
        BARQ_ASSERT(!m_applying_server_changeset);
    ...
    BARQ_ASSERT(new_version.version > 0);
    BARQ_ASSERT(
    ...
    version_info.barq_version = new_version.version;
...
{
    BARQ_ASSERT(transact->get_transact_stage() == DB::transact_Writing);
    ...
    try {
        for (auto& changeset : changesets_to_integrate) {
            BARQ_ASSERT(changeset.last_integrated_remote_version <= local_version);
            BARQ_ASSERT(changeset.origin_file_ident > 0 && changeset.origin_file_ident != sync_file_id);
...
{
    is_compressed = true;
    BARQ_ASSERT(version > m_sync_history_base_version);

    std::size_t index = to_size_t(version - m_sync_history_base_version) - 1;
    BARQ_ASSERT(index < sync_history_size());
                                            ...
                                            version_type& last_integrated_server_version) noexcept -> version_type
{
    ...
    BARQ_ASSERT(begin_version >= base_version);
    BARQ_ASSERT(end_version <= base_version + arrays.changesets.size());
                                                             ...
                                                             version_type end_version) const noexcept
{
    ...
    BARQ_ASSERT(m_arrays->changesets.is_attached());
    BARQ_ASSERT(m_arrays->origin_file_idents.is_attached());
    BARQ_ASSERT(end_version <= m_sync_history_base_version + sync_history_size());
...
{
    if (m_arrays) {
        BARQ_ASSERT(m_arrays->root.size() == s_root_size);
        return;
    }
...
Replication::version_type ClientHistory::add_changeset(BinaryData ct_changeset, BinaryData sync_changeset)
{
    ...
    BARQ_ASSERT(!m_applying_server_changeset || !m_applying_client_reset);
    ...
    if (m_applying_server_changeset) {
        ...
        BARQ_ASSERT(m_ct_history_base_version + ct_history_size() ==
                     m_sync_history_base_version + sync_history_size());
        BARQ_ASSERT(sync_changeset.size() == 0);
    ...
    // uploadable_bytes is updated at every local Barq change. The total
    // number of uploadable bytes must be persisted in the Barq, since the
...
{
    BARQ_ASSERT(m_arrays->changesets.size() == sync_history_size());
    BARQ_ASSERT(m_arrays->reciprocal_transforms.size() == sync_history_size());
    BARQ_ASSERT(m_arrays->remote_versions.size() == sync_history_size());
    BARQ_ASSERT(m_arrays->origin_file_idents.size() == sync_history_size());
    BARQ_ASSERT(m_arrays->origin_timestamps.size() == sync_history_size());
    ...
    m_arrays->remote_versions.insert(barq::npos, std::int_fast64_t(entry.remote_version));       // Throws
    m_arrays->origin_file_idents.insert(barq::npos, std::int_fast64_t(entry.origin_file_ident)); // Throws
    m_arrays->origin_timestamps.insert(barq::npos, std::int_fast64_t(entry.origin_timestamp));   // Throws
}
...
void ClientHistory::set_download_progress(Transaction& tr, DownloadableProgress p)
{
    ...
    BARQ_ASSERT(root.size() > s_progress_uploadable_bytes_iip);
    root.set(s_progress_downloadable_bytes_iip,
...
void ClientHistory::trim_ct_history()
{
    ...
    BARQ_ASSERT(m_ct_history_base_version + ct_history_size() == m_sync_history_base_version + sync_history_size());
}
...
void ClientHistory::do_trim_sync_history(std::size_t n)
{
    ...
    BARQ_ASSERT(m_arrays->origin_file_idents.size() == sync_history_size());
    BARQ_ASSERT(m_arrays->origin_timestamps.size() == sync_history_size());
    BARQ_ASSERT(n <= sync_history_size());
                                                                  ...
                                                                  file_ident_type client_file_ident)
{
    ...
    for (size_t i = 0; i < sync_history_size(); ++i) {
        // We could have opened a pre-provisioned barq file. In this case we can skip the entries downloaded
        // from the server.
        if (m_arrays->origin_file_idents.get(i) != 0)
            continue;
...
// Overriding member function in barq::_impl::History
...
{
    BARQ_ASSERT(begin_version <= end_version);
    BARQ_ASSERT(begin_version >= m_ct_history_base_version);
    BARQ_ASSERT(end_version <= m_ct_history_base_version + ct_history_size());
    std::size_t n = to_size_t(end_version - begin_version);
    BARQ_ASSERT(n == 0 || m_arrays);
...
{
    BARQ_ASSERT(version >= m_version_of_oldest_bound_snapshot);
...
void ClientHistory::verify() const
{
    ...
    BARQ_ASSERT(ct_history_size() != 0 || m_ct_history_base_version == s_initial_version + 0);

    if (!m_arrays) {
        BARQ_ASSERT(m_progress_download.server_version == 0);
        BARQ_ASSERT(m_progress_download.last_integrated_client_version == 0);
        return;
    }
    m_arrays->verify();
    ...
    version_type progress_download_client_version =
        version_type(root.get_as_ref_or_tagged(s_progress_download_client_version_iip).get_as_int());
    BARQ_ASSERT(progress_download_server_version == m_progress_download.server_version);
    BARQ_ASSERT(progress_download_client_version == m_progress_download.last_integrated_client_version);
    BARQ_ASSERT(progress_download_client_version <= m_sync_history_base_version + sync_history_size());
    ...
    BARQ_ASSERT(progress_download_server_version >= remote_version_of_last_entry);
    ...
    BARQ_ASSERT(cooked_history.get_ref_from_parent() == 0);
#endif // BARQ_DEBUG
}
    ...
    : Arrays(alloc)
{
    ...
    cooked_history.set_parent(&root, s_cooked_history_iip);
    // We should have no cooked history in existing Barqs.
    BARQ_ASSERT(cooked_history.get_ref_from_parent() == 0);
}
...
{
    root.init_from_ref(ref);
    BARQ_ASSERT(root.size() == s_root_size);
    {
...
void ClientHistory::Arrays::verify() const
{
    ...
    BARQ_ASSERT(root.size() == s_root_size);
    BARQ_ASSERT(reciprocal_transforms.size() == changesets.size());
    BARQ_ASSERT(remote_versions.size() == changesets.size());
    BARQ_ASSERT(origin_file_idents.size() == changesets.size());
    BARQ_ASSERT(origin_timestamps.size() == changesets.size());
#endif // BARQ_DEBUG
}
