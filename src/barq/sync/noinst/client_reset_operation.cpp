...
std::string get_fresh_path_for(const std::string& path)
{
    ...
    BARQ_ASSERT(path.length());
    BARQ_ASSERT_DEBUG_EX(
...
{
    BARQ_ASSERT(reset_config.mode != ClientResyncMode::Manual);
    BARQ_ASSERT(reset_config.fresh_copy);
    logger.debug(util::LogCategory::reset,
                 "Possibly beginning client reset operation: barq_path = %1, mode = %2, action = %3, error = %4",
    ...
    // if there is nothing in this Barq, then there is nothing to reset and
    // sync should be able to continue as normal
    auto latest_version = db.get_version_id_of_latest_snapshot();
    bool local_barq_exists = latest_version.version > 1;
    if (!local_barq_exists) {
        logger.debug(util::LogCategory::reset,
                     "Local Barq file has never been written to, so skipping client reset.");
        return false;
    }
