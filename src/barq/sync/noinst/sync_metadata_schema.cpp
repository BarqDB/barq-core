...
SyncMetadataSchemaVersionsReader::SyncMetadataSchemaVersionsReader(const TransactionRef& tr)
{
    ...
    BARQ_ASSERT_EX(tr->get_transact_stage() != DB::transact_Ready, tr->get_transact_stage());
    ...
    : SyncMetadataSchemaVersionsReader(tr)
{
    ...
    BARQ_ASSERT_EX((orig == DB::transact_Reading || orig == DB::transact_Writing), orig);
    ...
    if (!m_table) {
        ...
        if (BARQ_UNLIKELY(tr->has_table(c_sync_internal_schemas_table))) {
            load_sync_metadata_schema(*tr, &unified_schema_version_table_def);
        }
        else {
