...
#ifndef BARQ_NOINST_CLIENT_HISTORY_IMPL_HPP
#define BARQ_NOINST_CLIENT_HISTORY_IMPL_HPP
...
//
//  11   Path-based instruction format for Barq Sync (v10)
//
...
public:
    ...
    /// Get the version of the latest snapshot of the associated Barq, as well
    ...
    /// as the snapshot number of the Barq file depends on the history
    /// implementation.
    ///
    ...
    /// Barq file, such that it is available via get_status() during future
    /// synchronization sessions. It is an error to set this identifier more
    /// than once per Barq file.
    ///
    ...
    /// file identifiers generated on behalf of a particular server Barq are
    ...
    /// Stores the synchronization progress in the associated Barq file in a
    ...
    // Overriding member functions in barq::TransformHistory
...
private:
    ...
    // Overriding member functions in barq::_impl::History
...
public:
    ...
    // Overriding member functions in barq::Replication
...
inline void ClientHistory::clamp_sync_version_range(version_type& begin, version_type& end) const noexcept
{
    ...
    BARQ_ASSERT(m_progress_download.last_integrated_client_version <= begin);
...
/// \brief Create a "sync history" implementation of the barq::Replication
/// interface.
///
...
#endif // BARQ_NOINST_CLIENT_HISTORY_IMPL_HPP
