...
std::optional<PendingReset> PendingResetStore::has_pending_reset(const Group& group)
{
    ...
    BARQ_ASSERT(reset_store->m_pending_reset_table);
    ...
    if (reset_entry.get<String>(reset_store->m_version) != BARQ_VERSION_STRING) {
...
{
    BARQ_ASSERT(mode != ClientResyncMode::Manual);
    ...
    table->create_object(null_key, {
                                       {reset_store.m_version, Mixed(BARQ_VERSION_STRING)},
