#ifndef BARQ_NOINST_PROTOCOL_CODEC_HPP
#define BARQ_NOINST_PROTOCOL_CODEC_HPP
...
#include <barq/util/from_chars.hpp>
...
private:
    ...
    void parse_log_message(Connection& connection, HeaderLineParser& msg)
    {
        ...
        // See if the log_message contains the Barq request ID.
        if (auto it = message_body.find("co_id"); it != message_body.end() && it->is_string()) {
            connection.receive_barq_request_id(it->get<std::string_view>());
        }
...
#endif // BARQ_NOINST_PROTOCOL_CODEC_HPP
