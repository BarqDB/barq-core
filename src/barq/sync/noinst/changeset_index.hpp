...
#ifndef BARQ_NOINST_CHANGESET_INDEX_HPP
#define BARQ_NOINST_CHANGESET_INDEX_HPP
...
struct ChangesetIndex {
    using Changeset = barq::sync::Changeset;
    using GlobalID = barq::sync::GlobalID;
    using PrimaryKey = barq::sync::PrimaryKey;
...
struct ChangesetIndex::RangeIterator {
        ...
        , m_outer(ranges->begin())
    {
        if (m_outer != ranges->end()) {
            ...
            m_pos = m_inner->begin;
            BARQ_ASSERT(m_pos != m_inner->end); // empty ranges not allowed!
            check();
        }
    }
    ...
    {
        BARQ_ASSERT_DEBUG(m_ranges);
        BARQ_ASSERT_DEBUG(m_outer != m_ranges->end());
        BARQ_ASSERT_DEBUG(m_inner >= m_outer->second.begin());
        BARQ_ASSERT_DEBUG(m_inner < m_outer->second.end());
        BARQ_ASSERT_DEBUG(m_pos >= m_inner->begin);
        BARQ_ASSERT_DEBUG(m_pos < m_inner->end);
        BARQ_ASSERT_DEBUG(m_pos.m_inner >= m_outer->first->begin().m_inner);
        BARQ_ASSERT_DEBUG(m_pos.m_inner < m_outer->first->end().m_inner);
    }
    ...
    RangeIterator& operator++() noexcept
    {
        ...
        ++m_pos;
        if (BARQ_UNLIKELY(m_pos == m_inner->end)) {
            // Slow path
            inc_inner();
        }
        return *this;
    ...
    {
        BARQ_ASSERT(m_ranges == other.m_ranges);
...
#endif // BARQ_NOINST_CHANGESET_INDEX_HPP
