#include <barq/sync/noinst/client_impl_base.hpp>
...
#include <barq/sync/noinst/client_reset_operation.hpp>
#include <barq/sync/noinst/sync_schema_migration.hpp>
...
using namespace barq::sync::websocket;
...
std::chrono::milliseconds ClientImpl::ReconnectInfo::delay_interval()
{
    ...
    switch (*m_backoff_state.triggering_error) {
        ...
        default:
            ...
            BARQ_ASSERT(m_reconnect_mode == ReconnectMode::normal);
                                      ...
                                      port_type& port, std::string& path) const
{
    ...
    bool barq_scheme = (uri.get_scheme() == "barq:" || uri.get_scheme() == "barqs:");
    bool ws_scheme = (uri.get_scheme() == "ws:" || uri.get_scheme() == "wss:");
    bool good = ((barq_scheme || ws_scheme) && uri.get_auth(userinfo, address_2, port_2) && userinfo.empty() &&
                 !address_2.empty() && uri.get_query().empty() && uri.get_frag().empty()); // Throws
    if (BARQ_UNLIKELY(!good))
        return false;
    ProtocolEnvelope protocol_2;
    port_type port_3;
    if (barq_scheme) {
        if (uri.get_scheme() == "barq:") {
            protocol_2 = ProtocolEnvelope::barq;
            port_3 = (m_enable_default_port_hack ? 80 : 7800);
        }
        else {
            protocol_2 = ProtocolEnvelope::barqs;
            port_3 = (m_enable_default_port_hack ? 443 : 7801);
        }
    }
    else {
        BARQ_ASSERT(ws_scheme);
    ...
    if (!port_2.empty()) {
        ...
        in >> port_3;
        if (BARQ_UNLIKELY(!in || !in.eof() || port_3 < 1))
            return false;
    }
    ...
    , m_random{}
{
    ...
    logger.info("Barq sync client (%1)", BARQ_VER_CHUNK); // Throws
    ...
    BARQ_ASSERT_EX(m_socket_provider, "Must provide socket provider in sync Client config");
...
{
    util::CheckedLockGuard lock(m_drain_mutex);
    BARQ_ASSERT(m_outstanding_posts);
...
void Connection::initiate_session_deactivation(Session* sess)
{
    BARQ_ASSERT(sess);
    BARQ_ASSERT(&sess->m_conn == this);
    BARQ_ASSERT(m_num_active_sessions);
    ...
    if (BARQ_UNLIKELY(--m_num_active_sessions == 0)) {
...
{
    BARQ_ASSERT(sess->m_state == Session::Deactivated);
...
void Connection::force_close()
{
    ...
    BARQ_ASSERT_EX(m_state == ConnectionState::disconnected, m_state);
        ...
        if (prefix_matches(expected_prefix)) {
            ...
            if (in && in.eof() && value_2 >= 0) {
                ...
                if (good_version) {
                    ...
                    // will provide the Barq connection ID via a log message.
                    // TODO: Remove once the server starts sending the connection ID
                    receive_barq_request_id(m_websocket->get_barq_request_id());
...
{
    BARQ_ASSERT(m_activated);
    BARQ_ASSERT(!m_reconnect_delay_in_progress);
    BARQ_ASSERT(!m_disconnect_delay_in_progress);
...
{
    if (!status.is_ok()) {
        BARQ_ASSERT(status != ErrorCodes::OperationAborted);
        throw Exception(status);
    }
...
void Connection::handle_connect_wait(Status status)
{
    ...
    BARQ_ASSERT_EX(m_state == ConnectionState::connecting, m_state);
...
void Connection::schedule_urgent_ping()
{
    ...
    BARQ_ASSERT_EX(m_state == ConnectionState::connecting || m_waiting_for_pong, m_state);
    if (!m_send_ping)
        m_minimize_next_ping_delay = true;
}
...
{
    BARQ_ASSERT(!m_ping_delay_in_progress);
    BARQ_ASSERT(!m_waiting_for_pong);
    BARQ_ASSERT(!m_send_ping);
    ...
    if (!m_minimize_next_ping_delay) {
        ...
        BARQ_ASSERT_3(now, >=, m_pong_wait_started_at);
...
{
    BARQ_ASSERT_EX(m_state == ConnectionState::connected, m_state);
    BARQ_ASSERT(!m_sending_session);
    BARQ_ASSERT(!m_sending);
    if (m_send_ping) {
        send_ping(); // Throws
        return;
    }
...
void Connection::disconnect(const SessionErrorInfo& info)
{
    ...
    if (!m_barq_connection_id.empty()) {
        m_barq_connection_id.clear();
...
void Connection::receive_pong(milliseconds_type timestamp)
{
    ...
    if (m_ping_after_scheduled_reset_of_reconnect_info) {
        BARQ_ASSERT(m_reconnect_info.scheduled_reset);
        m_ping_after_scheduled_reset_of_reconnect_info = false;
...
void Connection::receive_error_message(const ProtocolErrorInfo& info, session_ident_type session_ident)
{
    ...
    if (BARQ_LIKELY(known_error_code)) {
        ProtocolError error_code = ProtocolError(info.raw_error_code);
        if (BARQ_LIKELY(!is_session_level_error(error_code))) {
...
void Connection::receive_barq_request_id(std::string_view coid)
{
    if (coid.empty() || !m_barq_connection_id.empty()) {
        return;
    }
    m_barq_connection_id = coid;
    logger.log(util::LogCategory::session, util::LogCategory::Level::info,
               "Connected to Barq sync with request id: \"%1\". Further log entries for this connection will be "
               "prefixed with \"Connection[%2:%1]\" instead of \"Connection[%2]\"",
               m_barq_connection_id, m_ident);
    logger.base_logger = make_logger(m_ident, m_barq_connection_id, get_client().logger.base_logger);
...
// associated with the same Barq file.
//
...
std::string Connection::get_active_barq_connection_id()
{
    return m_barq_connection_id;
}

void Session::cancel_resumption_delay()
{
    BARQ_ASSERT_EX(m_state == Active, m_state);
                                                 ...
                                                 std::vector<ProtocolErrorInfo>* out)
{
    ...
    BARQ_ASSERT_DEBUG(
        std::is_sorted(m_pending_compensating_write_errors.begin(), m_pending_compensating_write_errors.end(),
                       [](const ProtocolErrorInfo& lhs, const ProtocolErrorInfo& rhs) {
                           BARQ_ASSERT_DEBUG(lhs.compensating_write_server_version.has_value());
                           BARQ_ASSERT_DEBUG(rhs.compensating_write_server_version.has_value());
    ...
    while (!m_pending_compensating_write_errors.empty() &&
        ...
        BARQ_ASSERT_3(*cur_error.compensating_write_server_version, >=, changesets.front().version);
...
{
    BARQ_ASSERT_EX(m_state == Active, m_state);
    BARQ_ASSERT(!m_client_error && !m_error_to_send);
    ...
    BARQ_ASSERT(m_suspended || m_error_message_received || !m_unbind_message_sent);
...
{
    BARQ_ASSERT_EX(m_state == Active, m_state);
    BARQ_ASSERT_3(progress.download.server_version, >=, m_download_progress.server_version);
...
Session::~Session()
{
    //    BARQ_ASSERT_EX(m_state == Unactivated || m_state == Deactivated, m_state);
}
...
{
    BARQ_ASSERT_EX(m_state == Unactivated, m_state);

    logger.debug("Activating"); // Throws

    if (BARQ_LIKELY(!get_client().is_dry_run())) {
        bool file_exists = util::File::exists(get_barq_path());

        logger.info("client_reset_config = %1, Barq exists = %2, upload messages allowed = %3",
    ...
    BARQ_ASSERT_3(m_last_version_available, >=, m_progress.upload.client_version);
...
void Session::initiate_deactivation()
{
    ...
    if (m_enlisted_to_send) {
        BARQ_ASSERT(!unbind_process_complete());
        return;
    }
...
{
    BARQ_ASSERT_EX(m_state == Deactivating, m_state);
    m_state = Deactivated;
...
{
    BARQ_ASSERT_EX(m_state == Active || m_state == Deactivating, m_state);
    BARQ_ASSERT(m_enlisted_to_send);
    ...
    // not been initiated
    BARQ_ASSERT(!m_unbind_message_sent);
...
void Session::send_bind_message()
{
    ...
    // The file ident can be 0 when a client reset is being performed if a brand new local barq
...
{
    BARQ_ASSERT_EX(m_state == Active, m_state);
    BARQ_ASSERT(m_bind_message_sent);
    BARQ_ASSERT(!m_unbind_message_sent);
    BARQ_ASSERT(have_client_file_ident());
...
{
    BARQ_ASSERT_EX(m_state == Active, m_state);
    BARQ_ASSERT(m_ident_message_sent);
    BARQ_ASSERT(!m_unbind_message_sent);
    BARQ_ASSERT(m_pending_flx_sub_set);
    BARQ_ASSERT_3(m_pending_flx_sub_set->query_version, >, m_last_sent_flx_query_version);

    if (BARQ_UNLIKELY(get_client().is_dry_run())) {
        return;
    }
...
void Session::send_upload_message()
{
    ...
    if (m_pending_flx_sub_set) {
        BARQ_ASSERT(m_is_flx_sync_session);
...
void Session::send_mark_message()
{
    ...
    BARQ_ASSERT(m_ident_message_sent);
    BARQ_ASSERT(!m_unbind_message_sent);
    BARQ_ASSERT_3(m_target_download_mark, >, m_last_download_mark_sent);
...
{
    BARQ_ASSERT_EX(m_state == Deactivating || m_error_message_received || m_suspended, m_state);
    BARQ_ASSERT(m_bind_message_sent);
    BARQ_ASSERT(!m_unbind_message_sent);
...
void Session::send_json_error_message()
{
    ...
    BARQ_ASSERT(m_ident_message_sent);
    BARQ_ASSERT(!m_unbind_message_sent);
    BARQ_ASSERT(m_error_to_send);
    BARQ_ASSERT(m_client_error);
...
void Session::send_test_command_message()
{
    ...
    BARQ_ASSERT(it != m_pending_test_commands.end());
...
bool Session::client_reset_if_needed()
{
    ...
    try {
        // The file ident from the fresh barq will be copied over to the local barq
    ...
    // The fresh Barq has been used to reset the state
    logger.debug("Client reset is completed, path = %1", get_barq_path()); // Throws
    ...
    BARQ_ASSERT_EX(m_progress.download.last_integrated_client_version == 0,
                    m_progress.download.last_integrated_client_version);
    BARQ_ASSERT_EX(m_progress.upload.client_version == 0, m_progress.upload.client_version);
...
Status Session::receive_ident_message(SaltedFileIdent client_file_ident)
{
    ...
    // because in that case, the associated Barq and SessionWrapper must
...
Status Session::receive_download_message(const DownloadMessage& message)
{
    ...
    if (auto status = check_received_sync_progress(progress); BARQ_UNLIKELY(!status.is_ok())) {
    ...
    BARQ_ASSERT_EX(hook_action == SyncClientHookAction::NoAction, hook_action);
...
Status Session::receive_mark_message(request_ident_type request_ident)
{
    ...
    if (BARQ_UNLIKELY(!good_request_ident)) {
        return {
            ErrorCodes::SyncProtocolInvariantFailed,
            util::format(
...
Status Session::receive_unbound_message()
{
    ...
    BARQ_ASSERT_EX(m_state == Deactivating || m_suspended, m_state);
...
Status Session::receive_error_message(const ProtocolErrorInfo& info)
{
    ...
    if (status != ErrorCodes::UnknownError && BARQ_UNLIKELY(!is_session_level_error(protocol_error))) {
        return {ErrorCodes::SyncProtocolInvariantFailed,
    ...
    if (status == ErrorCodes::SyncCompensatingWrite) {
        ...
        if (m_state == Active) {
            BARQ_ASSERT(info.compensating_write_server_version.has_value());
...
{
    BARQ_ASSERT(!m_try_again_activation_timer);
...
{
    BARQ_ASSERT_3(m_target_download_mark, >=, m_last_download_mark_received);
    BARQ_ASSERT_3(m_last_download_mark_received, >=, m_last_triggering_download_mark);
    ...
    if (BARQ_UNLIKELY(m_delay_uploads)) {
