...
PendingBootstrapStore::PendingBatch PendingBootstrapStore::peek_pending(Transaction& tr, size_t limit_in_bytes)
{
    ...
    BARQ_ASSERT(bootstrap_table->size() == 1);
    ...
    for (size_t idx = 0; idx < changeset_list.size() && bytes_so_far < limit_in_bytes; ++idx) {
        ...
        if (ec == util::compression::error::decompress_unsupported) {
            BARQ_TERMINATE(
                "Synchronized Barq files with unprocessed pending bootstraps cannot be copied between platforms.");
        }
...
{
    BARQ_ASSERT_3(tr.get_transact_stage(), ==, DB::transact_Writing);
    auto bootstrap_table = tr.get_table(m_table);
    // We should only have one pending bootstrap at a time.
    BARQ_ASSERT(bootstrap_table->size() == 1);
    ...
    auto bootstrap_obj = bootstrap_table->get_object(0);
    auto changeset_list = bootstrap_obj.get_linklist(m_changesets);
    BARQ_ASSERT_3(changeset_list.size(), >=, count);
