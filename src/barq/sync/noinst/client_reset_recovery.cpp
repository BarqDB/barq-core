...
struct InternDictKey {
    bool is_null() const
    {
        return m_pos == barq::npos && m_size == barq::npos;
    }
...
private:
    ...
    size_t m_pos = barq::npos;
    size_t m_size = barq::npos;
};
...
private:
    ...
    BARQ_NORETURN void handle_error(const std::string& message) const;
...
#define BARQ_DECLARE_INSTRUCTION_HANDLER(X) void operator()(const Instruction::X&) override;
    BARQ_FOR_EACH_INSTRUCTION_TYPE(BARQ_DECLARE_INSTRUCTION_HANDLER)
#undef BARQ_DECLARE_INSTRUCTION_HANDLER
                       ...
                       uint32_t& remote_to_out)
{
    ...
    for (auto it = m_indices_allowed.begin(); it != m_indices_allowed.end(); ++it) {
        if (it->local == from) {
            BARQ_ASSERT(target_from == m_indices_allowed.end());
            target_from = it;
        }
        else if (it->local == to) {
            BARQ_ASSERT(target_to == m_indices_allowed.end());
            target_to = it;
        }
    }
    ...
    BARQ_ASSERT_EX(target_from->remote <= lst_size, from, to, target_from->remote, target_to->remote, lst_size);
    BARQ_ASSERT_EX(target_to->remote <= lst_size, from, to, target_from->remote, target_to->remote, lst_size);

    if (from < to) {
        for (auto it = m_indices_allowed.begin(); it != m_indices_allowed.end(); ++it) {
            if (it->local > from && it->local <= to) {
                BARQ_ASSERT(it->local != 0);
                BARQ_ASSERT(it->remote != 0);
                --it->local;
                --it->remote;
            }
        }
        ...
        for (auto it = m_indices_allowed.begin(); it != m_indices_allowed.end(); ++it) {
            if (it->local < from && it->local >= to) {
                BARQ_ASSERT_EX(it->remote + 1 < lst_size, it->remote, lst_size);
                ++it->local;
                ++it->remote;
            }
        }
...
std::string_view InterningBuffer::get_key(const InternDictKey& key) const
{
    ...
    BARQ_ASSERT(key.m_pos < m_dict_keys_buffer.size());
    BARQ_ASSERT(key.m_pos + key.m_size <= m_dict_keys_buffer.size());
...
BARQ_NORETURN void RecoverLocalChangesetsHandler::handle_error(const std::string& message) const
{
    ...
    throw barq::sync::ClientResetFailed(full_message);
}
...
void RecoverLocalChangesetsHandler::copy_lists_with_unrecoverable_changes()
{
    ...
    using namespace barq::converters;
    ...
    for (auto& [path, tracker] : m_lists) {
        ...
        bool did_translate = resolve(path, [&](LstBase& remote_list, LstBase& local_list) {
            ...
            InterBarqValueConverter value_converter(local_table, local_col_key, remote_table, remote_col_key,
                                                 ...
                                                 util::UniqueFunction<void(LstBase&, LstBase&)> callback)
{
    ...
    for (auto it = path.begin(); it != path.end();) {
        ...
        BARQ_ASSERT(it->type != ListPath::Element::Type::ListIndex);
        ...
        if (col.is_list()) {
            ...
            BARQ_ASSERT_EX(!col.is_collection(), col);
            BARQ_ASSERT_EX(col.get_type() == col_type_Link, col);
...
RecoverLocalChangesetsHandler::RecoveryResolver::on_list_index_advance(uint32_t index)
{
    if (m_recovery_applier->m_lists.count(m_list_path) != 0) {
        ...
        BARQ_ASSERT(cross_ndx->remote != uint32_t(-1));
...
{
    BARQ_ASSERT(m_it_begin != m_path_instr.path.begin());
    size_t distance = (m_it_begin - m_path_instr.path.begin()) - 1;
    BARQ_ASSERT_EX(distance < m_path_instr.path.size(), distance, m_path_instr.path.size());
    BARQ_ASSERT(mpark::holds_alternative<uint32_t>(m_path_instr.path[distance]));
    ...
    try {
        ...
        if (m_replication && table) {
            if (table->is_embedded()) {
                ...
                BARQ_ASSERT_EX(pk_col, class_name);
