...
#ifndef BARQ_NOINST_CLIENT_IMPL_BASE_HPP
#define BARQ_NOINST_CLIENT_IMPL_BASE_HPP
...
public:
    ...
    /// of this session, and the associated Barq file will not be accessed,
    ...
    ///
    /// After the initiation of the deactivation process, the associated Barq
    ...
    std::string get_active_barq_connection_id();
...
private:
    ...
    void receive_barq_request_id(std::string_view coid);
    ...
    std::string m_barq_connection_id;
...
/// A synchronization session between a local and a remote Barq file.
///
...
public:
    ...
    /// version of the Barq depends on the history implementation.
    ///
...
private:
    ...
    /// Fetch a reference to the remote virtual path of the Barq associated
    /// with this session.
    ///
    /// This function is always called by the event loop thread of the
    /// associated client object.
    ...
    const std::string& get_barq_path() const noexcept;
    ...
    // Config::ClientReset, the session will be initiated with a state Barq
    ...
    // - Client reset state means the session is going to be used to download a fresh barq.
    ...
    // update the cursor info, such as during a client reset fresh barq download
...
private:
    ...
    //
    // It is set to the current version of the local Barq at session activation
    // time (although always zero for the initial empty Barq
...
{
    BARQ_ASSERT(!was_voluntary(reason));
...
inline void ClientImpl::Connection::change_state_to_disconnected() noexcept
{
    ...
    BARQ_ASSERT(m_state != ConnectionState::disconnected);
...
{
    BARQ_ASSERT(m_num_active_unsuspended_sessions);
    if (--m_num_active_unsuspended_sessions != 0)
        return;
...
{
    BARQ_ASSERT(m_state == Active);

    bool resume_upload = do_recognize_sync_version(version);
    if (BARQ_LIKELY(resume_upload)) {
...
inline void ClientImpl::Session::request_download_completion_notification()
{
    ...
    BARQ_ASSERT(m_error_message_received || !m_unbind_message_sent);
...
{
    if (BARQ_LIKELY(version > m_last_version_available)) {
...
{
    BARQ_ASSERT(m_state == Active || m_state == Deactivating);
    ...
    if (m_state == Deactivating) {
        ...
        BARQ_ASSERT(m_state == Deactivated);
        return;
    }
...
inline void ClientImpl::Session::message_sent()
{
    ...
    BARQ_ASSERT(!m_unbind_message_send_complete);
...
#endif // BARQ_NOINST_CLIENT_IMPL_BASE_HPP
