                    ...
                    bool allow_schema_additions)
{
    ...
    BARQ_ASSERT_RELEASE(client_repl);
    ...
    //    schema changes that the local Barq cannot advance through.
    ...
    for (auto table_key : group_src.get_table_keys()) {
        ...
        if (!table_dst) {
            ...
            if (table_src->is_embedded()) {
                BARQ_ASSERT(!pk_col_src);
                group_dst.add_table(table_name, Table::Type::Embedded);
            }
            else {
                BARQ_ASSERT(pk_col_src); // a sync table will have a pk
                auto pk_col_src = table_src->get_primary_key_column();
    ...
    size_t num_tables;
    {
        ...
        BARQ_ASSERT_EX(allow_schema_additions || num_tables_src == num_tables_dst, num_tables_src, num_tables_dst);
            ...
            if (!col_key_dst) {
                ...
                if (Table::is_link_type(col_key.get_type())) {
                    ...
                    if (col_key.is_list()) {
                        ...
                        BARQ_ASSERT(!col_key.is_collection());
    ...
    for (auto table_key : group_src.get_table_keys()) {
        ...
        BARQ_ASSERT_DEBUG(pk_col); // sync barqs always have a pk
    ...
    for (auto table_key : group_src.get_table_keys()) {
        ...
        StringData table_name = table_src->get_name();
        TableRef table_dst = group_dst.get_table(table_name);
        BARQ_ASSERT_EX(allow_schema_additions || table_src->get_column_count() == table_dst->get_column_count(),
        ...
        converters::InterBarqObjectConverter converter(table_src, table_dst, &embedded_tracker);
                               ...
                               sync::SubscriptionStore* sub_store)
{
    ...
    // transform the local Barq such that all public tables become identical to the remote Barq
    transfer_group(*tr_remote, *wt_local, logger, false);

    // now that the state of the fresh and local Barqs are identical,
    // reset the local sync history and steal the fresh Barq's ident
