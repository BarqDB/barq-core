#ifndef BARQ_SYNC_HISTORY_HPP
#define BARQ_SYNC_HISTORY_HPP
...
struct VersionInfo {
    /// Barq snapshot version.
    version_type barq_version = 0;

    /// The synchronization version corresponding to `barq_version`.
    ///
    /// In the context of the client-side history type `sync_version.version`
    /// will currently always be equal to `barq_version` and
