#ifndef BARQ_SYNC_CLIENT_BASE_HPP
#define BARQ_SYNC_CLIENT_BASE_HPP
...
/// reset operation. If client_reset is util::none or if the local Barq does not
/// exist, an ordinary sync session will take place.
///
/// A session will perform client reset by downloading a fresh copy of the Barq
/// from the server at a different file path location. After download, the fresh
/// Barq will be integrated into the local Barq in a write transaction. The
/// application is free to read or write to the local barq during the entire client
...
/// local barq during client reset but before the client reset operation has
...
///
/// Client reset downloads its fresh Barq copy for a Barq at path "xyx.barq" to
/// "xyz.barq.fresh". It is assumed that this path is available for use and if
...
struct ClientConfig {
    ...
    /// initiated on behalf of an empty (or nonexisting) local Barq
    ...
    /// Disable sync to disk (fsync(), msync()) for all barq files managed
    /// by this client.
    ///
...
inline std::ostream& operator<<(std::ostream& os, ConnectionState state)
{
    ...
    BARQ_TERMINATE("Invalid ConnectionState value");
}
...
enum class SessionReason {
    ...
    Sync = 0,
    // Download a fresh barq
    ClientReset,
};

} // namespace barq::sync

#endif // BARQ_SYNC_CLIENT_BASE_HPP
