...
#include <barq/sync/noinst/changeset_index.hpp>
...
struct MajorSide : Side {
    ...
    {
        BARQ_ASSERT(position >= m_changeset->begin());
        BARQ_ASSERT(position != m_changeset->end());
        m_position = position;
        skip_tombstones();
        BARQ_ASSERT(position != m_changeset->end());
    ...
    {
        BARQ_ASSERT(m_position != m_changeset->end());
        do {
            ++m_position;
        } while (m_position != m_changeset->end() && !*m_position);
    }
...
struct MinorSide : Side {
    ...
    BARQ_NOINLINE void skip_tombstones_slow() noexcept
    {
    ...
    Instruction& get() noexcept final
    {
        ...
        BARQ_ASSERT(instr != nullptr);
        return *instr;
    }

    void init_with_instruction(Position position)
    {
        // BARQ_ASSERT(position >= Position(m_conflict_ranges));
        BARQ_ASSERT(position != end());
        m_position = position;
        update_changeset_pointer();
        skip_tombstones();
        BARQ_ASSERT(position != end());
...
#if defined(BARQ_DEBUG) // LCOV_EXCL_START Debug utilities
...
#endif // LCOV_EXCL_STOP BARQ_DEBUG
...
struct TransformerImpl {
    ...
    _impl::ChangesetIndex::Ranges* get_conflict_ranges_for_instruction(const Instruction& instr)
    {
        ...
        if (_impl::is_schema_change(instr)) {
            ...
            BARQ_ASSERT(num_major_ids <= 2);
            BARQ_ASSERT(num_major_ids >= 1);
#if BARQ_DEBUG // LCOV_EXCL_START
            if (m_trace) {
            ...
            if (num_major_ids == 2) {
                ...
                // two object IDs.
                BARQ_ASSERT(ranges == index.get_modifications_for_object(major_ids[1]));
            }
            return ranges;
        }
    }
    ...
    {
        BARQ_ASSERT(*m_major_side.m_position); // cannot prepend a tombstone
    ...
    {
        BARQ_ASSERT(*m_minor_side.m_position); // cannot prepend a tombstone
        ...
        BARQ_ASSERT(m_minor_end == m_minor_side.end());
    }
    ...
    void transform_prepended_major(size_t num_prepended)
    {
        ...
        BARQ_ASSERT(m_major_side.m_position != m_major_side.m_changeset->end());

#if defined(BARQ_DEBUG) // LCOV_EXCL_START
        if (m_trace) {
        ...
        for (size_t i = 0; i < num_prepended; ++i) {
            ...
            BARQ_ASSERT(m_major_side.m_position != m_major_side.m_changeset->end());
            m_major_side.init_with_instruction(m_major_side.m_position);
            BARQ_ASSERT(!m_major_side.was_discarded);
            BARQ_ASSERT(m_major_side.m_position != m_major_side.m_changeset->end());
            transform_major();
            if (!m_major_side.was_discarded) {
                // Discarding an instruction moves to the next.
...
#if defined(BARQ_DEBUG) // LCOV_EXCL_START Debug tracing
...
BARQ_NORETURN void throw_bad_merge(std::string msg)
{
    throw sync::TransformError{std::move(msg)};
}

template <class... Params>
BARQ_NORETURN void bad_merge(const char* msg, Params&&... params)
{
    throw_bad_merge(util::format(msg, std::forward<Params>(params)...));
}

BARQ_NORETURN void bad_merge(Side& side, Instruction::PathInstruction instr, const std::string& msg)
{
    std::stringstream ss;
...
struct MergeUtils {
    ...
    {
        BARQ_ASSERT(left.path.size() != 0);
        BARQ_ASSERT(left.path.size() < right.path.size());
        BARQ_ASSERT(mpark::holds_alternative<uint32_t>(left.path.back()));
...
///  GET READY!
///
///  Barq supports 14 instructions at the time of this writing. Each
...
DEFINE_MERGE(Instruction::ArrayInsert, Instruction::Update)
{
    if (same_container(left, right)) {
        BARQ_ASSERT(right.is_array_update());
...
DEFINE_MERGE(Instruction::Clear, Instruction::Update)
{
    ...
    // The two instructions are at the same level of nesting.
    if (same_path(left, right)) {
        BARQ_ASSERT(right.value.type != Type::Set);
...
void TransformerImpl::merge_instructions(MajorSide& their_side, MinorSide& our_side)
{
    ...
    if (their_side.get().get_if<Instruction::Update>()) {
        BARQ_ASSERT(their_side.m_path_len > 2);
    }
    if (our_side.get().get_if<Instruction::Update>()) {
        BARQ_ASSERT(our_side.m_path_len > 2);
    }
...
{
    BARQ_ASSERT(our_changesets.size() != 0);
    BARQ_ASSERT(their_changesets.size() != 0);
    bool trace = false;
#if BARQ_DEBUG && !BARQ_UWP
...
size_t Transformer::transform_remote_changesets(TransformHistory& history, file_ident_type local_file_ident,
                                                ...
                                                util::Logger& logger)
{
    BARQ_ASSERT(local_file_ident != 0);
...
void parse_remote_changeset(const RemoteChangeset& remote_changeset, Changeset& parsed_changeset)
{
    ...
    // origin.
    BARQ_ASSERT(remote_changeset.origin_file_ident != 0);
    BARQ_ASSERT(remote_changeset.remote_version != 0);
