...
void ChangesetEncoder::operator()(const Instruction::AddColumn& instr)
{
    ...
    BARQ_ASSERT(instr.type != Instruction::Payload::Type::Null || instr.nullable || is_dictionary);
...
InternString ChangesetEncoder::intern_string(StringData str)
{
    ...
    if (it == m_intern_strings_rev.end()) {
        ...
        BARQ_ASSERT_RELEASE_EX(index <= std::numeric_limits<uint32_t>::max(), index);
        bool inserted;
        std::tie(it, inserted) = m_intern_strings_rev.insert({std::string{str}, uint32_t(index)});
        BARQ_ASSERT_RELEASE_EX(inserted, str);
...
StringBufferRange ChangesetEncoder::add_string_range(StringData data)
{
    ...
    BARQ_ASSERT(data.size() <= std::numeric_limits<uint32_t>::max());
...
{
    BARQ_ASSERT(str.offset + str.size <= m_string_range.size());
...
{
    BARQ_ASSERT(str != InternString::npos);
