...
public:
    HistoryCursorPtr create_history_cursor(util::Optional<file_ident_type> reciprocal) override
    {
        ...
        if (BARQ_UNLIKELY(!cursor))
            return nullptr;
        if (reciprocal) {
            if (BARQ_UNLIKELY(!cursor->reciprocal(*reciprocal))) // Throws
                return nullptr;
        }
        cursor->init(); // Throws
        return cursor;
    ...
    HistoryCursorPtr create_history_cursor(util::Optional<file_ident_type> reciprocal, version_type version) override
    {
        ...
        if (BARQ_UNLIKELY(!cursor->init(version))) // Throws
            return nullptr;
        return cursor;
    }
                                           ...
                                           version_type end_version) override
    {
        ...
        if (BARQ_UNLIKELY(!cursor->init(begin_version, end_version))) // Throws
            return nullptr;
        return cursor;
    }
...
public:
    ServerHistoryCursor_6_to_10(Allocator& alloc, ref_type root_ref, int schema_version)
        ...
        , m_root{alloc}
    {
        BARQ_ASSERT(schema_version >= 6 && schema_version <= 10);
        ...
        BARQ_ASSERT(m_origin_files->size() == history_size);
        BARQ_ASSERT(m_client_versions->size() == history_size);
        BARQ_ASSERT(m_timestamps->size() == history_size);
        BARQ_ASSERT(m_changesets->size() == history_size);
...
public:
        ...
        : m_root{alloc}
    {
        ...
        BARQ_ASSERT(m_client_versions->size() == m_size);
        BARQ_ASSERT(m_rh_base_versions->size() == m_size);
        BARQ_ASSERT(m_proxy_files->size() == m_size);
        BARQ_ASSERT(!m_client_types || m_client_types->size() == m_size);
        BARQ_ASSERT(m_last_seen_timestamps->size() == m_size);
        BARQ_ASSERT(!m_locked_server_versions || m_locked_server_versions->size() == m_size);
        ...
        // Find the client file entry that corresponds to ourselves
        {
            ...
            BARQ_ASSERT_RELEASE(upstream_status_ref == 0 || partial_sync_ref == 0);
    ...
    LogicalClientType get_logical_client_type() const override final
    {
        ...
        if (BARQ_UNLIKELY(index == 0))
            return LogicalClientType::special;
        if (BARQ_UNLIKELY(index == 1)) {
...
private:
                                ...
                                file_ident_type proxy_file) const
    {
        if (client_file_ident == 0) {
            BARQ_ASSERT(client_type == ClientType(0));
                     ...
                     Format format, Summary summary, bool with_versions, std::ostream& out)
{
    ...
    while (cursor.next()) {
        ...
        if (BARQ_UNLIKELY(origin_file)) {
            if (BARQ_LIKELY(cursor.get_origin_file() != *origin_file))
                continue;
        }
        if (BARQ_UNLIKELY(expression)) {
            buffer.clear();
            ...
            for (auto instr : changeset) {
                ...
                if (BARQ_LIKELY(!did_match))
                    continue;
                instr_was_found = true;
                break;
            }
            if (BARQ_LIKELY(!instr_was_found))
                continue;
        }
        switch (format) {
            case Format::auto_: {
        ...
        ++num_history_entries;
        if (BARQ_UNLIKELY(version < min_version))
            min_version = version;
        if (BARQ_UNLIKELY(version > max_version))
            max_version = version;
        if (summary == Summary::full) {
            timestamp_type timestamp = cursor.get_origin_timestamp(); // Throws
            if (BARQ_UNLIKELY(timestamp < min_timestamp))
                min_timestamp = timestamp;
            if (BARQ_UNLIKELY(timestamp > max_timestamp))
                          ...
                          std::time_t max_last_seen_timestamp, version_type max_locked_version)
{
    ...
    while (cursor.next()) {
        ...
        ++num_client_files;
        if (is_unexpired) {
            if (BARQ_UNLIKELY(last_seen_timestamp < min_timestamp))
                min_timestamp = last_seen_timestamp;
            if (BARQ_UNLIKELY(last_seen_timestamp > max_timestamp))
...
int main(int argc, char* argv[])
{
    ...
    // Process command-line
    {
        ...
        if (help) {
            // clang-format off
            std::cerr <<
                "Synopsis: "<<prog<<" <barq file>\n"
                "          "<<prog<<" <barq file> <version>\n"
                "          "<<prog<<" <barq file> <begin version> <end version>\n"
                "          "<<prog<<" <barq file> (-c | --client-files)\n"
                "          "<<prog<<" <barq file> (-c | --client-files) <file ident>\n"
                "\n"
                "The first three forms are for inspecting a specific range of the\n"
                "synchronization history of the specified Barq file. In the first form, the\n"
                ...
                "                       `<barq/sync/instructions.hpp>` for the list of\n"
                ...
                "                       Access the Barq file using an encryption key. The\n"
        ...
        if (version) {
            ...
            build_mode = "Release";
#endif
            std::cerr << "BarqSync/" BARQ_VERSION_STRING " (build_mode=" << build_mode << ")\n";
            return EXIT_SUCCESS;
        }
    ...
    Group group{barq_path, encryption_key_3}; // Throws
