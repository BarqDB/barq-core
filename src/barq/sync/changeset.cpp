...
InternString Changeset::intern_string(StringData str)
{
    ...
    BARQ_ASSERT(m_string_buffer.size() < std::numeric_limits<uint32_t>::max());
    BARQ_ASSERT(m_strings.size() < std::numeric_limits<uint32_t>::max());
    BARQ_ASSERT(str.size() < std::numeric_limits<uint32_t>::max());
...
std::ostream& barq::sync::operator<<(std::ostream& os, const Changeset& changeset)
{
...
void Changeset::verify() const
{
    for (size_t i = 0; i < m_strings.size(); ++i) {
        ...
        BARQ_ASSERT(range.offset <= m_string_buffer.size());
        BARQ_ASSERT(range.offset + range.size <= m_string_buffer.size());
    }
    ...
    for (auto instr : *this) {
        ...
        if (auto table_instr = instr->get_if<Instruction::TableInstruction>()) {
            ...
            if (auto object_instr = instr->get_if<Instruction::ObjectInstruction>()) {
                ...
                mpark::visit(util::overload{
                                 [&](const Instruction::AddTable::TopLevelTable& spec) {
                                     BARQ_ASSERT(is_valid_key_type(spec.pk_type));
            ...
            BARQ_TERMINATE("Corrupt instruction type");
        }
    }
}
