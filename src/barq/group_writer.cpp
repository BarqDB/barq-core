...
public:
    ...
    ref_type write_array(const char* data, size_t size, uint32_t checksum) override
    {
        ...
        BARQ_ASSERT_RELEASE(dest_addr && (reinterpret_cast<size_t>(dest_addr) & 7) == 0);
...
size_t WriteWindowMgr::MapWindow::get_window_size(util::File& f, ref_type start_ref, size_t size)
{
    ...
    BARQ_ASSERT_DEBUG_EX(start_ref + size <= file_size, start_ref + size, file_size);
...
{
    barq::util::encryption_write_barrier(start_addr, size, m_map.get_encrypted_mapping());
}
...
#if BARQ_PLATFORM_APPLE && BARQ_MOBILE
    ...
    , m_free_versions(m_alloc)
{
    ...
    if (ref_type ref = m_free_lengths.get_ref_from_parent()) {
        ...
        BARQ_ASSERT_RELEASE_EX(m_free_positions.size() == m_free_lengths.size(), top.get_ref(),
    ...
    if (ref_type ref = m_free_versions.get_ref_from_parent()) {
        ...
        BARQ_ASSERT_RELEASE_EX(m_free_versions.size() == m_free_lengths.size(), top.get_ref(),
...
void GroupWriter::backdate()
{
    ...
    auto get_earlier = [&](uint64_t version) -> FreeList* {
        ...
        BARQ_ASSERT(it != old_freelists.end());
        BARQ_ASSERT(it != old_freelists.begin());
        --it;
        BARQ_ASSERT((*it)->version < version);
        return it->get();
    };
    ...
    auto find_cover_for = [&](const FreeSpaceEntry& entry, FreeList& free_list) -> std::optional<uint64_t> {
        ...
        BARQ_ASSERT(index < limit); // follows from above
        const auto start_pos = static_cast<ref_type>(free_list.positions.get(index));
        BARQ_ASSERT(start_pos <= entry.ref);
        ...
        BARQ_ASSERT(found_version <= entry.released_at_version);
...
ref_type GroupWriter::write_group()
{
    ...
    BARQ_ASSERT_3(reserve_size, >, max_free_space_needed);
    int_fast64_t value_4 = to_int64(reserve_pos + max_free_space_needed);

#if BARQ_ENABLE_MEMDEBUG
    ...
    BARQ_ASSERT(Array::get_wtype_from_header(Array::get_header_from_data(m_free_versions.m_data)) ==
                 Array::wtype_Bits);
    ...
    if (m_logical_size >= minimal_compaction_size && m_evacuation_limit == 0 && m_backoff == 0) {
        ...
        BARQ_ASSERT(m_logical_size > free_space);
    ...
    BARQ_ASSERT_3(size_t(end_ref), <=, reserve_pos + max_free_space_needed);
    ...
    // expansion
    BARQ_ASSERT_3(value_8, <=, Array::ubound_for_width(m_free_positions.get_width()));
    BARQ_ASSERT_3(value_9, <=, Array::ubound_for_width(m_free_lengths.get_width()));
    ...
    if (m_alloc.is_in_memory()) {
        ...
        BARQ_ASSERT_EX(free_sizes_ref >= reserve_ref && free_sizes_ref + free_sizes_size <= reserve_ref + used,
...
void GroupWriter::read_in_freelist()
{
    ...
    BARQ_ASSERT(m_free_lengths.is_attached());
    size_t limit = m_free_lengths.size();
    BARQ_ASSERT_RELEASE_EX(m_free_positions.size() == limit, limit, m_free_positions.size());
    BARQ_ASSERT_RELEASE_EX(m_free_versions.size() == limit, limit, m_free_versions.size());
...
size_t GroupWriter::recreate_freelist(size_t reserve_pos)
{
    ...
    BARQ_ASSERT(free_in_file.size() == nb_elements);
...

    {
        ...
        for (size_t i = 0; i < limit; ++i) {
            ...
            if (BARQ_UNLIKELY(prev_ref + prev_size > ref)) {
                ...
                for (const auto& elem : new_free_space) {
                    ...
                    for (const auto& locked : m_not_free_in_file) {
                        BARQ_ASSERT_RELEASE_EX(free_ref < locked.ref || free_ref >= (locked.ref + locked.size),
                                                locked.ref, locked.size, locked.released_at_version, free_ref,
                                                m_current_version, m_alloc.get_file_path_for_assertions());
                        BARQ_ASSERT_RELEASE_EX(locked.ref < free_ref || locked.ref >= (free_ref + free_sz),
                                                locked.ref, locked.released_at_version, free_ref, free_sz,
                                                m_current_version, m_alloc.get_file_path_for_assertions());
                    }
                }

                BARQ_ASSERT_RELEASE_EX(prev_ref + prev_size <= ref, prev_ref, prev_size, ref, i, limit,
        ...
        BARQ_ASSERT_RELEASE(reserve_ndx != barq::npos);
                                                ...
                                                std::multimap<size_t, size_t>& size_map)
{
    ...
    for (auto& elem : list) {
        ...
        if (elem.size) {
            BARQ_ASSERT_RELEASE_EX(!(elem.size & 7), elem.size);
            BARQ_ASSERT_RELEASE_EX(!(elem.ref & 7), elem.ref);
...
{
    BARQ_ASSERT_3(size % 8, ==, 0); // 8-byte alignment
    ...
    BARQ_ASSERT_3(chunk_size, >=, size);
    BARQ_ASSERT_RELEASE_EX(!(chunk_pos & 7), chunk_pos);
    BARQ_ASSERT_RELEASE_EX(!(chunk_size & 7), chunk_size);
...
inline GroupWriter::FreeListElement GroupWriter::split_freelist_chunk(FreeListElement it, size_t alloc_pos)
{
    ...
    m_size_map.erase(it);
    BARQ_ASSERT_RELEASE_EX(alloc_pos > start_pos, alloc_pos, start_pos);

    BARQ_ASSERT_RELEASE_EX(!(alloc_pos & 7), alloc_pos);
...
GroupWriter::FreeListElement GroupWriter::extend_free_space(size_t requested_size)
{
    ...
    BARQ_ASSERT_RELEASE_EX(!(new_file_size & 7), new_file_size);
    BARQ_ASSERT_3(logical_file_size, <, new_file_size);
    ...
    BARQ_ASSERT(new_file_size <= get_file_size());
    ...
    BARQ_ASSERT_RELEASE_EX(!(chunk_size & 7), chunk_size);
    BARQ_ASSERT_RELEASE(chunk_size != 0);
...
ref_type GroupWriter::write_array(const char* data, size_t size, uint32_t checksum)
{
    ...
    BARQ_ASSERT_RELEASE(is_aligned(dest_addr));
...
void GroupWriter::write_array_at(T* translator, ref_type ref, const char* data, size_t size)
{
    ...
    BARQ_ASSERT_3(pos + size, <=, to_size_t(m_group.m_top.get(2) / 2));
    // BARQ_ASSERT_3(pos + size, <=, m_file_map.get_size());
...
void GroupCommitter::commit(ref_type new_top_ref)
{
    ...
    BARQ_ASSERT(!util::int_cast_has_overflow<type_1>(file_format_version));
