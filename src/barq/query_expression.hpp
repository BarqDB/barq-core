...
#ifndef BARQ_QUERY_EXPRESSION_HPP
#define BARQ_QUERY_EXPRESSION_HPP

#include <barq/aggregate_ops.hpp>
#include <barq/array_timestamp.hpp>
#include <barq/array_binary.hpp>
#include <barq/array_string.hpp>
#include <barq/array_backlink.hpp>
#include <barq/array_list.hpp>
#include <barq/array_key.hpp>
#include <barq/array_bool.hpp>
#include <barq/array_fixed_bytes.hpp>
#include <barq/column_integer.hpp>
#include <barq/column_type_traits.hpp>
#include <barq/dictionary.hpp>
#if BARQ_ENABLE_GEOSPATIAL
#include <barq/geospatial.hpp>
#endif
...
#define BARQ_OLDQUERY_FALLBACK 1
...
public:
    ...
    BARQ_FORCEINLINE void fun_const(const ValueType& const_value, const ValueBase& right)
    {
        TOperator o;
    ...
    template <class TOperator>
    BARQ_FORCEINLINE void fun_const(const ValueBase& left, const ValueType& const_value)
    {
        TOperator o;
        // Operate on values one-by-one
        size_t sz = left.size();
    ...
    template <class TOperator>
    BARQ_FORCEINLINE void fun(const ValueBase& left, const ValueBase& right)
    {
        TOperator o;
        ...
        if (!left.m_from_list && !right.m_from_list) {
            ...
            BARQ_ASSERT_DEBUG(right.size() > 0);
            init(true, left.size());
    ...
    BARQ_FORCEINLINE static size_t compare(ValueBase& left, ValueBase& right,
                                            ...
                                            util::Optional<ExpressionComparisonType> right_cmp_type)
    {
        ...
        if (!left.m_from_list && !right.m_from_list) {
            ...
            BARQ_ASSERT_DEBUG(!left_cmp_type || *left_cmp_type == Compare::Any);
            BARQ_ASSERT_DEBUG(!right_cmp_type || *right_cmp_type == Compare::Any);
        ...
        if (left_size > 2 && right_size > 2) {
            ...
            else if constexpr (barq::is_any_v<TCond, Greater, GreaterEqual, Less, LessEqual>) {
        ...
        if constexpr (barq::is_any_v<TCond, BeginsWith, BeginsWithIns, EndsWith, EndsWithIns, Contains, ContainsIns,
                                      Like, LikeIns>) {
...
Query create(L left, const Subexpr2<R>& right)
{
    ...
    constexpr const bool supported_by_old_query_engine =
        ...
        (std::is_same_v<L, R> &&
         barq::is_any_v<L, double, float, Timestamp, StringData, BinaryData, ObjectId, UUID>);

    if constexpr (BARQ_OLDQUERY_FALLBACK && supported_by_old_query_engine) {
...
{
#ifdef BARQ_OLDQUERY_FALLBACK // if not defined, never fallback query_engine; always use query_expression
...
struct TrueExpression : Expression {
    ...
    {
        BARQ_ASSERT(start <= end);
        if (start != end)
            return start;

        return barq::not_found;
    }
...
public:
    ...
    {
        BARQ_ASSERT_DEBUG(!m_comparison_type || m_from_list);
...
public:
    ...
    {
        BARQ_ASSERT(m_only_unary_links);
        ObjKey result;
...
public:
    ...
    std::vector<ObjKey> find_all(Mixed value) const final
    {
        ...
        if (m_link_map.get_target_table()->get_primary_key_column() == m_column_key) {
            ...
            BARQ_ASSERT(index);
            if (value.is_null()) {
                index->find_all(result, barq::null{});
            }
            else {
                T val = value.get<T>();
...
// If we add a new Barq type T and quickly want Query support for it, then simply inherit from it like
    ...
    void evaluate(Subexpr::Index& index, ValueBase& destination) override
    {
        if (links_exist()) {
            ...
            if (m_link_map.only_unary_links()) {
                BARQ_ASSERT(destination.size() == 1);
                BARQ_ASSERT(!destination.m_from_list);
                ...
                if (link_translation_key) {
                    ...
                    if constexpr (barq::is_any_v<T, ObjectId, UUID>) {
...
class Columns<BinaryData> : public SimpleQuerySupport<BinaryData> {
    ...
    friend Query operator==(const Columns<BinaryData>& left, barq::null)
    {
        return create<Equal>(BinaryData(), left);
    }

    friend Query operator==(barq::null, const Columns<BinaryData>& right)
    {
...
inline std::enable_if_t<!barq::is_any_v<T, StringData, barq::null, const char*, std::string>, Query>
...
public:
    ...
    SubColumns<C> column(ColKey column_key) const
    {
        ...
        // having
        BARQ_ASSERT_DEBUG(!m_comparison_type);
    ...
    void set_cluster(const Cluster* cluster) override
    {
        BARQ_ASSERT(m_link_map.has_links());
        m_link_map.set_cluster(cluster);
    }
...
public:
    ...
    void evaluate(Subexpr::Index& index, ValueBase& destination) override
    {
        if constexpr (barq::is_any_v<T, ObjectId, Int, Bool, UUID>) {
    ...
    {
        if constexpr (barq::is_any_v<T, Int, Float, Double, Decimal128, Mixed>) {
            return max().clone();
        }
        else {
            return {};
    ...
    {
        if constexpr (barq::is_any_v<T, StringData, BinaryData, Mixed>) {
...
public:
    ...
    {
        BARQ_ASSERT(m_key_type == type_String);
    }
...
public:
    ...
    void evaluate(Subexpr::Index& index, ValueBase& destination) override
    {
        if (m_dictionary_key_type) {
            ...
            BARQ_ASSERT_DEBUG(sz > 0 || list_refs.m_from_list);
...
Query compare(const Subexpr2<Link>& left, const Obj& obj)
{
    ...
    if (column) {
        ...
        BARQ_ASSERT(link_map.get_target_table()->get_key() == obj.get_table()->get_key());
#ifdef BARQ_OLDQUERY_FALLBACK
...
class Columns : public ObjPropertyExpr<T> {
    constexpr static bool requires_null_column = barq::is_any_v<T, int64_t, bool>;
...
public:
    ...
    void evaluate_internal(size_t index, ValueBase& destination)
    {
        ...
        if (links_exist()) {
            BARQ_ASSERT(mpark::holds_alternative<mpark::monostate>(m_leaf));
            if (m_link_map.only_unary_links()) {
            ...
            if constexpr (std::is_same_v<U, int64_t>) {
                ...
                BARQ_ASSERT_3(ValueBase::chunk_size, ==, 8);
...
public:
    ...
    std::unique_ptr<Subexpr> max_of() override
    {
        if constexpr (barq::is_any_v<T, Int, Float, Double, Decimal128, Timestamp, Mixed>) {
            return max().clone();
        }
        else {
            return {};
...
public:
    SubQueryCount(const Query& q, const LinkMap& link_map)
        ...
        , m_link_map(link_map)
    {
        BARQ_ASSERT(q.produces_results_in_table_order());
        BARQ_ASSERT(m_query.get_table() == m_link_map.get_target_table());
    }
    ...
    std::string description(util::serializer::SerialisationState& state) const override
    {
        BARQ_ASSERT(m_link_map.get_base_table() != nullptr);
...
public:
    ...
    {
        BARQ_ASSERT(m_link_map.get_target_table() == m_query.get_table());
    }
...
public:
    ...
    ConstTableRef get_base_table() const override
    {
        ...
        BARQ_ASSERT(l == nullptr || r == nullptr || l == r);
...
public:
        ...
        , m_right(std::move(right))
    {
        ...
        BARQ_ASSERT(!(m_left_const_values && m_right_const_values));
    }
    ...
    size_t find_first_with_matches(size_t start, size_t end) const
    {
        ...
        BARQ_ASSERT(uint64_t(actual_key.value) >= m_cluster->get_offset());
...
#endif // BARQ_QUERY_EXPRESSION_HPP
