...
#ifndef BARQ_KEYS_HPP
#define BARQ_KEYS_HPP
...
template <>
struct hash<barq::ObjKey> {
    size_t operator()(barq::ObjKey key) const
    {
        return std::hash<uint64_t>{}(key.value);
    }
};
template <>
struct hash<barq::ColKey> {
    size_t operator()(barq::ColKey key) const
    {
        return std::hash<int64_t>{}(key.value);
    }
};
template <>
struct hash<barq::TableKey> {
    size_t operator()(barq::TableKey key) const
    {
