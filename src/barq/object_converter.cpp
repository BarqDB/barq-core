...
#include <barq/object_converter.hpp>

#include <barq/dictionary.hpp>
#include <barq/list.hpp>
#include <barq/set.hpp>

#include <barq/util/flat_map.hpp>

namespace barq::converters {

// Takes two lists, src and dst, and makes dst equal src. src is unchanged.
void InterBarqValueConverter::copy_list(const LstBase& src, LstBase& dst, bool* update_out) const
{
    ...
    for (size_t i = 0; i < len_min; i++) {
        InterBarqValueConverter::ConversionResult converted_src;
        ...
        if (cmp_src_to_dst(src_value, dst.get_any(ndx), &converted_src, update_out)) {
            if (converted_src.requires_new_embedded_object) {
                BARQ_ASSERT(dst_as_link_list); // this is the only type of list that supports embedded objects
                ...
                // which has been deleted in the dest Barq. Lists do not support
...
void InterBarqValueConverter::copy_set(const SetBase& src, SetBase& dst, bool* update_out) const
{
    ...
    for (auto ndx : to_insert) {
        ...
        BARQ_ASSERT(!converted_src.requires_new_embedded_object);
...
void InterBarqValueConverter::copy_dictionary(const Dictionary& src, Dictionary& dst, bool* update_out) const
{
    ...
    for (auto ndx : to_insert) {
        ...
        InterBarqValueConverter::ConversionResult converted_val;
...
void InterBarqValueConverter::copy_value(const Obj& src_obj, Obj& dst_obj, bool* update_out)
{
    if (m_src_col.is_list()) {
        ...
        BARQ_ASSERT(!m_src_col.is_collection());
        // nested collections
...
void InterBarqValueConverter::copy_list(const LstBase& src_list, LstBase& dst_list) const
{
    auto src_as_lst_mixed = dynamic_cast<const Lst<Mixed>*>(&src_list);
    auto dst_as_lst_mixed = dynamic_cast<Lst<Mixed>*>(&dst_list);
    BARQ_ASSERT(bool(src_as_lst_mixed) == bool(dst_as_lst_mixed));
    if (src_as_lst_mixed) {
...
//
void InterBarqValueConverter::handle_list_in_mixed(const Lst<Mixed>& src_list, Lst<Mixed>& dst_list) const
{
...
void InterBarqValueConverter::handle_dictionary_in_mixed(Dictionary& src_dictionary,
...
bool InterBarqValueConverter::check_if_list_elements_match(const Lst<Mixed>& src_list, Lst<Mixed>& dst_list,
                                                            size_t ndx_src, size_t ndx_dst) const
{
    BARQ_ASSERT(ndx_src < src_list.size() && ndx_dst < dst_list.size());
...
bool InterBarqValueConverter::check_if_dictionary_elements_match(const Dictionary& src_dictionary,
                                                                  const Dictionary& dst_dictionary,
                                                                  StringData key) const
{
    BARQ_ASSERT(src_dictionary.contains(key) && dst_dictionary.contains(key));
...
void InterBarqValueConverter::copy_list_in_mixed(const Lst<Mixed>& src_list, Lst<Mixed>& dst_list, size_t ndx_src,
...
void InterBarqValueConverter::copy_dictionary_in_mixed(const Dictionary& src_dictionary, Dictionary& dst_dictionary,
...
bool InterBarqValueConverter::is_collection(Mixed mixed) const
{
    BARQ_ASSERT_DEBUG(!mixed.is_type(type_Set));
    return mixed.is_type(type_List, type_Dictionary);
}

CollectionType InterBarqValueConverter::to_collection_type(Mixed mixed) const
{
...
{
    util::FlatMap<TableKey, InterBarqObjectConverter> converters;
    ...
    while (!embedded_pending.empty()) {
        ...
        if (it == converters.end()) {
            ...
            it = converters.insert({dst_table_key, InterBarqObjectConverter{src_table, dst_table, this}}).first;
        }
        InterBarqObjectConverter& converter = it->second;
        converter.copy(pending.embedded_in_src, pending.embedded_in_dst, nullptr);
    }
}

InterBarqValueConverter::InterBarqValueConverter(ConstTableRef src_table, ColKey src_col, ConstTableRef dst_table,
...
void InterBarqValueConverter::track_new_embedded(const Obj& src, const Obj& dst) const
{
    m_embedded_converter->track(src, dst);
}

// convert `src` to the destination Barq and compare that value with `dst`
// If `converted_src_out` is provided, it will be set to the converted src value
int InterBarqValueConverter::cmp_src_to_dst(Mixed src, Mixed dst, ConversionResult* converted_src_out,
                                             bool* did_update_out) const
{
    ...
    if (m_primitive_types_only || !src.is_type(type_Link, type_TypedLink)) {
        ...
        if (m_is_embedded_link) {
            ...
            BARQ_ASSERT_DEBUG(src_embedded.is_valid());
            if (dst.is_type(type_Link, type_TypedLink)) {
                ...
                BARQ_ASSERT_DEBUG(dst_embedded.is_valid());
            ...
            if (m_opposite_of_dst == m_opposite_of_src) {
                // if this is the same Barq, we can use the ObjKey
                dst_link = m_opposite_of_dst->get_object(src_link_key);
            }
            else {
                // in different Barqs we create a new object
        ...
        if (src_link.is_unresolved()) {
            ...
            BARQ_ASSERT_EX(src_link_table, src_link.get_table_key());
            TableRef dst_link_table = m_dst_table->get_parent_group()->get_table(src_link_table->get_name());
            BARQ_ASSERT_EX(dst_link_table, src_link_table->get_name());
            // embedded tables should always be covered by the m_opposite_of_src case above.
            BARQ_ASSERT_EX(!src_link_table->is_embedded(), src_link_table->get_name());
            ...
            if (src_link_table->get_primary_key_column()) {
                ...
                // no pk, but this is the same Barq, so convert by ObjKey
                Obj dst_link = dst_link_table->get_object(src_link.get_obj_key());
                converted_src = ObjLink{dst_link_table->get_key(), dst_link.get_key()};
            }
            else {
                // no pk, and different Barq, create an object
...
InterBarqObjectConverter::InterBarqObjectConverter(ConstTableRef table_src, TableRef table_dst,
    ...
    : m_embedded_tracker(embedded_tracker)
{
    ...
    for (ColKey col_key_src : table_src->get_column_keys()) {
        ...
        BARQ_ASSERT(col_key_dst);
        m_columns_cache.emplace_back(table_src, col_key_src, table_dst, col_key_dst, m_embedded_tracker);
    }
}

void InterBarqObjectConverter::copy(const Obj& src, Obj& dst, bool* update_out)
{
