...
#ifndef BARQ_IMPL_TRANSACT_LOG_HPP
#define BARQ_IMPL_TRANSACT_LOG_HPP
...
private:
    ...
    BARQ_COLD BARQ_NORETURN void parser_error() const;
...
inline void TransactLogBufferStream::transact_log_reserve(size_t size, char** inout_new_begin, char** out_new_end)
{
    ...
    BARQ_ASSERT(*inout_new_begin >= data);
    BARQ_ASSERT(*inout_new_begin <= (data + m_buffer.size()));
                                                         ...
                                                         char** out_new_end)
{
    ...
    *out_new_begin = barq::safe_copy_n(data, size, *out_new_begin);
}
...
{
    BARQ_ASSERT(free_begin <= free_end);
...
{
    BARQ_ASSERT_DEBUG(m_transact_log_free_begin <= ptr);
    BARQ_ASSERT_DEBUG(ptr <= m_transact_log_free_end);
...
char* TransactLogEncoder::encode_int(char* ptr, T value)
{
    ...
    if (negative) {
        ...
        BARQ_DIAG_PUSH();
        BARQ_DIAG_IGNORE_UNSIGNED_MINUS();
        value = -(value + 1);
        BARQ_DIAG_POP();
    }
...
void TransactLogParser::parse_one(InstructionHandler& handler)
{
    ...
    switch (instr) {
        ...
        case instr_DictionaryInsert: {
            ...
            BARQ_ASSERT(type == int(type_String));
...
#endif // BARQ_IMPL_TRANSACT_LOG_HPP
