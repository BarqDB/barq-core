...
#ifndef BARQ_COPY_REPLICATION_HPP
#define BARQ_COPY_REPLICATION_HPP
...
private:
    Table* get_table_in_destination_barq()
    {
    ...
    ColKey get_colkey_in_destination_barq(ColKey c)
    {
        auto col_name = m_current.table->get_column_name(c);
        return get_table_in_destination_barq()->get_column_key(col_name);
    }
    ...
    TransactionRef m_tr;
    struct State {
        // Table and object in source barq
        const Table* table = nullptr;
        ObjKey obj_key;
        // Corresponding object in destination barq
...
#endif /* BARQ_COPY_REPLICATION_HPP */
