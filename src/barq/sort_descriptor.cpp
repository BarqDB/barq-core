...
{
    BARQ_ASSERT_EX(m_ascending.empty() || m_ascending.size() == m_column_keys.size(), m_ascending.size(),
                    m_column_keys.size());
...
{
    BARQ_ASSERT(!column_lists.empty());
    BARQ_ASSERT_EX(column_lists.size() == ascending.size(), column_lists.size(), ascending.size());
    ...
    for (size_t i = 0; i < column_lists.size(); ++i) {
        ...
        auto sz = columns.size();
        BARQ_ASSERT_EX(!columns.empty(), i);
...
{
    BARQ_ASSERT(!m_column_keys.empty());
...
{
    BARQ_ASSERT(index < m_descriptors.size());
...
barq::util::Optional<size_t> DescriptorOrdering::get_min_limit() const
{
    barq::util::Optional<size_t> min_limit;
