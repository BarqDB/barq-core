...
#ifndef BARQ_GLOBAL_KEY_HPP
#define BARQ_GLOBAL_KEY_HPP
...
struct GlobalKey {
    ...
    constexpr GlobalKey(barq::util::None = barq::util::none)
        : m_lo(-1)
        , m_hi(-1)
    {
    }
    ...
    // in this barq (sync_file_id == hi) then 0 is used for hi. In this
...
template <>
struct hash<barq::GlobalKey> {
    size_t operator()(barq::GlobalKey oid) const
    {
