...
{
    return size() ? m_values->find_first(value) : barq::not_found;
}
...
{
    return find_any_key(key) != barq::npos;
}
...
{
    BARQ_ASSERT(m_dictionary_top);
    BARQ_ASSERT(ndx != barq::npos);
...
{
    return update() ? m_values->find_first(value) : barq::not_found;
}
...
{
    size_t ndx = update() ? m_values->find_first(value) : barq::npos;
    return (ndx == barq::npos) ? Mixed{} : do_get_key(ndx);
}
...
void Dictionary::verify() const
{
    m_keys->verify();
    m_values->verify();
    BARQ_ASSERT(m_keys->size() == m_values->size());
}
...
{
    BARQ_ASSERT_EX(col_key.get_type() == col_type_Link, col_key.get_type());
}

DictionaryLinkValues::DictionaryLinkValues(const Dictionary& source)
    : m_source(source)
{
    BARQ_ASSERT_EX(source.get_value_data_type() == type_Link, source.get_value_data_type());
}
