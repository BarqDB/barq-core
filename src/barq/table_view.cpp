...
Mixed TableView::aggregate(ColKey column_key, size_t* result_count, ObjKey* return_key) const
{
    ...
    BARQ_ASSERT(m_table->valid_column(column_key));
...
{
    BARQ_ASSERT(m_query);
    BARQ_ASSERT(m_query->m_table);
    BARQ_ASSERT(m_query->m_table == q.m_table);
