#ifndef BARQ_OBJECT_STORE_C_API_UTIL_HPP
#define BARQ_OBJECT_STORE_C_API_UTIL_HPP
...
inline const ObjectSchema& schema_for_table(const std::shared_ptr<Barq>& barq, TableKey table_key)
{
    // Validate the table key.
    barq->read_group().get_table(table_key);
...
inline void report_type_mismatch(const SharedBarq& barq, const Table& table, ColKey col_key)
{
    auto& schema = schema_for_table(barq, table.get_key());
...
inline void check_value_assignable(const SharedBarq& barq, const Table& table, ColKey col_key, Mixed val)
{
    if (val.is_null()) {
    ...
    if (val.get_type() == type_TypedLink && (col_type == col_type_Link)) {
        ...
        if (table.get_link_target(col_key)->get_key() != obj_link.get_table_key()) {
            report_type_mismatch(barq, table, col_key);
        }
    }
    else {
...
inline void check_value_assignable(const barq::object_store::Collection& list, Mixed val)
{
    auto barq = list.get_barq();
    auto table_key = list.get_parent_table_key();
    auto table = barq->read_group().get_table(table_key);
    auto col_key = list.get_parent_column_key();
    return check_value_assignable(barq, *table, col_key, val);
}
...
#endif // BARQ_OBJECT_STORE_C_API_UTIL_HPP
