...
#if BARQ_PLATFORM_APPLE && !defined(BARQ_NO_THREAD_LOCAL)
#define BARQ_NO_THREAD_LOCAL
#endif

#if defined(BARQ_NO_THREAD_LOCAL)
#include <pthread.h>
#endif
...
void ErrorStorage::assign(std::exception_ptr eptr) noexcept
{
    ...
    auto populate_error = [&](const std::exception& ex, ErrorCodes::Error error_code) {
        m_err->error = barq_errno_e(error_code);
        ...
        try {
            ...
            m_err->error = BARQ_ERR_OUT_OF_MEMORY;
            m_err->message = "Out of memory while creating barq_error_t";
        }
    };
...
bool ErrorStorage::get_as_barq_error_t(barq_error_t* out) const noexcept
{
    if (!m_err) {
        return false;
    }
...
{
    return ErrorStorage::get_thread_local()->get_as_barq_error_t(err);
}

BARQ_API bool barq_clear_last_error()
{
...
BARQ_API barq_async_error_t* barq_get_last_error_as_async_error(void)
{
    ...
    return new barq_async_error_t{*ErrorStorage::get_thread_local()};
}

BARQ_API bool barq_get_async_error(const barq_async_error_t* async_err, barq_error_t* out_err)
{
    if (!async_err)
        return false;

    return async_err->error_storage.get_as_barq_error_t(out_err);
}

} // namespace barq::c_api

BARQ_EXPORT bool barq_wrap_exceptions(void (*func)()) noexcept
{
    return barq::c_api::wrap_err([=]() {
        (func)();
        return true;
    });
}

BARQ_API void barq_register_user_code_callback_error(void* user_code_error) noexcept
{
    barq::c_api::ErrorStorage::get_thread_local()->set_user_code_error(user_code_error);
}
