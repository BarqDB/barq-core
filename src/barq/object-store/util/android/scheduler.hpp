...
#define LOGE(...)                                                                                                    \
    do {                                                                                                             \
        ...
        __android_log_print(ANDROID_LOG_ERROR, "BARQ", __VA_ARGS__);                                                \
    } while (0)
...
private:
    ...
    void init()
    {
        ...
        // See https://github.com/barq/barq-java/issues/3945 .
        if (pipe(message_pipe)) {
            int err = errno;
            LOGE("could not create WeakBarqNotifier ALooper message pipe: %s.", strerror(err));
            return;
        }
        ...
        if (ALooper_addFd(m_looper, message_pipe[0], ALOOPER_POLL_CALLBACK, ALOOPER_EVENT_INPUT, &looper_callback,
                          this) != 1) {
            LOGE("Error adding WeakBarqNotifier callback to looper.");
    ...
    static int looper_callback(int fd, int events, void* data)
    {
        ...
        if ((events & ALOOPER_EVENT_ERROR) != 0) {
            LOGE("Unexpected error on WeakBarqNotifier's ALooper message pipe.");
        }
