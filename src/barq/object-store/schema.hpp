...
#ifndef BARQ_SCHEMA_HPP
#define BARQ_SCHEMA_HPP
...
// schema of the Barq file. By default, any properties or object types not in
// the requested schema are simply ignored entirely and the Barq's in-memory
// schema will always exactly match the requested one.
struct SchemaSubsetMode {
    // Add additional tables present in the Barq file to the schema. This is
    // applicable to all schema modes except for Manual and ResetFile.
    bool include_types : 1;

    // Add additional columns in the tables present in the Barq file to the
    ...
    // Additional object classes present in the Barq file are added to the
    ...
    // Additional properties present in the Barq file are added to the
...
#define BARQ_FOR_EACH_SCHEMA_CHANGE_TYPE(macro)                                                                     \
...
class SchemaChange {
public:
#define BARQ_SCHEMA_CHANGE_CONSTRUCTOR(name)                                                                        \
    SchemaChange(schema_change::name value)                                                                          \
        : m_kind(Kind::name)                                                                                         \
    {                                                                                                                \
        name = value;                                                                                                \
    }
    BARQ_FOR_EACH_SCHEMA_CHANGE_TYPE(BARQ_SCHEMA_CHANGE_CONSTRUCTOR)
#undef BARQ_SCHEMA_CHANGE_CONSTRUCTOR

    template <typename Visitor>
    auto visit(Visitor&& visitor) const
    {
        switch (m_kind) {
#define BARQ_SWITCH_CASE(name)                                                                                      \
    case Kind::name:                                                                                                 \
        return visitor(name);
            BARQ_FOR_EACH_SCHEMA_CHANGE_TYPE(BARQ_SWITCH_CASE)
#undef BARQ_SWITCH_CASE
        }
...
private:
    enum class Kind {
#define BARQ_SCHEMA_CHANGE_TYPE(name) name,
        BARQ_FOR_EACH_SCHEMA_CHANGE_TYPE(BARQ_SCHEMA_CHANGE_TYPE)
#undef BARQ_SCHEMA_CHANGE_TYPE

    } m_kind;
    union {
#define BARQ_DEFINE_FIELD(name) schema_change::name name;
        BARQ_FOR_EACH_SCHEMA_CHANGE_TYPE(BARQ_DEFINE_FIELD)
#undef BARQ_DEFINE_FIELD
    };
};

#undef BARQ_FOR_EACH_SCHEMA_CHANGE_TYPE
} // namespace barq

#endif /* defined(BARQ_SCHEMA_HPP) */
