    ...
    switch (op) {
        case Results::SectionedResultsOperator::FirstLetter:
            if (results.get_type() == PropertyType::Object) {
                ...
                return [col_key](Mixed value, const SharedBarq& barq) {
                    auto link = value.get_link();
                    auto v = barq->read_group().get_object(link).get<StringData>(col_key);
                    return v.size() > 0 ? v.prefix(1) : "";
                };
            }
            else {
                return [](Mixed value, const SharedBarq&) {
...
void SectionedResults::calculate_sections()
{
    ...
    for (size_t i = 0; i < size; ++i) {
        Mixed key = m_callback(m_results.get_any(i), m_results.get_barq());
    ...
    if (!m_has_performed_initial_evaluation) {
        BARQ_ASSERT_EX(m_previous_key_to_index.size() == 0, m_previous_key_to_index.size());
        BARQ_ASSERT_EX(m_previous_index_to_key.size() == 0, m_previous_index_to_key.size());
...
SectionedResults SectionedResults::freeze(std::shared_ptr<Barq> const& frozen_barq)
{
    return copy(m_results.freeze(frozen_barq));
}
