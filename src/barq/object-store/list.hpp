...
#ifndef BARQ_OS_LIST_HPP
#define BARQ_OS_LIST_HPP
...
public:
    ...
    List freeze(std::shared_ptr<Barq> const& frozen_barq) const;
...
private:
    ...
    {
        BARQ_ASSERT_DEBUG(dynamic_cast<LstBase*>(m_coll_base.get()));
...
auto& List::as() const
{
    BARQ_ASSERT_DEBUG(dynamic_cast<Lst<T>*>(m_coll_base.get()));
...
{
    BARQ_ASSERT_DEBUG(dynamic_cast<LnkLst*>(m_coll_base.get()));
...
namespace std {
template <>
struct hash<barq::List> {
    size_t operator()(barq::List const&) const;
};
} // namespace std

#endif // BARQ_OS_LIST_HPP
