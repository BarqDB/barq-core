...
#ifndef BARQ_AUDIT_HPP
#define BARQ_AUDIT_HPP
...
struct AuditConfig {
    // User to open the audit Barqs with. If null, the user used to open the
    // Barq being audited is used
    std::shared_ptr<SyncUser> audit_user;
    // Prefix added to the start of generated partition keys for audit Barqs.
    ...
    // Logger for audit events. If null, the sync logger from the Barq under
    // audit is used.
    std::shared_ptr<util::Logger> logger;
    // Error handler which is called if fatal sync errors occur on the sync
    // Barq. If null, an error is logged then abort() is called.
    ...
    // Root directory to store audit Barqs
...
std::shared_ptr<AuditInterface> make_audit_context(std::shared_ptr<DB>, BarqConfig const& parent_config);
...
inline std::shared_ptr<AuditInterface> make_audit_context(std::shared_ptr<DB>, BarqConfig const&)
{
    BARQ_TERMINATE("Audit not supported on this platform");
}
#endif

} // namespace barq

#endif // BARQ_AUDIT_HPP
