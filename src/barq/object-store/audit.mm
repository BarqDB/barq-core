...
#include <barq/object-store/impl/collection_change_builder.hpp>
...
private:
    ...
    {
        BARQ_TERMINATE("Unexpected transaction log instruction encountered");
    }
...
public:
    AuditEventWriter(DB& db, MetadataSchema const& metadata, StringData activity_name, Table& audit_table,
        ...
        , m_repl_buffer([this]() -> const util::AppendBuffer<char>& {
            BARQ_ASSERT(typeid(m_repl) == typeid(sync::ClientReplication));
...
private:
    ...
    bool write_event(Timestamp timestamp, StringData activity, StringData event_type, StringData data)
    {
        ...
        // We never read from the Barq file, and so we can significantly improve
        // performance and reduce the size of the Barq file by bypassing
...
// A pool of audit Barqs for a sync user. Each audit scope asks the pool for
// a Barq to write the scope to. The pool takes care of rotating between Barqs
// when the current one grows too large and ensuring that all of the Barqs are
// uploaded to the server.
class AuditBarqPool : public std::enable_shared_from_this<AuditBarqPool> {
    struct Private {};
...
public:
    ...
    static std::shared_ptr<AuditBarqPool> get_pool(std::shared_ptr<SyncUser> user, const AuditConfig& config,
                                                    const std::shared_ptr<util::Logger>& logger);

    // Write to a pooled Barq. The Transaction should not be retained outside
    // of the callback.
    void write(util::FunctionRef<void(Transaction&)> func) REQUIRES(!m_mutex);

    explicit AuditBarqPool(Private, std::shared_ptr<SyncUser> user, const AuditConfig& config,
                            const std::shared_ptr<util::Logger>& logger);

    // Block the calling thread until all pooled Barqs have been fully uploaded,
...
private:
    ...
    std::shared_ptr<Barq> m_current_barq;
    ...
    void open_new_barq() REQUIRES(!m_mutex);
    void wait_for_upload(std::shared_ptr<SyncSession>) REQUIRES(m_mutex);
    void scan_for_barqs_to_upload() REQUIRES(!m_mutex);
    bool clean_up_backup(const std::string& file_name);
    std::string prefixed_partition(std::string const& partition);
};

std::shared_ptr<AuditBarqPool>
AuditBarqPool::get_pool(std::shared_ptr<SyncUser> user, const AuditConfig& config,
                         const std::shared_ptr<util::Logger>& logger) NO_THREAD_SAFETY_ANALYSIS
{
    struct CachedPool {
        ...
        std::string app_id;
        std::weak_ptr<AuditBarqPool> pool;
    };
    ...
    auto pool = std::make_shared<AuditBarqPool>(Private(), user, config, logger);
    pool->scan_for_barqs_to_upload();
    s_pools.push_back({user->user_id(), config.partition_value_prefix, app_id, pool});
    return pool;
}

AuditBarqPool::AuditBarqPool(Private, std::shared_ptr<SyncUser> user, const AuditConfig& config,
                               const std::shared_ptr<util::Logger>& logger)
    : m_user(user)
    ...
    , m_path_root([&] {
        ...
        const char separator[] = "/";
#endif
        // "$root/barq-audit/$appId/$userId/$partitonPrefix/"
        return util::format("%2%1barq-audit%1%3%1%4%1%5%1", separator, config.base_file_path, m_user->app_id(),
...
void AuditBarqPool::write(util::FunctionRef<void(Transaction&)> func)
{
    if (m_current_barq) {
        auto size = util::File::get_size_static(m_current_barq->config().path);
        if (size > g_max_partition_size) {
            m_logger->info("Events: Closing Barq at '%1': size %2 > max size %3", m_current_barq->config().path,
                           size, g_max_partition_size.load());
            auto sync_session = m_current_barq->sync_session();
            {
                // If we're offline and already have a Barq waiting to upload,
                // just close this Barq and we'll come back to it later.
                // Otherwise keep it open and upload it.
                util::CheckedLockGuard lock(m_mutex);
                if (m_upload_sessions.empty() ||
                    sync_session->connection_state() == barq::SyncSession::ConnectionState::Connected) {
                    wait_for_upload(sync_session);
                }
                else {
                    sync_session->force_close();
                    m_open_paths.erase(m_current_barq->config().path);
                }
            }
            m_current_barq->close();
            m_current_barq = nullptr;
        }
        else {
            m_logger->detail("Events: Reusing existing Barq at '%1'", m_current_barq->config().path);
        }
    }
    ...
    BARQ_ASSERT(m_current_barq);
    m_current_barq->begin_transaction();
    try {
        func(static_cast<Transaction&>(m_current_barq->read_group()));
    }
    catch (...) {
        m_current_barq->cancel_transaction();
        throw;
    }
    m_current_barq->commit_transaction();
}

void AuditBarqPool::wait_for_upload(std::shared_ptr<SyncSession> session)
{
    ...
    session->wait_for_upload_completion([this, weak_self = weak_from_this(), session](Status status) {
...

        {
            ...
            BARQ_ASSERT(it != m_upload_sessions.end());
        ...
        scan_for_barqs_to_upload();
    });
    session->revive_if_needed();
}

std::string AuditBarqPool::prefixed_partition(std::string const& partition)
{
    return util::format("\"%1-%2\"", m_partition_prefix, partition);
}

void AuditBarqPool::scan_for_barqs_to_upload()
{
    util::CheckedLockGuard lock(m_mutex);
    m_logger->trace("Events: Scanning for Barqs in '%1' to upload", m_path_root);
    ...
    while (dir.next(file_name)) {
        ...
        if (tr->get_history()->no_pending_local_changes(tr->get_version())) {
            m_logger->info("Events: Barq at '%1' is fully uploaded", path);
            tr = nullptr;
            db->close();
            util::File::remove(path);
            continue;
...
bool AuditBarqPool::clean_up_backup(const std::string& file_name)
{
    ...
    auto base_barq_path = util::format("%1%2.barq", m_path_root, base_name);
    ...
    if (!util::File::exists(base_barq_path)) {
    ...
    // should be removed even though the base Barq still exists
    auto path = m_path_root + file_name;
...
void AuditBarqPool::open_new_barq()
{
    ...
    sync_config->recovery_directory = std::string("io.barq.audit");
    ...
    config.path = util::format("%1%2.barq", m_path_root, partition);
    ...
    m_logger->info("Events: Opening new Barq at '%1'", config.path);
    m_current_barq = Barq::get_shared_barq(std::move(config));
    util::CheckedLockGuard lock(m_mutex);
    m_open_paths.insert(m_current_barq->config().path);
}

void AuditBarqPool::wait_for_uploads()
{
...
public:
    AuditContext(std::shared_ptr<DB> source_db, BarqConfig const& parent_config, AuditConfig const& audit_config);
    ~AuditContext();
...
private:
    ...
    std::shared_ptr<AuditBarqPool> m_barq_pool;
...
AuditContext::AuditContext(std::shared_ptr<DB> source_db, BarqConfig const& parent_config,
    ...
    , m_queue(dispatch_queue_create("Barq audit worker", DISPATCH_QUEUE_SERIAL_WITH_AUTORELEASE_POOL))
{
    validate_metadata(m_metadata->metadata);
    BARQ_ASSERT(parent_config.sync_config);
    ...
    if (parent_sync_config.flx_sync_requested && audit_user == parent_sync_config.user) {
        throw InvalidArgument("Auditing a flexible sync barq requires setting the audit "
    ...
    m_barq_pool = AuditBarqPool::get_pool(audit_user, audit_config, m_logger);
}
...
void AuditContext::process_scope(AuditContext::Scope& scope) const
{
    ...
    try {
        ...
        m_barq_pool->write([&](Transaction& tr) {
...
namespace barq {
std::shared_ptr<AuditInterface> make_audit_context(std::shared_ptr<DB> db, BarqConfig const& config)
{
    BARQ_ASSERT(config.audit_config);
