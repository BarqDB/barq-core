...
#if BARQ_ENABLE_SYNC
#include <barq/object-store/sync/impl/sync_file.hpp>
...
bool BarqConfig::needs_file_format_upgrade() const
{
    return DB::needs_file_format_upgrade(path, encryption_key);
}

Barq::Barq(Config config, util::Optional<VersionID> version, std::shared_ptr<_impl::BarqCoordinator> coordinator,
             Private)
...
Barq::~Barq()
{
    ...
    if (m_coordinator) {
        m_coordinator->unregister_barq(this);
    }
}
...
Transaction& Barq::transaction() const
{
    // one day we should change the way we use constness
    Barq* nc_barq = const_cast<Barq*>(this);
    return nc_barq->transaction();
}

std::shared_ptr<Transaction> Barq::transaction_ref()
{
    return m_transaction;
}

std::shared_ptr<Transaction> Barq::duplicate() const
{
    auto version = read_transaction_version(); // does the validity check first
    return m_coordinator->begin_read(version, is_frozen());
}

std::shared_ptr<DB>& Barq::Internal::get_db(Barq& barq)
{
    return barq.m_coordinator->m_db;
}

void Barq::Internal::begin_read(Barq& barq, VersionID version_id)
{
    barq.begin_read(version_id);
}

void Barq::begin_read(VersionID version_id)
{
    BARQ_ASSERT(!m_transaction);
    m_transaction = m_coordinator->begin_read(version_id, bool(m_frozen_version));
    add_schema_change_handler();
    read_schema_from_group_if_needed();
}

SharedBarq Barq::get_shared_barq(Config config)
{
    auto coordinator = BarqCoordinator::get_coordinator(config.path);
    return coordinator->get_barq(std::move(config), util::none);
}

SharedBarq Barq::get_frozen_barq(Config config, VersionID version)
{
    auto coordinator = BarqCoordinator::get_coordinator(config.path);
    return coordinator->get_barq(std::move(config), version);
}

SharedBarq Barq::get_shared_barq(ThreadSafeReference ref, std::shared_ptr<util::Scheduler> scheduler)
{
    if (!scheduler)
        scheduler = util::Scheduler::make_default();
    SharedBarq barq = ref.resolve<std::shared_ptr<Barq>>(nullptr);
    BARQ_ASSERT(barq);
    auto& config = barq->config();
    auto coordinator = BarqCoordinator::get_coordinator(config.path);
    if (auto barq = coordinator->get_cached_barq(config, scheduler))
        return barq;
    barq->m_scheduler = scheduler;
    coordinator->bind_to_context(*barq);
    return barq;
}

#if BARQ_ENABLE_SYNC
std::shared_ptr<AsyncOpenTask> Barq::get_synchronized_barq(Config config)
{
    auto coordinator = BarqCoordinator::get_coordinator(config.path);
    return coordinator->get_synchronized_barq(std::move(config));
}

std::shared_ptr<SyncSession> Barq::sync_session() const
{
    return m_coordinator ? m_coordinator->sync_session() : nullptr;
}

sync::SubscriptionSet Barq::get_latest_subscription_set()
{
    ...
    BARQ_ASSERT(flx_sub_store);
    return flx_sub_store->get_latest();
}

sync::SubscriptionSet Barq::get_active_subscription_set()
{
    if (!m_config.sync_config || !m_config.sync_config->flx_sync_requested) {
        throw IllegalOperation("Flexible sync is not enabled");
    }
...
void Barq::set_schema(Schema const& reference, Schema schema)
{
...
void Barq::read_schema_from_group_if_needed()
{
...
bool Barq::reset_file(Schema& schema, std::vector<SchemaChange>& required_changes)
{
...
bool Barq::schema_change_needs_write_transaction(Schema& schema, std::vector<SchemaChange>& changes,
                                                  uint64_t version)
{
    ...
    switch (m_config.schema_mode) {
        ...
        case SchemaMode::Manual:
            ...
            if (version == m_schema_version) {
                ...
                BARQ_UNREACHABLE(); // changes is non-empty so above line always throws
            }
            return true;
    }
    BARQ_COMPILER_HINT_UNREACHABLE();
}

// Schema version is not allowed to decrease for local and pbs barqs.
void Barq::verify_schema_version_not_decreasing(uint64_t version)
{
#if BARQ_ENABLE_SYNC
    if (m_config.sync_config && m_config.sync_config->flx_sync_requested)
        return;
...
Class Barq::get_class(StringData object_type)
{
...
std::vector<Class> Barq::get_classes()
{
...
void Barq::set_schema_subset(Schema schema)
{
    verify_thread();
    verify_open();
    BARQ_ASSERT(m_dynamic_schema);
    BARQ_ASSERT(m_schema_version != ObjectStore::NotVersioned);
...
void Barq::update_schema(Schema schema, uint64_t version, MigrationFunction migration_function,
                          DataInitializationFunction initialization_function, bool in_transaction)
{
    ...
    // Frozen Barqs never modify the schema on disk and we just need to verify
    ...
    if (migration_function && !additive) {
        auto wrapper = [&] {
            ...
            // Don't go through the normal codepath for opening a Barq because
            // we're using a mismatched config
            auto old_barq = std::make_shared<Barq>(std::move(config), none, m_coordinator, Private());
            // block autorefresh for the old barq
            old_barq->m_auto_refresh = false;
            migration_function(old_barq, shared_from_this(), m_schema);
        };

        // migration function needs to see the target schema on the "new" Barq
...
void Barq::rename_property(Schema schema, StringData object_type, StringData old_name, StringData new_name)
{
    ObjectStore::rename_property(read_group(), schema, object_type, old_name, new_name);
}

void Barq::add_schema_change_handler()
{
    if (m_config.immutable())
        return;
...
void Barq::translate_schema_error()
{
...
void Barq::notify_schema_changed()
{
...
static void check_can_create_write_transaction(const Barq* barq)
{
    barq->verify_thread();
    barq->verify_open();
    if (barq->config().immutable() || barq->config().read_only()) {
        throw WrongTransactionState("Can't perform transactions on read-only Barqs.");
    }
    if (barq->is_frozen()) {
        throw WrongTransactionState("Can't perform transactions on a frozen Barq");
    }
    if (!barq->is_closed() && barq->get_number_of_versions() > barq->config().max_number_of_active_versions) {
        throw WrongTransactionState(
            util::format("Number of active versions (%1) in the Barq exceeded the limit of %2",
                         barq->get_number_of_versions(), barq->config().max_number_of_active_versions));
    }
}

void Barq::verify_thread() const
{
    if (m_scheduler && !m_scheduler->is_on_thread())
        throw LogicError(ErrorCodes::WrongThread, "Barq accessed from incorrect thread.");
}

void Barq::verify_in_write() const
{
...
void Barq::verify_open() const
{
    if (is_closed()) {
        throw LogicError(ErrorCodes::ClosedBarq, "Cannot access barq that has been closed.");
    }
}

bool Barq::verify_notifications_available(bool throw_on_error) const
{
    if (is_frozen()) {
        if (throw_on_error)
            throw WrongTransactionState(
    ...
    if (config().immutable()) {
        if (throw_on_error)
            throw WrongTransactionState("Cannot create asynchronous query for immutable Barqs");
        return false;
    }
    if (throw_on_error) {
...
VersionID Barq::read_transaction_version() const
{
    verify_thread();
    verify_open();
    BARQ_ASSERT(m_transaction);
...
util::Optional<DB::version_type> Barq::latest_snapshot_version() const
{
...
void Barq::enable_wait_for_change()
{
    verify_open();
    m_coordinator->enable_wait_for_change();
}

bool Barq::wait_for_change()
{
    verify_open();
...
void Barq::call_completion_callbacks()
{
...
void Barq::run_async_completions()
{
    call_completion_callbacks();
    check_pending_write_requests();
}

void Barq::check_pending_write_requests()
{
...
void Barq::end_current_write(bool check_pending)
{
    if (!m_transaction) {
        return;
    }
...
void Barq::run_writes()
{
    if (!m_transaction) {
        // Barq might have been closed
        return;
    }
    ...
    while (!m_async_write_q.empty() && m_transaction) {
        ...
        // Barq may have been closed in the write function
        if (!m_transaction) {
            return;
        }
...
auto Barq::async_begin_transaction(util::UniqueFunction<void()>&& the_write_block, bool notify_only) -> AsyncHandle
{
...
auto Barq::async_commit_transaction(util::UniqueFunction<void(std::exception_ptr)>&& completion, bool allow_grouping)
    -> AsyncHandle
{
    check_can_create_write_transaction(this);
    if (m_is_running_async_commit_completions) {
    ...
    BARQ_ASSERT(m_transaction->holds_write_mutex());
    BARQ_ASSERT(!m_notify_only);
    // auditing is not supported
    BARQ_ASSERT(!audit_context());
...
bool Barq::async_cancel_transaction(AsyncHandle handle)
{
    verify_thread();
    verify_open();
...
void Barq::begin_transaction()
{
    check_can_create_write_transaction(this);

    if (is_in_transaction()) {
        throw WrongTransactionState("The Barq is already in a write transaction");
    }
...
void Barq::do_begin_transaction()
{
...
void Barq::invalidate()
{
    ...
    if (m_is_sending_notifications) {
        // This was originally because closing the Barq during notification
...
bool Barq::compact()
{
    ...
    if (m_config.immutable() || m_config.read_only()) {
        throw WrongTransactionState("Can't compact a read-only Barq");
    }
    if (is_in_transaction()) {
        throw WrongTransactionState("Can't compact a Barq within a write transaction");
    }
...
void Barq::convert(const Config& config, bool merge_into_existing)
{
    verify_thread();
    verify_open();
    ...
    if (dst_is_flx_sync && !src_is_flx_sync) {
        throw IllegalOperation(
            "Barq cannot be converted to a flexible sync barq unless flexible sync is already enabled");
    }
    if (dst_is_pbs_sync && src_is_flx_sync) {
        throw IllegalOperation(
            "Barq cannot be converted from a flexible sync barq to a partition based sync barq");
    }

#endif

    if (merge_into_existing && util::File::exists(config.path)) {
        auto destination_barq = Barq::get_shared_barq(config);
        destination_barq->begin_transaction();
        auto destination = destination_barq->transaction_ref();
        m_transaction->copy_to(destination);
        destination_barq->commit_transaction();
        return;
    }
...
OwnedBinaryData Barq::write_copy()
{
    verify_thread();
...
bool Barq::do_refresh()
{
    // Frozen Barqs never change.
    if (is_frozen()) {
        return false;
    }

    if (m_config.immutable()) {
        throw WrongTransactionState("Can't refresh an immutable Barq.");
    }
...
void Barq::set_auto_refresh(bool auto_refresh)
{
    if (is_frozen() && auto_refresh) {
        throw WrongTransactionState("Auto-refresh cannot be enabled for frozen Barqs.");
    }
    m_auto_refresh = auto_refresh;
}


bool Barq::can_deliver_notifications() const noexcept
{
...
uint64_t Barq::get_schema_version(const Barq::Config& config)
{
    auto coordinator = BarqCoordinator::get_coordinator(config.path);
    auto version = coordinator->get_schema_version();
    if (version == ObjectStore::NotVersioned)
        version = ObjectStore::get_schema_version(coordinator->get_barq(config, util::none)->read_group());
    return version;
}
...
bool Barq::is_frozen() const
{
    ...
    BARQ_ASSERT_DEBUG(!result || !m_transaction || m_transaction->is_frozen());
    return result;
}

SharedBarq Barq::freeze()
{
    read_group(); // Freezing requires a read transaction
    return m_coordinator->freeze_barq(*this);
}

void Barq::copy_schema_from(const Barq& source)
{
    BARQ_ASSERT(is_frozen());
    BARQ_ASSERT(m_frozen_version == source.read_transaction_version());
...
void Barq::delete_files(const std::string& barq_file_path, bool* did_delete_barq)
{
    bool lock_successful = false;
    try {
        lock_successful = DB::call_with_lock(barq_file_path, [=](auto const& path) {
            DB::delete_files(path, did_delete_barq);
        });
    }
        ...
        if (did_delete_barq) {
            *did_delete_barq = false;
        }
        return;
    }
    if (!lock_successful) {
        throw FileAccessError(
            ErrorCodes::DeleteOnOpenBarq,
            util::format("Cannot delete files of an open Barq: '%1' is still in use.", barq_file_path),
            barq_file_path);
    }
}

AuditInterface* Barq::audit_context() const noexcept
{
...
KeyPathArray Barq::create_key_path_array(StringData table_name, const std::vector<std::string>& key_paths)
{
...
KeyPathArray Barq::create_key_path_array(TableKey table_key, size_t num_key_paths, const char** all_key_paths)
{
    auto object_schema = m_schema.find(table_key);
    BARQ_ASSERT(object_schema != m_schema.end());
...
#ifdef BARQ_DEBUG
void Barq::print_key_path_array(const KeyPathArray& kpa)
{
    auto& g = read_group();
    for (auto& kp : kpa) {
        for (auto [tk, ck] : kp) {
