    ...
    while (to_visit.size() > 0) {
        ...
        for (auto& prop : current.object.persisted_properties) {
            if (prop.type == PropertyType::Object) {
                ...
                BARQ_ASSERT(it != schema.end()); // this succeeds if the schema is otherwise valid
...
void Schema::copy_keys_from(barq::Schema const& other, SchemaSubsetMode subset_mode)
{
...
#define BARQ_SC_COMPARE(type, ...)                                                                                  \
    ...
    {                                                                                                                \
        ...
        BARQ_SC_COMPARE(AddIndex, v.object, v.property, v.type)
        BARQ_SC_COMPARE(AddProperty, v.object, v.property)
        BARQ_SC_COMPARE(AddInitialProperties, v.object)
        BARQ_SC_COMPARE(AddTable, v.object)
        BARQ_SC_COMPARE(RemoveTable, v.object)
        BARQ_SC_COMPARE(ChangeTableType, v.object, v.old_table_type, v.new_table_type)
        BARQ_SC_COMPARE(ChangePrimaryKey, v.object, v.property)
        BARQ_SC_COMPARE(ChangePropertyType, v.object, v.old_property, v.new_property)
        BARQ_SC_COMPARE(MakePropertyNullable, v.object, v.property)
        BARQ_SC_COMPARE(MakePropertyRequired, v.object, v.property)
        BARQ_SC_COMPARE(RemoveIndex, v.object, v.property)
        BARQ_SC_COMPARE(RemoveProperty, v.object, v.property)

#undef BARQ_SC_COMPARE
    } visitor{lft};
