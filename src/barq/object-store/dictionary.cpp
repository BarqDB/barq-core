...
public:
    ...
    {
        BARQ_TERMINATE("not implemented");
    }
...
void DictionaryChangeSet::add(std::vector<Mixed>& arr, const Mixed& key)
{
    ...
    if (key.is_type(type_String)) {
        BARQ_ASSERT(m_string_store.size() < m_string_store.capacity());
...
{
    verify_attached();
    return Results(m_barq,
                   std::make_shared<DictionaryKeyAdapter>(std::dynamic_pointer_cast<barq::Dictionary>(m_coll_base)));
}
...
public:
    NotificationHandler(barq::Dictionary& dict, Dictionary::CBFunc cb)
        : m_dict(dict)
        , m_prev_rt(static_cast<Transaction*>(dict.get_table()->get_parent_group())->duplicate())
        , m_prev_dict(std::dynamic_pointer_cast<barq::Dictionary>(m_prev_rt->import_copy_of(dict)))
        , m_cb(std::move(cb))
    {
    }
    ...
    void operator()(CollectionChangeSet const& c)
    {
        ...
        if (c.collection_root_was_deleted) {
            ...
            BARQ_ASSERT(m_dict.is_attached());
...
Dictionary Dictionary::freeze(const std::shared_ptr<Barq>& frozen_barq) const
{
    auto frozen_dictionary(frozen_barq->import_copy_of(*m_coll_base));
    if (frozen_dictionary) {
        return Dictionary(frozen_barq, std::move(frozen_dictionary));
    }
    else {
        return Dictionary{};
    }
