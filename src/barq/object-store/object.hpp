...
#ifndef BARQ_OS_OBJECT_HPP
#define BARQ_OS_OBJECT_HPP
...
struct CreatePolicy {
    ...
    /// created in the Barq? False for pure lookup functions such as find(),
    /// index_of() and queries, true for everything else.
    bool create : 1;
    /// Should the input object be copied into the Barq even if it is already
    /// an object managed by the current Barq? True for barq.create(), false
...
public:
    Object();
    Object(const std::shared_ptr<Barq>& r, Obj const& o);
    Object(const std::shared_ptr<Barq>& r, ObjectSchema const& s, Obj const& o, Obj const& parent = {},
           ColKey incoming_column = {});
    Object(const std::shared_ptr<Barq>& r, StringData object_type, ObjKey key);
    Object(const std::shared_ptr<Barq>& r, StringData object_type, size_t index);
    Object(const std::shared_ptr<Barq>& r, ObjLink link);
    ...
    std::shared_ptr<Barq> const& barq() const
    {
        return m_barq;
    }
    std::shared_ptr<Barq> const& get_barq() const
    {
        return m_barq;
    }
    ...
    // Freeze a copy of this object in the context of the frozen Barq.
    // Equivalent to producing a thread-safe reference and resolving it in the frozen barq.
    Object freeze(std::shared_ptr<Barq> frozen_barq) const;
    ...
    template <typename ValueType, typename ContextType>
    static Object create(ContextType& ctx, std::shared_ptr<Barq> const& barq, const ObjectSchema& object_schema,
                         ValueType value, CreatePolicy policy = CreatePolicy::ForceCreate,
                         ObjKey current_obj = ObjKey(), Obj* = nullptr);

    template <typename ValueType, typename ContextType>
    static Object create(ContextType& ctx, std::shared_ptr<Barq> const& barq, StringData object_type,
                         ValueType value, CreatePolicy policy = CreatePolicy::ForceCreate,
                         ObjKey current_obj = ObjKey(), Obj* = nullptr);

    template <typename ValueType, typename ContextType>
    static Object get_for_primary_key(ContextType& ctx, std::shared_ptr<Barq> const& barq,
                                      const ObjectSchema& object_schema, ValueType primary_value);

    template <typename ValueType, typename ContextType>
    static Object get_for_primary_key(ContextType& ctx, std::shared_ptr<Barq> const& barq, StringData object_type,
...
private:
    ...
    Object(std::shared_ptr<Barq> r, const ObjectSchema* s, Obj const& o, Obj const& parent = {},
           ColKey incoming_column = {});
    template <typename Key>
    Object(const std::shared_ptr<Barq>& r, const ObjectSchema* s, Key key);
