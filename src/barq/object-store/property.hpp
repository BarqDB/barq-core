...
#ifndef BARQ_PROPERTY_HPP
#define BARQ_PROPERTY_HPP
...
struct Property {
    ...
    // The internal column name used in the Barq file.
    std::string name;

    // The public name used by the binding to represent the internal column name in the Barq file. Bindings can use
...
inline std::string Property::type_string() const
{
    ...
    if (is_set(type)) {
        BARQ_ASSERT(type != PropertyType::LinkingObjects);
        if (type == PropertyType::Object)
