...
{
    verify_attached();
    return Results(m_barq, std::dynamic_pointer_cast<LnkSet>(m_coll_base), get_query().and_query(std::move(q)));
}

Set Set::freeze(const std::shared_ptr<Barq>& frozen_barq) const
{
    auto frozen_set(frozen_barq->import_copy_of(*m_coll_base));
    if (frozen_set) {
        return Set(frozen_barq, std::move(frozen_set));
    }
    else {
        return Set{};
    }
...
#define BARQ_PRIMITIVE_SET_TYPE(T)                                                                                  \
...
BARQ_PRIMITIVE_SET_TYPE(bool)
BARQ_PRIMITIVE_SET_TYPE(int64_t)
BARQ_PRIMITIVE_SET_TYPE(float)
BARQ_PRIMITIVE_SET_TYPE(double)
BARQ_PRIMITIVE_SET_TYPE(StringData)
BARQ_PRIMITIVE_SET_TYPE(BinaryData)
BARQ_PRIMITIVE_SET_TYPE(Timestamp)
BARQ_PRIMITIVE_SET_TYPE(ObjKey)
BARQ_PRIMITIVE_SET_TYPE(ObjectId)
BARQ_PRIMITIVE_SET_TYPE(Decimal)
BARQ_PRIMITIVE_SET_TYPE(UUID)
BARQ_PRIMITIVE_SET_TYPE(util::Optional<bool>)
BARQ_PRIMITIVE_SET_TYPE(util::Optional<int64_t>)
BARQ_PRIMITIVE_SET_TYPE(util::Optional<float>)
BARQ_PRIMITIVE_SET_TYPE(util::Optional<double>)
BARQ_PRIMITIVE_SET_TYPE(util::Optional<ObjectId>)
BARQ_PRIMITIVE_SET_TYPE(util::Optional<UUID>)

#undef BARQ_PRIMITIVE_SET_TYPE
...
namespace std {
size_t hash<barq::object_store::Set>::operator()(barq::object_store::Set const& set) const
{
    return set.hash();
}
} // namespace std
