...
void ChunkedRangeVector::push_back(value_type value)
{
    if (!empty() && m_data.back().data.size() < max_size) {
        ...
        BARQ_ASSERT(range.end <= value.first);
...
void ChunkedRangeVector::verify() const noexcept
{
    ...
    size_t prev_end = -1;
    for (auto range : *this) {
        BARQ_ASSERT(range.first < range.second);
        BARQ_ASSERT(prev_end == size_t(-1) || range.first > prev_end);
        prev_end = range.second;
    }

    for (auto& chunk : m_data) {
        BARQ_ASSERT(!chunk.data.empty());
        BARQ_ASSERT(chunk.data.front().first == chunk.begin);
        BARQ_ASSERT(chunk.data.back().second == chunk.end);
        BARQ_ASSERT(chunk.count <= chunk.end - chunk.begin);
        size_t count = 0;
        for (auto range : chunk.data)
            count += range.second - range.first;
        BARQ_ASSERT(count == chunk.count);
    }
#endif
}
...
IndexSet::iterator IndexSet::find(size_t index, iterator begin) noexcept
{
    ...
    BARQ_ASSERT_DEBUG(inner != it->data.end());
...
#ifdef BARQ_DEBUG
    BARQ_ASSERT((size_t)std::distance(as_indexes().begin(), as_indexes().end()) == expected);
#endif
}
...
void IndexSet::shift_for_insert_at(barq::IndexSet const& values)
{
...
void IndexSet::remove(barq::IndexSet const& values)
{
    auto it = begin();
...
{
    BARQ_ASSERT_DEBUG(!contains(index));
...
IndexSet::iterator IndexSet::do_add(iterator it, size_t index)
{
    ...
    BARQ_ASSERT(!more_before || index >= std::prev(it)->second);
