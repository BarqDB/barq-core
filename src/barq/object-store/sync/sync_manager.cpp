...
#include <barq/object-store/util/uuid.hpp>
...
void SyncManager::tear_down_for_testing()
{
...

    {
        ...
        BARQ_ASSERT_RELEASE(no_sessions);
...
std::shared_ptr<SyncSession> SyncManager::get_session(std::shared_ptr<DB> db, const BarqConfig& config)
{
    auto& client = get_sync_client(); // Throws
#ifndef __EMSCRIPTEN__
    auto path = db->get_path();
    BARQ_ASSERT_EX(path == config.path, path, config.path);
#else
...
void SyncManager::unregister_session(const std::string& path)
{
    ...
    if (auto existing_session = it->second->existing_external_reference()) {
        ...
        // can happen if the user was logged out, or if all Barqs using the
...
{
    BARQ_ASSERT(!sync_route.empty()); // Cannot be set to empty string
    {
        util::CheckedLockGuard lock(m_mutex);
