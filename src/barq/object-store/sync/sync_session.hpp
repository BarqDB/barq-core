...
#ifndef BARQ_OS_SYNC_SESSION_HPP
#define BARQ_OS_SYNC_SESSION_HPP

#include <barq/object-store/feature_checks.hpp>
...
private:
    ...
    // client reset in the future, we might need to reset the progress state variables if the Barq
    // is rolled back.
...
public:
    ...
    explicit SyncSession(Private, _impl::SyncClient&, std::shared_ptr<DB>, const BarqConfig&,
    ...
    // The on-disk path of the Barq file backing the Barq this `SyncSession` represents.
    std::string const& path() const;

    // Returns the salted file ident of the Barq file if one has been assigned, or returns
    // zero for the file ident/salt if no file ident has been assigned yet. This value can
    // change after a client reset or during the initial download of the barq.
    ...
    // The session may resume after calling this if a new Barq is opened for the underlying DB
    ...
    // Shut down the synchronization session (sync::Session) and wait for the Barq file to no
    ...
    // same user and reference barq url as the old configuration. The session will immediately
    ...
    // Swift SDK in `setCustomRequestHeaders` and is defined in the barq-js bindgen definitions.
    void update_configuration(SyncConfig new_config)
        REQUIRES(!m_state_mutex, !m_config_mutex, !m_connection_state_mutex);

    // An object representing the user who owns the Barq this `SyncSession` represents.
    ...
    // A copy of the configuration object describing the Barq this `SyncSession` represents.
    ...
    // created. When the next backend HTTP request is made via the App object, the location info
    ...
    std::string full_barq_url() const REQUIRES(!m_config_mutex)
    {
        util::CheckedLockGuard lock(m_config_mutex);
        return m_server_url;
    }
    ...
    // to the server.
    bool barq_url_verified() const REQUIRES(!m_config_mutex)
    {
        util::CheckedLockGuard lock(m_config_mutex);
    ...
    struct OnlyForTesting {
        ...
        static std::string get_barq_connection_id(SyncSession& session)
        {
            return session.get_barq_connection_id();
        }

        // Supported commands are handled by the test sync server.
...
private:
    ...
    friend class barq::SyncManager;
    // Called by SyncManager {
    static std::shared_ptr<SyncSession> create(_impl::SyncClient& client, std::shared_ptr<DB> db,
                                               const BarqConfig& config, SyncManager* sync_manager)
    {
        BARQ_ASSERT(config.sync_config);
    ...
    static util::UniqueFunction<void(std::optional<networking::NetworkError>)>
    ...
    void download_fresh_barq(const sync::SessionErrorInfo& error_info)
        REQUIRES(!m_config_mutex, !m_state_mutex, !m_connection_state_mutex);
    void handle_fresh_barq_downloaded(DBRef db, Status result, const sync::SessionErrorInfo& cr_error_info,
    ...
    std::string get_barq_connection_id() const REQUIRES(!m_state_mutex);
    ...
    BarqConfig m_config GUARDED_BY(m_config_mutex);
    ...
    // The fully-resolved URL of this Barq, including the server and the path.
...
#endif // BARQ_OS_SYNC_SESSION_HPP
