...
char decoded_char_for(const std::string& percent_encoding, size_t index)
{
    ...
    BARQ_ASSERT(percent_encoding[index] == '%');
...
{
    BARQ_ASSERT_DEBUG(template_string.find("XXXXXX") != std::string::npos);
...
SyncFileManager::SyncFileManager(std::string base_file_path, std::string app_id)
...
void SyncFileManager::remove_user_barqs(const std::string& user_id) const
{
...
bool SyncFileManager::remove_barq(const std::string& absolute_path) const
{
    BARQ_ASSERT(absolute_path.length() > 0);
    bool success = true;
    try {
        constexpr bool delete_lockfile = true;
        barq::DB::delete_files(absolute_path, &success, delete_lockfile);
    }
...
bool SyncFileManager::copy_barq_file(const std::string& old_path, const std::string& new_path) const
{
    BARQ_ASSERT(old_path.length() > 0);
    try {
...
bool SyncFileManager::remove_barq(const std::string& user_id, const std::vector<std::string>& legacy_user_identities,
                                   const std::string& raw_barq_path, const std::string& partition) const
{
    auto existing = get_existing_barq_file_path(user_id, legacy_user_identities, raw_barq_path, partition);
    if (existing) {
        return remove_barq(*existing);
    }
...
SyncFileManager::get_existing_barq_file_path(const std::string& user_id,
                                              const std::vector<std::string>& legacy_user_identities,
                                              const std::string& barq_file_name, const std::string& partition) const
{
    std::string preferred_name_without_suffix = preferred_barq_path_without_suffix(user_id, barq_file_name);
    if (try_file_exists(preferred_name_without_suffix)) {
        return preferred_name_without_suffix;
    }

    std::string preferred_name_with_suffix = preferred_name_without_suffix + c_barq_file_suffix;
    if (try_file_exists(preferred_name_with_suffix)) {
        return preferred_name_with_suffix;
    }

    // Shorten the Barq path to just `<rootDir>/<hashedAbsolutePath>.barq`
    std::string hashed_name = fallback_hashed_barq_file_path(preferred_name_without_suffix);
    std::string hashed_path = hashed_name + c_barq_file_suffix;
    ...
    for (auto& legacy_identity : legacy_user_identities) {
        ...
        std::string old_path = legacy_barq_file_path(legacy_identity, barq_file_name);
...
std::string SyncFileManager::barq_file_path(const std::string& user_id,
                                             const std::vector<std::string>& legacy_user_identities,
                                             const std::string& barq_file_name, const std::string& partition) const
{
    auto existing_path = get_existing_barq_file_path(user_id, legacy_user_identities, barq_file_name, partition);
    if (existing_path) {
    ...
    std::string preferred_name_with_suffix = preferred_name_without_suffix + c_barq_file_suffix;
    try {
        std::string test_path = preferred_name_without_suffix + c_barq_file_test_suffix;
        ...
        std::string hashed_path = hashed_name + c_barq_file_suffix;
        try {
            std::string test_hashed_path = hashed_name + c_barq_file_test_suffix;
            ...
            throw LogicError(ErrorCodes::InvalidArgument,
                             util::format("A valid barq path cannot be created for the "
                                          "Barq identity '%1' at neither '%2' nor '%3'. %4",
                                          barq_file_name, preferred_name_with_suffix, hashed_path, e_hashed.what()));
        }
    }
...
std::string SyncFileManager::metadata_path() const
{
    auto dir_path = file_path_by_appending_component(get_utility_directory(), c_metadata_directory,
                                                     util::FilePathType::Directory);
    util::try_make_dir(dir_path);
    return util::file_path_by_appending_component(dir_path, c_metadata_barq);
}

bool SyncFileManager::remove_metadata_barq() const
{
    auto dir_path = file_path_by_appending_component(get_utility_directory(), c_metadata_directory,
                                                     util::FilePathType::Directory);
    try {
...
std::string SyncFileManager::preferred_barq_path_without_suffix(const std::string& user_id,
                                                                 const std::string& barq_file_name) const
{
    auto escaped_file_name = util::validate_and_clean_path(barq_file_name);
    std::string preferred_name = util::file_path_by_appending_component(user_directory(user_id), escaped_file_name);
    if (StringData(preferred_name).ends_with(c_barq_file_suffix)) {
        preferred_name = preferred_name.substr(0, preferred_name.size() - strlen(c_barq_file_suffix));
    }
    return preferred_name;
}

std::string SyncFileManager::fallback_hashed_barq_file_path(const std::string& preferred_path) const
{
                                                          ...
                                                          const std::string& partition) const
{
    ...
    std::string legacy_partition_path = util::file_path_by_appending_component(
        get_user_directory_path(user_id), legacy_hashed_file_name + c_barq_file_suffix);
    return legacy_partition_path;
}

std::string SyncFileManager::legacy_barq_file_path(const std::string& local_user_identity,
                                                    const std::string& barq_file_name) const
{
    auto path =
    ...
    path = util::file_path_by_appending_component(path, util::validate_and_clean_path(local_user_identity),
                                                  util::FilePathType::Directory);
    path = util::file_path_by_appending_component(path, util::validate_and_clean_path(barq_file_name));
    return path;
}
                                                        ...
                                                        const std::string& barq_file_name) const
{
    ...
    std::string path = path_name + c_barq_file_suffix;
...
std::string SyncFileManager::path_for_barq(const SyncConfig& config,
                                            std::optional<std::string> custom_file_name) const
{
    ...
    auto file_name = [&]() -> std::string {
        ...
        if (config.flx_sync_requested) {
            BARQ_ASSERT_DEBUG(config.partition_value.empty());
            return "flx_sync_default";
        }
        return string_from_partition(config.partition_value);
    }();
    auto path = barq_file_path(user->user_id(), user->legacy_identities(), file_name, config.partition_value);
    user->track_barq(path);
    return path;
}
