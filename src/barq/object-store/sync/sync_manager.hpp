...
#ifndef BARQ_OS_SYNC_MANAGER_HPP
#define BARQ_OS_SYNC_MANAGER_HPP

#include <barq/object-store/sync/sync_client_config.hpp>
...
public:
    ...
    // https://github.com/barq/barq-sync/blob/develop/src/barq/sync/client.hpp#L126 The user agent can only be set
    ...
    /// re-establish connectivity with the Barq Object Server. It is presumed that
    ...
    std::shared_ptr<SyncSession> get_session(std::shared_ptr<DB> db, const BarqConfig& config)
    ...
    // session is not guaranteed to stop immediately when a Barq is closed. Using this method
...
#endif // BARQ_OS_SYNC_MANAGER_HPP
