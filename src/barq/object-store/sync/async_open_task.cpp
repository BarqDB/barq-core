...
AsyncOpenTask::AsyncOpenTask(Private, std::shared_ptr<_impl::BarqCoordinator> coordinator,
                             std::shared_ptr<barq::SyncSession> session, bool db_first_open)
                                                   ...
                                                   Status status)
{
    ...
    SharedBarq shared_barq;
    try {
        shared_barq = coordinator->get_barq(util::Scheduler::make_dummy(), m_db_first_open);
    }
    catch (...) {
        async_open_complete(std::move(callback), coordinator, exception_to_status());
        return;
    }
    const auto subscription_set = shared_barq->get_latest_subscription_set();
    ...
    if (sub_state != sync::SubscriptionSet::State::Complete) {
        ...
        subscription_set.get_state_change_notification(sync::SubscriptionSet::State::Complete)
            .get_async([self, coordinator, callback = std::move(callback)](
                           StatusWith<barq::sync::SubscriptionSet::State> state) mutable {
...
void AsyncOpenTask::async_open_complete(AsyncOpenCallback&& callback,
                                        std::shared_ptr<_impl::BarqCoordinator> coordinator, Status status)
{
    {
        util::CheckedLockGuard lock(m_mutex);
    ...
    if (status.is_ok()) {
        ThreadSafeReference barq;
        try {
            barq = coordinator->get_unbound_barq();
        }
        catch (...) {
            return callback({}, std::current_exception());
        }
        return callback(std::move(barq), nullptr);
    }
