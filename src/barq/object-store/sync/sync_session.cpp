...
#include <barq/sync/network/websocket_error.hpp>
...
{
    BARQ_ASSERT(m_state != State::Active);
    ...
    if (m_migration_sentinel_query_version) {
        m_flx_subscription_store->get_by_version(*m_migration_sentinel_query_version)
            ...
            .get_async([=, weak_self = weak_from_this()](StatusWith<sync::SubscriptionSet::State> s) {
                ...
                BARQ_ASSERT(s.get_value() == sync::SubscriptionSet::State::Complete);
...
{
    BARQ_ASSERT(m_state != State::Dying);
    m_state = State::Dying;
...
{
    BARQ_ASSERT(m_state != State::Inactive);
...
{
    BARQ_ASSERT(m_state != State::Paused);
...
{
    BARQ_ASSERT(m_state != State::WaitingForAccessToken);
    ...
    return [weak_session, restart_session](std::optional<networking::NetworkError> error) {
        ...
        if (!session_user) {
            ...
            if (ErrorCodes::error_categories(error->code()).test(ErrorCategory::client_error)) {
                ...
                // happen here.
                util::CheckedUniqueLock lock(session->m_state_mutex);
                // If updating access token while opening barq, just become active at this point
...
SyncSession::SyncSession(Private, SyncClient& client, std::shared_ptr<DB> db, const BarqConfig& config,
...
{
    BARQ_ASSERT(m_config.sync_config);
    ...
    // After a migration to FLX, if the user opens the barq with a flexible sync configuration, we need to first
    ...
    if (m_migration_store->is_migrated() && m_original_sync_config->flx_sync_requested) {
        ...
        BARQ_ASSERT(m_migration_sentinel_query_version);
    }
}
...
void SyncSession::update_error_and_mark_file_for_deletion(SyncError& error, ShouldBackup should_backup)
{
    ...
    // Add a SyncFileActionMetadata marking the Barq as needing to be deleted.
    auto original_path = path();
    error.user_info[SyncError::c_original_file_path_key] = original_path;
    using Action = SyncFileAction;
    auto action = should_backup == ShouldBackup::yes ? Action::BackUpThenDeleteBarq : Action::DeleteBarq;
...
void SyncSession::download_fresh_barq(const sync::SessionErrorInfo& error_info)
{
    ...
    try {
        ...
        handle_fresh_barq_downloaded(nullptr, exception_to_status(), error_info);
        return;
    }
    ...
    // the fresh Barq may apply writes to this db after it has outlived its sync session
    ...
    // The fresh barq uses flexible sync.
    if (auto fresh_sub_store = fresh_sync_session->get_flx_subscription_store()) {
        auto fresh_sub = fresh_sub_store->get_latest();
        // The local barq uses flexible sync as well so copy the active subscription set to the fresh barq.
        ...
        fresh_sub.get_state_change_notification(SubscriptionState::Complete)
            .then([=](SubscriptionState) -> util::Future<sync::SubscriptionSet> {
                ...
                // fresh_sync_session is using a new barq file that doesn't have the migration_store info
                ...
                BARQ_ASSERT(query_string);
                // Create subscriptions in the fresh barq based on the schema instructions received in the bootstrap
                // message.
            ...
            .get_async([=](StatusWith<sync::SubscriptionSet>&& subs) {
                ...
                if (subs.is_ok()) {
                    self->handle_fresh_barq_downloaded(db, Status::OK(), error_info, std::move(subs.get_value()));
                }
                else {
                    self->handle_fresh_barq_downloaded(nullptr, std::move(subs.get_status()), error_info);
                }
            });
    }
    else { // pbs
        fresh_sync_session->wait_for_download_completion([=, weak_self = weak_from_this()](Status status) {
            ...
            if (auto strong_self = weak_self.lock()) {
                if (status.is_ok()) {
                    strong_self->handle_fresh_barq_downloaded(db, Status::OK(), error_info);
                }
                else {
                    strong_self->handle_fresh_barq_downloaded(nullptr, std::move(status), error_info);
                }
            }
        });
    }
...
void SyncSession::handle_fresh_barq_downloaded(DBRef db, Status result, const sync::SessionErrorInfo& cr_error_info,
...
util::Future<void> SyncSession::pause_async()
{
    ...
    // sync::Client::notify_session_terminated() in order to be notified when the Barq file is closed. This works
    ...
    if (error.status == ErrorCodes::AutoClientResetFailed) {
        ...
        switch (error.server_requests_action) {
            case sync::ProtocolErrorInfo::Action::NoAction:
                BARQ_UNREACHABLE(); // This is not sent by the server
            ...
            case sync::ProtocolErrorInfo::Action::Transient:
                ...
                return;
            case sync::ProtocolErrorInfo::Action::DeleteBarq:
                next_state = NextStateAfterError::inactive;
            ...
            case sync::ProtocolErrorInfo::Action::MigrateToFLX:
                ...
                BARQ_ASSERT(!m_original_sync_config->flx_sync_requested);
                BARQ_ASSERT(error.migration_query_string && !error.migration_query_string->empty());
...
make_client_reset_config(const BarqConfig& base_config, const std::shared_ptr<SyncConfig>& sync_config,
                         DBRef&& fresh_copy, sync::SessionErrorInfo&& error_info, bool schema_migration_detected)
{
    BARQ_ASSERT(sync_config->client_resync_mode != ClientResyncMode::Manual);
    ...
    // Currently, a schema migration involves breaking changes so opening the barq
    // with the new schema results in a crash.
    if (schema_migration_detected)
        return config;

    BarqConfig barq_config = base_config;
    barq_config.sync_config = std::make_shared<SyncConfig>(*sync_config); // deep copy
    barq_config.scheduler = util::Scheduler::make_dummy();

    if (sync_config->notify_after_client_reset) {
        config.notify_after_client_reset = [barq_config](VersionID previous_version, bool did_recover) {
            auto coordinator = _impl::BarqCoordinator::get_coordinator(barq_config);
            ThreadSafeReference active_after = coordinator->get_unbound_barq();
            SharedBarq frozen_before = coordinator->get_barq(barq_config, previous_version);
            BARQ_ASSERT(frozen_before);
            BARQ_ASSERT(frozen_before->is_frozen());
            barq_config.sync_config->notify_after_client_reset(std::move(frozen_before), std::move(active_after),
                                                                did_recover);
        };
    }
    config.notify_before_client_reset = [config = std::move(barq_config)]() -> VersionID {
        // Opening the Barq live here may make a write if the schema is different
        // than what exists on disk. It is necessary to pass a fully usable Barq
        ...
        auto before = Barq::get_shared_barq(config);
        if (auto& notify_before = config.sync_config->notify_before_client_reset) {
            notify_before(config.sync_config->freeze_before_reset_barq ? before->freeze() : before);
        }
        // Note that if the SDK wrote to the Barq (hopefully by requesting a
...
void SyncSession::create_sync_session()
{
    ...
    session_config.barq_identifier = sync_config.partition_value;
    ...
    session_config.fresh_barq_download = client_reset::is_fresh_path(m_config.path);
...

    {
        ...
        BARQ_ASSERT_EX(!sync_route.empty(), "Server URL cannot be empty");
...
void SyncSession::close(util::CheckedUniqueLock lock)
{
    switch (m_state) {
        ...
        case State::Inactive: {
            ...
            // the user from deleting the barq when it's in the paused/inactive state.
            if (m_sync_manager) {
                m_sync_manager->unregister_session(m_db->get_path());
            }
            m_state_mutex.unlock(lock);
...
{
    {
        ...
        // Barq file to be closed. This works so long as this SyncSession object remains in the
...
sync::SaltedFileIdent SyncSession::get_file_ident() const
{
    ...
    BARQ_ASSERT(repl);
    BARQ_ASSERT(dynamic_cast<sync::ClientReplication*>(repl));
...
std::string SyncSession::get_barq_connection_id() const
{
    util::CheckedLockGuard lk(m_state_mutex);
    if (!m_session) {
        return {};
    }
    return m_session->get_barq_connection_id();
}
...
void SyncSession::update_configuration(SyncConfig new_config)
{
    while (true) {
        ...
        BARQ_ASSERT(m_state == State::Inactive || m_state == State::Paused);
        BARQ_ASSERT(!m_session);
        BARQ_ASSERT(m_config.sync_config->user == new_config.user);
...
void SyncSession::update_subscription_store(bool flx_sync_requested, std::optional<sync::SubscriptionSet> new_subs)
{
    ...
    // If migrated to FLX, create subscriptions in the local barq to cover the existing data.
...
{
    BARQ_ASSERT(!m_flx_subscription_store);
...
void SyncSession::migrate_schema(util::UniqueFunction<void(Status)>&& callback)
{
    ...
    //  2. Once the sync client releases the barq file:
