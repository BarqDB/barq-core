...
#include <barq/object-store/sectioned_results.hpp>
...
Results::Results(SharedBarq r, Query q, DescriptorOrdering o)
    : m_barq(std::move(r))
    , m_query(std::move(q))
    ...
    , m_mode(Mode::Query)
    , m_mutex(m_barq && m_barq->is_frozen())
{
}

Results::Results(const Class& cls)
    : Results(cls.get_barq(), cls.get_table())
{
}


Results::Results(SharedBarq r, ConstTableRef table)
    : m_barq(std::move(r))
    , m_table(table)
    , m_table_view(m_table)
    , m_mode(Mode::Table)
...
Results::Results(std::shared_ptr<Barq> r, std::shared_ptr<CollectionBase> coll, util::Optional<Query> q,
                 SortDescriptor s)
    : m_barq(std::move(r))
...
Results::Results(std::shared_ptr<Barq> r, std::shared_ptr<CollectionBase> coll, DescriptorOrdering o)
    : m_barq(std::move(r))
    , m_table(coll->get_target_table())
    , m_descriptor_ordering(std::move(o))
    , m_collection(std::move(coll))
...
Results::Results(std::shared_ptr<Barq> r, TableView tv, DescriptorOrdering o)
    : m_barq(std::move(r))
...
{
    validate_read();
    if (!m_barq || !m_barq->is_in_transaction())
...
const ObjectSchema& Results::get_object_schema() const
{
    ...
    if (!object_schema) {
        BARQ_ASSERT(m_barq);
        auto it = m_barq->schema().find(get_object_type());
        BARQ_ASSERT(it != m_barq->schema().end());
...
void Results::ensure_up_to_date(EvaluateMode mode)
{
    if (m_update_policy == UpdatePolicy::Never) {
        BARQ_ASSERT(m_mode == Mode::TableView);
        return;
    }
    ...
    switch (m_mode) {
        ...
        case Mode::Collection: {
            ...
            if (m_notifier && m_notifier->get_list_indices(m_list_indices) && !m_barq->is_in_transaction())
                return;
        ...
        case Mode::Query:
            ...
            if (m_notifier && m_notifier->get_tableview(m_table_view)) {
                m_mode = Mode::TableView;
                if (auto audit = m_barq->audit_context())
                    audit->record_query(m_barq->read_transaction_version(), m_table_view);
                return;
            }
...
Mixed Results::get_any(size_t ndx)
{
    ...
    switch (m_mode) {
        ...
        case Mode::Collection:
            ...
            break;
        case Mode::Query:
            BARQ_UNREACHABLE(); // should always be in TV mode
        case Mode::TableView: {
...
{
    util::CheckedUniqueLock lock(m_mutex);
    BARQ_ASSERT(m_mode == Mode::Collection);
    ensure_up_to_date();
    if (size_t actual = actual_index(ndx); actual < m_collection->size()) {
        return List{m_barq, m_collection->get_list(m_collection->get_path_element(actual))};
    }
...
object_store::Dictionary Results::get_dictionary(size_t ndx)
{
    ...
    ensure_up_to_date();
    if (size_t actual = actual_index(ndx); actual < m_collection->size()) {
        return object_store::Dictionary{m_barq,
...
std::pair<StringData, Mixed> Results::get_dictionary_element(size_t ndx)
{
    ...
    BARQ_ASSERT(typeid(dict) == typeid(Dictionary));
...
Results Results::sort(std::vector<std::pair<std::string, bool>> const& keypaths) const
{
    ...
    for (auto& keypath : keypaths) {
        column_keys.push_back(parse_keypath(keypath.first, m_barq->schema(), &get_object_schema()));
...
Results Results::filter(Query&& q) const
{
    ...
    return Results(m_barq, get_query().and_query(std::move(q)), m_descriptor_ordering);
}
...
Results Results::distinct(std::vector<std::string> const& keypaths) const
{
    ...
    column_keys.reserve(keypaths.size());
    for (auto& keypath : keypaths)
        column_keys.push_back(parse_keypath(keypath, m_barq->schema(), &get_object_schema()));
...
void Results::prepare_async(ForCallback force) NO_THREAD_SAFETY_ANALYSIS
{
    ...
    if (m_notifier)
        return;
    if (!m_barq->verify_notifications_available(force))
        return;
    if (m_update_policy == UpdatePolicy::Never) {
        if (force)
    ...
    _impl::BarqCoordinator::register_notifier(m_notifier);
}
...
{
    BARQ_ASSERT(!m_barq || !m_barq->is_frozen());
    switch (m_mode) {
        case Mode::Empty:
        case Mode::Table:
            return true;
...
#define BARQ_RESULTS_TYPE(T)                                                                                        \
    template T Results::get<T>(size_t);                                                                              \
    template util::Optional<T> Results::first<T>();                                                                  \
    template util::Optional<T> Results::last<T>();

BARQ_RESULTS_TYPE(bool)
BARQ_RESULTS_TYPE(int64_t)
BARQ_RESULTS_TYPE(float)
BARQ_RESULTS_TYPE(double)
BARQ_RESULTS_TYPE(StringData)
BARQ_RESULTS_TYPE(BinaryData)
BARQ_RESULTS_TYPE(Timestamp)
BARQ_RESULTS_TYPE(ObjectId)
BARQ_RESULTS_TYPE(Decimal)
BARQ_RESULTS_TYPE(UUID)
BARQ_RESULTS_TYPE(Mixed)
BARQ_RESULTS_TYPE(Obj)
BARQ_RESULTS_TYPE(util::Optional<bool>)
BARQ_RESULTS_TYPE(util::Optional<int64_t>)
BARQ_RESULTS_TYPE(util::Optional<float>)
BARQ_RESULTS_TYPE(util::Optional<double>)
BARQ_RESULTS_TYPE(util::Optional<ObjectId>)
BARQ_RESULTS_TYPE(util::Optional<UUID>)

#undef BARQ_RESULTS_TYPE

Results Results::import_copy_into_barq(std::shared_ptr<Barq> const& barq)
{
    util::CheckedUniqueLock lock(m_mutex);
    ...
    switch (m_mode) {
        case Mode::Table:
            return Results(barq, barq->import_copy_of(m_table));
        case Mode::Collection:
            if (std::shared_ptr<CollectionBase> collection = barq->import_copy_of(*m_collection)) {
                return Results(barq, collection, m_descriptor_ordering);
            }
            // If collection is gone, fallback to empty selection on table.
            return Results(barq, TableView(barq->import_copy_of(m_table)));
            break;
        case Mode::Query:
            return Results(barq, *barq->import_copy_of(m_query, PayloadPolicy::Copy), m_descriptor_ordering);
        case Mode::TableView: {
            Results results(barq, *barq->import_copy_of(m_table_view, PayloadPolicy::Copy), m_descriptor_ordering);
...
Results Results::freeze(std::shared_ptr<Barq> const& frozen_barq)
{
    return import_copy_into_barq(frozen_barq);
}

bool Results::is_frozen() const
{
    return !m_barq || m_barq->is_frozen();
}
