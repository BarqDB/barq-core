...
Object Object::freeze(std::shared_ptr<Barq> frozen_barq) const
{
    return Object(frozen_barq, frozen_barq->import_copy_of(m_obj));
}
...
static const ObjectSchema* find_object_schema(Barq& barq, Key key)
{
    auto object_schema = barq.schema().find(key);
    BARQ_ASSERT(object_schema != barq.schema().end());
    return &*object_schema;
}

static const ObjectSchema* find_object_schema(Barq& barq, Obj const& o)
{
    auto table = o.get_table();
    if (!table) {
        return nullptr;
    }
    BARQ_ASSERT(&barq.read_group() == _impl::TableFriend::get_parent_group(*table));
    auto object_schema = barq.schema().find(ObjectStore::object_type_for_table_name(table->get_name()));
    BARQ_ASSERT(object_schema != barq.schema().end());
    return &*object_schema;
}

Object::Object(std::shared_ptr<Barq> r, const ObjectSchema* s, Obj const& o, Obj const& parent,
    ...
    , m_object_schema(s)
{
    if (auto audit = m_barq->audit_context())
        audit->record_read(m_barq->read_transaction_version(), m_obj, parent, incoming_column);
}

Object::Object(const std::shared_ptr<Barq>& r, ObjectSchema const& s, Obj const& o, Obj const& parent,
               ColKey incoming_column)
    : Object(r, &s, o, parent, incoming_column)
{
}

Object::Object(const std::shared_ptr<Barq>& r, Obj const& o)
    : Object(r, find_object_schema(*r, o), o)
{
}

template <typename Key>
Object::Object(const std::shared_ptr<Barq>& r, const ObjectSchema* s, Key key)
    : Object(r, s, r->read_group().get_table(s->table_key)->get_object(key))
{
}

Object::Object(const std::shared_ptr<Barq>& r, StringData object_type, ObjKey key)
    : Object(r, find_object_schema(*r, object_type), key)
{
}

Object::Object(const std::shared_ptr<Barq>& r, StringData object_type, size_t index)
    : Object(r, find_object_schema(*r, object_type), index)
{
}

Object::Object(const std::shared_ptr<Barq>& r, ObjLink link)
                                                    ...
                                                    std::optional<KeyPathArray> key_path_array) &
{
    ...
    if (!m_notifier) {
        m_notifier = std::make_shared<_impl::ObjectNotifier>(m_barq, m_obj);
