...
void KVOAdapter::before(Transaction& sg)
{
    ...
    for (auto& list : m_lists) {
        ...
        if (list.observer->changes.empty()) {
            BARQ_ASSERT_DEBUG(tables[list.observer->table_key].deletions_contains(list.observer->obj_key));
            continue;
        }
        // otherwise the column should have been marked as modified
        auto it = list.observer->changes.find(list.col_key.value);
        BARQ_ASSERT(it != list.observer->changes.end());
        ...
        // KVO can't express multiple types of changes at once
            ...
            BARQ_ASSERT(!builder.deletions.empty());
...
public:
    ...
    // Removing or renaming things while a Barq is open is never supported
                                ...
                                _impl::NotifierPackage& notifiers)
{
    ...
    if (observers.empty() && (!notifiers || notifiers.version())) {
        ...
        // Barq. Just return early if that happens.
...
UnsupportedSchemaChange::UnsupportedSchemaChange()
    : std::logic_error(
          "Schema mismatch detected: another process has modified the Barq file's schema in an incompatible way")
{
}
