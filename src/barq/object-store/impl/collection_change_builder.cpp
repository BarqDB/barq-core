...
void CollectionChangeBuilder::move(size_t from, size_t to)
{
    ...
    for (auto& move : moves) {
        ...
        BARQ_ASSERT(!updated_existing_move);
...
#ifdef BARQ_DEBUG
    for (auto&& move : moves) {
        BARQ_ASSERT(deletions.contains(move.from));
        BARQ_ASSERT(insertions.contains(move.to));
    }
#endif
}
...
private:
    ...
    Match find_longest_match(size_t begin1, size_t end1, size_t begin2, size_t end2)
    {
        ...
        auto for_each_b_match = [&](size_t i, auto&& f) {
            ...
            BARQ_ASSERT(it != end(b) && it->key == ai);
        ...
        for (size_t i = begin1; i < end1; ++i) {
            ...
            for_each_b_match(i, [&](size_t j) {
                ...
                BARQ_ASSERT(best.i >= begin1 && best.i + best.size <= end1);
                BARQ_ASSERT(best.j >= begin2 && best.j + best.size <= end2);
            });
        }
        return best;
    }
