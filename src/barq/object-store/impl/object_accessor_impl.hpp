...
#ifndef BARQ_OS_OBJECT_ACCESSOR_IMPL_HPP
#define BARQ_OS_OBJECT_ACCESSOR_IMPL_HPP
...
public:
    ...
    CppContext(CppContext& c, Obj parent, Property const& prop)
        : barq(c.barq)
        , object_schema(prop.type == PropertyType::Object ? &*barq->schema().find(prop.object_type)
                                                          : c.object_schema)
    ...
    CppContext() = default;
    CppContext(std::shared_ptr<Barq> barq, const ObjectSchema* os = nullptr)
        : barq(std::move(barq))
        , object_schema(os)
    {
    }
                                                ...
                                                size_t /* property_index */) const
    {
        ...
        if (auto geo = std::any_cast<Geospatial>(&dict)) {
            ...
            BARQ_ASSERT_EX(false, prop.name); // unexpected property type
        }
#endif
    ...
    // `create` is false then when given something which is not a managed Barq
    // object `unbox()` should simply return a detached obj, while if it's
    // true then `unbox()` should create a new object in the context's Barq
...
inline Obj CppContext::unbox(std::any& v, CreatePolicy policy, ObjKey current_obj) const
{
    ...
    BARQ_ASSERT(object_schema);
    return Object::create(const_cast<CppContext&>(*this), barq, *object_schema, v, policy, current_obj).get_obj();
}
...
inline Mixed CppContext::unbox(std::any& v, CreatePolicy policy, ObjKey) const
{
    if (v.has_value()) {
        ...
        if (this_type == typeid(Mixed)) {
            ...
            auto os = barq->schema().find(unmanaged_obj.object_type);
            CppContext child_ctx(barq, &*os);
...
#endif // BARQ_OS_OBJECT_ACCESSOR_IMPL_HPP
