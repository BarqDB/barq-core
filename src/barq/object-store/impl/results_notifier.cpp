...
// by BarqCoordinator using the "notifier lock" which also guards registering
...
ResultsNotifier::ResultsNotifier(Results& target)
    : ResultsNotifierBase(target.get_barq())
...
void ResultsNotifier::run()
{
...

    {
        ...
        if (!get_barq() || (!have_callbacks() && !m_results_were_used))
            return;
    }
...
void ResultsNotifier::do_prepare_handover(Transaction& sg)
{
    ...
    if (m_run_tv.is_attached()) {
        BARQ_ASSERT(m_run_tv.is_in_sync());
        if (!m_handover_transaction)
...
bool ResultsNotifier::prepare_to_deliver()
{
    ...
    if (!m_handover_tv) {
        bool transaction_is_stale =
            m_delivered_transaction &&
            (!barq->is_in_read_transaction() ||
             barq->read_transaction_version() > m_delivered_transaction->get_version_of_current_transaction());
...
{
    BARQ_ASSERT(target.get_type() != PropertyType::Object);
    ...
    if (m_change.paths.size()) {
        if (auto coll = dynamic_cast<CollectionParent*>(m_list.get())) {
            for (auto& p : m_change.paths) {
                if (auto ndx = coll->find_index(p); ndx != barq::not_found)
