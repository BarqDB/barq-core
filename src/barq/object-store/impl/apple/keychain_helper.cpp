...
#include <barq/object-store/impl/apple/keychain_helper.hpp>
...
const CFStringRef s_service = CFSTR("io.barq.sync.keychain");
...
void delete_key(CFStringRef account, CFStringRef service, CFStringRef group)
{
    ...
    BARQ_ASSERT(status == errSecSuccess || status == errSecItemNotFound);
}

CFPtr<CFStringRef> bundle_service()
{
    if (CFStringRef bundle_id = CFBundleGetIdentifier(CFBundleGetMainBundle())) {
        return adoptCF(CFStringCreateWithFormat(nullptr, nullptr, CFSTR("%@ - Barq Sync Metadata Key"), bundle_id));
    }
                                                                 ...
                                                                 std::string_view access_group)
{
    ...
    //
    // The original location was (account: "metadata", service: "io.barq.sync.keychain").
    // For processes with a bundle ID, we then switched to (account: "metadata",
    // service: "$bundleId - Barq Sync Metadata Key")
    // The current preferred location on non-macOS (account: appId, service: "io.barq.sync.keychain"),
    // and on macOS is (account: appId, service: "$bundleId - Barq Sync Metadata Key").
    //
    ...
    // application using Barq to have separate state. Using multiple server apps
    // in one client is unusual, but when it's done we want each metadata barq to
    // have a separate key.
...
std::optional<std::vector<char>> create_new_metadata_barq_key(std::string_view app_id, std::string_view access_group)
{
    auto cf_app_id = string_view_to_cfstring(app_id);
    std::optional<std::vector<char>> key;
