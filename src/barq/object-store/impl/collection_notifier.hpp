...
#ifndef BARQ_BACKGROUND_COLLECTION_HPP
#define BARQ_BACKGROUND_COLLECTION_HPP
...
public:
    CollectionNotifier(std::shared_ptr<Barq>);
    ...
    // ------------------------------------------------------------------------
    // API for BarqCoordinator to manage running things and calling callbacks

    bool is_for_barq(Barq&) const noexcept;
    Barq* get_barq() const noexcept
    {
        return m_barq.get();
    }

    // precondition: BarqCoordinator::m_notifier_mutex is locked
    ...
    // precondition: BarqCoordinator::m_notifier_mutex is unlocked
    void before_advance() REQUIRES(!m_callback_mutex);
    void after_advance() REQUIRES(!m_callback_mutex);

    bool is_alive() const noexcept;

    // precondition: BarqCoordinator::m_notifier_mutex is locked *or* is called on worker thread
    bool has_run() const noexcept
    {
        return m_has_run;
    }

    // Detach from the source Barq's transaction and attach to either an existing
...
private:
    ...
    mutable std::mutex m_barq_mutex;
...
// A package of CollectionNotifiers for a single Barq instance which is passed
...
public:
    ...
    NotifierPackage(std::vector<std::shared_ptr<CollectionNotifier>> notifiers, BarqCoordinator* coordinator);
...
private:
    ...
    BarqCoordinator* m_coordinator = nullptr;
};
...
#endif /* BARQ_BACKGROUND_COLLECTION_HPP */
