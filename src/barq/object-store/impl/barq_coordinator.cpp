...
#include <barq/object-store/impl/transact_log_handler.hpp>
...
static auto& s_coordinators_per_path = *new std::unordered_map<std::string, std::weak_ptr<BarqCoordinator>>;

std::shared_ptr<BarqCoordinator> BarqCoordinator::get_coordinator(StringData path)
{
    ...
    auto coordinator = std::make_shared<BarqCoordinator>(Private());
    weak_coordinator = coordinator;
    return coordinator;
}

std::shared_ptr<BarqCoordinator>
BarqCoordinator::get_coordinator(const Barq::Config& config) NO_THREAD_SAFETY_ANALYSIS
{
    auto coordinator = get_coordinator(config.path);
    util::CheckedLockGuard lock(coordinator->m_barq_mutex);
    coordinator->set_config(config);
    coordinator->open_db();
    return coordinator;
}

std::shared_ptr<BarqCoordinator> BarqCoordinator::get_existing_coordinator(StringData path)
{
    std::lock_guard<std::mutex> lock(s_coordinator_mutex);
    return s_coordinators_per_path[path].lock();
}

void BarqCoordinator::set_config(const Barq::Config& config)
{
    ...
    if (config.schema_mode == SchemaMode::Immutable && config.sync_config)
        throw InvalidArgument(ErrorCodes::IllegalCombination,
                              "Synchronized Barqs cannot be opened in immutable mode");
    ...
    if (config.in_memory && !config.encryption_key.empty()) {
        throw InvalidArgument(ErrorCodes::IllegalCombination, "Encryption is not supported for in-memory barqs");
    }
    ...
    if (config.sync_config) {
        ...
        if (!config.sync_config->user) {
            throw InvalidArgument(ErrorCodes::IllegalCombination,
                                  "A user must be provided to open a synchronized Barq.");
        }
    }
#endif

    bool no_existing_barq =
        std::all_of(begin(m_weak_barq_notifiers), end(m_weak_barq_notifiers), [](auto& notifier) {
    ...
    if (no_existing_barq) {
        ...
        if (m_config.immutable() != config.immutable()) {
            throw LogicError(
                ...
                util::format("Barq at path '%1' already opened with different read permissions.", config.path));
        }
        if (m_config.in_memory != config.in_memory) {
            throw LogicError(
                ErrorCodes::MismatchedConfig,
                util::format("Barq at path '%1' already opened with different inMemory settings.", config.path));
        }
        if (m_config.encryption_key != config.encryption_key) {
            throw LogicError(
                ErrorCodes::MismatchedConfig,
                util::format("Barq at path '%1' already opened with a different encryption key.", config.path));
        }
        if (m_config.schema_mode != config.schema_mode) {
            throw LogicError(
                ErrorCodes::MismatchedConfig,
                util::format("Barq at path '%1' already opened with a different schema mode.", config.path));
        }
        ...
        if (config.schema && m_schema_version != ObjectStore::NotVersioned &&
            ...
            throw LogicError(
                ErrorCodes::MismatchedConfig,
                util::format("Barq at path '%1' already opened with different schema version.", config.path));
        }
        ...
        if (bool(m_config.sync_config) != bool(config.sync_config)) {
            throw LogicError(
                ErrorCodes::MismatchedConfig,
                util::format("Barq at path '%1' already opened with different sync configurations.", config.path));
        }
        ...
        if (config.sync_config) {
            ...
            if (old_user != new_user) {
                throw LogicError(
                    ErrorCodes::MismatchedConfig,
                    util::format("Barq at path '%1' already opened with different sync user.", config.path));
            }
            if (m_config.sync_config->partition_value != config.sync_config->partition_value) {
                throw LogicError(
                    ErrorCodes::MismatchedConfig,
                    util::format("Barq at path '%1' already opened with different partition value.", config.path));
            }
            if (m_config.sync_config->flx_sync_requested != config.sync_config->flx_sync_requested) {
                throw LogicError(ErrorCodes::MismatchedConfig,
                                 util::format("Barq at path '%1' already opened in a different synchronization mode",
                                              config.path));
            }
        }
#endif
        // Mixing cached and uncached Barqs is allowed
        m_config.cache = config.cache;

        // Barq::update_schema() handles complaining about schema mismatches
    }
}

std::shared_ptr<Barq> BarqCoordinator::get_cached_barq(Barq::Config const& config,
                                                          std::shared_ptr<util::Scheduler> scheduler)
{
    if (!config.cache)
        return nullptr;
    util::CheckedUniqueLock lock(m_barq_mutex);
    return do_get_cached_barq(config, scheduler);
}

std::shared_ptr<Barq> BarqCoordinator::do_get_cached_barq(Barq::Config const& config,
                                                             std::shared_ptr<util::Scheduler> scheduler)
{
    if (!config.cache)
        return nullptr;
    ...
    for (auto& cached_barq : m_weak_barq_notifiers) {
        if (!cached_barq.is_cached_for_scheduler(scheduler))
            continue;
        // can be null if we jumped in between ref count hitting zero and
        // unregister_barq() getting the lock
        if (auto barq = cached_barq.barq()) {
            // If the file is uninitialized and was opened without a schema,
            // do the normal schema init
            if (barq->schema_version() == ObjectStore::NotVersioned)
                break;

            // Otherwise if we have a barq schema it needs to be an exact
            // match (even having the same properties but in different
            // orders isn't good enough)
            if (config.schema && barq->schema() != *config.schema)
                throw LogicError(
                    ErrorCodes::MismatchedConfig,
                    util::format("Barq at path '%1' already opened on current thread with different schema.",
                                 config.path));
...
std::shared_ptr<Barq> BarqCoordinator::get_barq(Barq::Config config, util::Optional<VersionID> version)
{
    BARQ_ASSERT(!version || *version != VersionID());
    if (!config.scheduler)
        config.scheduler = version ? util::Scheduler::make_frozen(*version) : util::Scheduler::make_default();
    // barq must be declared before lock so that the mutex is released before
    // we release the strong reference to barq, as Barq's destructor may want
    // to acquire the same lock
    std::shared_ptr<Barq> barq;
    util::CheckedUniqueLock lock(m_barq_mutex);
    set_config(config);
    if ((barq = do_get_cached_barq(config))) {
        BARQ_ASSERT(!version || barq->read_transaction_version() == *version);
        return barq;
    }
    do_get_barq(std::move(config), barq, version, lock);
    if (version) {
        barq->read_group();
    }
    return barq;
...
std::shared_ptr<Barq> BarqCoordinator::get_barq(std::shared_ptr<util::Scheduler> scheduler, bool first_time_open)
{
    std::shared_ptr<Barq> barq;
    util::CheckedUniqueLock lock(m_barq_mutex);
    ...
    do_get_barq(std::move(config), barq, none, lock, first_time_open);
    return barq;
}

std::shared_ptr<Barq> BarqCoordinator::freeze_barq(const Barq& source_barq)
{
    std::shared_ptr<Barq> barq;
    util::CheckedUniqueLock lock(m_barq_mutex);

    auto version = source_barq.read_transaction_version();
    auto scheduler = util::Scheduler::make_frozen(version);
    if ((barq = do_get_cached_barq(source_barq.config(), scheduler))) {
        return barq;
    }

    auto config = source_barq.config();
    config.scheduler = scheduler;
    barq = Barq::make_shared_barq(std::move(config), version, shared_from_this());
    Barq::Internal::copy_schema(*barq, source_barq);
    m_weak_barq_notifiers.emplace_back(barq, config.cache);
    return barq;
}

ThreadSafeReference BarqCoordinator::get_unbound_barq()
{
    std::shared_ptr<Barq> barq;
    util::CheckedUniqueLock lock(m_barq_mutex);
    do_get_barq(BarqConfig(m_config), barq, none, lock);
    return ThreadSafeReference(barq);
}

void BarqCoordinator::do_get_barq(BarqConfig&& config, std::shared_ptr<Barq>& barq,
                                    util::Optional<VersionID> version, util::CheckedUniqueLock& barq_lock,
                                    bool first_time_open)
{
    const auto db_created = open_db();
#ifdef BARQ_ENABLE_SYNC
    ...
    if (barq->config().audit_config) {
        if (m_audit_context)
            m_audit_context->update_metadata(barq->config().audit_config->metadata);
        else
            m_audit_context = make_audit_context(m_db, barq->config());
    }
#else
    if (barq->config().audit_config)
        BARQ_TERMINATE("Cannot use Audit interface if Barq Core is built without Sync");
#endif

    // Cached frozen Barqs need to initialize their schema before releasing
    ...
    // transaction, so unlike with live Barqs this is safe to do.
    if (config.cache && version && schema) {
        barq->update_schema(std::move(*schema));
        schema.reset();
    }

    barq_lock.unlock_unchecked();
    if (schema) {
        barq->update_schema(std::move(*schema), config.schema_version, std::move(migration_function),
    ...
    // 1. this is the first time we are creating the barq file
    ...
    if (subscription_function) {
        const auto current_subscription = barq->get_latest_subscription_set();
        ...
        // we are actually creating a barq. For async open this does not apply, in fact db_created
        // will always be false.
        if (!first_time_open)
            first_time_open = db_created;
        if (subscription_version == 0 || (first_time_open && rerun_on_open)) {
            bool was_in_read = barq->is_in_read_transaction();
            subscription_function(barq);
            if (!was_in_read)
                barq->invalidate();
        }
    }
...
void BarqCoordinator::bind_to_context(Barq& barq)
{
    util::CheckedLockGuard lock(m_barq_mutex);
    for (auto& cached_barq : m_weak_barq_notifiers) {
        if (!cached_barq.is_for_barq(&barq))
            continue;
        cached_barq.bind_to_scheduler();
        return;
    }
    BARQ_TERMINATE("Invalid Barq passed to bind_to_context()");
}

#if BARQ_ENABLE_SYNC
std::shared_ptr<AsyncOpenTask> BarqCoordinator::get_synchronized_barq(Barq::Config config)
{
    if (!config.sync_config)
        throw LogicError(ErrorCodes::IllegalOperation,
                         "This method is only available for fully synchronized Barqs.");
...
bool BarqCoordinator::open_db()
{
    ...
    if (m_config.sync_config) {
        BARQ_ASSERT(m_config.sync_config->user);
        // If we previously opened this Barq, we may have a lingering sync
        // session which outlived its BarqCoordinator. If that happens we
    ...
    try {
        if (m_config.immutable() && m_config.barq_data) {
            m_db = DB::create(m_config.barq_data, false);
            return true;
        }
...
#else
            BARQ_TERMINATE("Barq was not built with sync enabled");
#endif
        }
        else if (!m_config.immutable()) {
            history = make_in_barq_history();
        }
        ...
        if (history) {
            ...
            // or teach BarqConfig and SDKs about pure in-memory barqs.
    ...
    catch (barq::FileFormatUpgradeRequired const&) {
...
void BarqCoordinator::init_external_helpers()
{
...
void BarqCoordinator::close()
{
    m_db->close();
    m_db = nullptr;
}

void BarqCoordinator::delete_and_reopen()
{
    util::CheckedLockGuard lock(m_barq_mutex);
    close();
    util::File::remove(m_config.path);
...
TransactionRef BarqCoordinator::begin_read(VersionID version, bool frozen_transaction)
{
    BARQ_ASSERT(m_db);
    return frozen_transaction ? m_db->start_frozen(version) : m_db->start_read(version);
}

uint64_t BarqCoordinator::get_schema_version() const noexcept
{
    util::CheckedLockGuard lock(m_schema_cache_mutex);
    return m_schema_version;
}

bool BarqCoordinator::get_cached_schema(Schema& schema, uint64_t& schema_version,
...
void BarqCoordinator::cache_schema(Schema const& new_schema, uint64_t new_schema_version,
...
void BarqCoordinator::clear_schema_cache_and_set_schema_version(uint64_t new_schema_version)
{
    util::CheckedLockGuard lock(m_schema_cache_mutex);
...
void BarqCoordinator::advance_schema_cache(uint64_t previous, uint64_t next)
{
    util::CheckedLockGuard lock(m_schema_cache_mutex);
    if (!m_cached_schema)
        return;
    BARQ_ASSERT(previous <= m_schema_transaction_version_max);
...
BarqCoordinator::BarqCoordinator(Private) {}

BarqCoordinator::~BarqCoordinator()
{
    {
...
void BarqCoordinator::unregister_barq(Barq* barq)
{
    util::CheckedLockGuard lock(m_barq_mutex);
    ...
    {
        auto new_end = remove_if(begin(m_weak_barq_notifiers), end(m_weak_barq_notifiers), [=](auto& notifier) {
            return notifier.expired() || notifier.is_for_barq(barq);
        });
        m_weak_barq_notifiers.erase(new_end, end(m_weak_barq_notifiers));
    }
}

// Thread-safety analysis doesn't reasonably handle calling functions on different
// instances of this type
void BarqCoordinator::clear_cache() NO_THREAD_SAFETY_ANALYSIS
{
    std::vector<std::shared_ptr<BarqCoordinator>> coordinators;
    {
        std::lock_guard<std::mutex> lock(s_coordinator_mutex);
    ...
    for (auto& coordinator : coordinators) {
        ...
        std::vector<std::shared_ptr<Barq>> barqs_to_close;
        {
            // Gather a list of all of the barqs which will be removed
            util::CheckedLockGuard lock(coordinator->m_barq_mutex);
            for (auto& weak_barq_notifier : coordinator->m_weak_barq_notifiers) {
                if (auto barq = weak_barq_notifier.barq()) {
                    barqs_to_close.push_back(barq);
                }
            }
        }

        // Close all of the previously cached Barqs. This can't be done while
        // locks are held as it may try to re-lock them.
        for (auto& barq : barqs_to_close)
            barq->close();
    }
}

void BarqCoordinator::clear_all_caches()
{
    std::vector<std::weak_ptr<BarqCoordinator>> to_clear;
    {
        std::lock_guard<std::mutex> lock(s_coordinator_mutex);
...
void BarqCoordinator::assert_no_open_barqs() noexcept
{
#ifdef BARQ_DEBUG
    std::lock_guard<std::mutex> lock(s_coordinator_mutex);
    BARQ_ASSERT(s_coordinators_per_path.empty());
#endif
}

void BarqCoordinator::wake_up_notifier_worker()
{
    if (m_notifier) {
...
void BarqCoordinator::commit_write(Barq& barq, bool commit_to_disk)
{
    BARQ_ASSERT(!m_config.immutable());
    BARQ_ASSERT(barq.is_in_transaction());

    Transaction& tr = Barq::Internal::get_transaction(barq);
    VersionID new_version;
    {
        ...
        // transactions. If we have a notifier for the current Barq, then we
        ...
        bool have_notifiers = std::any_of(m_notifiers.begin(), m_notifiers.end(), [&](auto&& notifier) {
            return notifier->is_for_barq(barq) && notifier->have_callbacks();
        });
        if (have_notifiers) {
            BARQ_ASSERT(!m_notifier_skip_version);
            BARQ_ASSERT(m_notifier_transaction);
            BARQ_ASSERT_3(m_notifier_transaction->get_transact_stage(), ==, DB::transact_Reading);
            BARQ_ASSERT_3(m_notifier_transaction->get_version() + 1, ==, new_version.version);
            m_notifier_skip_version = tr.duplicate();
        }
    }

    if (barq.m_binding_context) {
        barq.m_binding_context->did_change({}, {});
    }
    // note: no longer safe to access `barq` or `this` after this point as
    // did_change() may have closed the Barq.
}

void BarqCoordinator::enable_wait_for_change()
{
    m_db->enable_wait_for_change();
}

bool BarqCoordinator::wait_for_change(std::shared_ptr<Transaction> tr)
{
    return m_db->wait_for_change(tr);
}

void BarqCoordinator::wait_for_change_release()
{
    m_db->wait_for_change_release();
}

bool BarqCoordinator::can_advance(Barq& barq)
{
    return barq.last_seen_transaction_version() != m_db->get_version_of_latest_snapshot();
}

// Thread-safety analysis doesn't reasonably handle calling functions on different
// instances of this type
void BarqCoordinator::register_notifier(std::shared_ptr<CollectionNotifier> notifier) NO_THREAD_SAFETY_ANALYSIS
{
    auto& self = Barq::Internal::get_coordinator(*notifier->get_barq());
    {
...
void BarqCoordinator::clean_up_dead_notifiers()
{
...
void BarqCoordinator::on_commit(DB::version_type)
{
    if (m_notifier) {
        m_notifier->notify_others();
    }
...
void BarqCoordinator::on_change()
{
#if BARQ_ENABLE_SYNC
    // Invoke barq sync if another process has notified for a change
    if (m_sync_session) {
    ...
    util::CheckedLockGuard lock(m_barq_mutex);
    for (auto& barq : m_weak_barq_notifiers) {
        barq.notify();
    }
}

void BarqCoordinator::run_async_notifiers()
{
    ...
    if (version != m_notifier_transaction->get_version_of_current_transaction()) {
        ...
        // adding a new notifier in a different process sharing this Barq file,
        // closing the Barq in a different process, and possibly some other cases.
        notifiers = m_notifiers;
    }
    else {
        BARQ_ASSERT(!skip_version);
    ...
    if (skip_version && skip_version->get_version_of_current_transaction() != version) {
        BARQ_ASSERT(!notifiers.empty());
        BARQ_ASSERT(version >= skip_version->get_version_of_current_transaction());
...
void BarqCoordinator::advance_to_ready(Barq& barq)
{
    // If callbacks close the Barq the last external reference may go away
    // while we're in this function
    auto self = shared_from_this();
    auto tr = Barq::Internal::get_transaction_ref(barq);
    ...
    std::shared_ptr<Transaction> handover_version_tr;
    {
        ...
        // If there are any new notifiers for this Barq then by definition they
        // haven't run yet and aren't ready
        for (auto& notifier : m_new_notifiers) {
            if (notifier->is_for_barq(barq))
                return;
        }
    ...
    if (notifiers.empty()) {
        // If we have no notifiers for this Barq, just advance to latest
        return transaction::advance(tr, barq.m_binding_context.get(), {});
    }
    ...
    // If the most recent write was performed via the Barq instance being
    // advanced, the notifiers can be at an older version than the Barq.
    ...
    // on this Barq instance. There might also be a newer version but we ignore
    // it if so.
    if (notifier_version == current_version) {
        if (barq.m_binding_context)
            barq.m_binding_context->will_send_notifications();
        if (barq.is_closed())
            return;
        for (auto& notifier : notifiers)
        ...
        if (barq.is_closed())
            return;
        if (barq.m_binding_context)
            barq.m_binding_context->did_send_notifications();
        return;
    }

    // We have notifiers for a newer version, so advance to that
    transaction::advance(tr, barq.m_binding_context.get(),
                         _impl::NotifierPackage(std::move(notifiers), handover_version_tr));
}

std::vector<std::shared_ptr<_impl::CollectionNotifier>> BarqCoordinator::notifiers_for_barq(Barq& barq)
{
    auto pred = [&](auto& notifier) {
        return notifier->is_for_barq(barq);
    };
...
bool BarqCoordinator::advance_to_latest(Barq& barq)
{
    // If callbacks close the Barq the last external reference may go away
    // while we're in this function
    auto self = shared_from_this();
    ...
    {
        util::CheckedUniqueLock lock(m_notifier_mutex);
        notifiers = notifiers_for_barq(barq);
    }
    ...
    transaction::advance(tr, barq.m_binding_context.get(), _impl::NotifierPackage(std::move(notifiers), pin_tr));
    return !barq.is_closed() && prev_version != tr->get_version_of_current_transaction();
}

void BarqCoordinator::promote_to_write(Barq& barq)
{
    BARQ_ASSERT(!barq.is_in_transaction());
    // If callbacks close the Barq the last external reference may go away
    // while we're in this function
    ...
    util::CheckedUniqueLock lock(m_notifier_mutex);
    auto notifiers = notifiers_for_barq(barq);
    lock.unlock();

    transaction::begin(Barq::Internal::get_transaction_ref(barq), barq.m_binding_context.get(),
                       {std::move(notifiers), this});
}

void BarqCoordinator::process_available_async(Barq& barq)
{
    BARQ_ASSERT(!barq.is_in_transaction());
    // If callbacks close the Barq the last external reference may go away
    // while we're in this function
    ...
    auto current_version = barq.current_transaction_version();
    std::vector<std::shared_ptr<_impl::CollectionNotifier>> notifiers;

    {
        ...
        for (auto& notifier : m_notifiers) {
            if (!notifier->is_for_barq(barq) || !notifier->has_run() || !notifier->package_for_delivery())
                continue;
            notifiers.push_back(notifier);
        }
    }
    ...
    if (barq.m_binding_context)
        barq.m_binding_context->will_send_notifications();
    if (barq.is_closed()) // i.e. the Barq was closed in the callback above
        return;
    for (auto& notifier : notifiers)
        notifier->after_advance();
    if (barq.is_closed()) // i.e. the Barq was closed in the callback above
...
TransactionRef BarqCoordinator::package_notifiers(NotifierVector& notifiers, VersionID::version_type target_version)
{
    auto ready = [&] {
...
bool BarqCoordinator::compact()
{
    return m_db->compact();
}

void BarqCoordinator::write_copy(std::string_view path, const char* key)
{
    m_db->write_copy(path, key);
}

void BarqCoordinator::async_request_write_mutex(Barq& barq)
{
    auto tr = Barq::Internal::get_transaction_ref(barq);
    m_db->async_request_write_mutex(tr, [barq = barq.shared_from_this()]() mutable {
        auto& scheduler = *barq->scheduler();
        scheduler.invoke([barq = std::move(barq)] {
            Barq::Internal::run_writes(*barq);
        });
    });
}
