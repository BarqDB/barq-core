...
#define LOGE(...)                                                                                                    \
    do {                                                                                                             \
        ...
        ANDROID_LOG(ANDROID_LOG_ERROR, "BARQ", __VA_ARGS__);                                                        \
    } while (0)
...
void notify_fd(int fd, bool read_first = true)
{
    while (true) {
        ...
        BARQ_ASSERT_RELEASE(ret < 0);
        int err = errno;
        if (err == EWOULDBLOCK || err == EAGAIN) {
            BARQ_ASSERT_RELEASE(read_first); // otherwise this is just an infinite loop.
            continue;
        }
        throw std::system_error(err, std::system_category());
    }
...
public:
    ...
    void add(int fd, BarqCoordinator*) REQUIRES(!m_mutex);
    void remove(int fd, BarqCoordinator*) REQUIRES(!m_mutex, !m_running_on_change_mutex);
...
private:
    ...
    // currently BarqCoordinators and when we get an event, check that the
    // pointer is still in this vector while holding the mutex.
    std::vector<BarqCoordinator*> m_live_coordinators GUARDED_BY(m_mutex);

    // We want destroying an ExternalCommitHelper to block if it's currently
    // running on a background thread to ensure that `Barq::close()`
...
void DaemonThread::add(int fd, BarqCoordinator* coordinator)
{
    {
...
void DaemonThread::remove(int fd, BarqCoordinator* coordinator)
{
    {
...
{
    pthread_setname_np(pthread_self(), "Barq notification listener");
    ...
    while (true) {
        ...
        // inversions with one of BarqCoordinator's mutexes.
        ...
        BarqCoordinator* coordinator = static_cast<BarqCoordinator*>(ev.data.ptr);
        {
            util::CheckedLockGuard lock(m_mutex);
            auto it = std::find(m_live_coordinators.begin(), m_live_coordinators.end(), coordinator);
            if (it == m_live_coordinators.end()) {
    ...
    : m_parent(parent)
{
    ...
    // it is a problem when saving Barqs on external storage:
    ...
    //  1) Next to the Barq file itself
    //  2) A location defined by `Barq::Config::fifo_files_fallback_path`
    //  3) A location defined by `DBOptions::set_sys_tmp_dir()`
    //
    // Core has a similar policy for its named pipes.
    //
    // Also see https://github.com/barq/barq-java/issues/3140
    ...
    bool fifo_created = barq::util::try_create_fifo(path, !temp_dir.empty() || !sys_temp_dir.empty());
    if (!fifo_created && !temp_dir.empty()) {
        path = DB::get_core_file(util::format("%1barq_%2", temp_dir, std::hash<std::string>()(config.path)),
                                 DB::CoreFileType::Note);
        fifo_created = barq::util::try_create_fifo(path, !sys_temp_dir.empty());
    }
    if (!fifo_created && !sys_temp_dir.empty()) {
        path = DB::get_core_file(util::format("%1barq_%2", sys_temp_dir, std::hash<std::string>()(config.path)),
                                 DB::CoreFileType::Note);
        barq::util::create_fifo(path);
    }
