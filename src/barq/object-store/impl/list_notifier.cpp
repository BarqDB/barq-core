...
ListNotifier::ListNotifier(std::shared_ptr<Barq> barq, CollectionBase const& list, PropertyType type)
    : CollectionNotifier(std::move(barq))
    , m_type(type)
...
void ListNotifier::run()
{
    ...
    if (m_change.paths.size()) {
        BARQ_ASSERT(m_collection_parent);
        BARQ_ASSERT(m_type == PropertyType::Mixed);
        for (auto& p : m_change.paths) {
            if (auto ndx = m_collection_parent->find_index(p); ndx != barq::not_found)
