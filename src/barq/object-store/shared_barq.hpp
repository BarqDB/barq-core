...
#ifndef BARQ_BARQ_HPP
#define BARQ_BARQ_HPP
...
struct AuditConfig;
struct SyncConfig;
typedef std::shared_ptr<Barq> SharedBarq;
typedef std::weak_ptr<Barq> WeakBarq;
...
class BarqCoordinator;
class BarqFriend;
} // namespace _impl

// A callback function to be called during a migration for Automatic and
// Manual schema modes. It is passed a SharedBarq at the version before
// the migration, the SharedBarq in the migration, and a mutable reference
// to the barq's Schema. Updating the schema with changes made within the
...
using MigrationFunction = std::function<void(SharedBarq old_barq, SharedBarq barq, Schema&)>;

// A callback function to be called the first time when a schema is created.
// It is passed a SharedBarq which is in a write transaction with the schema
// initialized. So it is possible to create some initial objects inside the callback
// with the given SharedBarq. Those changes will be committed together with the
// schema creation in a single transaction.
using DataInitializationFunction = std::function<void(SharedBarq barq)>;

// A callback function called when opening a SharedBarq when no cached
// version of this Barq exists. It is passed the total bytes allocated for
...
struct BarqConfig {
    ...
    std::string path;
    BinaryData barq_data;
    // User-supplied encryption key. Must be either empty or 64 bytes.
    std::vector<char> encryption_key;

    // Core and Object Store will in some cases need to create named pipes alongside the Barq file.
    ...
    // the Barq is actually opened and not just retrieved from the cache
    ...
    // If false, always return a new Barq instance, and don't return
    // that Barq instance for other requests for a cached Barq. Useful
    // for dynamic Barqs and for tests that need multiple instances on
    // one thread
    bool cache = false;
    ...
    // The Scheduler which this Barq should be bound to. If not supplied,
    // a default one for the current thread will be used.
    std::shared_ptr<util::Scheduler> scheduler;

    /// A data structure storing data used to configure the Barq for sync support.
    std::shared_ptr<SyncConfig> sync_config;

    // Open the Barq using the sync history mode even if a sync
    ...
    // Maximum number of active versions in the Barq file allowed before an exception
    // is thrown.
    ...
    // sync metadata barq which is automatically deleted if it can't be used.
    bool clear_on_invalid_file = false;
};

class Barq : public std::enable_shared_from_this<Barq> {
    struct Private {};

public:
    using Config = BarqConfig;

    // Returns a thread-confined live Barq for the given configuration
    static SharedBarq get_shared_barq(Config config);

    // Get a Barq for the given scheduler (or current thread if `none`)
    // from the thread safe reference.
    static SharedBarq get_shared_barq(ThreadSafeReference, std::shared_ptr<util::Scheduler> = nullptr);

#if BARQ_ENABLE_SYNC
    // Open a synchronized Barq and make sure it is fully up to date before
    // returning it.
    //
    ...
    static std::shared_ptr<AsyncOpenTask> get_synchronized_barq(Config config);

    std::shared_ptr<SyncSession> sync_session() const;

    // Returns the latest/active subscription set for a FLX-sync enabled barq.
    // Throws an exception for a non-FLX barq
    sync::SubscriptionSet get_latest_subscription_set();
    sync::SubscriptionSet get_active_subscription_set();
#endif

    // Returns a frozen Barq for the given Barq. This Barq can be accessed from any thread.
    static SharedBarq get_frozen_barq(Config config, VersionID version);

    // Updates a Barq to a given schema, using the Barq's pre-set schema mode.
    ...
    // Set the schema used for this Barq, but do not update the file's schema
    // if it is not compatible (and instead throw an error).
    // Cannot be called multiple times on a single Barq instance or an instance
    ...
    //   associated with the current barq. It will run after the write
    ...
    //   the current barq. It will run after the commit has reached stable storage.
    ...
    // Returns a frozen copy for the current version of this Barq
    // If called from within a write transaction, the returned Barq will
    ...
    // Returns `true` if the Barq is frozen, `false` otherwise.
    bool is_frozen() const;

    // Returns true if the Barq is either in a read or frozen transaction
    ...
    Group& read_group();
    // Get the version of the current read or frozen transaction, or `none` if the Barq
    ...
    /**
     * Copy this Barq's data into another Barq file.
     *
     * If the file at `config.path` already exists and \a merge_into_existing
     * is true, the contents of this Barq will be copied into the existing
     * Barq at that path. If \a merge_into_existing is false, an exception
     ...
     * configuration is a non-sync local Barq configuration, a compacted copy
     ...
     *
     * If the target configuration is a sync configuration and the source Barq
     * is a local Barq, a sync Barq with no file identifier is created and
     * sync history is synthesized for all of the current objects in the Barq.
     *
     * If the target configuration is a sync configuration and the source Barq
     * is also a sync Barq, a sync Barq with no file identifier is created,
     * but the existing history is retained instead of synthesizing new
     * history. This mode requires that the source Barq does not have any
     * unuploaded changesets, and will thrown an exception if that is not the
     * case.
     *
     * @param config The barq configuration that specifies what file should be
     *               produced. This can be a local or a synced Barq, encrypted or not.
     * @param merge_into_existing If true, converting into an existing file
     *                            will write this Barq's data into that file
    ...
    // Close this Barq. Continuing to use a Barq after closing it will throw Exception(ClosedBarq)
    // Closing a Barq will wait for any asynchronous writes which have been commited but not synced
     ...
     * @throws Exception(DeleteOnOpenBarq) if the function was called on an open Barq.
     */
    static void delete_files(const std::string& barq_file_path, bool* did_delete_barq = nullptr);

    bool has_pending_async_work() const;

    explicit Barq(Config config, util::Optional<VersionID> version,
                   std::shared_ptr<_impl::BarqCoordinator> coordinator, Private);
    Barq(const Barq&) = delete;
    Barq& operator=(const Barq&) = delete;
    Barq(Barq&&) = delete;
    Barq& operator=(Barq&&) = delete;
    ~Barq();
    ...
    static SharedBarq make_shared_barq(Config config, util::Optional<VersionID> version,
                                         std::shared_ptr<_impl::BarqCoordinator> coordinator)
    {
        return std::make_shared<Barq>(std::move(config), std::move(version), std::move(coordinator), Private());
    }
    ...
    class Internal {
        ...
        friend class _impl::BarqCoordinator;
        friend class TestHelper;
        friend class ThreadSafeReference;

        static Transaction& get_transaction(Barq& barq)
        {
            return barq.transaction();
        }
        static std::shared_ptr<Transaction> get_transaction_ref(Barq& barq)
        {
            return barq.transaction_ref();
        }

        static void run_writes(Barq& barq)
        {
            barq.run_writes();
        }

        static void copy_schema(Barq& target_barq, const Barq& source_barq)
        {
            target_barq.copy_schema_from(source_barq);
        }

        // CollectionNotifier needs to be able to access the owning
        // coordinator to wake up the worker thread when a callback is
        // added, and coordinators need to be able to get themselves from a Barq
        static _impl::BarqCoordinator& get_coordinator(Barq& barq)
        {
            return *barq.m_coordinator;
        }

        static std::shared_ptr<DB>& get_db(Barq& barq);
        static void begin_read(Barq&, VersionID);
    };
...
private:
    std::shared_ptr<_impl::BarqCoordinator> m_coordinator;
    ...
    // True while we're performing a schema migration via this Barq instance
    ...
    void copy_schema_from(const Barq&);
