...
#include <barq/object-store/collection.hpp>

#include <barq/object-store/audit.hpp>
#include <barq/object-store/impl/list_notifier.hpp>
...
Collection::Collection(const Object& parent_obj, const Property* prop)
    : Collection(std::shared_ptr(parent_obj.get_barq()), parent_obj.get_obj().get_collection_ptr(prop->column_key),
                 prop->type)
{
}

Collection::Collection(std::shared_ptr<Barq> r, const Obj& parent_obj, ColKey col)
    : Collection(std::move(r), parent_obj.get_collection_ptr(col),
                 ObjectSchema::from_core_type(col) & ~PropertyType::Collection)
{
}

Collection::Collection(std::shared_ptr<Barq> r, const CollectionBase& coll)
    : Collection(std::move(r), coll.clone_collection(),
                 ObjectSchema::from_core_type(coll.get_col_key()) & ~PropertyType::Collection)
{
}

Collection::Collection(std::shared_ptr<Barq> r, CollectionBasePtr coll)
    : Collection(std::move(r), std::move(coll),
                 ObjectSchema::from_core_type(coll->get_col_key()) & ~PropertyType::Collection)
{
}

Collection::Collection(std::shared_ptr<Barq>&& r, CollectionBasePtr&& coll, PropertyType type)
    : m_barq(std::move(r))
    , m_type(type)
...
{
    if (!m_barq || !m_coll_base)
        return false;
    m_barq->verify_thread();
    if (!m_barq->is_in_read_transaction())
        return false;
...
void Collection::verify_attached() const
{
    ...
    throw LogicError(ErrorCodes::InvalidatedObject,
                     util::format("%1 is no longer valid. Either the parent object was deleted or the containing "
                                  "Barq has been invalidated or closed.",
                                  type_name()));
}
...
{
    verify_attached();
    if (BARQ_UNLIKELY(!m_barq->is_in_transaction())) {
...
{
    verify_attached();
    return Results(m_barq, m_coll_base);
}

Results Collection::sort(SortDescriptor order) const
{
    verify_attached();
    return Results(m_barq, m_coll_base, util::none, std::move(order));
}
...
{
    verify_attached();
    m_barq->verify_notifications_available();
    ...
    if (m_notifier && !m_notifier->have_callbacks())
        m_notifier.reset();
    if (!m_notifier) {
        m_notifier = std::make_shared<_impl::ListNotifier>(m_barq, *m_coll_base, m_type);
...
void Collection::record_audit_read(const Obj& obj) const
{
    if (auto audit = m_barq->audit_context()) {
        audit->record_read(m_barq->read_transaction_version(), obj, m_coll_base->get_obj(),
...
{
    if (auto audit = m_barq->audit_context(); audit && value.is_type(type_TypedLink)) {
        audit->record_read(m_barq->read_transaction_version(),
                           m_barq->read_group().get_object(value.get<ObjLink>()), m_coll_base->get_obj(),
                           m_coll_base->get_col_key());
    }
}
...
{
    return List{m_barq, m_coll_base->get_list(path)};
}

Dictionary Collection::get_dictionary(const PathElement& path) const
{
    return Dictionary{m_barq, m_coll_base->get_dictionary(path)};
}
