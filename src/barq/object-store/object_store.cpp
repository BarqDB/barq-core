...
const char* const c_development_mode_msg =
    "If your app is running in development mode, you can delete the barq and restart the app to update your schema.";
...
{
    BARQ_ASSERT(type != PropertyType::Object); // Link columns have to be handled differently
...
ColKey add_column(Group& group, Table& table, Property const& property)
{
    ...
    BARQ_ASSERT(property.type != PropertyType::LinkingObjects);
                                  ...
                                  StringData new_name)
{
    ...
    if (target_object_schema == target_schema.end()) {
        throw LogicError(
            ErrorCodes::NoSuchTable,
            util::format("Cannot rename properties for type '%1' because it has been removed from the Barq.",
                         object_type));
    }
