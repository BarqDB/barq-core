...
#ifndef BARQ_RESULTS_HPP
#define BARQ_RESULTS_HPP
...
public:
    ...
    Results();
    Results(const Class&);
    Results(std::shared_ptr<Barq> r, ConstTableRef table);
    Results(std::shared_ptr<Barq> r, Query q, DescriptorOrdering o = {});
    Results(std::shared_ptr<Barq> r, TableView tv, DescriptorOrdering o = {});
    Results(std::shared_ptr<Barq> r, const Obj& obj, TableKey src_table, ColKey src_col_key)
        : Results(r, obj.get_backlink_view(r->read_group().get_table(src_table), src_col_key))
    {
    }
    Results(std::shared_ptr<Barq> r, std::shared_ptr<CollectionBase> list, DescriptorOrdering o);
    Results(std::shared_ptr<Barq> r, std::shared_ptr<CollectionBase> collection, util::Optional<Query> q = {},
            SortDescriptor s = {});
    ~Results();
    ...
    // Delete all of the rows in this Results from the Barq
    ...
    // the object from the Barq. This would work just fine with an in-memory collection but fail
    // with Barq collections that are not snapshotted.
    ...
    Results freeze(std::shared_ptr<Barq> const& frozen_barq) REQUIRES(!m_mutex);
    ...
    enum class Mode {
        ...
        // inteded for read-only Barqs which are missing tables.
        Empty,
    ...
    // Is this Results associated with a Barq that has not been invalidated?
    bool is_valid() const;
    ...
    SectionedResults sectioned_results(
        util::UniqueFunction<Mixed(Mixed value, const std::shared_ptr<Barq>& barq)>&& section_key_func);
...
private:
    ...
    Results import_copy_into_barq(std::shared_ptr<Barq> const& barq) REQUIRES(!m_mutex);
...
void Results::set_property_value(ContextType& ctx, StringData prop_name, ValueType value) NO_THREAD_SAFETY_ANALYSIS
{
    ...
    if (prop->is_primary && !m_barq->is_in_migration()) {
    ...
    for (size_t i = 0; i < size; ++i) {
        Object obj(m_barq, *m_object_schema, snapshot.get(i));
