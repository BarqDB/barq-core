...
#ifndef BARQ_SECTIONED_RESULTS_HPP
#define BARQ_SECTIONED_RESULTS_HPP
...
public:
    ...
    using SectionKeyFunc = util::UniqueFunction<Mixed(Mixed value, const std::shared_ptr<Barq>& barq)>;
    ...
    SectionedResults freeze(std::shared_ptr<Barq> const& frozen_barq) REQUIRES(!m_mutex);
...
private:
    ...
    friend class barq::ResultsSection;
    ...
    // This is due to the fact that such values can reference the memory address of the value in the barq.
    // We can not rely on that because it would not produce stable keys.
    // So we perform a deep copy to produce stable key values that will not change if the barq is modified.
...
#endif /* BARQ_SECTIONED_RESULTS_HPP */
