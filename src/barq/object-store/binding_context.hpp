...
#include <barq/object-store/index_set.hpp>
...
// a SharedBarq. It can be used to store additional data associated with the
// Barq which is needed by the binding, and there are several methods which
// can be overridden to receive notifications of state changes within the Barq.
//
...
public:
    ...
    std::weak_ptr<Barq> barq;

    // Called when the Barq is about to send notifications about Barq,
    ...
    // Called when the Barq is done sending all change notifications. This method
    // will be called even if no notification callbacks have been registered.
    virtual void did_send_notifications() {}

    // Called by the Barq when refresh called or a notification arrives which
    // is triggered through write transaction committed by itself or a different
    // Barq instance.
    virtual void before_notify() {}

    // Called by the Barq when a write transaction is committed to the file by
    // a different Barq instance (possibly in a different process)
    ...
    // requested or if the Barq is not actually in a read transaction, although
    ...
    // Called immediately after the corresponding Barq's schema is changed through
    // update_schema()/set_schema_subset() or the schema is changed by another Barq
    // instance. The parameter is a schema reference which is the same as the return
    // value of Barq::schema().
