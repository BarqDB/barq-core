...
#ifndef BARQ_OS_OBJECT_ACCESSOR_HPP
#define BARQ_OS_OBJECT_ACCESSOR_HPP
...
struct ValueUpdater {
    ...
    void operator()(T*)
    {
        ...
        if (!attr_changed) {
            ...
            if constexpr (std::is_same<T, barq::Mixed>::value) {
                                     ...
                                     bool is_default)
{
    ...
    if (is_dictionary(property.type)) {
        ContextType child_ctx(ctx, m_obj, property);
        object_store::Dictionary dict(m_barq, m_obj, col);
    ...
    if (is_set(property.type)) {
        ...
        ContextType child_ctx(ctx, m_obj, property);
        object_store::Set set(m_barq, m_obj, col);
...
ValueType Object::get_property_value_impl(ContextType& ctx, const Property& property) const
{
    ...
    if (is_array(property.type) && property.type != PropertyType::LinkingObjects)
        return ctx.box(List(m_barq, m_obj, column));
    if (is_set(property.type) && property.type != PropertyType::LinkingObjects)
        return ctx.box(object_store::Set(m_barq, m_obj, column));
    if (is_dictionary(property.type))
        return ctx.box(object_store::Dictionary(m_barq, m_obj, column));
    ...
    switch (property.type & ~PropertyType::Flags) {
        ...
        case PropertyType::Object: {
            auto linkObjectSchema = m_barq->schema().find(property.object_type);
            auto linked = const_cast<Obj&>(m_obj).get_linked_object(column);
            return ctx.box(Object(m_barq, *linkObjectSchema, linked, m_obj, column));
        }
        case PropertyType::LinkingObjects: {
            auto target_object_schema = m_barq->schema().find(property.object_type);
            auto link_property = target_object_schema->property_for_name(property.link_origin_property_name);
            return ctx.box(Results(m_barq, m_obj, target_object_schema->table_key, link_property->column_key));
        }
        default:
            BARQ_UNREACHABLE();
    }
...
template <typename ValueType, typename ContextType>
Object Object::create(ContextType& ctx, std::shared_ptr<Barq> const& barq, StringData object_type, ValueType value,
                      CreatePolicy policy, ObjKey current_obj, Obj* out_row)
{
    auto object_schema = barq->schema().find(object_type);
    BARQ_ASSERT(object_schema != barq->schema().end());
    return create(ctx, barq, *object_schema, value, policy, current_obj, out_row);
}
...
template <typename ValueType, typename ContextType>
Object Object::create(ContextType& ctx, std::shared_ptr<Barq> const& barq, ObjectSchema const& object_schema,
                      ValueType value, CreatePolicy policy, ObjKey current_obj, Obj* out_row)
{
    barq->verify_in_write();
    ...
    Obj obj;
    auto table = barq->read_group().get_table(object_schema.table_key);
    ...
    if (object_schema.table_type == ObjectSchema::ObjectType::TopLevelAsymmetric) {
        BARQ_ASSERT(!policy.update);
        BARQ_ASSERT(!current_obj);
        BARQ_ASSERT(object_schema.primary_key_property());
    }
    ...
    if (auto primary_prop = object_schema.primary_key_property()) {
        ...
        if (table->get_primary_key_column() == ColKey{}) {
            BARQ_ASSERT(barq->is_in_migration());
            if (policy.update) {
    ...
    Object object(barq, object_schema, obj);
...
template <typename ValueType, typename ContextType>
Object Object::get_for_primary_key(ContextType& ctx, std::shared_ptr<Barq> const& barq, StringData object_type,
                                   ValueType primary_value)
{
    auto object_schema = barq->schema().find(object_type);
    BARQ_ASSERT(object_schema != barq->schema().end());
    return get_for_primary_key(ctx, barq, *object_schema, primary_value);
}

template <typename ValueType, typename ContextType>
Object Object::get_for_primary_key(ContextType& ctx, std::shared_ptr<Barq> const& barq,
                                   const ObjectSchema& object_schema, ValueType primary_value)
{
    ...
    if (object_schema.table_key)
        table = barq->read_group().get_table(object_schema.table_key);
    if (!table)
        return Object(barq, object_schema, Obj());
    ...
    return Object(barq, object_schema, key ? table->get_object(key) : Obj{});
}
...
void List::set(Context& ctx, size_t list_ndx, T&& value, CreatePolicy policy)
{
    ...
    dispatch([&](auto t) {
        ...
        if constexpr (std::is_same_v<U, barq::Mixed>) {
            if (auto type = _impl::collection_type(new_val)) {
...
#endif // BARQ_OS_OBJECT_ACCESSOR_HPP
