...
#ifndef BARQ_OS_SET_HPP
#define BARQ_OS_SET_HPP
...
public:
    ...
    Set freeze(const std::shared_ptr<Barq>& barq) const;
...
private:
    ...
    {
        BARQ_ASSERT_DEBUG(dynamic_cast<SetBase*>(m_coll_base.get()));
...
auto& Set::as() const
{
    BARQ_ASSERT_DEBUG(dynamic_cast<barq::Set<T>*>(m_coll_base.get()));
    return static_cast<barq::Set<T>&>(*m_coll_base);
}

template <>
inline auto& Set::as<Obj>() const
{
    BARQ_ASSERT_DEBUG(dynamic_cast<LnkSet*>(m_coll_base.get()));
...
namespace std {
template <>
struct hash<barq::object_store::Set> {
    size_t operator()(barq::object_store::Set const&) const;
};
} // namespace std

#endif // BARQ_OS_SET_HPP
