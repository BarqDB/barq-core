...
static bool type_is_valid(barq::StringData str_type)
{
...
std::optional<GeoPoint> Geospatial::point_from_obj(const Obj& obj, ColKey type_col, ColKey coords_col)
{
    if (!type_col) {
        ...
        BARQ_ASSERT_EX(type_col.get_type() == ColumnType(ColumnType::Type::String), type_col.get_type());
        BARQ_ASSERT(!type_col.is_collection());
    }
    if (!coords_col) {
        coords_col = obj.get_table()->get_column_key(StringData(c_geo_point_coords_col_name));
    }
    else {
        BARQ_ASSERT_EX(coords_col.get_type() == ColumnType(ColumnType::Type::Double), coords_col.get_type());
        BARQ_ASSERT(coords_col.is_list());
    }
...
// The following validation follows the query engine behavior.
...
static Status coord_to_point(double lng, double lat, S2Point& out)
{
    ...
    // Note that it's (lat, lng) for S2 but (lng, lat) for stored coordinates.
    S2LatLng ll = S2LatLng::FromDegrees(lat, lng).Normalized();
    // This shouldn't happen since we should only have valid lng/lats.
    BARQ_ASSERT_EX(ll.is_valid(), util::format("coords invalid after normalization, lng = %1, lat = %2", lng, lat));
    out = ll.ToPoint();
    return Status::OK();
}
