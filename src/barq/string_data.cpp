...
BARQ_FORCEINLINE bool matchlike(const StringData& text, const StringData& pattern,
                                 const StringData* alternate_pattern = nullptr) noexcept
{
    ...
    BARQ_ASSERT_DEBUG(has_alternate_pattern == bool(alternate_pattern));
    BARQ_ASSERT_DEBUG(!alternate_pattern || pattern.size() == alternate_pattern->size());
...
bool StringData::matchlike(const barq::StringData& text, const barq::StringData& pattern) noexcept
{
...
size_t barq::murmur2_or_cityhash(const unsigned char* data, size_t len) noexcept
{
    return size_t(Murmur2OrCityHash<>{}(data, len));
}

uint_least32_t barq::murmur2_32(const unsigned char* data, size_t len) noexcept
{
    // This implementation is copied from libc++.
    // See: https://github.com/llvm-mirror/libcxx/blob/master/include/utility

    BARQ_ASSERT_DEBUG(len <= std::numeric_limits<uint_least32_t>::max());
...
uint_least64_t barq::cityhash_64(const unsigned char* data, size_t len) noexcept
{
