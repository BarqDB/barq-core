...
size_t ParentNode::find_first(size_t start, size_t end)
{
    ...
    while (BARQ_LIKELY(start < end)) {
        ...
        if (BARQ_LIKELY(nb_cond_to_test == 0))
            return m;
...
void MixedNode<Equal>::init(bool will_query_ranges)
{
    ...
    BARQ_ASSERT(bool(m_index_evaluator) ==
...
size_t IndexEvaluator::do_search_index(const Cluster* cluster, size_t start, size_t end)
{
    ...
    if (m_results_ndx < m_results_end) {
        ...
        BARQ_ASSERT(uint64_t(m_actual_key.value) >= cluster->get_offset());
...
bool StringNode<Equal>::do_consume_condition(ParentNode& node)
{
    ...
    BARQ_ASSERT(m_condition_column_key == other.m_condition_column_key);
...
void StringNodeFulltext::_search_index_init()
{
    ...
    BARQ_ASSERT(index && index->is_fulltext_index());
    m_index_matches.clear();
...
size_t size_of_list_from_ref(ref_type ref, Allocator& alloc, ColumnType col_type, bool is_nullable)
{
    ...
    BARQ_TERMINATE("Unsupported column type.");
}
...
size_t NotNode::find_first_covers_known(size_t start, size_t end)
{
    ...
    // [    ######    ]
    BARQ_ASSERT_DEBUG(start <= m_known_range_start && end >= m_known_range_end);
...
{
    BARQ_ASSERT_DEBUG(start < m_known_range_start && end >= m_known_range_start && end <= m_known_range_end);
...
{
    BARQ_ASSERT_DEBUG(start <= m_known_range_end && start >= m_known_range_start && end > m_known_range_end);
...
{
    BARQ_ASSERT_DEBUG((start < m_known_range_start && end < m_known_range_start) ||
...
size_t LinksToNode<NotEqual>::find_first_local(size_t start, size_t end)
{
    ...
    BARQ_ASSERT(m_target_keys.size() == 1);
