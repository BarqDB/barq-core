...
#include <barq/handover_defs.hpp>
#include <barq/impl/changeset_input_stream.hpp>
...
#include <barq/util/interprocess_mutex.hpp>
...
public:
    ...
    using VersionID = barq::VersionID;
    ...
    using CallbackWithLock = util::FunctionRef<void(const std::string& barq_path)>;
    static bool call_with_lock(const std::string& barq_path, CallbackWithLock&& callback);
    ...
    /// \param barq_path The path for the main Barq file.
    ...
    static std::string get_core_file(const std::string& barq_path, CoreFileType type);
...
private:
    ...
    struct ReadLockInfo {
        ...
        {
            BARQ_ASSERT_RELEASE_EX((m_top_ref & 7) == 0 && m_top_ref < m_file_size, m_version, m_reader_idx,
