...
#ifndef BARQ_ARRAY_BASIC_TPL_HPP
#define BARQ_ARRAY_BASIC_TPL_HPP
...
{
    BARQ_ASSERT(type == NodeHeader::type_Normal);
    BARQ_ASSERT(!context_flag);
    size_t length = 0;
...
size_t BasicArrayNull<T>::find_first_null(size_t begin, size_t end) const
{
    ...
    BARQ_ASSERT(begin <= sz && end <= sz && begin <= end);
    while (begin != end) {
...
#endif // BARQ_ARRAY_BASIC_TPL_HPP
