...
static StringData reconstruct_string(size_t offset, StringIndex::key_type key, StringData new_string)
{
    ...
    BARQ_ASSERT(offset + rest_len <= new_string.size());
...
int64_t IndexArray::index_string(const Mixed& value, InternalFindResult& result_ref,
                                 const ClusterColumn& column) const
{
    // Return`barq::not_found`, or an index to the (any) match
...
{
    BARQ_ASSERT(!m_array->is_inner_bptree_node()); // only works in leaves
    ...
    BARQ_ASSERT(m_array->size() == values.size() + 1);
...
#ifdef BARQ_DEBUG // LCOV_EXCL_START ignore debug code
                                               ...
                                               const Mixed& value)
{
    ...
    if (m_array->is_inner_bptree_node()) {
        ...
        BARQ_ASSERT(m_array->size() == keys.size() + 1);
        ...
        if (keys.size() < BARQ_MAX_BPNODE_SIZE) {
        ...
        switch (node_ndx) {
            ...
            case BARQ_MAX_BPNODE_SIZE: // insert after
                if (nc.type == NodeChange::change_Split)
        ...
        BARQ_ASSERT_EX(m_array->size() == old_offsets_size + 1, m_array->size(), old_offsets_size + 1);

        bool noextend = old_offsets_size >= BARQ_MAX_BPNODE_SIZE;
...
{
    BARQ_ASSERT(m_array->is_inner_bptree_node());
    BARQ_ASSERT(new_ref);
    ...
    BARQ_ASSERT(m_array->size() == offsets.size() + 1);
    BARQ_ASSERT(ndx < offsets.size());
    BARQ_ASSERT(offsets.size() < BARQ_MAX_BPNODE_SIZE);
    ...
    if (!Array::get_context_flag_from_header(header)) {
        ...
        if (value_exists_in_list) {
            ...
            if (index_data == index_data_2 || suboffset > s_max_offset) {
                ...
                BARQ_ASSERT_DEBUG(contains_only_duplicates);
#endif
...
void StringIndex::do_delete(ObjKey obj_key, StringData index_data, size_t offset)
{
    ...
    if (m_array->is_inner_bptree_node()) {
        ...
        if (ref & 1) {
            BARQ_ASSERT(int64_t(ref >> 1) == obj_key.value);
            values.erase(pos);
            m_array->erase(pos_refs);
        }
        else {
            ...
            if (Array::get_context_flag_from_header(header)) {
                ...
                BARQ_ASSERT_EX(r != sub_size, r, sub_size);
                sub.erase(r);
...
void StringIndex::erase_string(ObjKey key, StringData value)
{
    ...
    while (m_array->is_inner_bptree_node()) {
        BARQ_ASSERT(m_array->size() > 1); // node cannot be empty
        if (m_array->size() > 2)
            break;
...
void StringIndex::set(ObjKey key, const Mixed& new_value)
{
    ...
    if (this->m_target_column.tokenize()) {
        ...
        if (BARQ_LIKELY(new_value != old_value)) {
    ...
    if (m_array->is_inner_bptree_node()) {
        ...
        for (size_t i = 1; i < array_size; ++i) {
            ...
            if (ref & 1) {
                ...
                BARQ_ASSERT_EX(r < column_size, r, column_size);
            }
            else {
                // A real ref either points to a list or a subindex
                ...
                if (Array::get_context_flag_from_header(header)) {
                    ...
                    while (it != it_end) {
                        ...
                        BARQ_ASSERT(previous <= it_data);
                        if (it != sub.cbegin() && previous == it_data) {
                            BARQ_ASSERT_EX(it_row > last_row, it_row, last_row);
                        }
                        last_row = it_row;
...
void StringIndex::verify_entries(const ClusterColumn& column) const
{
    ...
    while (it != end) {
        ...
        BARQ_ASSERT(ndx != results.end());
        size_t found = count(value);
        BARQ_ASSERT_EX(found >= 1, found);
        results.clear();
    }
}
