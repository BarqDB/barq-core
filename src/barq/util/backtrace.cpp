...
#if !defined(BARQ_HAVE_BACKTRACE) && (BARQ_PLATFORM_APPLE || (defined(__linux__) && defined(__GNUC__)))
// we detect the backtrace facility in CMake, but if building outside it we assume
// it's available on Apple or Linux/glibc
#define BARQ_HAVE_BACKTRACE 1
#endif

#if BARQ_HAVE_BACKTRACE
#if defined(BARQ_BACKTRACE_HEADER)
#include BARQ_BACKTRACE_HEADER
#else
#include <execinfo.h>
#endif
#endif
...
Backtrace Backtrace::capture() noexcept
{
    ...
    if (BARQ_UNLIKELY(frames <= 1)) {
        ...
        if (BARQ_UNLIKELY(memory == nullptr)) {
