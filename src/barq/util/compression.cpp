...
public:
    ...
    {
        return "barq::util::compression::error";
    }
    std::string message(int err) const override final
    {
        using error = barq::util::compression::error;
        error e = error(err);
        switch (e) {
...
{
    using Alloc = barq::util::compression::Alloc;
...
void grow_arena(compression::CompressMemoryArena& compress_memory_arena)
{
    ...
    BARQ_ASSERT(n != 0);
    BARQ_ASSERT(n != std::numeric_limits<std::size_t>::max());
                                ...
                                bool has_header)
{
    ...
    do {
        ...
        while (in_offset < compressed_buf.size()) {
            ...
            int rc = inflate(&strm, Z_SYNC_FLUSH);
            BARQ_ASSERT(rc != Z_STREAM_ERROR && rc != Z_MEM_ERROR);
                                      ...
                                      std::size_t& compressed_size, int compression_level, Alloc* custom_allocator)
{
    ...
    while (rc == Z_OK || rc == Z_BUF_ERROR) {
        BARQ_ASSERT(strm.next_in + strm.avail_in == uncompressed_ptr + next_in_ndx);
        BARQ_ASSERT(strm.next_out + strm.avail_out == compressed_ptr + next_out_ndx);
        ...
        BARQ_ASSERT(rc != Z_STREAM_END || flush == Z_FINISH);
    }
