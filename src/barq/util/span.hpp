...
#ifndef BARQ_UTIL_SPAN_HPP
#define BARQ_UTIL_SPAN_HPP
...
public:
    ...
    constexpr explicit Span(pointer ptr, size_type count)
        : m_data{ptr}
    {
        BARQ_ASSERT(extent == count);
    }
    constexpr explicit Span(pointer begin, pointer end)
        : m_data{begin}
    {
        BARQ_ASSERT(extent == std::distance(begin, end));
    }
    ...
    {
        BARQ_ASSERT(offset <= size());
        BARQ_ASSERT(count <= size() || count == dynamic_extent);
        if (count == dynamic_extent)
            return {data() + offset, size() - offset};
        BARQ_ASSERT(count <= size() - offset);
...
public:
    ...
    {
        BARQ_ASSERT(offset <= m_size);
        BARQ_ASSERT(count == dynamic_extent || count <= m_size - offset);
    ...
    {
        BARQ_ASSERT(offset <= m_size);
        BARQ_ASSERT(count <= m_size || count == dynamic_extent);
        if (count == dynamic_extent)
            return {m_data + offset, m_size - offset};
        BARQ_ASSERT(count <= m_size - offset);
