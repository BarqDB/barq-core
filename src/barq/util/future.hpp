...
public:
    ...
    {
        BARQ_ASSERT_DEBUG(m_refs.load(std::memory_order_relaxed) == (count - 1));
...
protected:
    ...
    template <typename>
    friend class ::barq::util::bind_ptr;
...
struct SharedStateBase : public FutureRefCountable {
    ...
    void wait() noexcept
    {
        ...
        if (BARQ_UNLIKELY(
            ...
            BARQ_ASSERT_DEBUG(old_state == SSBState::Finished);
            return;
        }
    ...
    void transition_to_finished() noexcept
    {
        ...
        BARQ_ASSERT_DEBUG(old_state == SSBState::Waiting);
    ...
    {
        BARQ_ASSERT_DEBUG(m_state.load() < SSBState::Finished);
...
struct SharedStateImpl final : public SharedStateBase {
    ...
    {
        BARQ_ASSERT_DEBUG(m_state.load() < SSBState::Finished);
        BARQ_ASSERT_DEBUG(other.m_state.load() == SSBState::Finished);
        BARQ_ASSERT_DEBUG(m_owned_by_promise.load());
    ...
    void disown() const
    {
        BARQ_ASSERT(m_owned_by_promise.exchange(false));
    }

    void claim() const
    {
        BARQ_ASSERT(!m_owned_by_promise.exchange(true));
    }
...
public:
    ...
    ~Promise()
    {
        if (BARQ_UNLIKELY(m_shared_state)) {
    ...
    {
        BARQ_ASSERT_DEBUG(!status.is_ok());
        set_impl([&](auto& shared_state) {
 ...
 */
template <typename T>
class BARQ_NODISCARD future_details::Future {
public:
...
private:
    ...
    auto general_impl(SuccessFunc&& success, FailFunc&& fail, NotReady&& notReady) noexcept
    {
        ...
        auto guard = util::make_scope_exit([&]() noexcept {
            auto old_state = SSBState::Init;
            if (BARQ_UNLIKELY(!m_shared->m_state.compare_exchange_strong(old_state, SSBState::Waiting,
    ...
    {
        BARQ_ASSERT(!m_shared->m_callback && !m_shared->m_continuation);
 ...
 */
template <>
class BARQ_NODISCARD future_details::Future<void> {
public:
