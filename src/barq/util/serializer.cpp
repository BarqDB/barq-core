...
template <>
std::string print_value<>(barq::Timestamp t)
{
    if (t.is_null()) {
        return "NULL";
    }
...
template <>
std::string print_value<>(barq::ObjectId oid)
{
    return "oid(" + oid.to_string() + ")";
}

template <>
std::string print_value<>(barq::ObjKey k)
{
    std::stringstream ss;
    if (!k) {
        ss << "NULL";
...
std::string print_value(barq::ObjLink link, Group* g)
{
    if (!link) {
        return "NULL";
    }
...
template <>
std::string print_value<>(barq::UUID uuid)
{
    return "uuid(" + uuid.to_string() + ")";
}

template <>
std::string print_value<>(barq::TypeOfValue type)
{
...
template <>
std::string print_value<>(const barq::Geospatial& geo)
{
...
std::string SerialisationState::get_backlink_column_name(ConstTableRef from, ColKey col_key)
{
    ...
    BARQ_ASSERT_EX(col_type == col_type_Link, col_type);
