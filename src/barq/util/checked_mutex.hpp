...
#ifndef BARQ_OS_CHECKED_MUTEX_HPP
#define BARQ_OS_CHECKED_MUTEX_HPP
...
#define BARQ_THREAD_ANNOTATION_ATTRIBUTE__(x) __attribute__((x))
#else
#define BARQ_THREAD_ANNOTATION_ATTRIBUTE__(x)
#endif

#define CAPABILITY(x) BARQ_THREAD_ANNOTATION_ATTRIBUTE__(capability(x))

#define RETURN_CAPABILITY(x) BARQ_THREAD_ANNOTATION_ATTRIBUTE__(return_capability(x))

#define SCOPED_CAPABILITY BARQ_THREAD_ANNOTATION_ATTRIBUTE__(scoped_lockable)

#define GUARDED_BY(x) BARQ_THREAD_ANNOTATION_ATTRIBUTE__(guarded_by(x))

#define REQUIRES(...) BARQ_THREAD_ANNOTATION_ATTRIBUTE__(requires_capability(__VA_ARGS__))

#define ACQUIRE(...) BARQ_THREAD_ANNOTATION_ATTRIBUTE__(acquire_capability(__VA_ARGS__))

#define RELEASE(...) BARQ_THREAD_ANNOTATION_ATTRIBUTE__(release_capability(__VA_ARGS__))

#define EXCLUDES(...) BARQ_THREAD_ANNOTATION_ATTRIBUTE__(locks_excluded(__VA_ARGS__))

#define NO_THREAD_SAFETY_ANALYSIS BARQ_THREAD_ANNOTATION_ATTRIBUTE__(no_thread_safety_analysis)
#define ASSERT_CAPABILITY(x) BARQ_THREAD_ANNOTATION_ATTRIBUTE__(assert_capability(x))
...
public:
    ...
    {
        BARQ_ASSERT(lock.owns_lock());
        BARQ_ASSERT(lock.native_handle().mutex() == &m_mutex);
...
#endif // BARQ_OS_CHECKED_MUTEX_HPP
