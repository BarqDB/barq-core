...
#ifndef BARQ_UTIL_COMPRESSION_HPP
#define BARQ_UTIL_COMPRESSION_HPP
...
#ifndef BARQ_USE_LIBCOMPRESSION
#define BARQ_USE_LIBCOMPRESSION BARQ_PLATFORM_APPLE
#endif

namespace barq::util::compression {
...
template <>
struct is_error_code_enum<barq::util::compression::error> {
...
public:
    void* alloc(size_t size) noexcept override final
    {
        ...
        if (padding > m_size - offset)
            return nullptr;
        offset += padding;
        BARQ_ASSERT(offset % alignof(std::max_align_t) == 0);
...
#endif // BARQ_UTIL_COMPRESSION_HPP
