...
#ifndef BARQ_UTIL_CIRCULAR_BUFFER_HPP
#define BARQ_UTIL_CIRCULAR_BUFFER_HPP
...
public:
    ...
    {
        BARQ_ASSERT(m_buffer == i.m_buffer);
...
void CircularBuffer<T>::reserve(size_type capacity)
{
    ...
    if (BARQ_UNLIKELY(int_add_with_overflow_detect(min_allocated_size, 1)))
    ...
    if (BARQ_UNLIKELY(int_multiply_with_overflow_detect(new_allocated_size, 2)))
...
inline auto CircularBuffer<T>::emplace_front(Args&&... args) -> reference
{
    ...
    BARQ_ASSERT(m_allocated_size > 0);
    T* memory = get_memory_ptr();
...
inline auto CircularBuffer<T>::circular_inc(size_type index) noexcept -> size_type
{
    ...
    if (BARQ_LIKELY(index_2 < m_allocated_size))
        return index_2;
    return 0;
}
...
{
    BARQ_ASSERT(new_allocated_size > 1);
    BARQ_ASSERT(new_allocated_size > m_size);
...
#endif // BARQ_UTIL_CIRCULAR_BUFFER_HPP
