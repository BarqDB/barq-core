...
#ifndef BARQ_UTIL_THREAD_HPP
#define BARQ_UTIL_THREAD_HPP
...
protected:
    ...
    BARQ_NORETURN static void init_failed(int);
    BARQ_NORETURN static void attr_init_failed(int);
    BARQ_NORETURN static void destroy_failed(int) noexcept;
    BARQ_NORETURN static void lock_failed(int) noexcept;
...
inline void RobustMutex::lock(Func recover_func)
{
    ...
    if (BARQ_LIKELY(no_thread_has_died))
        return;
    try {
        recover_func(); // Throws
        mark_as_consistent();
...
inline void CondVar::wait(LockGuard& l) noexcept
{
    ...
    if (BARQ_UNLIKELY(r != 0))
        BARQ_TERMINATE("pthread_cond_wait() failed");
#endif
}
