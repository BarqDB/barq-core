...
namespace {
bool for_each_helper(const std::string& path, const std::string& dir, barq::util::File::ForEachHandler& handler)
{
    using File = barq::util::File;
    barq::util::DirScanner ds{path}; // Throws
    std::string name;
...
void throwIfCreateDirectoryError(std::error_code error, const std::string& path)
{
    ...
    if (error == errc::permission_denied || error == errc::read_only_file_system) {
        throw barq::FileAccessError(barq::ErrorCodes::PermissionDenied, error.message(), path);
    }
    else {
        throw barq::FileAccessError(barq::ErrorCodes::FileOperationFailed, error.message(), path);
    }
}
...
#if BARQ_ANDROID
    std::string buffer = "/data/local/tmp/barq_XXXXXX";
#else
...
#endif
        BARQ_ASSERT(size > 0 && size % c_min_supported_page_size == 0);
...
void File::close() noexcept
{
    ...
    BARQ_ASSERT_RELEASE(r);
#else // POSIX version
    int r = ::close(m_fd);
    BARQ_ASSERT_RELEASE(r == 0);
#endif
...
size_t File::read_static(FileDesc fd, SizeType pos, char* data, size_t size)
{
    ...
    while (0 < size) {
        ...
        BARQ_ASSERT_RELEASE(size_t(r) <= n);
        size -= size_t(r);
        data += size_t(r);
        pos += r;
    }
...
void File::write_static(FileDesc fd, SizeType pos, const char* data, size_t size)
{
    ...
    while (0 < size) {
        ...
        if (!WriteFile(fd, data, n, &r, &o))
            goto error;
        BARQ_ASSERT_RELEASE(r == n); // Partial writes are not possible.
        size -= size_t(r);
        data += size_t(r);
        pos += r;
    }
...
void File::write(SizeType pos, const char* data, size_t size)
{
    ...
    if (m_encryption) {
        ...
        barq::util::encryption_write_barrier(write_map, 0, size);
        return;
    }
...
void File::prealloc(SizeType size)
{
    ...
    if (m_encryption) {
        ...
        BARQ_ASSERT(size == encrypted_size_to_data_size(new_size));
    }
...
#if BARQ_HAVE_POSIX_FALLOCATE
    // Mostly Linux only
    if (!prealloc_if_supported(0, new_size)) {
        manually_consume_space();
    }
#elif BARQ_PLATFORM_APPLE // Non-atomic fallback
...
#elif BARQ_ANDROID || defined(_WIN32) || defined(__EMSCRIPTEN__)
    manually_consume_space();
#else
#error Please check if/how your OS supports file preallocation
#endif // BARQ_HAVE_POSIX_FALLOCATE
}
...
bool File::prealloc_if_supported(SizeType offset, SizeType size)
{
    ...
    BARQ_ASSERT_RELEASE(is_prealloc_supported());
...
bool File::rw_lock(bool exclusive, bool non_blocking)
{
    ...
    BARQ_ASSERT(!exclusive || non_blocking);

#ifndef BARQ_FILELOCK_EMULATION
    return lock(exclusive, non_blocking);
#else
    BARQ_ASSERT(!m_has_exclusive_lock && !has_shared_lock());
    ...
    BARQ_ASSERT_RELEASE(m_pipe_fd == -1);
    ...
    m_pipe_fd = fd;
    return true;
#endif // BARQ_FILELOCK_EMULATION
}

bool File::lock(bool exclusive, bool non_blocking)
{
    BARQ_ASSERT_RELEASE(is_attached());
    BARQ_ASSERT_RELEASE(!m_have_lock);
...
void File::rw_unlock() noexcept
{
    ...
    if (has_shared_lock()) {
        ...
        BARQ_ASSERT(status == 0);
        // close the pipe (== release the shared lock)
        ::close(m_pipe_fd);
        m_pipe_fd = -1;
    }
    else {
        BARQ_ASSERT(m_has_exclusive_lock);
    }
    _unlock(m_fd);
...
{
    BARQ_ASSERT_RELEASE(is_attached());
    BARQ_ASSERT_RELEASE(f.is_attached());
...
void File::MapBase::unmap() noexcept
{
    if (!m_addr)
        return;
    BARQ_ASSERT(m_reservation_size);
#if BARQ_ENABLE_ENCRYPTION
...
bool File::MapBase::try_extend_to(size_t size) noexcept
{
    ...
    if (m_encrypted_mapping) {
        ...
        if (got_addr == MAP_FAILED)
            return false;
        BARQ_ASSERT(got_addr == extension_start_addr);
        m_size = size;
...
void File::MapBase::sync()
{
    ...
    barq::util::msync(m_fd, m_addr, m_size);
}
...
bool DirScanner::next(std::string& name)
{
#if !defined(__linux__) && !BARQ_PLATFORM_APPLE && !BARQ_WINDOWS && !BARQ_UWP && !BARQ_ANDROID &&                \
...
// in 32-bits.
#if BARQ_HAVE_READDIR64
#define BARQ_READDIR(...) readdir64(__VA_ARGS__)
#else
#define BARQ_READDIR(...) readdir(__VA_ARGS__)
#endif

    for (;;) {
        using DirentPtr = decltype(BARQ_READDIR(m_dirp));
        DirentPtr dirent;
        do {
