#ifndef BARQ_UTIL_QUOTE_HPP
#define BARQ_UTIL_QUOTE_HPP
...
quote:
    ...
    if (BARQ_LIKELY(sentry)) {
        out.put(dquote); // Throws
        bool follows_hex = false;
        for (C ch : view) {
            if (BARQ_LIKELY(ctype.is(ctype.print, ch))) {
                if (BARQ_LIKELY(!follows_hex || !ctype.is(ctype.xdigit, ch))) {
                    if (BARQ_LIKELY(ch != dquote && ch != bslash))
                        goto put_char;
                    goto escape_char;
                }
            }
