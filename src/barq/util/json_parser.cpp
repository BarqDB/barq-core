...
int hexdigit_to_int(int digit) noexcept
{
    ...
    if (digit >= 'A' && digit <= 'F')
        return 10 + (digit - 'A');
    BARQ_UNREACHABLE(); // LCOV_EXCL_LINE
}
...
{
    BARQ_ASSERT_EX(utf32 <= 0x7fffffff, utf32);
...
{
    return "barq::util::JSONParser::ErrorCategory";
}
...
{
    BARQ_ASSERT_EX(type == EventType::string, static_cast<int>(type));
    size_t i, o;
    bool escape = false;
