...
int Mixed::compare(const Mixed& b) const noexcept
{
    ...
    switch (type) {
        ...
        default:
            ...
            BARQ_ASSERT_RELEASE(false && "Compare not supported for this column type");
            break;
    }

    // Comparing rank of types as a fallback makes it possible to sort of a list of Mixed
    BARQ_ASSERT(sorting_rank[m_type] != sorting_rank[b.m_type]);
...
size_t Mixed::hash() const
{
    ...
    switch (get_type()) {
        ...
        case type_Mixed:
        case type_Link:
            BARQ_ASSERT_RELEASE(false && "Hash not supported for this column type");
            break;
    }
...
StringData Mixed::get_index_data(std::array<char, 16>& buffer) const noexcept
{
    ...
    switch (type) {
        case type_Int: {
            ...
            barq::safe_copy_n(c, sizeof(int64_t), buffer.data());
        ...
        case type_Float: {
            ...
            barq::safe_copy_n(src, sizeof(float), buffer.data());
        ...
        case type_Double: {
            ...
            const char* src = reinterpret_cast<const char*>(&v2);
            barq::safe_copy_n(src, sizeof(double), buffer.data());
        ...
        case type_Timestamp: {
            ...
            barq::safe_copy_n(s_buf, sizeof(s), buffer.data());
            barq::safe_copy_n(ns_buf, sizeof(ns), buffer.data() + sizeof(s));
        ...
        case type_Decimal: {
            ...
            const char* src = reinterpret_cast<const char*>(&v2);
            barq::safe_copy_n(src, sizeof(v2), buffer.data());
        ...
        case type_TypedLink: {
            ...
            barq::safe_copy_n(src, sizeof(k1), buffer.data());
            src = reinterpret_cast<const char*>(&k2);
            barq::safe_copy_n(src, sizeof(k2), buffer.data() + sizeof(k1));
    ...
    BARQ_ASSERT_RELEASE(false && "Index not supported for this column type");
    return {};
}
