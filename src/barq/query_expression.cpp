...
void LinkMap::set_base_table(ConstTableRef table)
{
    ...
    for (size_t i = 0; i < m_link_column_keys.size(); i++) {
        ...
        BARQ_ASSERT(Table::is_link_type(type) || type == col_type_BackLink);
        ...
        BARQ_ASSERT(table->valid_column(link_column_key));
...
bool LinkMap::map_links(size_t column, ObjKey key, LinkMapFunction lm) const
{
    ...
    if (column_key.is_collection()) {
        ...
        if (pe.is_all()) {
            ...
            BARQ_ASSERT(column_key.is_dictionary());
            ...
            BARQ_ASSERT(column_key.is_list());
        ...
        BARQ_TERMINATE("Invalid column type in LinkMap::map_links()");
    }
    return true;
}
            ...
            if (leaf.get(row)) {
                ...
                for (; start < end; start++) {
                    ...
                    if (m.is_type(type_TypedLink)) {
                        ...
                        BARQ_ASSERT(link.get_table_key() == this->m_tables[column + 1]->get_key());
