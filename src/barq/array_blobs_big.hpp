...
#ifndef BARQ_ARRAY_BIG_BLOBS_HPP
#define BARQ_ARRAY_BIG_BLOBS_HPP
...
{
    BARQ_ASSERT_DEBUG(!(!m_nullable && value.is_null()));
                                            ...
                                            bool nullable = true) noexcept
{
    ...
    BARQ_ASSERT_DEBUG(!(!nullable && bin.is_null()));
    if (bin.is_null())
        return barq::null();
    else
        return StringData(bin.data(), bin.size() - 1); // Do not include terminating zero
...
#endif // BARQ_ARRAY_BIG_BLOBS_HPP
