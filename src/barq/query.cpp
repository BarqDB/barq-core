    ...
    , m_source_collection(list.clone_obj_list())
{
    ...
    BARQ_ASSERT_DEBUG(m_view);
    BARQ_ASSERT_DEBUG(list.get_target_table() == m_table);
    create();
}
    ...
    , m_source_collection(std::move(list_ptr))
{
    m_view = m_source_collection.get();
    BARQ_ASSERT_DEBUG(m_view);
    BARQ_ASSERT_DEBUG(m_view->get_target_table() == m_table);
    create();
}
...
BARQ_NOINLINE BARQ_COLD BARQ_NORETURN void throw_type_mismatch_error()
{
...
struct MakeConditionNode {
    ...
    template <class T>
    BARQ_FORCEINLINE static std::unique_ptr<ParentNode> make(ColKey, T&&)
    {
...
std::unique_ptr<ParentNode> make_condition_node(const Table& table, ColKey column_key, T value)
{
    ...
    switch (type) {
        ...
        case type_Link:
            if constexpr (std::is_same_v<T, Mixed> && barq::is_any_v<Cond, Equal, NotEqual>) {
                ObjKey key;
...
BARQ_FORCEINLINE Query& Query::add_condition(ColKey column_key, T value)
{
...
Query& Query::between(ColKey column_key, int64_t from, int64_t to)
{
    if (column_key.is_nullable()) {
        add_node(std::unique_ptr<barq::ParentNode>(new BetweenNode<ArrayIntNull>(from, to, column_key)));
    }
    else {
        add_node(std::unique_ptr<barq::ParentNode>(new BetweenNode<ArrayInteger>(from, to, column_key)));
    }
    return *this;
}
...
{
    add_node(std::unique_ptr<barq::ParentNode>(new BetweenNode<ArrayTimestamp>(from, to, column_key)));
    return *this;
}
// ------------- ObjectId
...
{
    BARQ_ASSERT(!column_key.is_collection());
            ...
            if (col_type == col_type_Mixed) {
                ...
                for (const Mixed* it = begin; it != end; ++it) {
                    if (it->is_type(DataType(col_type))) {
                        ...
                        cond.add_node(make_condition_node<Equal>(*m_table, column_key, barq::null()));
                        cond.Or();
                    }
                }
            }
...
TableView Query::find_all(const DescriptorOrdering& descriptor) const
{
    ...
    for (size_t i = 0; i < descriptor.size(); ++i) {
        if (descriptor.get_type(i) != DescriptorType::Limit) {
            ...
            BARQ_ASSERT(dynamic_cast<const LimitDescriptor*>(descriptor[i]));
...
size_t Query::count(const DescriptorOrdering& descriptor) const
{
    if (!m_table)
        return 0;
    barq::util::Optional<size_t> min_limit = descriptor.get_min_limit();
...
void Query::add_node(std::unique_ptr<ParentNode> node)
{
    ...
    switch (current_group.m_state) {
        case QueryGroup::State::OrCondition: {
            BARQ_ASSERT_DEBUG(dynamic_cast<OrNode*>(current_group.m_root_node.get()));
...
Query& Query::and_query(Query&& q)
{
    if (q.root_node()) {
        ...
        if (q.m_source_collection) {
            BARQ_ASSERT(!m_source_collection || (m_source_collection->matches(*q.m_source_collection)));
