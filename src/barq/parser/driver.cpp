...
#include "barq/parser/driver.hpp"
...
#include "barq/parser/generated/query_flex.hpp"
...
{
    switch (type) {
        case barq::query_parser::AggrNode::MAX:
            return ".@max";
        case barq::query_parser::AggrNode::MIN:
            return ".@min";
        case barq::query_parser::AggrNode::SUM:
            return ".@sum";
        case barq::query_parser::AggrNode::AVG:
            return ".@avg";
    }
    return "";
}
...
{
    if constexpr (barq::is_any<T, float, double>::value || std::numeric_limits<T>::is_iec559) {
        ...
        if (auto link_column = dynamic_cast<const Columns<Link>*>(left.get())) {
            if (link_column->link_map().get_nb_hops() == 1 &&
                link_column->get_comparison_type().value_or(ExpressionComparisonType::Any) ==
                    ExpressionComparisonType::Any) {
                BARQ_ASSERT(dynamic_cast<const Value<ObjKey>*>(right.get()));
        ...
        if (prop && !prop->links_exist() && !prop->has_path()) {
            ...
            if (val.is_null()) {
                switch (op) {
                    case CompareType::EQUAL:
                    case CompareType::IN:
                        return drv->m_base_table->where().equal(col_key, barq::null());
                    case CompareType::NOT_EQUAL:
                        return drv->m_base_table->where().not_equal(col_key, barq::null());
                    default:
                        break;
                }
            }
...
Query RelationalNode::visit(ParserDriver* drv)
{
    ...
    BARQ_ASSERT(!(left_type_is_null && right_type_is_null));
...
Query GeoWithinNode::visit(ParserDriver* drv)
{
    ...
    BARQ_ASSERT_3(argument.size(), >, 1);
    BARQ_ASSERT_3(argument[0], ==, '$');
...
Query TrueOrFalseNode::visit(ParserDriver* drv)
{
    Query q = drv->m_base_table->where();
    if (true_or_false) {
        q.and_query(std::unique_ptr<barq::Expression>(new TrueExpression));
    }
    else {
        q.and_query(std::unique_ptr<barq::Expression>(new FalseExpression));
    }
    return q;
}
...
std::unique_ptr<Subexpr> SubqueryNode::visit(ParserDriver* drv, DataType)
{
    ...
    if (!(col_key.is_list() || col_type == col_type_BackLink)) {
        throw InvalidQueryError(util::format("A subquery must operate on a list property, but '%1' is type '%2'",
                                             identifier, barq::get_data_type_name(DataType(col_type))));
    }
...
void ConstantNode::decode_b64()
{
    ...
    BARQ_ASSERT_DEBUG_EX(*decoded_size <= encoded_size, *decoded_size, encoded_size);
...
std::unique_ptr<Subexpr> ConstantNode::visit(ParserDriver* drv, DataType hint)
{
    ...
    if (value.is_null()) {
        if (hint == type_String) {
            ...
            return std::make_unique<Value<null>>(barq::null());
        }
    }
...
std::unique_ptr<Subexpr> ListNode::visit(ParserDriver* drv, DataType hint)
{
    ...
    for (auto constant : elements) {
        ...
        if (auto value = dynamic_cast<const ValueBase*>(evaulated_constant.get())) {
            BARQ_ASSERT_EX(value->size() == 1, value->size());
...
{
    BARQ_ASSERT(m_link_cols.size() > 0);
