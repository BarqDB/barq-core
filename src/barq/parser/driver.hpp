...
#include "barq/parser/generated/query_bison.hpp"
YY_DECL;
    ...
    void do_canonicalize(std::vector<LogicalNode*>& todo)
    {
        ...
        while (index < children.size()) {
            ...
            if (child_type == my_type) {
                ...
                BARQ_ASSERT_EX(logical_node->children.size() == 2, logical_node->children.size());
...
public:
    ...
    GeospatialNode(Ts&&...)
    {
        throw barq::LogicError(ErrorCodes::NotSupported, "Support for Geospatial queries is not enabled");
    }
