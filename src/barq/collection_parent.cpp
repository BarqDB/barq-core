...
{
    BARQ_ASSERT(col_key.get_attrs().test(col_attr_List) || col_key.get_type() == col_type_Mixed);
...
{
    BARQ_ASSERT(col_key.get_attrs().test(col_attr_Set));
...
void CollectionParent::set_key(BPlusTreeMixed& tree, size_t index)
{
    ...
    while (tree.find_key(key) != barq::not_found) {
        key++;
    }
