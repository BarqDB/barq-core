...
template <typename T>
constexpr bool is_status_or_status_with = std::is_same_v<T, ::barq::Status> || is_status_with<T>;
 ...
 */
template <typename T>
class BARQ_NODISCARD StatusWith {
...
public:
    ...
    {
        BARQ_ASSERT_DEBUG(is_ok());
        BARQ_ASSERT_RELEASE(m_value);
        return *m_value;
    }
