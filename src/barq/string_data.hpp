...
#ifndef BARQ_STRING_HPP
#define BARQ_STRING_HPP
...
constexpr inline StringData::StringData(const char* external_data, size_t data_size) noexcept
    ...
    , m_size(data_size)
{
    BARQ_ASSERT_DEBUG(external_data || data_size == 0);
}
...
namespace std {
template <>
struct hash<::barq::StringData> {
    inline size_t operator()(const ::barq::StringData& str) const noexcept
    {
        return str.hash();
    }
};
