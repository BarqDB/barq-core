...
//
// `N_c` must always be a power of `BARQ_MAX_BPNODE_SIZE`.
//
...
ref_type Array::do_write_shallow(_impl::ArrayWriterBase& out) const
{
    ...
    BARQ_ASSERT_3(new_ref % 8, ==, 0);                                    // 8-byte alignment
    return new_ref;
}
...
{
    BARQ_ASSERT_3(begin, <=, end);
    BARQ_ASSERT_3(end, <=, m_size);
    BARQ_ASSERT_3(dest_begin, <=, m_size);
    BARQ_ASSERT_3(end - begin, <=, m_size - dest_begin);
    BARQ_ASSERT(!(dest_begin >= begin && dest_begin < end)); // Required by std::copy
    ...
    barq::safe_copy_n(begin_2, end_2 - begin_2, dest_begin_2);
}
...
{
    BARQ_ASSERT(value >= 0);
    BARQ_ASSERT(&m_alloc == &Allocator::get_default());
...
void Array::insert(size_t ndx, int_fast64_t value)
{
    ...
    if (do_expand) {
        ...
        BARQ_ASSERT_DEBUG(width > m_width);
...
{
    BARQ_TEMPEX(return sum, m_width, (start, end));
}
...
int64_t Array::sum(size_t start, size_t end) const
{
    ...
    BARQ_ASSERT_EX(end <= m_size && start <= end, start, end, m_size);
    ...
    if (w == 1 || w == 2 || w == 4) {
        ...
        for (size_t t = 0; t < chunks; t++) {
            if (w == 1) {
#if 0
#if defined(USE_SSE42) && defined(_MSC_VER) && defined(BARQ_PTR_64)
                s += __popcnt64(data[t]);
#elif !defined(_MSC_VER) && defined(USE_SSE42) && defined(BARQ_PTR_64)
...
{
    BARQ_ASSERT(width != 0 && (width & (width - 1)) == 0); // Is a power of two
...
MemRef Array::clone(MemRef mem, Allocator& alloc, Allocator& target_alloc)
{
    ...
    if (!get_hasrefs_from_header(header)) {
        ...
        barq::safe_copy_n(src_begin, src_end - src_begin, dst_begin);
    ...
    BARQ_ASSERT_3(get_wtype_from_header(header), ==, wtype_Bits);
...
MemRef Array::create(Type type, bool context_flag, WidthType width_type, size_t size, int_fast64_t value,
                     Allocator& alloc)
{
    BARQ_ASSERT_7(value, ==, 0, ||, width_type, ==, wtype_Bits);
    BARQ_ASSERT_7(size, ==, 0, ||, width_type, !=, wtype_Ignore);
    ...
    if (value != 0) {
        ...
        BARQ_TEMPEX(fill_direct, width, (data, begin, end, value));
    }
...
void Array::update_width_cache_from_header() noexcept
{
    ...
    BARQ_TEMPEX(m_vtable = &VTableForWidth, width, ::vtable);
...
void Array::verify() const
{
    ...
    BARQ_ASSERT(m_width == 0 || m_width == 1 || m_width == 2 || m_width == 4 || m_width == 8 || m_width == 16 ||
    ...
    BARQ_ASSERT_3(ref_in_parent, ==, m_ref);
#endif
}

size_t Array::lower_bound_int(int64_t value) const noexcept
{
    BARQ_TEMPEX(return lower_bound, m_width, (m_data, m_size, value));
}

size_t Array::upper_bound_int(int64_t value) const noexcept
{
    BARQ_TEMPEX(return upper_bound, m_width, (m_data, m_size, value));
}
