...
#ifndef BARQ_QUERY_ENGINE_HPP
#define BARQ_QUERY_ENGINE_HPP
...
#if BARQ_X86_OR_X64_TRUE && defined(_MSC_FULL_VER) && _MSC_FULL_VER >= 160040219
#include <immintrin.h>
#endif
...
public:
    ...
    bool do_consume_condition(ParentNode& node) override
    {
        ...
        BARQ_ASSERT(this->m_condition_column_key == other.m_condition_column_key);
        if (m_needles.empty()) {
    ...
    std::string describe(util::serializer::SerialisationState& state) const override
    {
        BARQ_ASSERT(this->m_condition_column_key);
...
public:
    ...
    size_t find_first_local(size_t start, size_t end) override
    {
        ...
        auto find = [&](bool nullability) {
            ...
            for (size_t s = start; s < end; ++s) {
                ...
                BARQ_ASSERT(!(null::is_null_float(v) && !nullability));
    ...
    std::string describe(util::serializer::SerialisationState& state) const override
    {
        BARQ_ASSERT(m_condition_column_key);
...
public:
    ...
    void reset_cache()
    {
        ...
        BARQ_ASSERT_DEBUG(m_condition_column_key.is_list());
    }
...
public:
    ...
    std::string describe(util::serializer::SerialisationState& state) const override
    {
        ...
        return state.describe_column(ParentNode::m_table, this->m_condition_column_key) + " " +
               ...
               (this->m_value_is_null ? util::serializer::print_value(barq::null())
...
public:
    ...
    {
        BARQ_ASSERT(column.get_type() == col_type_Mixed);
        get_ownership();
        m_condition_column_key = column;
    }
...
public:
    ...
    size_t find_first_local(size_t start, size_t end) override
    {
        ...
        for (size_t i = start; i < end; i++) {
            ...
            if constexpr (barq::is_any_v<TConditionFunction, BeginsWith, BeginsWithIns, EndsWith, EndsWithIns, Like,
...
public:
    ...
    std::string describe(util::serializer::SerialisationState& state) const override
    {
        BARQ_ASSERT(m_condition_column_key1 && m_condition_column_key2);
...
public:
        ...
        : m_target_keys(target_keys)
    {
        ...
        BARQ_ASSERT(column_type == col_type_Link);
        BARQ_ASSERT(!m_target_keys.empty());
    }
