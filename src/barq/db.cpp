...
#include <barq/util/file_mapper.hpp>
...
struct VersionList {
    ...
    ReadCount* try_allocate_entry(uint64_t top, uint64_t size, uint64_t version)
    {
        ...
        auto& rc = data()[i];
        BARQ_ASSERT(rc.count_frozen == 0);
        BARQ_ASSERT(rc.count_live == 0);
        BARQ_ASSERT(rc.count_full == 0);
        rc.current_top = top;
        rc.filesize = size;
        rc.activate(version);
    ...
    void purge_versions(uint64_t& oldest_live_v, TopRefMap& top_refs, bool& any_new_unreachables)
    {
        ...
        for (auto* rc = data(); rc < data() + entries; ++rc) {
            ...
            if (rc->count_frozen == 0 && rc->count_live == 0 && rc->version < oldest_full_v) {
                ...
                BARQ_ASSERT(index_of(*rc) != index_of_newest);
                free_entry(rc);
            }
        }
        BARQ_ASSERT(oldest_v != std::numeric_limits<uint64_t>::max());
        BARQ_ASSERT(oldest_live_v != std::numeric_limits<uint64_t>::max());
    }
    ...
    , shared_controlmutex() // Throws
{
    ...
    BARQ_ASSERT(!util::int_cast_has_overflow<decltype(history_type)>(ht + 0));
    BARQ_ASSERT(!util::int_cast_has_overflow<decltype(history_schema_version)>(hsv));
    history_type = ht;
...
public:
    ...
    void release_read_lock(const ReadLockInfo& read_lock) REQUIRES(!m_local_readers_mutex, !m_info_mutex)
    {
        {
            util::CheckedLockGuard lock(m_local_readers_mutex);
            BARQ_ASSERT(read_lock.m_reader_idx < m_local_readers.size());
        ...
        BARQ_ASSERT(read_lock.m_reader_idx < m_local_max_entry);
        auto& r = m_info->readers.get(read_lock.m_reader_idx);
        BARQ_ASSERT(read_lock.m_version == r.version);
        ...
        REQUIRES(!m_local_readers_mutex, !m_info_mutex)
    {
...

        {
            ...
            BARQ_ASSERT(newest != VersionList::nil);
...

        {
            ...
            BARQ_ASSERT_EX(field_for_type(r2, type) == 0, type, r2.count_full, r2.count_live, r2.count_frozen);
    ...
    void add_version(ref_type new_top_ref, size_t new_file_size, uint64_t new_version) REQUIRES(!m_info_mutex)
    {
        ...
        BARQ_ASSERT_EX(success, new_entries, new_version);
    }
    ...
    static uint32_t& field_for_type(VersionList::ReadCount& r, ReadLockInfo::Type type)
    {
        switch (type) {
            ...
            case ReadLockInfo::Full:
                return r.count_full;
            default:
                BARQ_UNREACHABLE(); // silence a warning
        }
    }
...
public:
        ...
        , m_file(file)
    {
        ...
        while (size < required_size) {
            ...
            BARQ_ASSERT(required_size >= size);
        }
    }
...
void DB::open(const std::string& path, const DBOptions& options)
{
    ...
    BARQ_ASSERT(!is_attached());
    BARQ_ASSERT(path.size());
...
#pragma GCC diagnostic pop
        ...
        if (info->init_complete == 0)
            continue;
        BARQ_ASSERT(info->init_complete == 1);
        ...
        // - Waiting for and signalling database changes
        {
            ...
            if (BARQ_UNLIKELY(!file_format_ok)) {
            ...
            if (begin_new_session) {
                ...
                switch (openers_hist_type) {
                    ...
                    case Replication::hist_OutOfBarq:
                        BARQ_ASSERT(false); // No longer in use
                        break;
                ...
                BARQ_ASSERT(stored_hist_schema_version >= 0);
        ...
        m_logger->log(util::Logger::Level::debug, "   Durability: %1", [&] {
            switch (options.durability) {
                ...
                case DBOptions::Durability::MemOnly:
                    return "MemOnly";
                case barq::DBOptions::Durability::Unsafe:
                    return "Unsafe";
            }
            return "";
        }());
...
{
    BARQ_ASSERT(!m_fake_read_lock_if_immutable);
...
void DB::release_all_read_locks() noexcept
{
    ...
    BARQ_ASSERT(m_transaction_count == 0);
}
...
public:
    ...
    void blocking_begin_write()
    {
        ...
        BARQ_ASSERT(can_lock_on_caller || m_running || InterprocessMutex::is_thread_confined);
    ...
    {
        std::unique_lock lg(m_mutex);
        BARQ_ASSERT(m_has_write_mutex);
        BARQ_ASSERT(m_owns_write_mutex || !InterprocessMutex::is_thread_confined);
...
void DB::close_internal(std::unique_lock<InterprocessMutex> lock, bool allow_open_read_transactions)
{
    ...
    SharedInfo* info = m_info;
    {
        ...
        if (m_is_sync_agent) {
            BARQ_ASSERT(info->sync_agent_present);
    ...
    while (m_running) {
        ...
        if (m_has_write_mutex) {
            if (auto cb = std::move(m_pending_sync)) {
                ...
                // and pending release.
                BARQ_ASSERT(!m_pending_mx_release);
                lg.unlock();
                cb();
            ...
            if (m_pending_mx_release) {
                BARQ_ASSERT(!InterprocessMutex::is_thread_confined || m_owns_write_mutex);
                m_db->do_end_write();
            ...
            BARQ_ASSERT(!m_pending_sync && !m_pending_mx_release);
            ...
            if (!m_waiting_for_write_mutex && has_pending_write_requests()) {
                ...
                BARQ_ASSERT(!m_pending_writes.empty());
...
bool DB::needs_file_format_upgrade(const std::string& file, Span<const char> encryption_key)
{
    ...
    if (!encryption_key.empty()) {
        BARQ_ASSERT(encryption_key.size() == 64);
                             ...
                             int current_hist_schema_version, int target_hist_schema_version)
{
    ...
    BARQ_ASSERT(current_file_format_version <= target_file_format_version);
    BARQ_ASSERT(current_hist_schema_version <= target_hist_schema_version);
...
// millisleep(200);
        ...
        BARQ_ASSERT(current_hist_schema_version_2 == current_hist_schema_version ||
        ...
        // concurrent DB object.
        BARQ_ASSERT(current_file_format_version_2 == current_file_format_version ||
...
void DB::do_end_write() noexcept
{
    ...
    CheckedLockGuard local_lock(m_mutex);
    BARQ_ASSERT(m_write_transaction_open);
    m_alloc.set_read_only(true);
...
void DB::low_level_commit(uint_fast64_t new_version, Transaction& transaction, bool commit_to_disk)
{
    ...
    BARQ_ASSERT(oldest_version <= new_version);
    ...
    }
    {
        ...
        info->commit_in_critical_phase = 1;
        {
            ...
            // BARQ_ASSERT(m_alloc.matches_section_boundary(new_file_size));
            BARQ_ASSERT(new_top_ref < new_file_size);
        }
...
bool DB::call_with_lock(const std::string& barq_path, CallbackWithLock&& callback)
{
    auto lockfile_path = get_core_file(barq_path, CoreFileType::Lock);
...
{
    if (m_fake_read_lock_if_immutable) {
        BARQ_ASSERT(false && "Can't write an immutable DB");
    }
    if (nonblocking) {
...
{
    {
        ...
        BARQ_ASSERT(tr->m_async_stage == Transaction::AsyncState::Idle);
...
void DB::release_sync_agent()
{
    ...
    if (!m_is_sync_agent)
        return;
    BARQ_ASSERT(m_info->sync_agent_present);
