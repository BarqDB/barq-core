...
#ifndef BARQ_TABLE_HPP
#define BARQ_TABLE_HPP
...
#define BARQ_EXERCISE_OBJECT_ID_COLLISION 0
...
class Table {
public:
    /// The type of tables supported by a barq.
    ...
    /// <barq/object-store/object_schema.hpp>.
    ...
    {
        if (BARQ_UNLIKELY(!valid_column(col_key)))
    ...
    /// index of this table within the group. Otherwise it returns barq::npos.
...
public:
    ...
    ColKey operator*()
    {
        if (m_pos < m_table->get_column_count()) {
            BARQ_ASSERT(m_table->m_spec.get_key(m_pos) == m_table->spec_ndx2colkey(m_pos));
...
public:
    ...
    inline Columns<T> column(ColKey col_key)
    {
        ...
        // Check if user-given template type equals Barq type.
...
inline StringData Table::get_column_name(ColKey column_key) const
{
    ...
    BARQ_ASSERT_3(spec_ndx, <, get_column_count());
