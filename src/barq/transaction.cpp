    ...
    for (auto elem : cols) {
        ...
        if (embedded_table) {
            update_embedded = [&](Mixed val) {
                ...
                BARQ_ASSERT(val.is_type(type_Link, type_TypedLink));
...
void Transaction::upgrade_file_format(int target_file_format_version)
{
    ...
    // forget it.
    BARQ_ASSERT_EX(target_file_format_version == 24, target_file_format_version);
...
void Transaction::prepare_for_close()
{
    ...
    switch (m_async_stage) {
        ...
        case AsyncState::Requesting:
            ...
            BARQ_ASSERT(m_transact_stage == DB::transact_Reading);
            BARQ_ASSERT(!m_oldest_version_not_persisted);
...
void Transaction::do_end_read() noexcept
{
    ...
    // those commits.
    if (m_oldest_version_not_persisted) {
        BARQ_ASSERT(m_async_commit_has_failed);
...
void Transaction::close_read_with_lock()
{
    ...
    {
        util::CheckedLockGuard lck(m_async_mutex);
        BARQ_ASSERT_EX(m_async_stage == AsyncState::Idle, size_t(m_async_stage));
    }

    detach();
    BARQ_ASSERT_EX(!m_oldest_version_not_persisted, m_oldest_version_not_persisted->m_type,
...
public:
    ...
    bool trv(Array& current_node, unsigned level, std::vector<size_t>& progress)
    {
        ...
        if (current_node.has_refs()) {
            ...
            BARQ_ASSERT_EX(level < progress.size(), level, progress.size());
