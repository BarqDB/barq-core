...
// Takes a row of payload and returns a vector of Barq types that can represent them. If a value can be represented
// by multiple Barq types, it prioritizes Bool > Int > Float > Double > String. If Empty_as_string == true, then
...
// Takes two vectors of Barq types, and for each field finds best type that can represent both.
