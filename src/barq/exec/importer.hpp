...
#ifndef BARQ_IMPORTER_HPP
#define BARQ_IMPORTER_HPP
...
---------------------------------------------------------------------------------------------------------------------
    ...
    * Newline inside data fields, plus auto detection of non-conforming non-quoted newlines (as in some IBM sample
      files)
    * Barq types String, Integer, Bool, Float and Double
    * Auto detection of header and naming of Barq columns accordingly
...
---------------------------------------------------------------------------------------------------------------------
    ...
    contains 'false, false, false, hello' and we detect and create Barq table scheme using the first 3 rows, we fail
...
import_csv(csv file handle, barq table)
...
// Number of rows to csv-parse + insert into barq in each iteration.
