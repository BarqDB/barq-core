/*
 * Usage: barq-trawler <barq-file-name>
 *
 * This tool will dump the structure of a barq file and print out any inconsistencies it finds.
 *
...
public:
    Node() {}
    Node(barq::Allocator& alloc, uint64_t ref)
    {
        init(alloc, ref);
    }
    void init(barq::Allocator& alloc, uint64_t ref);
    bool valid() const
    {
        return m_valid;
    }
    ...
    char* data()
    {
        return barq::Array::get_data_from_header(m_header);
    }
...
public:
    Array() {}
    Array(barq::Allocator& alloc, uint64_t ref)
        : Node(alloc, ref)
    {
        init(alloc, ref);
    }
    ...
    {
        return barq::NodeHeader::get_is_inner_bptree_node_from_header(m_header);
    }
    void init(barq::Allocator& alloc, uint64_t ref)
    {
        Node::init(alloc, ref);
    ...
    {
        int64_t val = barq::get_direct(m_data, width(), ndx);
    ...
    std::string get_string(size_t ndx) const
    {
        ...
        if (valid()) {
            ...
            auto w = width();
            BARQ_ASSERT(ndx * w < sz);
            str = m_data + ndx * w;
        }
        return str;
    }

    uint64_t mem_usage(barq::Allocator& alloc) const
    {
        uint64_t mem = 0;
...
private:
    ...
    void _mem_usage(barq::Allocator& alloc, uint64_t& mem) const
    {
        if (m_has_refs) {
...
public:
    Table() = default;
    Table(barq::Allocator& alloc, uint64_t ref)
        : Array(alloc, ref)
    {
        if (valid()) {
            ...
            if (spec.size() > 5) {
                ...
                for (size_t spec_ndx = 0; spec_ndx < num_spec_cols; ++spec_ndx) {
                    barq::ColKey col_key{m_column_colkeys.get_val(spec_ndx)};
            ...
            if (size() > 11) {
                ...
                if (pk_col)
                    m_pk_col = barq::ColKey(pk_col);
            }
            if (size() > 12) {
                auto flags = get_val(12);
                m_table_type = static_cast<barq::Table::Type>(flags & 0x3);
            }
        }
    }
    std::string get_column_name(barq::ColKey col_key) const
    {
        return m_column_names.get_string(m_leaf_ndx2spec_ndx[col_key.get_index().val]);
    }
    void print_columns(const Group&) const;
    size_t get_size(barq::Allocator& alloc) const
    {
        size_t ret = 0;
        if (m_clusters.valid()) {
            if (m_clusters.is_inner_bptree_node()) {
                ...
                if (uint64_t key_ref = m_clusters.get_ref(0)) {
                    auto header = alloc.translate(barq::to_ref(key_ref));
                    ret = barq::NodeHeader::get_size_from_header(header);
                }
                else {
...
private:
    ...
    {
        BARQ_ASSERT(column_ndx <= m_column_names.size());
        ...
        for (size_t i = 0; i != column_ndx; ++i) {
            auto type = barq::ColumnType(m_column_types.get_val(i));
            if (type == barq::col_type_Link) {
                subspec_ndx += 1; // index of dest column
            }
            else if (type == barq::col_type_BackLink) {
    ...
    Array m_clusters;
    barq::ColKey m_pk_col;
    barq::Table::Type m_table_type = barq::Table::Type::TopLevel;
    std::vector<size_t> m_leaf_ndx2spec_ndx;
};

class Group : public Array {
public:
    Group(barq::Allocator& alloc, uint64_t ref)
        : Array(alloc, ref)
        , m_alloc(alloc)
    {
        m_valid &= (size() <= 12);
    ...
    std::vector<barq::BinaryData> get_changesets()
    {
        std::vector<barq::BinaryData> ret;
        if (int(get_val(7)) == 2) {
...
class BarqFile {
public:
    BarqFile(const std::string& file_path, const char* encryption_key, uint64_t top_ref = 0);
...
void Table::print_columns(const Group& group) const
{
    ...
    for (unsigned i = 0; i < m_column_names.size(); i++) {
        ...
        auto attr = barq::ColumnAttr(m_column_attributes.get_val(i));
        std::string type_str;
        barq::ColKey col_key;
        if (this->m_column_colkeys.valid()) {
            // core6
            col_key = barq::ColKey(m_column_colkeys.get_val(i));
        }
        if (type == 13) {
            // type_LinkList == 13
            type = 12;
        ...
        if (type == 12) {
            ...
            if (col_key) {
                // core6
                barq::TableKey opposite_table_key(uint32_t(m_opposite_table.get_val(col_key.get_index().val)));
            ...
            type_str = get_data_type_name(barq::DataType(type));
        }
        if (col_key) {
            if (col_key.is_list())
                type_str += "[]";
            ...
            if (col_key.is_dictionary()) {
                auto key_type = barq::DataType(int(m_column_types.get_val(i)) >> 16);
                type_str = std::string("{") + get_data_type_name(key_type) + ", " + type_str + "}";
            }
        }
        if (attr & barq::col_attr_Nullable)
            type_str += "?";
        if (attr & barq::col_attr_Indexed)
            type_str += " (indexed)";
...
void Node::init(barq::Allocator& alloc, uint64_t ref)
{
    m_ref = ref;
    m_header = alloc.translate(barq::to_ref(ref));
...
static std::vector<Entry> get_nodes(barq::Allocator& alloc, uint64_t ref)
{
...
BarqFile::BarqFile(const std::string& file_path, const char* encryption_key, uint64_t top_ref)
{
    barq::SlabAlloc::Config config;
...
void BarqFile::schema_info()
{
    m_group->print_schema();
}

void BarqFile::memory_leaks()
{
    if (m_group->valid()) {
        ...
        if (nodes.size() > 1) {
            ...
            BARQ_ASSERT(it->length == m_group->get_file_size());
            std::cout << "No memory leaks\n";
        }
    }
}

void BarqFile::free_list_info() const
{
...
public:
    ...
    bool select_table(barq::TableKey key)
    {
    ...
    bool insert_group_level_table(barq::TableKey key)
    {
        std::cout << "Create table: " << m_group->get_table_name(key.value) << std::endl;
        return true;
    }

    bool erase_class(barq::TableKey)
    {
        return true;
    }

    bool rename_class(barq::TableKey)
    {
        return true;
    }

    bool create_object(barq::ObjKey key)
    {
        std::cout << "Create object: " << key << std::endl;
        return true;
    }

    bool remove_object(barq::ObjKey key)
    {
        std::cout << "Remove object: " << key << std::endl;
        return true;
    }

    bool modify_object(barq::ColKey col_key, barq::ObjKey key)
    {
    ...
    bool set_link_type(barq::ColKey)
    {
        return true;
    }

    bool insert_column(barq::ColKey col_key)
    {
        std::cout << "Add column: " << m_table->get_column_name(col_key) << std::endl;
        return true;
    }

    bool erase_column(barq::ColKey)
    {
        return true;
    }

    bool rename_column(barq::ColKey)
    {
        return true;
    }

    bool select_collection(barq::ColKey col_key, barq::ObjKey key, const barq::StablePath&)
    {
    ...
    bool typed_link_change(barq::ColKey, barq::TableKey)
    {
        return true;
    }
...
void BarqFile::changes() const
{
    barq::_impl::TransactLogParser parser;
    HistoryLogger logger(m_group.get());

    auto changesets = m_group->get_changesets();

    for (auto c : changesets) {
        barq::util::SimpleInputStream stream(c);
        try {
...
int main(int argc, const char* argv[])
{
    if (argc > 1) {
        try {
            ...
            for (int curr_arg = 1; curr_arg < argc; curr_arg++) {
                if (strcmp(argv[curr_arg], "--keyfile") == 0) {
                    ...
                    BarqFile rf(argv[curr_arg], key_ptr, alternate_top);
                    if (free_list_info) {
                        rf.free_list_info();
                    }
                    if (memory_leaks) {
        ...
        std::cout << "Usage: barq-trawler [-cfmsw] [--keyfile file-with-binary-crypt-key] [--hexkey "
