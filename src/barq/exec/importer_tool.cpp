...
const char* legend =
    ...
    "  csv <.csv file | -stdin> <.barq file>\n"
    "\n"
    "Advanced auto-detection of scheme:\n"
    "  csv [-a=N] [-n=N] [-e] [-f] [-q] [-l tablename] <.csv file | -stdin> <.barq file>\n"
    "\n"
    "Manual specification of scheme:\n"
    "  csv -t={s|i|b|f|d}{s|i|b|f|d}... name1 name2 ... [-s=N] [-n=N] <.csv file | -stdin> <.barq file>\n"
    "\n"
    " -a: Use the first N rows to auto-detect scheme (default =10000). Lower is faster but more error prone\n"
    " -e: Barq does not support null values. Set the -e flag to import a column as a String type column if\n"
    ...
    "\n"
    "Examples:\n"
    "  csv file.csv file.barq\n"
    "  csv -a=200000 -e file.csv file.barq\n"
    "  csv -t=ssdbi Name Email Height Gender Age file.csv -s=1 file.barq\n"
    "  csv -stdin file.barq < cat file.csv\n"
    "  csv -p=$'\\t' file.tsv file.barq";
...
{
#if 0
    char* hey[] = {"self", "-f", "d:/csv/managers.csv", "d:/csv/test.barq"};
