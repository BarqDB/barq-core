...
const char* legend =
    "Simple tool to output the JSON representation of a Barq:\n"
    "  barq2json [--output-mode N] [--filter <filterexpr>] <.barq file>\n"
    "\n"
    "Options:\n"
    " --schema: Just output the schema of the barq\n"
    " --output-mode: Optional formatting for the output \n"
    "      0 - JSON Object\n"
    "      1 - Extended JSON (XJSON)\n"
...
int main(int argc, char const* argv[])
{
    ...
    barq::JSONOutputMode output_mode = barq::output_mode_json;
    ...
    for (int idx = 1; idx < argc - 1; ++idx) {
        ...
        if (arg == "--schema") {
            ...
            switch (output_mode_val) {
                case 0: {
                    output_mode = barq::output_mode_json;
                    break;
                }
                case 1: {
                    output_mode = barq::output_mode_xjson;
                    break;
                }
                case 2: {
                    output_mode = barq::output_mode_xjson_plus;
                    break;
                }
            }
        }
    ...
    auto print = [&](barq::TransactionRef tr) {
        if (output_schema) {
            tr->schema_to_json(std::cout);
        }
        else if (table_filter.size()) {
            barq::TableRef target = tr->get_table(table_filter);
            abort_if(!target, "table not found: '%s'", table_filter.c_str());
            barq::Query q = target->query(query_filter);
            barq::TableView results = q.find_all();
            std::cout << barq::util::format("filter '%1' found %2 results", query_filter, results.size())
                      << std::endl;
    ...
    auto hist = barq::make_in_barq_history();
    barq::DBOptions options;
    ...
    for (;;) {
        try {
            auto db = barq::DB::create(*hist, path, options);
        ...
        catch (const barq::IncompatibleHistories&) {
            hist = barq::sync::make_client_replication();
            options.allow_file_format_upgrade = false;
            options.is_immutable = true;
        }
        catch (const barq::FileAccessError& e) {
            if (e.code() != barq::ErrorCodes::FileFormatUpgradeRequired) {
                throw;
            }
