...
#ifndef BARQ_MIXED_HPP
#define BARQ_MIXED_HPP
...
{
    BARQ_ASSERT(get_type() == type_Bool);
    return bool_val;
}
...
{
    BARQ_ASSERT(get_type() == type_Float);
    return float_val;
}
...
{
    BARQ_ASSERT(get_type() == type_Double);
    return double_val;
}
...
inline StringData Mixed::get<StringData>() const noexcept
{
    if (is_null())
        return StringData();
    BARQ_ASSERT(get_type() == type_String);
    return string_val;
}
...
inline BinaryData Mixed::get<BinaryData>() const noexcept
{
    if (is_null())
        return BinaryData();
    BARQ_ASSERT(get_type() == type_Binary);
    return binary_val;
}
...
{
    BARQ_ASSERT(get_type() == type_Timestamp);
    return date_val;
}
...
{
    BARQ_ASSERT(get_type() == type_Decimal);
    return decimal_val;
}
...
{
    BARQ_ASSERT(get_type() == type_ObjectId);
    return id_val;
}
...
{
    BARQ_ASSERT(get_type() == type_UUID);
    return uuid_val;
}
...
inline ObjKey Mixed::get<ObjKey>() const noexcept
{
    ...
    BARQ_ASSERT(get_type() == type_Link);
...
{
    BARQ_ASSERT(get_type() == type_TypedLink);
    return link_val;
}
...
{
    auto type = get_type();
    BARQ_ASSERT_EX(type == type_Geospatial, type);
...
namespace std {
template <>
struct hash<::barq::Mixed> {
    inline size_t operator()(const ::barq::Mixed& m) const noexcept
    {
        return m.hash();
    }
};
