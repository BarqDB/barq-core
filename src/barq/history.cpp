...
class InBarqHistory : public _impl::History {
public:
    ...
    void prepare_for_write()
    {
        if (!m_changesets) {
            ...
            gf::prepare_history_parent(*m_group, *m_changesets, Replication::hist_InBarq, g_history_schema_version,
                                       0); // Throws
            m_changesets->create();
        }
    }
...
InBarqHistory::version_type InBarqHistory::add_changeset(BinaryData changeset)
{
    prepare_for_write();
...
void InBarqHistory::get_changesets(version_type begin_version, version_type end_version,
                                    BinaryIterator* buffer) const noexcept
{
    BARQ_ASSERT_EX(begin_version <= end_version, begin_version, end_version, m_base_version);
    BARQ_ASSERT_EX(begin_version >= m_base_version, begin_version, end_version, m_base_version);
    BARQ_ASSERT_EX(end_version <= m_base_version + m_size, end_version, m_base_version, m_size);
    version_type n_version_type = end_version - begin_version;
    version_type offset_version_type = begin_version - m_base_version;
    BARQ_ASSERT_EX(!util::int_cast_has_overflow<size_t>(n_version_type) &&
...
void InBarqHistory::set_oldest_bound_version(version_type version)
{
    BARQ_ASSERT(version >= m_base_version);
    if (version > m_base_version) {
        ...
        // history empty.
        BARQ_ASSERT_EX(num_entries_to_erase < m_size, num_entries_to_erase, m_size);
...
void InBarqHistory::verify() const
{
#ifdef BARQ_DEBUG
    if (m_changesets)
        m_changesets->verify();
...
void InBarqHistory::update_from_parent(version_type version)
{
    using gf = _impl::GroupFriend;
...
void InBarqHistory::update_from_ref_and_version(ref_type ref, version_type version)
{
    if (ref == 0) {
        // No history
...
class InBarqHistoryImpl : public Replication {
public:
    ...
    {
        auto hist = std::make_unique<InBarqHistory>();
        hist->initialize(m_history.get_alloc());
        return hist;
    }

private:
    InBarqHistory m_history;
};
...
std::unique_ptr<Replication> make_in_barq_history()
{
    return std::unique_ptr<InBarqHistoryImpl>(new InBarqHistoryImpl()); // Throws
}
