...
protected:
    ...
    BARQ_COLD BARQ_NORETURN void throw_invalid_null()
    {
...
template <class T>
BARQ_NOINLINE auto Set<T>::find_impl(const T& value) const -> iterator
{
    auto b = this->begin();
...
template <class T>
BARQ_NOINLINE size_t Set<T>::size() const
{
...
BARQ_NOINLINE void set_sorted_indices(size_t sz, std::vector<size_t>& indices, bool ascending);
...
{
    BARQ_ASSERT(!value.is_unresolved());
    update_if_needed();
