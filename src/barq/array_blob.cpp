...
ref_type Array::blob_replace(size_t begin, size_t end, const char* data, size_t data_size, bool add_zero_term)
{
    ...
    BARQ_ASSERT(get_context_flag());
    size_t sz = blob_size();
    BARQ_ASSERT_3(begin, <=, end);
    BARQ_ASSERT_3(end, <=, sz);
    BARQ_ASSERT(data_size == 0 || data);

    BARQ_ASSERT((begin == 0 || begin == sz) && end == sz); // Only append or total replace supported
    if (begin == sz && end == sz) {
        // Append
...
ref_type ArrayBlob::replace(size_t begin, size_t end, const char* data, size_t data_size, bool add_zero_term)
{
    ...
    // Insert the data
    modify_begin = barq::safe_copy_n(data, data_size, modify_begin);
    if (add_zero_term)
        *modify_begin = 0;
