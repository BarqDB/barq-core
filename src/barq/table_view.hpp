...
public:
    ...
    /// within this view is returned, otherwise `barq::not_found` is returned.
    ...
    void knnsearch(ColKey column, const std::vector<float>& query_data, size_t k, size_t ef = 0, bool exact = false);
...
{
    BARQ_ASSERT(m_collection_source);
    m_key_values.create();
    if (m_table) {
        m_last_seen_versions.emplace_back(m_table->get_key(), m_table->get_content_version());
    }
