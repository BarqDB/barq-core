...
#ifndef BARQ_TIMESTAMP_HPP
#define BARQ_TIMESTAMP_HPP
...
public:
    ...
    constexpr Timestamp(int64_t seconds, int32_t nanoseconds)
        ...
        , m_is_null(false)
    {
        BARQ_ASSERT_EX(-nanoseconds_per_second < nanoseconds && nanoseconds < nanoseconds_per_second, nanoseconds);
        const bool both_non_negative = seconds >= 0 && nanoseconds >= 0;
        const bool both_non_positive = seconds <= 0 && nanoseconds <= 0;
        BARQ_ASSERT_EX(both_non_negative || both_non_positive, both_non_negative, both_non_positive);
    }
    constexpr Timestamp() = default;
    constexpr Timestamp(barq::null) {}
...
namespace std {
template <>
struct numeric_limits<barq::Timestamp> {
    static constexpr bool is_integer = false;
    static constexpr barq::Timestamp min()
    {
        return barq::Timestamp(numeric_limits<int64_t>::min(), 0);
    }
    static constexpr barq::Timestamp lowest()
    {
        return barq::Timestamp(numeric_limits<int64_t>::lowest(), 0);
    }
    static constexpr barq::Timestamp max()
    {
        return barq::Timestamp(numeric_limits<int64_t>::max(), 0);
    }
};
} // namespace std

#endif // BARQ_TIMESTAMP_HPP
