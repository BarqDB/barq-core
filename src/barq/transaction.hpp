...
#ifndef BARQ_TRANSACTION_HPP
#define BARQ_TRANSACTION_HPP
...
inline bool Transaction::promote_to_write(O* observer, bool nonblocking)
{
    ...
    try {
        ...
        BARQ_ASSERT(repl); // Presence of `repl` follows from the presence of `hist`
...
inline bool Transaction::internal_advance_read(O* observer, VersionID version_id, _impl::History& hist, bool writable)
{
    ...
    BARQ_ASSERT(new_read_lock.m_version >= m_read_lock.m_version);
...
{
    BARQ_ASSERT(m_transact_stage != DB::transact_Ready);
    BARQ_ASSERT(end <= m_read_lock.m_version);
    auto hist = get_history(); // Throws
    BARQ_ASSERT(hist);
