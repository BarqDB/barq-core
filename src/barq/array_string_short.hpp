...
#ifndef BARQ_ARRAY_STRING_SHORT_HPP
#define BARQ_ARRAY_STRING_SHORT_HPP
...
xxx0 xx01 x002 0003 0004 (strings "xxx",. "xx", "x", "", barq::null())

So each string is 0 terminated, and the last byte in a block tells how many 0s are present, except
for a barq::null() which has the byte set to m_width (4). The byte is used to compute the length of a string
in various functions.

New: If m_witdh = 0, then all elements are barq::null(). So to add an empty string we must expand m_width
...
inline StringData ArrayStringShort::get(size_t ndx) const noexcept
{
    ...
    if (m_width == 0)
        return m_nullable ? barq::null() : StringData("");
    ...
    BARQ_ASSERT_EX(data[array_size] == 0, data[array_size],
