...
#ifndef BARQ_OBJECT_CONVERTER_HPP
#define BARQ_OBJECT_CONVERTER_HPP
...
struct InterBarqValueConverter {
    InterBarqValueConverter(ConstTableRef src_table, ColKey src_col, ConstTableRef dst_table, ColKey dst_col,
...
struct InterBarqObjectConverter {
    InterBarqObjectConverter(ConstTableRef table_src, TableRef table_dst, EmbeddedObjectConverter* embedded_tracker);
    void copy(const Obj& src, Obj& dst, bool* update_out);

private:
    EmbeddedObjectConverter* m_embedded_tracker;
    std::vector<InterBarqValueConverter> m_columns_cache;
};

} // namespace barq::converters

#endif // BARQ_OBJECT_CONVERTER_HPP
