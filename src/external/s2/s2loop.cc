...
void S2Loop::Init(vector<S2Point> const& vertices) {
  ...
  if (vertices.empty()) {
    ...
    // void* casts to silence a -Wclass-memaccess warning.
...
bool S2Loop::IsValid(string* err) const {
  ...
  for (int i = 0; i < num_vertices(); ++i) {
    if (!S2::IsUnitLength(vertex(i))) {
      ...
      if (err) *err = barq::util::format("Vertex %1 is not unit length", i);
      return false;
    }
  }
  ...
  for (int i = 0; i < num_vertices(); ++i) {
    if (!vmap.insert(make_pair(vertex(i), i)).second) {
        ...
        if (err) *err = barq::util::format("Duplicate vertices: %1 and %2", vmap[vertex(i)], i);
        return false;
    }
  }
  ...
  for (int i = 0; i < num_vertices(); ++i) {
    ...
    for (it.GetCandidates(vertex(i), vertex(i+1)); !it.Done(); it.Next()) {
      ...
      if (ai > i+1) {
        ...
        previous_index = ai + 1;
        if (crosses) {
            std::string msg = barq::util::format("Edges %1 and %2 cross. Edge locations in degrees: %3-%4 and %5-%6", i, ai, S2LatLng(vertex(i)), S2LatLng(vertex(i + 1)), S2LatLng(vertex(ai)), S2LatLng(vertex(ai + 1)));
