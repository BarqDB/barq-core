...
bool S2Polygon::IsValid(const vector<S2Loop*>& loops, string* err) {
  ...
  if (loops.size() > 1) {
    ...
    for (size_t i = 0; i < loops.size(); ++i) {
      ...
      for (int j = 0; j < lp->num_vertices(); ++j) {
        ...
        if (err) {
            *err = barq::util::format("Duplicate edge: ring %1, edge %2 and ring %3, edge %4", i, j, other.first, other.second);
        }
        else {
  ...
  for (size_t i = 0; i < loops.size(); ++i) {
    if (!loops[i]->IsNormalized()) {
        ...
        if (err) *err = barq::util::format("Ring %1 encloses more than half the sphere", i);
      return false;
    }
    for (size_t j = i + 1; j < loops.size(); ++j) {
      ...
      if (loops[i]->ContainsOrCrosses(loops[j]) < 0) {
          ...
          if (err) *err = barq::util::format("Ring %1 crosses ring %2", i, j);
        return false;
      }
    }
  }
...
bool S2Polygon::IsNormalized(string* err) const {
  ...
  for (int i = 0; i < num_loops(); ++i) {
    ...
    if (count > 1) {
      if (err) {
          *err = barq::util::format("Ring %1 shares more than one vertex with its parent ring %2", i, GetParent(i));
      }
      return false;
    }
  }
